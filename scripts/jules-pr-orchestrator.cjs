...
async function cmdTriggerPr(ids) {
  const triggerPrompt =
    'Please create a GitHub pull request for this completed changeset against the main branch of whodaniel/tnf-monorepo. Include a clear title, summary of changes, and verification notes.';
...
async function cmdAdvance(ids, promptOverride) {
  ...
  const prompt =
    promptOverride ||
    "Required follow-up: use 'Publish Branch' now, then open a GitHub PR to whodaniel/tnf-monorepo main. Reply with BRANCH and PR_URL.";
