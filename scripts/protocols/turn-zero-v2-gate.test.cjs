...
test('blocks restricted personal content from public OSS destination', () => {
  const result = validateClassification({ workDomain:'personal', artifactDestination:'oss_runtime', dataResidency:'bounded_working', sensitivity:'restricted' });
  assert.strictEqual(result.ok, false);
  assert.match(result.errors.join('\n'), /cannot target oss_runtime/);
});

test('accepts a complete public corporate OSS classification', () => {
  const result = validateClassification({ workDomain:'corporate', artifactDestination:'oss_runtime', dataResidency:'product_state', sensitivity:'public' });
  assert.deepStrictEqual(result, { ok:true, unresolved:false, errors:[] });
});

test('unknown values are non-write-ready rather than guessed', () => {
  const result = validateClassification({ workDomain:'corporate', artifactDestination:'unknown', dataResidency:'unknown', sensitivity:'unknown' });
  assert.strictEqual(result.ok, false);
