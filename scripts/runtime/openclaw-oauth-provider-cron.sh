...
SYNC_CMD="cd '$RUNTIME_DIR' && bash scripts/cloud-run/sync-openclaw-oauth-instances.sh --no-wait"
HEALTH_CMD="cd '$RUNTIME_DIR' && bash scripts/cloud-run/check-zeroclaw-instances.sh openclaw-cloud openclaw-primary openclaw-sandbox-cloud openclaw-oc004"
...
run_once() {
  ...
  (
    ...
    while [ "$attempt" -le "$retries" ]; do
      if bash scripts/cloud-run/check-zeroclaw-instances.sh openclaw-cloud openclaw-primary openclaw-sandbox-cloud openclaw-oc004; then
        return 0
      fi
