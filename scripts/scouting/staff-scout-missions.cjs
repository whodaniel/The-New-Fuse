...
function maybeRunAgent(payload) {
  ...
  const args = fs.existsSync(cli)
    ? [cli, 'agents', 'run', '--task-file', OUT_MD, '--quiet']
    : ['--import', 'tsx', src, 'agents', 'run', '--task-file', OUT_MD, '--quiet'];
  const result = spawnSync(process.execPath, args, {
    cwd: ROOT,
    encoding: 'utf8',
    env: { ...process.env, TNF_AGENT_ID: 'tnf-cli-agent' },
    timeout: Number(process.env.TNF_SCOUT_RUN_TIMEOUT_MS || 180000),
  });
  return {
    skipped: false,
...
function main() {
  ...
  const receipt = { ok: queueBuild.ok !== false, queueBuild, mission: payload, agent };
  if (opts.json) {
      ...
      JSON.stringify(
        {
          ...
          agent: agent.skipped ? 'brief-only' : `exit=${agent.status}`,
        },
        null,
        2
      )
