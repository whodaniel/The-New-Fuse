...
class SelfImprovementOrchestrator {
  constructor() {
    ...
    this.prompts = {
      baseContextPrompt:
        'You are an autonomous AI system that helps improve The New Fuse framework. Remember your core mission, the available MCP tools, and your current task.',
      reflectionPrompt:
        'Based on the completed tasks and current state of the framework, what improvements would be most valuable next? Consider both technical improvements and documentation updates.',
      explorationPrompt:
        'Explore the framework to discover new components, integrations, or opportunities for enhancement. Focus on the MCP integration specifically.',
    };
  }
  ...
  async verifyMcpConfig() {
    ...
    try {
      ...
      if (mcpConfigExists) {
        ...
        const missingServers = essentialServers.filter(
  ...
  async checkAndProcessTasks() {
    ...
    this.log(
      `Processing task: ${task.description} [ID: ${task.id}]`,
  ...
  async handleExplorationTask(task) {
    ...
    fs.writeFileSync(
      resultFile,
      `Exploration Task: ${task.description}\nStarted: ${new Date().toISOString()}\nResult: ${result}\n`,
      'utf8'
    );
  ...
  async handleEnhancementTask(task) {
    ...
    fs.writeFileSync(
      resultFile,
      `Enhancement Task: ${task.description}\nStarted: ${new Date().toISOString()}\nResult: ${result}\n`,
      'utf8'
    );
  ...
  async handleReflectionTask(task) {
    ...
    fs.writeFileSync(
      resultFile,
      `Reflection Task: ${task.description}\nStarted: ${new Date().toISOString()}\nResult: ${result}\n`,
      'utf8'
    );
  ...
  /**
   * System-wide "is a human actively at the keyboard/mouse right now"
   * check, via macOS's HIDIdleTime (nanoseconds since last HID event,
   * any app). Unlike a per-app buffer read, this works regardless of
   * which app has focus — needed here because executeAppleScript()
   * force-activates "Claude" and types blindly via `keystroke`, which
   * has no per-app way to check for an unsubmitted line the way
   * scripts/lib/tnf-terminal-attention.cjs does for Terminal.app.
   * Intentionally conservative: on any error, treat as "active" (skip)
   * rather than risk injecting.
   */
  ...
  async executeAppleScript(script) {
    ...
    const minIdleSeconds = Number.parseInt(
      process.env.TNF_SELF_IMPROVE_MIN_IDLE_SECONDS || '20',
      10
    );
    if (idleSeconds < minIdleSeconds) {
      this.log(
        `Skipping AppleScript injection: system active ${idleSeconds}s ago (< ${minIdleSeconds}s threshold) — a human may be at the keyboard.`,
        colors.yellow
      );
      return 'Skipped: system recently active (human may be present)';
    }
