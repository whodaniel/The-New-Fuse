#!/bin/bash
# Deploy The New Fuse services to Railway
# Run this AFTER creating empty services in Railway dashboard

set -e

echo "========================================="
echo "The New Fuse - Railway Service Deployment"
echo "========================================="
echo ""
...
deploy_service() {
    ...
    # Link to the service
    echo "Linking to Railway service: $service_name..."
    if ! railway link --service "$service_name" 2>&1; then
        echo -e "${RED}❌ Failed to link to service: $service_name${NC}"
        echo "Make sure the service exists in Railway dashboard:"
        echo "https://railway.com/project/041cee9d-8648-4074-b5a6-0eae436de1d1"
        return 1
    fi
    ...
    if railway up --detach 2>&1; then
        ...
        echo "Watch logs: railway logs --service $service_name"
        return 0
    else
...
echo "Make sure you've created these services in Railway dashboard:"
echo "- api"
echo "- backend"
echo "- api-gateway"
echo "- frontend"
echo ""
read -p "Have you created all services in Railway dashboard? (y/n) " -n 1 -r
echo
if [[ ! $REPLY =~ ^[Yy]$ ]]; then
    echo ""
    echo -e "${YELLOW}⚠️  Please create services first:${NC}"
    echo "1. Visit: https://railway.com/project/041cee9d-8648-4074-b5a6-0eae436de1d1"
...
if [ $services_failed -gt 0 ]; then
    ...
    echo "Check that services exist in Railway dashboard"
