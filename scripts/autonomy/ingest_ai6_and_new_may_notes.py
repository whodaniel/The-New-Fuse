...
1) YouTube playlist "AI 6" metadata snapshot
...
DEFAULT_PLAYLIST_JSON = ROOT / "data" / "ai-6-playlist.json"
...
DEFAULT_MANIFEST = ROOT / "data" / "ingestion-runs" / "ai6-new-may-2026-manifest.json"
DEFAULT_SCOUT_QUEUE = ROOT / "data" / "ingestion-runs" / "ai6-new-may-2026-scout-queue.json"
DEFAULT_ACTION_QUEUE = ROOT / "data" / "ingestion-runs" / "ai6-new-may-2026-action-queue.json"
...
def main(argv: Sequence[str]) -> int:
    parser = argparse.ArgumentParser(description="Ingest AI 6 playlist + NEW- May-2026 notes into TNF intelligence artifacts")
    ...
    raw_playlist: Any = read_json(playlist_path)
    if isinstance(raw_playlist, dict) and "videos" in raw_playlist:
        playlist = raw_playlist["videos"]
        playlist_id = raw_playlist.get("playlistId", "unknown")
    elif isinstance(raw_playlist, list):
        playlist = raw_playlist
        playlist_id = extract_playlist_id(playlist[0]["url"]) if playlist and "url" in playlist[0] else "unknown"
    else:
        raise RuntimeError("Playlist JSON must be an array or object containing a 'videos' array")

    notes_data = fetch_notes_from_folder(notes_db_path, args.notes_folder)
    # Enforce strict chronological order (oldest to newest)
    notes_data["notes"].sort(key=lambda x: (x.get("createdAt") or x.get("modifiedAt") or "", x.get("notePk", 0)))
    ...
    for video in video_items:
        source_id = f"yt-ai6-{video.get('videoId')}"
    ...
    write_json(
        ...
        {
            "generatedAt": now_iso(),
            "seedSource": "ai6-and-new-may-2026-intake",
