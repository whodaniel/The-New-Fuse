...
REPO_ROOT = Path("/Users/<owner>/Desktop/A1-Inter-LLM-Com/The-New-Fuse")
CANONICAL_LIBRARY = Path("/Users/<owner>/Projects/virtual-library-blueprints")
MONOREPO_LIBRARY = REPO_ROOT / "apps/virtual-library-blueprints"
KILO_HOME = Path("/Users/<owner>/.kilo")
OPENCODE_HOME = Path("/Users/<owner>/.opencode")
GEMINI_HOME = Path("/Users/<owner>/.gemini")
...
def git_info(path: Path) -> GitInfo:
    if not path.exists():
        return GitInfo(str(path), False, False, None, None, None, None)
    is_repo = run(["git", "-C", str(path), "rev-parse", "--is-inside-work-tree"]) == "true"
    if not is_repo:
        return GitInfo(str(path), True, False, None, None, None, None)
    return GitInfo(
        path=str(path),
        exists=True,
        is_repo=True,
...
def build_surface_map() -> dict[str, Any]:
    ...
    return {
        ...
        "canonicalization": {
            "canonical_codebase": str(CANONICAL_LIBRARY),
            "monorepo_mirror_codebase": str(MONOREPO_LIBRARY),
            "drift": drift,
        },
        "surfaces": {
            "codebases": {
        ...
        "process_contract": {
            "authoritative_edit_path": str(CANONICAL_LIBRARY),
