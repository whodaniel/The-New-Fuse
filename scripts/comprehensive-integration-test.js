...
async function testAgencyManagement() {
  ...
  if (createResponse.success && createResponse.data.success) {
    ...
    logTest(
      'Agency Creation',
      'FAIL',
      `Failed to create agency: ${createResponse.error || 'Unknown error'}`
    );
    return;
  }
  ...
  if (getResponse.success && getResponse.data.id === testAgencyId) {
    ...
    logTest(
      'Agency Retrieval',
      'FAIL',
      `Failed to retrieve agency: ${getResponse.error || 'Unknown error'}`
    );
  }
  ...
  if (listResponse.success && listResponse.data.success && Array.isArray(listResponse.data.data)) {
    ...
    logTest(
      'Agency Listing',
      'FAIL',
      `Failed to list agencies: ${listResponse.error || 'Unknown error'}`
    );
  }
...
async function testSwarmOrchestration() {
  ...
  const statusResponse = await httpRequest(
    'GET',
    `${TEST_CONFIG.API_URL}/agency/${testAgencyId}/metrics`
  );
  ...
  if (statusResponse.success && statusResponse.data.success) {
    ...
    logTest(
      'Swarm Status Check',
      'FAIL',
      `Failed to get swarm status: ${statusResponse.error || 'Unknown error'}`
    );
  }

  // Test swarm toggle (disable then re-enable)
  const disableResponse = await httpRequest(
    'PUT',
    `${TEST_CONFIG.API_URL}/agency/${testAgencyId}/swarm`,
    { enabled: false }
  );
  ...
  if (disableResponse.success && disableResponse.data.success) {
    ...
    const enableResponse = await httpRequest(
      'PUT',
      `${TEST_CONFIG.API_URL}/agency/${testAgencyId}/swarm`,
      { enabled: true }
    );
    ...
    if (enableResponse.success && enableResponse.data.success) {
      ...
      logTest(
        'Swarm Enable',
        'FAIL',
        `Failed to re-enable swarm: ${enableResponse.error || 'Unknown error'}`
      );
    }
  } else {
    logTest(
      'Swarm Disable',
      'FAIL',
      `Failed to disable swarm: ${disableResponse.error || 'Unknown error'}`
    );
  }
...
async function testEndToEndWorkflow() {
  ...
  try {
    ...
    const initialMetrics = await httpRequest(
      'GET',
      `${TEST_CONFIG.API_URL}/agency/${testAgencyId}/metrics`
    );
    ...
    // Get final metrics
    const finalMetrics = await httpRequest(
      'GET',
      `${TEST_CONFIG.API_URL}/agency/${testAgencyId}/metrics`
    );
  ...
  try {
    ...
    if (successRate >= 90) {
      logTest(
        ...
        'PASS',
        `${successRate}% success rate (${successCount}/${concurrentRequests})`
      );
    } else {
      logTest(
        'Concurrent API Requests',
        'FAIL',
        `Low success rate: ${successRate}% (${successCount}/${concurrentRequests})`
      );
    }
  } catch (error) {
...
async function cleanup() {
  ...
  if (testAgencyId) {
    const deleteResponse = await httpRequest(
      'DELETE',
      `${TEST_CONFIG.API_URL}/agency/${testAgencyId}`
    );
    if (deleteResponse.success) {
      ...
      logTest(
        'Test Agency Cleanup',
        'FAIL',
        `Failed to delete test agency: ${deleteResponse.error || 'Unknown error'}`
      );
    }
  }
