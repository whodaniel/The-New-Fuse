...
department: tech
description:
  Reference for Google Jules CLI commands. Use this skill when delegating
  complex coding tasks to the Jules asynchronous agent.
---
...
- `jules remote pull --session ID`: Retrieve the results of a session for
  review.
- `jules remote pull --session ID --apply`: Retrieve and apply changes to the
  local repo.
...
- Use parallel sessions for independent tasks (e.g., implementing multiple
  routes).
