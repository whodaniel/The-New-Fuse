...
- `bridge`: connects multiple skills with explicit handoffs
- `protocol`: contracts and best practices
- `integrator`: wires systems together (auth/provider swaps, service wiring)

Use `skill-types.md` and enforce pairing:

- every `operator` should have a `validator`
- every `bridge` should end with a `validator`

## Bridging Skills (First-Class)

Bridging is a skill type, not an afterthought. A bridge must have:
