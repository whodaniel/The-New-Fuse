...
department: tech
description:
  Implement or repair per-task execution audit logging across The New Fuse
  stack. Use when requests involve task execution visibility, audit trails,
  timeline logging, or “what happened during task/workflow execution” features
  in TNF. Trigger for tasks that require wiring logs through backend API
  endpoints, workflow engine telemetry/router/worker events, and Task Detail
  frontend filtering/presets/shareable log views.
---
...
Implement execution logging as a single chain: API persistence, workflow
emission, and UI consumption. Keep behavior additive and backward compatible.
...
5. Run focused lint/type/test checks and report pre-existing failures
   separately.
...
- Service method to append logs into `task_executions` using structured JSON
  payload in `output`.
- Timeline mirror event (typically `historical_event`) with payload category
  `task_execution_log`.

Require `message`, `level`, `actor`, `source` in POST body. Use ownership checks
so users can only access their own task logs.
...
- Optional interval override via `TASK_HEALTH_CHECK_INTERVAL_MS` (default
  `1800000`).

Wire `TaskModule` to `UnifiedLedgerModule` if timeline creation is needed.

Read [references/contracts.md](references/contracts.md) for payload and response
shapes.
...
- Optional API persistence path gated by env (`TNF_API_BASE_URL` or
  `API_BASE_URL`).
...
If worker writes context, ensure engine exposes the method worker calls (for
example `updateExecutionState`).

## Step 3: Frontend Task Detail

Implement or verify in `apps/frontend/src/pages/Tasks/Detail.tsx` and
...
- Add presets (`All`, `Errors Only`, `Router Events`, `Dispatch Stage`,
  `Worker Execution`).
- Add active preset visual state and `Custom` indicator when manual filters
  diverge.
...
If full frontend typecheck is known to hang in this repo, state that explicitly
and use file-scoped lint as fallback.

## Guardrails

- Avoid introducing new event enums if existing timeline types are sufficient.
- Keep persistence best-effort from workflow engine; never fail execution flow
  only because logging API failed.
