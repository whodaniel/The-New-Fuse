...
department: tech
description:
  Convert ambiguous constraints into adaptive, node-based workflows with
  explicit data handoff schemas. Use when users describe a limitation, blocker,
  or failed approach and need a resilient execution design that maps node
  responsibilities, input/output contracts, and parsed-content transfer formats.
---
...
Translate a limitation into a concrete challenge objective, then build a
schema-first node workflow so each step passes structured, validated data to the
next step.
...
- Require envelope fields (`trace_id`, `node_id`, `status`, `payload`) for
  observability and debugging.

4. Add adaptation hooks

- Add `confidence`, `errors`, and `next_action` fields so downstream nodes can
  adapt.
...
- Use `references/node-schema-patterns.md` for canonical contract structure and
  naming rules.
- Use `scripts/generate_workflow_contract.py` to bootstrap a schema package that
