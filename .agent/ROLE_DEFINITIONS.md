...
- `MASTER_LIBRARY_ID` / `KNOWLEDGE_TREE.json` `vector_id` is the
  **intelligence-indexer namespace** — generated by `scripts/autonomy/generate_merkle_tree.py`
  from hash bytes.

  **Updated 2026-08-09:** this namespace no longer shares the `ID#:` prefix. The
  2026-06-14 encoding audit migrated it to **`VEC#:`** (`vector_embeddings` rows
  `ID#:INTEL-*` → `VEC#:INTEL-*`; `KNOWLEDGE_TREE.json` rebuilt with 645 leaves
  on `VEC#:`). The prefixes are now disjoint, and the warning below is kept only
  as history:

  | Prefix | Namespace | Encodes |
  | ------ | --------- | ------- |
  | `ID#:` | federated agent identity (`agents.id_number`) | sequential int → Base58 |
  | `VEC#:` | intelligence indexer / vector embeddings | hash bytes → Base58 |

  Anything still minting `ID#:` for indexer content is a regression against that
  migration.

### Provisional edge IDs (added 2026-08-09)

`idNumber` allocation requires Redis, which a browser content script cannot
reach. `deterministicIdNumber()` — mirrored across
`apps/chrome-extension/src/v6/shared/federation-identity.ts`,
`scripts/lib/federation-protocol.cjs`, and
`packages/relay-core/src/contracts/recovery-federation.ts` — hashes the agent id
to give an unregistered edge agent a stable handle until the registry assigns
the real one.

**It is provisional, not authoritative.** A server-supplied `idNumber` always
wins; never overwrite one with a hashed value.

#### `idNumber` band allocation

An `ID#`'s **provenance is readable from its integer value**. Bands are
normative — a new minting site must claim a band, not invent a range:

| Band | Minter | Status |
| ---- | ------ | ------ |
| `1 – 999,999,999` | `FederatedIdentityService` (Redis `INCR`) | **authoritative** |
| `1e9 – 2e9` | `deterministicIdNumber` / `deterministicBridgeIdNumber` | **provisional** |
| `2e9 – 3e9` | `deterministicSeedNumber` (seeder) | **placeholder** |

Verified disjoint and collision-free against the live 194-agent roster on
2026-08-09: 194 distinct values in each band, zero overlap between them.

There are **four** mirrors of the provisional hash, which MUST stay in
lock-step — a divergence yields a different `ID#` for the same agent depending
on which side computed it:

1. `apps/chrome-extension/src/v6/shared/federation-identity.ts`
2. `scripts/lib/federation-protocol.cjs`
3. `packages/relay-core/src/contracts/recovery-federation.ts`
4. `packages/relay-core/src/agent-registry-bridge.ts` — **on the live
   registration path**

**Caveats, stated rather than hidden:**

- The seeder previously used `1,000 + (h % 9,000)` — a band that sat **inside**
  the production range and overlapped the old provisional band. Measured, it
  also collided: `backend-specialist` and `cto-agent` both hashed to `3755`.
  Moved to `2e9 – 3e9`. **Re-seeding therefore assigns new `ID#`s to
  already-seeded agents.** Seeded values are placeholders, but anything that
  persisted an old one must be refreshed.
- `FederatedIdentityService.generateIdNumber()` has **no production callers** —
  registration currently mints provisional bridge IDs instead. The authoritative
  band is therefore unused in practice. See "Next" below.

Its original space was `5000 + (h % 10000)` — 10,000 values, and it **overlapped
the seeder band** at 5,000–9,999, so the "visually distinct" intent recorded in
the bridge comment did not hold. Measured against
the live 194-agent roster on 2026-08-09 this produced **two real collisions**
(`ID#:4gV` → `brand-outreach-agent` + `temporal-agent-reclassifier`; `ID#:3Ub` →
`interoperability-protocol-agent` + `research-agent`). Because
`resolveMessageTarget()` routes on `@ID#:…`, a collision is an **ambiguous
address**, not a cosmetic clash. The space is now `1e6 + (h % 1e9)`; the same
roster yields 194 distinct values. Collisions scale as n²/2N, so re-measure
before the fleet grows an order of magnitude.
