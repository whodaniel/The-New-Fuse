...
**Core Tenet**: PARODY + ASSIMILATE the BEST practices from ALL CLI agents, then codify into TNF-native capabilities.

## 📦 Assimilated CLI Patterns

### From **Jules CLI**:
- `parallel-task-execution` — dispatch multiple coding sessions concurrently
- `async-code-generation` — non-blocking code generation with deferred result retrieval
...
1. **No Implicit Authority**: Patch application ≠ commit authority. All commits require `TNF_OPERATOR_CONFIRM` per `docs/core/AGENTS.md`
2. **Honest Failure Reporting**: If a CLI returns an error, report it as such—never synthesize success
3. **Agent Switching Transparency**: Record all `/agent` or persona changes in session logs
