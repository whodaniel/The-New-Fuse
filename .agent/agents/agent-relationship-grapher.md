...
```cypher
// Required axes on every TNF authority/fleet node:
//   daccRole, platform, workerAction, batonHolder
// Baton root for authority graphs = master-clock, NOT orchestrator-agent persona.

CREATE (baton:Agent {
  name: 'master-clock',
  type: 'infra',
  daccRole: 'orchestrator',
  platform: 'master-clock',
  batonHolder: true,
  baton_identity: 'ORCHESTRATOR-{timestamp}',
  capabilities: ['heartbeat', 'stall-detection', 'onboarding'],
  created: datetime()
})
