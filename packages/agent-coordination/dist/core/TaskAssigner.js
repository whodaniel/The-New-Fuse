...
exports.TaskAssigner = void 0;
const events_1 = require("events");
const types_js_1 = require("./types.js");
/**
 * Task assignment and load balancing
 */
class TaskAssigner extends events_1.EventEmitter {
    constructor(config = {}) {
        super();
     ...
     */
    filterEligibleAgents(task, agents) {
        return agents.filter((agent) => {
            // Check agent status
            if (agent.status !== types_js_1.AgentStatus.IDLE && agent.status !== types_js_1.AgentStatus.BUSY) {
                return false;
            }
            ...
            if (this.config.loadBalancing?.considerCapabilities &&
                task.requiredCapabilities) {
                const agentCapabilityNames = agent.capabilities.map((c) => c.name);
                const hasAllCapabilities = task.requiredCapabilities.every((required) => agentCapabilityNames.includes(required));
    ...
    selectLeastLoadedAgent(agents) {
        return agents.reduce((least, current) => current.currentLoad < least.currentLoad ? current : least);
    }
    /**
     ...
     */
    selectCapabilityBasedAgent(agents) {
    ...
    getAgentAssignments(agentId) {
        return Array.from(this.assignments.values()).filter((assignment) => assignment.agentId === agentId);
    }
    /**
...
exports.TaskAssigner = TaskAssigner;
//# sourceMappingURL=TaskAssigner.js.map