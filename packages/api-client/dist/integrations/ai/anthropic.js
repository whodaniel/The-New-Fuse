...
export class AnthropicIntegration {
    ...
    async execute(action, params = {}) {
        ...
        switch (action) {
            case 'chat_completion':
                return this.createChatCompletion(params.model || this.config.model, params.messages, params.system, // Anthropic supports a top-level system prompt
     ...
     */
    async createChatCompletion(model, messages, // Content can be complex for vision
    system, options) {
        if (!model) {
        ...
        try {
            ...
            console.error(`Anthropic chat completion error for model ${model}: ${error instanceof Error ? error.message : String(error)}`, error?.response?.data);
 ...
 */
export function createAnthropicIntegration(config = {}) {
    const defaultConfig = {
        id: 'anthropic',
        name: 'Anthropic',
