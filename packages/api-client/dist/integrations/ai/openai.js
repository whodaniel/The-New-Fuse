...
export class OpenAIIntegration {
    ...
    async connect() {
        ...
        try {
            ...
            this.isConnected = false;
            console.error(`OpenAI connection error: ${(error instanceof Error ? error.message : String(error))}`, error.response?.data);
    ...
    async execute(action, params) {
        ...
        switch (action) {
            case 'chat_completion':
                return this.createChatCompletion(params.model || this.config.model || 'gpt-3.5-turbo', params.messages, params.options // Includes temperature, max_tokens etc.
                );
            case 'completion': // Legacy
                return this.createCompletion(params.model || this.config.model || 'text-davinci-003', // Adjust default legacy model if needed
                params.prompt, params.options);
            case 'embedding':
                return this.createEmbedding(params.model || 'text-embedding-ada-002', // Default embedding model
                params.input);
            case 'list_models':
    ...
    async createChatCompletion(model, messages, options) {
        ...
        try {
            ...
            if (error instanceof Error) {
                console.error(`OpenAI chat completion error for model ${model}: ${error.message}`, error.response?.data);
    ...
    async createCompletion(model, prompt, options) {
        ...
        try {
            ...
            if (error instanceof Error) {
                console.error(`OpenAI completion error for model ${model}: ${error.message}`, error.response?.data);
    ...
    async createEmbedding(model, input) {
        ...
        try {
            ...
            if (error instanceof Error) {
                console.error(`OpenAI embedding error for model ${model}: ${error.message}`, error.response?.data);
    ...
    */
    async listModels() {
        try {
            ...
            if (error instanceof Error) {
                console.error(`OpenAI list models error: ${error.message}`, error.response?.data);
 ...
 */
export function createOpenAIIntegration(config = {}) {
    const defaultConfig = {
        id: 'openai',
        name: 'OpenAI',
