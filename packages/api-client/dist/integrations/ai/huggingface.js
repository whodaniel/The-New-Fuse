...
export class HuggingFaceIntegration {
    ...
    async runInference(model, inputs, options) {
        ...
        try {
            ...
            console.error(`Hugging Face inference error for model ${model}: ${error instanceof Error ? error.message : String(error)}`, error?.response?.data);
 ...
 */
export function createHuggingFaceIntegration(config = {}) {
    const defaultConfig = {
        id: 'huggingface',
        name: 'Hugging Face',
