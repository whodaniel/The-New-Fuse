...
export class StabilityAIIntegration {
    ...
    async execute(action, params) {
        ...
        switch (action) {
            case 'text_to_image':
                return this.generateImageFromText(params.prompt, params.negative_prompt, params.engine, params.options);
            case 'image_to_image':
                return this.generateImageFromImage(params.init_image, params.prompt, params.negative_prompt, params.engine, params.options);
            case 'upscale':
                return this.upscaleImage(params.image, params.width, params.height, params.engine, params.options);
            case 'inpainting':
                return this.inpaintImage(params.image, params.mask, params.prompt, params.negative_prompt, params.engine, params.options);
            case 'masking':
     ...
     */
    async generateImageFromText(prompt, negative_prompt, engine, options = {}) {
        try {
     ...
     */
    async generateImageFromImage(init_image, prompt, negative_prompt, engine, options = {}) {
        try {
            const engineId = engine || this.config.engine || 'stable-diffusion-xl-1024-v1-0';
     ...
     */
    async upscaleImage(image, width, height, engine, options = {}) {
        try {
     ...
     */
    async inpaintImage(image, mask, prompt, negative_prompt, engine, options = {}) {
        try {
            const engineId = engine || this.config.engine || 'stable-diffusion-xl-1024-v1-0';
            const formData = new FormData();
