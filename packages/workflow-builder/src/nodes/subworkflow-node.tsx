...
import { useWorkflowHost } from '../host/context.js';
import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from '../ui/select-compat.js';
import { BaseNode } from './base-node.js';

const SubworkflowNode: React.FC<NodeProps> = memo(({ id, data }) => {
  // Supplied by the host surface rather than imported from one app's
  // services — see host/types.ts for why.
  const { workflowDatabase: workflowDatabaseService, workflowExecution: workflowExecutionService } =
    useWorkflowHost();
