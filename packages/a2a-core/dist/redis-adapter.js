...
import { A2AConnectionError, A2AError, A2AMessageSchema, A2AMessageType, A2APriority, A2ATimeoutError, A2AValidationError, AgentHeartbeatSchema, AgentRegistrationSchema, AgentStatus, ConversationSchema, } from './types.js';
let A2ARedisAdapter = A2ARedisAdapter_1 = class A2ARedisAdapter extends EventEmitter {
    config;
    redisService;
    logger = new Logger(A2ARedisAdapter_1.name);
    keyPrefix;
    ...
    ensureConnected() {
        if (!this.isConnected) {
            throw new A2AConnectionError('A2A Redis Adapter is not connected - operating in degraded mode');
        }
    }
    ...
    async handleRedisMessage(channel, message) {
        try {
            ...
            if (channel.endsWith(':messages:global')) {
                ...
                if (parsedMessage.type === A2AMessageType.DATA_RESPONSE &&
...
A2ARedisAdapter = A2ARedisAdapter_1 = __decorate([
    Injectable(),
    __metadata("design:paramtypes", [Object, UnifiedRedisService])
], A2ARedisAdapter);
export { A2ARedisAdapter };
