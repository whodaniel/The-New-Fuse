...
import { A2AMessageType, A2APriority, AgentStatus, LoadBalancingStrategy, } from './types.js';
let A2AService = A2AService_1 = class A2AService {
    ...
    signatureWrapper;
    pointerResolver;
    logger = new Logger(A2AService_1.name);
    agentRegistry = new Map();
    activeConnections = new Map(); // WebSocket connections
    ...
    heartbeatInterval;
    registryCleanupInterval;
    metricsInterval;
    constructor(configService, ap2ProtocolService, redisService, signatureWrapper, pointerResolver) {
     ...
     */
    async resolvePointers(message) {
        const resolved = {};
        if (!message.resourcePointers)
            return resolved;
        for (const [key, pointer] of Object.entries(message.resourcePointers)) {
            try {
                ...
                this.logger.error(`Failed to resolve pointer ${key}: ${error.message}`);
                throw error;
            }
        }
        return resolved;
     ...
     */
    async sendSecureMessage(fromAgent, toAgent, type, payload, options) {
