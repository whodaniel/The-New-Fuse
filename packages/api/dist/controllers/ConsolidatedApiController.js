...
let ConsolidatedApiController = class ConsolidatedApiController {
    ...
    async getDashboardData(user) {
        ...
        systemHealth,] = await Promise.all([
            ...
            this.workflowService.getWorkflows(user.id).then(workflows => workflows.filter(w => w.status === 'ACTIVE').length),
            ...
            this.workflowService.getWorkflows(user.id).then(workflows => workflows.filter(w => w.status === 'ACTIVE').slice(0, 5)),
