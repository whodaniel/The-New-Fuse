...
class TaskSchedulerService {
    constructor(config, logger, redisClient, // RedisClientManager instance
    emitActivityEvent) {
        ...
        const run = () => {
            void this.pollAndQueueTasks().catch((error) => {
                ...
                if (this.taskPollFailureCount <= 3 || this.taskPollFailureCount % 10 === 0) {
                    this.logger('warn', 'TASK-POLL', `Task polling failed (${this.taskPollFailureCount}): ${error.message || String(error)}`);
                }
            });
        };
    ...
    targetQueueForTask(task) {
        ...
        if (lane === 'suggestion_vote')
            return this.config.REDIS_KEYS.SUGGESTIONS;
        if (lane === 'changelog_suggestion')
            return this.config.REDIS_KEYS.CHANGELOG;
        if (lane === 'kanban_delivery')
            return this.config.REDIS_KEYS.KANBAN;
        if (this.isRealtimeDispatchCandidate(task))
            return this.config.REDIS_KEYS.TASKS_REALTIME;
