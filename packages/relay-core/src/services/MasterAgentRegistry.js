...
const VCIssuanceService_1 = require("./VCIssuanceService");
const BlockchainService_1 = require("./shared/BlockchainService");
...
class MasterAgentRegistry extends events_1.EventEmitter {
    ...
    constructor(prisma, logger, blockchainConfig, vcPrivateKey) {
        ...
        if (blockchainConfig) {
            this.blockchainService = new BlockchainService_1.BlockchainService(blockchainConfig, logger);
    ...
    async registerAgent(profile) {
      ...
      const completeProfile: MasterAgentProfile = {
        id: agentId,`, name, profile.name || `${profile.type || 'Unknown'}`, Agent, type, profile.type || database_js_1.AgentType.BASIC, status, database_js_1.AgentStatus.INACTIVE, // Start as inactive until onboarding complete
            description, profile.description || Registered, $, { profile, : .type }, agent, systemPrompt, profile.systemPrompt, configuration, profile.configuration || {}, userId, profile.userId || 'system', // Default to system user
            platform, profile.platform || 'unknown', location, profile.location || 'unknown', capabilities, {
            ...
            }, lastSeen, new Date(), lastHeartbeat, new Date(), metrics, {
                totalTasks: 0,
                completedTasks: 0,
                failedTasks: 0,
                ...
                escalationCount: 0,
                ...profile.metrics
            }, todoList, [], registeredAt, new Date(), lastVerified, new Date(), verificationHash, '', onboardingCompleted, false, protocolChecklistCompleted, false, onChainData, {
                isOnChain: false,
                tokenId: undefined,
        ...
        finally // 1. Store in Prisma database (single source of truth)
         { }
        ;
        ...
        if (!dbAgent) {
            `
        throw new Error(Failed to retrieve agent ${agentId}`;
            after;
            creation.;
            ;
        }
  ...
  async startUniversalOnboarding(agentId: string): Promise<{
    ...
    const workspaceAccess = '/Users/<owner>/Desktop/A1-Inter-LLM-Com/The New Fuse';
    ...
    const onboardingSteps: OnboardingChecklistItem[] = [
      {
        ...
        status: 'pending',
        orientationMaterials: [`, $, { workspaceAccess } `/README.md,
          ${workspaceAccess}` / DOCUMENTATION_INDEX.md);
                }
                {
                    ...
                    'Master Registry Integration',
                        description;
                    'Confirm agent is properly registered in Master Agent Registry',
                        verificationMethod;
                    'automatic',
                        status;
                    'pending',
                ...
                }
                {
                    id: 'relay_system_connection',
                        requirement;
                    'Relay System Connection',
                        description;
                    'Establish connection to The New Fuse relay communication system',
                        verificationMethod;
                    'automatic',
                        status;
                    'pending',
                ...
                }
                {
                    id: 'heartbeat_protocol_setup',
                        requirement;
                    'Heartbeat Protocol Setup',
                        description;
                    'Initialize heartbeat monitoring and anti-stagnation systems',
                        verificationMethod;
                    'automatic',
                        status;
                    'pending',
                ...
                }
                {
                    id: 'handoff_template_training',
                        requirement;
                    'Handoff Template Training',
                        description;
                    'Learn standardized agent handoff protocols',
                        verificationMethod;
                    'manual',
                        status;
                    'pending',
...
`;
    ...
    async addAgentTodo(agentId, todoData) {
        ...
        `
`;
        const todoId = $, { agentId };
        `_todo_${Date.now()}_${Math.random().toString(36).substr(2, 9)};
    ...
    if (todoData.category !== 'onboarding') { // Don't clutter Task table with onboarding todos
      const prismaTask = await this.prisma.task.create({
        ...
        todo.category,
            status;
        this.convertTodoStatusToTaskStatus(todo.status),
            priority;
        this.convertTodoPriorityToTaskPriority(todo.priority),
            assignedToId;
        agentId,
            userId;
        agent.userId,
...
return todoId;
generateVerificationHash(profile, MasterAgentProfile);
string;
{
    const data = {
        id: profile.id,
...
buildAgentMerkleTree(nodes, { hash: string, agentId: string }[]);
AgentMerkleNode;
{
    if (nodes.length === 0) {
        return {
    ...
    while (merkleNodes.length > 1) {
        ...
        for (let i = 0; i < merkleNodes.length; i += 2) {
            ...
            newLevel.push({} `
          hash: combinedHash,`, agentId, $, { left, : .agentId } `_${right.agentId},
          left,
          right
        });
      }
  ...
  private async syncAgentToSpreadsheet(agent: MasterAgentProfile): Promise<string | undefined> {
    ...
    async;
    syncAllAgentsToSpreadsheet();
    Promise < void  > {
        : .spreadsheetIntegration.enabled, return: ,
        try: {
  ...
  private performSystemHealthCheck(): void {
    ...
    // Log health issues`
    if (healthScore < 90) {
        this.logger.warn(System, health, degraded, $, { healthScore } `% (${stalledAgents} stalled agents));
    }
  }
  ...
  private async loadExistingAgents(): Promise<void> {
    try {
      ...
      this.updateSystemMetrics();`, this.logger.info(Loaded, $, { existingAgents, : .length } ` existing agents from database);
    } catch (error) {
  ...
  async issueAgentCredential(
    ...
    try {
      ...
      `;
            this.logger.info(Verifiable, Credential, issued);
            for (agent; ; )
                : $;
            {
                agentId;
  ...
  async verifyAgentCredential(credential: any): Promise<boolean> {
    ...
    VCIssuanceService_1.VCIssuanceService | null;
    {
    ...
    BlockchainService_1.BlockchainService | null;
    {
    ...
    };
    {
        ...
        async;
        verifyAgentCompliance(agentId, string);
        Promise < boolean > {
            const: agent = this.agentProfiles.get(agentId),
        ...
        convertToLegacyType(type, database_js_1.AgentType);
        any;
        {
            const mapping = {
                'BASIC': 'LLM',
        ...
        convertToLegacyStatus(status, database_js_1.AgentStatus);
        any;
        {
            const mapping = {
                'ACTIVE': 'ACTIVE',
        ...
        extractLegacyCapabilities(capabilities, MasterAgentProfile['capabilities']);
        string[];
        {
        ...
        convertTodoStatusToTaskStatus(status, MasterAgentTodo['status']);
        database_js_1.TaskStatus;
        {
            const mapping = {
                'pending': 'PENDING',
        ...
        convertTodoPriorityToTaskPriority(priority, MasterAgentTodo['priority']);
        database_js_1.TaskPriority;
        {
            const mapping = {
                'low': 'LOW',
                'medium': 'MEDIUM',
        ...
        generateAgentId(type, string, platform, string);
        string;
        {
...
async;
mintAgentNFT(profile, MasterAgentProfile);
Promise < OnChainAgentData | null > {
    const: agentNFTContract = this.blockchainService?.getContract('AgentNFTContract'),
    const: wallet = this.blockchainService?.getWallet(),
    const: blockchainConfig = this.blockchainService?.getConfig(),
    if(, agentNFTContract) { }
} || !wallet || !blockchainConfig;
{
...
try {
    ...
    const gasPrice = ethers_1.ethers.utils.parseUnits(blockchainConfig.maxGasPrice, 'gwei');
    // Mint the Agent NFT
...
async;
updateAgentOnChainMetadata(agentId, string, newMetadataURI, string);
Promise < boolean > {} `
    const agent = this.agentProfiles.get(agentId);`;
if (!agent || !agent.onChainData.isOnChain || !agent.onChainData.tokenId) {
    ...
    try {
      ...
      `;
    this.logger.info(Updated, on - chain, metadata);
    for (agent; $; { agentId } `);
      return true;
    } catch (error) {
...
getBlockchainConfig();
BlockchainService_1.BlockchainConfig;
{
    if (!this.blockchainService) {
        throw new Error('BlockchainService not initialized');
    }
...
async;
getOnChainAgentData(agentId, string);
Promise < OnChainAgentData | null > {
    const: agent = this.agentProfiles.get(agentId),
...
generateSystemStatusReport();
{
    ...
    agentSummary: {
        ...
        recentActivity: {
            agentId: string;
            lastSeen: Date;
            status: database_js_1.AgentStatus;
        }
        [];
    }
    ;
...
groupAgentsByStatus(agents, MasterAgentProfile[]);
Record < database_js_1.AgentStatus, number > {
    const: groups
};
{
    'ACTIVE';
...
return groups;
groupAgentsByPlatform(agents, MasterAgentProfile[]);
...
return groups;
groupTodosByStatus(todos, MasterAgentTodo[]);
Record < string, number > {
    const: groups
};
{ }
