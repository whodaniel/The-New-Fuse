...
export interface AgentAchievement {
  ...
  evidenceURI: string;
  issuer: string;
  value: number; // Quantifiable value if applicable
}
...
export class VCIssuanceService extends EventEmitter {
  ...
  async issueCredential(request: VCIssuanceRequest): Promise<VerifiableCredential> {
    try {
      ...
      const capabilities = await this.gatherCapabilityEvidence(
        request.agentId,
   ...
   */
  private async gatherCapabilityEvidence(
    agentId: string,
    requestedCapabilities: string[]
  ...
  private determineProficiencyLevel(score: number): ProficiencyLevel {
    if (score >= 0.9) return ProficiencyLevel.MASTER;
    if (score >= 0.8) return ProficiencyLevel.EXPERT;
    if (score >= 0.7) return ProficiencyLevel.ADVANCED;
    if (score >= 0.5) return ProficiencyLevel.INTERMEDIATE;
   ...
   */
  private async generateProof(credentialSubject: CredentialSubject): Promise<Proof> {
   ...
   */
  private isStructurallyValid(credential: VerifiableCredential): boolean {
    return !!(
      credential['@context'] &&
      credential.id &&
      credential.type &&
   ...
   */
  private isTrustedIssuer(issuerId: string): boolean {
   ...
   */
  async getAgentCredentials(agentId: string): Promise<VerifiableCredential[]> {
