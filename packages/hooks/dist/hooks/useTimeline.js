...
export const useTimeline = () => {
    ...
    const loadTimelineData = useCallback(async (recordId) => {
        const rows = await parse(await fetch(`/api/timeline/events?recordId=${encodeURIComponent(recordId)}`));
        setEventsCount(rows.length);
        setIsActive(rows.length > 0);
        return rows;
    }, []);
    const createBranch = useCallback(async (params) => {
        return parse(await fetch('/api/timeline/events', {
            method: 'POST',
    ...
    const mergeBranch = useCallback(async (params) => {
        return parse(await fetch('/api/timeline/events', {
            method: 'POST',
            headers: JSON_HEADERS,
            body: JSON.stringify({
    ...
    const executeWorkflowStep = useCallback(async (params) => {
        return parse(await fetch('/api/timeline/events', {
            method: 'POST',
            headers: JSON_HEADERS,
            body: JSON.stringify({
