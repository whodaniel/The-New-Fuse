...
const redis_agent_registry_js_1 = require("./redis-agent-registry.js");
// Mock Redis client
...
describe('RedisAgentRegistry', () => {
    ...
    let mockMulti;
    beforeEach(() => {
        registry = new redis_agent_registry_js_1.RedisAgentRegistry({ redisUrl: 'redis://mock' });
        mockMulti = {
    ...
    describe('register', () => {
        it('should register a new agent and index its capabilities', async () => {
            ...
            expect(mockMulti.hSet).toHaveBeenCalledWith('tnf:registry:agents:agent-1', expect.objectContaining({ id: 'agent-1' }));
            expect(mockMulti.expire).toHaveBeenCalledWith('tnf:registry:agents:agent-1', 60);
            expect(mockMulti.sAdd).toHaveBeenCalledWith('tnf:registry:agents:capability:test-capability-1', 'agent-1');
        ...
        it('should update an existing agent and its capability indexes', async () => {
            ...
            await registry.register(agent1);
            expect(mockMulti.sAdd).toHaveBeenCalledWith('tnf:registry:agents:capability:test-capability-1', 'agent-1');
            expect(mockMulti.sRem).toHaveBeenCalledWith('tnf:registry:agents:capability:old-capability', 'agent-1');
        });
    });
    describe('unregister', () => {
        it('should unregister an agent and remove it from capability sets', async () => {
            ...
            expect(mockRedisClient.multi).toHaveBeenCalledTimes(1);
            expect(mockMulti.sRem).toHaveBeenCalledWith('tnf:registry:agents:capability:test-capability-1', 'agent-1');
    ...
    describe('updateHeartbeat', () => {
        it('should update the lastSeen timestamp and refresh the TTL', async () => {
            ...
            expect(mockRedisClient.multi).toHaveBeenCalledTimes(1);
            expect(mockMulti.hSet).toHaveBeenCalledWith('tnf:registry:agents:agent-1', 'lastSeen', expect.any(Number));
            expect(mockMulti.expire).toHaveBeenCalledWith('tnf:registry:agents:agent-1', 60);
            expect(mockMulti.exec).toHaveBeenCalledTimes(1);
        });
    });
...
//# sourceMappingURL=redis-agent-registry.test.js.map