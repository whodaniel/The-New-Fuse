...
class NotificationProcessor extends BaseProcessor_1.BaseProcessor {
    ...
    // Inject other dependencies (e.g., WebSocket service for UI updates)
        ...
        this.logger = new core_1.Logger(`NotificationProcessor [Agent ${this.agentId}]`);
    ...
    async process(message) {
        if (message.type !== types_1.MessageType.NOTIFICATION || typeof message.content !== 'object' || message.content === null) {
        ...
        const notification = message.content; // Type assertion, consider validation
        if (!notification.level || !notification.text) {
            ...
            await this.alertService.warn(`Received incomplete notification: ${JSON.stringify(notification)}`, `Agent ${this.agentId} / NotificationProcessor`, { originalMessageId: message.id });
        ...
        try {
            ...
            await this.alertService.error(`Failed to process notification ${message.id}`, `Agent ${this.agentId} / NotificationProcessor`, { error: error.message, originalNotification: notification });
        }
    }
}
