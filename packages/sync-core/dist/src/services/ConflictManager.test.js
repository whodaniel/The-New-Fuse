...
const SyncDatabaseService_1 = require("../database/SyncDatabaseService");
...
describe('ConflictManager', () => {
    ...
    beforeEach(async () => {
        const module = await testing_1.Test.createTestingModule({
            providers: [
                ConflictManager_1.ConflictManager,
                {
                    provide: drizzle_1.DrizzleClient,
                    useValue: mockDrizzleClient,
                },
                {
                    provide: SyncDatabaseService_1.SyncDatabaseService,
                    useValue: mockSyncDatabaseService,
                },
            ],
        }).compile();
        service = module.get(ConflictManager_1.ConflictManager);
        drizzle = module.get(drizzle_1.DrizzleClient);
        syncDb = module.get(SyncDatabaseService_1.SyncDatabaseService);
        // Reset all mocks
        jest.clearAllMocks();
    });
    describe('detectConflict', () => {
        it('should return null when no existing sync state', async () => {
            ...
            const result = await service.detectConflict('agent', 'test-agent-id', { version: 1 }, { version: 2 });
        ...
        it('should return null when no conflict detected', async () => {
            ...
            const originalCalculateChecksum = service.calculateChecksum;
            service.calculateChecksum = jest.fn().mockReturnValue('same-checksum');
            const result = await service.detectConflict('agent', 'test-agent-id', { version: 1 }, { version: 1 });
            expect(result).toBeNull();
            // Restore original method
            service.calculateChecksum = originalCalculateChecksum;
        });
        ...
        it('should handle tenant-specific conflicts', async () => {
            ...
            syncDb.getSyncState.mockResolvedValue(mockSyncState);
            await service.detectConflict('agent', 'test-agent-id', { version: 1 }, { version: 2 }, tenantId);
    ...
    describe('resolveConflict', () => {
        it('should resolve conflict with latest_wins strategy', async () => {
            ...
            syncDb.upsertSyncState.mockResolvedValue({});
        ...
        it('should throw error for already resolved conflict', async () => {
            ...
            await expect(service.resolveConflict(conflictId, 'latest_wins', 'test-resolver')).rejects.toThrow('already resolved');
        });
        it('should throw error for manual resolution strategy', async () => {
            ...
            await expect(service.resolveConflict(conflictId, 'manual', 'test-resolver')).rejects.toThrow('Manual resolution strategy requires human intervention');
        });
    });
    ...
    describe('autoResolveConflicts', () => {
        it('should auto-resolve version conflicts with latest_wins', async () => {
            ...
            expect(resolveConflictSpy).toHaveBeenCalledWith('conflict-1', 'latest_wins', 'system-auto-resolver', { tenantId, permissions: [], isolationLevel: 'strict' });
    ...
    describe('error handling', () => {
        it('should handle database errors in detectConflict', async () => {
            ...
            await expect(service.detectConflict('agent', 'test-agent-id', { version: 1 }, { version: 2 })).rejects.toThrow('Database connection failed');
        });
        it('should handle transaction errors in resolveConflict', async () => {
            const error = new Error('Transaction failed');
            drizzle.$transaction.mockRejectedValue(error);
            await expect(service.resolveConflict('conflict-id', 'latest_wins', 'test-resolver')).rejects.toThrow('Transaction failed');
        });
    });
    describe('private methods', () => {
        ...
        it('should merge versions correctly', () => {
            ...
            const merged = service.mergeVersions(local, remote);
        ...
        it('should detect merge compatibility', () => {
            ...
            expect(service.canMerge(compatible1, compatible2)).toBe(true);
            expect(service.canMerge(incompatible1, incompatible2)).toBe(false);
            expect(service.canMerge(null, compatible1)).toBe(false);
            expect(service.canMerge('string', compatible1)).toBe(false);
        });
    });
});
//# sourceMappingURL=ConflictManager.test.js.map