...
exports.batchAutoResolutionExample = batchAutoResolutionExample;
exports.customResolutionExample = customResolutionExample;
exports.detectAgentConflictExample = detectAgentConflictExample;
exports.errorHandlingExample = errorHandlingExample;
exports.manualConflictResolutionExample = manualConflictResolutionExample;
exports.monitoringAndMaintenanceExample = monitoringAndMaintenanceExample;
exports.workflowIntegrationExample = workflowIntegrationExample;
...
async function setupConflictManager() {
    const drizzle = new drizzle_1.DrizzleClient();
    const syncDb = new SyncDatabaseService_1.SyncDatabaseService(drizzle);
    const conflictManager = new ConflictManager_1.ConflictManager(drizzle, syncDb);
...
async function detectAgentConflictExample() {
    ...
    try {
        // Detect conflict
        const conflict = await conflictManager.detectConflict('agent', 'agent-123', localAgentVersion, remoteAgentVersion, 'tenant-456');
        if (conflict) {
            ...
            if (conflict.conflictType === 'version') {
                const resolution = await conflictManager.resolveConflict(conflict.id, 'latest_wins', 'system-auto-resolver');
...
async function manualConflictResolutionExample() {
    ...
    try {
        ...
        for (const conflict of pendingConflicts) {
            ...
            if (strategy !== 'manual') {
                try {
                    const resolution = await conflictManager.resolveConflict(conflict.id, strategy, tenantContext.userId, tenantContext);
...
async function monitoringAndMaintenanceExample() {
    ...
    try {
        ...
        const agentConflicts = await conflictManager.getResourceConflicts('agent', 'problematic-agent-id', 'tenant-456');
...
async function workflowIntegrationExample() {
    ...
    async function syncAgentConfiguration(agentId, localConfig, remoteConfig, tenantId) {
        try {
            // Step 1: Detect conflicts
            const conflict = await conflictManager.detectConflict('agent', agentId, localConfig, remoteConfig, tenantId);
            if (!conflict) {
            ...
            if (conflict.conflictType === 'version') {
                const resolution = await conflictManager.resolveConflict(conflict.id, 'latest_wins', 'sync-service');
    ...
    // Example usage
    const result = await syncAgentConfiguration('agent-789', { version: 1, config: { temp: 0.7 } }, { version: 2, config: { temp: 0.8 } }, 'tenant-123');
...
async function customResolutionExample() {
    ...
    async function resolveTemplateConflict(conflictId) {
        try {
            ...
            if (localVersion.structure === remoteVersion.structure) {
                ...
                const resolution = await conflictManager.resolveConflict(conflictId, strategy, 'template-resolver');
...
//# sourceMappingURL=ConflictManager.example.js.map