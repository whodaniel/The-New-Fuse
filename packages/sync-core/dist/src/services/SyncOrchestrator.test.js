...
describe('SyncOrchestrator', () => {
    ...
    beforeEach(async () => {
        ...
        const module = await testing_1.Test.createTestingModule({
            providers: [
                SyncOrchestrator_1.SyncOrchestrator,
                {
                    provide: infrastructure_1.UnifiedRedisService,
                ...
                {
                    provide: database_1.DrizzleService,
                    useValue: mockDbService,
                },
                {
                    provide: prompt_templating_1.PromptTemplateServiceImpl,
                    useValue: mockPromptTemplateService,
                },
            ],
        }).compile();
        service = module.get(SyncOrchestrator_1.SyncOrchestrator);
        redisService = module.get(infrastructure_1.UnifiedRedisService);
        wsService = module.get('IWebSocketService');
        dbService = module.get(database_1.DrizzleService);
        promptTemplateService = module.get(prompt_templating_1.PromptTemplateServiceImpl);
        // Setup default mocks
    ...
    describe('syncTenantData', () => {
        ...
        it('should sync tenant data successfully', async () => {
            ...
            expect(redisService.publish).toHaveBeenCalledWith('sync:user-1:agent', expect.objectContaining({
            ...
            expect(wsService.sendMessage).toHaveBeenCalledWith('user-1', expect.objectContaining({
                type: 'sync_update',
        ...
        it('should throw error for unknown tenant', async () => {
            const testData = { id: 'test-1', name: 'Test Data' };
            await expect(service.syncTenantData('unknown-tenant', 'agent', testData)).rejects.toThrow('Tenant context not found: unknown-tenant');
        });
    });
    describe('syncGlobalData', () => {
        ...
        it('should sync global data successfully', async () => {
            ...
            expect(dbService.$executeRaw).toHaveBeenCalled();
            expect(redisService.publish).toHaveBeenCalledWith('sync:global:template', expect.objectContaining({
                operation: expect.objectContaining({
                    resourceType: 'template',
                    data: testData,
                }),
            }));
            expect(wsService.broadcastToAllUsers).toHaveBeenCalledWith(expect.objectContaining({
                type: 'sync_update',
                payload: expect.objectContaining({
                    dataType: 'template',
                    global: true,
    ...
    describe('resolveConflict', () => {
        ...
        it('should resolve conflict with merge strategy', async () => {
            ...
            expect(resolution.resolvedData).toEqual(expect.objectContaining({
    ...
    describe('error handling', () => {
        ...
        it('should handle Redis publish errors gracefully', async () => {
            ...
            const testData = { id: 'test-1', name: 'Test Data' };
            await expect(service.syncTenantData('user-1', 'agent', testData)).rejects.toThrow('Redis error');
        });
        it('should handle database errors gracefully', async () => {
            dbService.$executeRaw.mockRejectedValue(new Error('Database error'));
            const testData = { id: 'test-1', name: 'Test Data' };
            await expect(service.syncTenantData('user-1', 'agent', testData)).rejects.toThrow('Database error');
        });
...
//# sourceMappingURL=SyncOrchestrator.test.js.map