 ...
 */
let ConflictManager = ConflictManager_1 = class ConflictManager extends events_1.EventEmitter {
    constructor(drizzle, syncDb) {
        super();
        this.drizzle = drizzle;
        this.syncDb = syncDb;
        this.logger = new common_1.Logger(ConflictManager_1.name);
        this.errorHandler = new core_error_handling_1.BaseErrorHandler({
     ...
     */
    async detectConflict(resourceType, resourceId, localVersion, remoteVersion, tenantId) {
        try {
            ...
            const conflictError = {
                ...
                timestamp: new Date(),
                retryable: true,
                severity: core_error_handling_1.ErrorSeverity.HIGH,
                category: core_error_handling_1.ErrorCategory.SYSTEM,
                resourceType,
                resourceId,
                tenantId,
     ...
     */
    async resolveConflict(conflictId, strategy, resolvedBy, context) {
        try {
            return await this.drizzle.$transaction(async (tx) => {
                // Get the conflict
                ...
                await this.syncDb.upsertSyncState({
                    ...
                    version: resolution.resolvedData?.version || 1,
     ...
     */
    async autoResolveConflicts(tenantId) {
        let resolvedCount = 0;
        try {
            ...
            for (const conflict of pendingConflicts) {
                try {
                    ...
                    if (autoStrategy) {
                        await this.resolveConflict(conflict.id, autoStrategy, 'system-auto-resolver', tenantId ? { tenantId, permissions: [], isolationLevel: 'strict' } : undefined);
                        resolvedCount++;
                    }
                }
                catch (error) {
     ...
     */
    async getConflictStatistics(tenantId) {
        try {
     ...
     */
    determineConflictType(currentState, localVersion, remoteVersion) {
        // Version conflict
     ...
     * Apply resolution strategy
     */
    async applyResolutionStrategy(strategy, conflict, context) {
        switch (strategy) {
            case 'latest_wins':
                return {
                    strategy,
     ...
     */
    determineAutoResolutionStrategy(conflict) {
     ...
     */
    canMerge(localVersion, remoteVersion) {
...
exports.ConflictManager = ConflictManager;
exports.ConflictManager = ConflictManager = ConflictManager_1 = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [typeof (_a = typeof drizzle_1.DrizzleClient !== "undefined" && drizzle_1.DrizzleClient) === "function" ? _a : Object, SyncDatabaseService_1.SyncDatabaseService])
], ConflictManager);
