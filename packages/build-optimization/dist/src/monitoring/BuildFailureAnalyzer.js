...
export class BuildFailureAnalyzer extends EventEmitter {
     ...
     */
    async analyzeBuildFailure(buildResult, buildMetrics, systemResources, errorLogs = []) {
        // Perform failure analysis
        const analysis = this.performFailureAnalysis(buildResult, buildMetrics, systemResources, errorLogs);
     ...
     */
    analyzeMemoryIssues(memoryHistory, systemResources) {
     ...
     */
    generateSystemRecommendations(systemResources, buildMetrics) {
        const recommendations = [];
     ...
     */
    generateTroubleshootingLog(analysis, buildMetrics, systemResources) {
...
`;
     ...
     */
    performFailureAnalysis(buildResult, buildMetrics, systemResources, errorLogs) {
     ...
     */
    matchesPattern(pattern, buildResult, buildMetrics, errorLogs) {
        ...
        if (pattern.memoryPatterns) {
            ...
            if (peakUsage && buildMetrics.peakMemoryUsage < peakUsage)
                return false;
            if (threshold && buildMetrics.performanceStats.memoryViolations === 0)
                return false;
        ...
        if (pattern.eventPatterns) {
            ...
            const hasEventPattern = pattern.eventPatterns.every((eventType) => eventTypes.includes(eventType));
            if (!hasEventPattern)
                return false;
        }
        return true;
     ...
     */
    createAnalysisFromPattern(pattern, buildResult, buildMetrics, systemResources) {
     ...
     */
    performHeuristicAnalysis(buildResult, buildMetrics, systemResources, errorLogs) {
     ...
     */
    generateRecommendations(analysis, buildMetrics, systemResources) {
     ...
     */
    calculateMemoryGrowthRate(memoryHistory) {
     ...
     */
    getFailureDescription(type, buildResult, buildMetrics) {
        switch (type) {
            case 'memory-exhaustion':
     ...
     */
    getRootCause(type, buildMetrics, systemResources) {
        switch (type) {
            case 'memory-exhaustion':
                if (buildMetrics.peakMemoryUsage > systemResources.totalMemory * 0.9) {
     ...
     */
    getMemoryExhaustionRecommendations(buildMetrics, systemResources) {
        return [
            {
                category: 'memory',
                priority: 'critical',
     ...
     */
    getCompilationErrorRecommendations(analysis) {
        return [
            {
     ...
     */
    getDependencyErrorRecommendations(analysis) {
        return [
            {
                category: 'dependency',
                priority: 'high',
     ...
     */
    getTimeoutRecommendations(buildMetrics, systemResources) {
        return [
            {
                category: 'configuration',
                priority: 'medium',
     ...
     */
    getSystemResourceRecommendations(systemResources) {
        return [
            {
                category: 'system',
                priority: 'medium',
     ...
     */
    getGenericRecommendations(buildMetrics, systemResources) {
        return [
            {
                category: 'configuration',
                priority: 'low',
...
//# sourceMappingURL=BuildFailureAnalyzer.js.map