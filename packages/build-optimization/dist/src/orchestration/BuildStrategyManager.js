...
export class BuildStrategyManager {
     ...
     */
    selectOptimalStrategy(systemResources, environment) {
    ...
    createConfigurationFromEnvironment() {
        ...
        return {
            ...
            monorepoConfig: {
                ...
                packageManager: process.env.PACKAGE_MANAGER || 'bun',
            },
        };
    }
    /**
    ...
    validateConfiguration(config) {
        ...
        if (!validStrategies.includes(config.strategy)) {
            throw new ConfigurationValidationError(`strategy must be one of: ${validStrategies.join(', ')}`, 'strategy');
        }
    }
    /**
    ...
    mergeWithDefaults(config) {
        ...
        const result = {
            ...
            ideConfig: {
                ...
                estimatedMemoryUsage: config.ideConfig?.estimatedMemoryUsage ??
            ...
            monorepoConfig: {
                ...
                priorityPackages: config.monorepoConfig?.priorityPackages ??
                ...
                maxPackagesPerStage: config.monorepoConfig?.maxPackagesPerStage ??
                    defaults.monorepoConfig?.maxPackagesPerStage ??
                    8,
                workspaceRoot: config.monorepoConfig?.workspaceRoot ??
     ...
     */
    getUltraMemoryOptimizedStrategy(systemResources) {
        return {
     ...
     */
    getDevelopmentStrategy(systemResources) {
        return {
     ...
     */
    getProductionStrategy(systemResources) {
        const availableMemoryGB = systemResources.availableMemory / 1024;
        return {
     ...
     */
    getCIOptimizedStrategy(systemResources) {
        const availableMemoryGB = systemResources.availableMemory / 1024;
     ...
     */
    getDefaultStrategyForEnvironment(environment) {
        switch (environment) {
            case 'production':
                return 'production';
            case 'ci':
     ...
     */
    parseEnvNumber(envVar, defaultValue) {
     ...
     */
    parseEnvBoolean(envVar, defaultValue) {
        const value = process.env[envVar];
        if (!value)
            return defaultValue;
