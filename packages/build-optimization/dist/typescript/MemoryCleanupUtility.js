...
export class MemoryCleanupUtility {
    ...
    async performCleanup() {
        ...
        try {
            ...
            for (let attempt = 1; attempt <= this.config.maxCleanupAttempts; attempt++) {
                ...
                if (memoryFreed >= this.config.memoryThreshold ||
    ...
    clearModuleCache() {
        try {
            if (!this.config.clearModuleCache)
                return;
     ...
     */
    async clearTypeScriptCompilerMemory() {
        try {
            if (!this.config.clearTypeScriptCache)
                return;
     ...
     */
    verifyMemoryCleanup(beforeMemory, afterMemory) {
        ...
        return (memoryFreed >= this.config.memoryThreshold ||
            percentageFreed >= 10 ||
     ...
     */
    async executeCleanupStep(errors) {
        try {
     ...
     */
    isTypeScriptRelated(key) {
        return (key.includes('typescript') ||
            key.includes('.ts') ||
            key.includes('tsc') ||
     ...
     * Clear TypeScript diagnostic cache
     */
    clearTypeScriptDiagnosticCache() {
        try {
...
//# sourceMappingURL=MemoryCleanupUtility.js.map