...
export class WorkflowValidator {
  ...
  private warningRules: Map<string, (workflow: UnifiedWorkflow) => ValidationWarning[]> = new Map();
   ...
   */
  async validateWorkflow(workflow: UnifiedWorkflow): Promise<WorkflowValidationResult> {
    ...
    try {
      ...
      for (const [ruleName, rule] of this.validationRules.entries()) {
        try {
          ...
          errors.push({
            ...
            message: `Validation rule ${ruleName} failed: ${getErrorMessage(error)}`,
            severity: 'error'
          });
        }
      }
      ...
      for (const [ruleName, rule] of this.warningRules.entries()) {
        try {
          ...
          this.logger.warn(`Warning rule ${ruleName} failed: ${getErrorMessage(_error)}`);
        }
      }
      ...
      this.logger.error(`Workflow validation failed: ${getErrorMessage(error)}`);
      errors.push({
   ...
   */
  private validateWorkflowDefinition(definition: WorkflowDefinition): ValidationError[] {
    const errors: ValidationError[] = [];
  ...
  private validateNode(node: WorkflowNode): ValidationError[] {
    ...
    if (!Object.values(WorkflowNodeType).includes(node.type)) {
      errors.push({
   ...
   */
  private validateNodeConfiguration(node: WorkflowNode): ValidationError[] {
    const errors: ValidationError[] = [];
  ...
  private validateAgentTaskNode(node: WorkflowNode): ValidationError[] {
    const errors: ValidationError[] = [];
  ...
  private validateAgentHandoffNode(node: WorkflowNode): ValidationError[] {
    const errors: ValidationError[] = [];
    const config = node.config as AgentHandoffNodeConfig;
  ...
  private validateConditionNode(node: WorkflowNode): ValidationError[] {
    const errors: ValidationError[] = [];
    ...
    if (!config.expression || typeof config.expression !== 'string') {
      ...
      try {
        ...
        errors.push({
          ...
          message: `Invalid condition expression: ${getErrorMessage(error)}`,
          nodeId: node.id,
          severity: 'error'
        });
      }
  ...
  private validateLLMPromptNode(node: WorkflowNode): ValidationError[] {
    const errors: ValidationError[] = [];
    const config = node.config as LLMPromptNodeConfig;
  ...
  private validateAPICallNode(node: WorkflowNode): ValidationError[] {
    const errors: ValidationError[] = [];
   ...
   */
  private validateNodeInputsOutputs(node: WorkflowNode): ValidationError[] {
    const errors: ValidationError[] = [];
    ...
    for (const input of node.inputs) {
      ...
      if (!Object.values(VariableType).includes(input.type)) {
        errors.push({
    ...
    for (const output of node.outputs) {
      ...
      if (!Object.values(VariableType).includes(output.type)) {
        errors.push({
   ...
   * Validate connection
   */
  private validateConnection(connection: WorkflowConnection, definition: WorkflowDefinition): ValidationError[] {
    const errors: ValidationError[] = [];
    ...
    if (connection.condition) {
      try {
        ...
        errors.push({
          ...
          message: `Invalid connection condition: ${getErrorMessage(error)}`,
          connectionId: connection.id,
          severity: 'error'
        });
      }
   ...
   * Validate variable
   */
  private validateVariable(variable: WorkflowVariable): ValidationError[] {
    const errors: ValidationError[] = [];
  ...
  private validateVariableValue(value: any, type: VariableType): boolean {
    switch (type) {
  ...
  private initializeValidationRules(): void {
    ...
    this.validationRules.set('must_have_start', (workflow) => {
      if (!this.config.requireStartNode) return [];
    ...
    this.validationRules.set('must_have_end', (workflow) => {
      if (!this.config.requireEndNode) return [];
    ...
    this.validationRules.set('no_circular_references', (workflow) => {
      if (this.config.allowCircularReferences) return [];
  ...
  private initializeWarningRules(): void {
    ...
    this.warningRules.set('unreachable_nodes', (workflow) => {
      ...
      // Find start nodes
      const startNodes = workflow.definition.nodes.filter(n => n.type === WorkflowNodeType.START);
      ...
      for (const node of workflow.definition.nodes) {
        if (node.type !== WorkflowNodeType.START && !reachable.has(node.id)) {
          warnings.push({
   ...
   */
  private generatePerformanceWarnings(workflow: UnifiedWorkflow): ValidationWarning[] {
    if (!this.config.enablePerformanceValidation) return [];
  ...
  private calculateMaxDepth(definition: WorkflowDefinition): number {
    const visited = new Set<string>();
    const startNodes = definition.nodes.filter(n => n.type === WorkflowNodeType.START);
   ...
   * Public API methods
   */
  async validateNodeConfig(nodeType: WorkflowNodeType, config: NodeConfiguration): Promise<ValidationError[]> {
