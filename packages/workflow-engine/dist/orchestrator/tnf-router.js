...
exports.TNFRouter = void 0;
const event_emitter_1 = require("@nestjs/event-emitter");
const agent_1 = require("@the-new-fuse/agent");
const relay_core_1 = require("@the-new-fuse/relay-core");
const system_queue_service_1 = require("./services/system-queue.service");
const TelemetryService_js_1 = require("../telemetry/TelemetryService.js");
class RouterAgentInbox {
    constructor(agentId, redisService, eventEmitter) {
...
class TNFRouter {
    constructor(redisService, config = {}, eventEmitter) {
        this.redisService = redisService;
        this.config = {
        ...
        this.registry = new agent_1.RedisAgentRegistry({ redisUrl: this.config.redisUrl });
        this.systemQueue = new system_queue_service_1.SystemQueueService(this.config.redisUrl);
    ...
    async handleIngressMessage(rawMessage) {
        try {
            const envelope = (0, relay_core_1.validateTNFEnvelope)(JSON.parse(rawMessage));
    ...
    async routeToInbox(agentId, envelope) {
        try {
            ...
            await this.logTaskExecution(envelope.id, `Inbox delivery failed for ${agentId}`, {
                level: 'warn',
                stage: 'dispatch',
                metadata: { agentId, error: String(error?.message || error) },
            });
     ...
     */
    async selectBestAgent(agentIds) {
     ...
     */
    async routeSystemTask(capability, envelope) {
        ...
        try {
            ...
            switch (capability) {
                case 'system:email':
                    queueName = system_queue_service_1.SystemQueueName.EMAIL;
                    jobType = 'send-email';
                    break;
                case 'system:report':
                    queueName = system_queue_service_1.SystemQueueName.REPORT_GENERATION;
                    break;
                case 'system:data-sync':
                    queueName = system_queue_service_1.SystemQueueName.DATA_SYNC;
                    break;
                case 'system:agent-exec':
                    queueName = system_queue_service_1.SystemQueueName.AGENT_EXECUTION;
                    break;
                default:
     ...
     */
    async forwardToAgent(agentId, envelope) {
     ...
     * Send error response
     */
    async sendError(originalEnvelope, code, message) {
        const errorEnvelope = (0, relay_core_1.createTNFEnvelope)('response', { agentId: 'orchestrator', role: 'orchestrator' }, originalEnvelope.from, {
            success: false,
            error: { code, message },
        }, {
    ...
    async logTaskExecution(taskId, message, options) {
        await TelemetryService_js_1.telemetry.emitAndPersistTaskExecutionLog({
            taskId,
            message,
...
exports.TNFRouter = TNFRouter;
//# sourceMappingURL=tnf-router.js.map