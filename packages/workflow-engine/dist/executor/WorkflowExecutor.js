...
class WorkflowExecutor extends events_1.EventEmitter {
    ...
    async executeStep(node, context, execution) {
        ...
        try {
            ...
            nodeExecution.status = WorkflowTypes_js_1.NodeExecutionStatus.RUNNING;
            ...
            // Mark as completed
            nodeExecution.status = WorkflowTypes_js_1.NodeExecutionStatus.COMPLETED;
            ...
            nodeExecution.status = WorkflowTypes_js_1.NodeExecutionStatus.FAILED;
            nodeExecution.completedAt = new Date();
    ...
    async executeNodeByType(node, context, nodeExecution) {
        switch (node.type) {
            case WorkflowTypes_js_1.WorkflowNodeType.START:
                return this.executeStartNode(node, context);
            case WorkflowTypes_js_1.WorkflowNodeType.END:
            ...
            case WorkflowTypes_js_1.WorkflowNodeType.AGENT_COORDINATION:
            ...
            case WorkflowTypes_js_1.WorkflowNodeType.LOOP:
                return this.executeLoopNode(node, context);
            case WorkflowTypes_js_1.WorkflowNodeType.PARALLEL:
                return this.executeParallelNode(node, context);
            case WorkflowTypes_js_1.WorkflowNodeType.MERGE:
            ...
            case WorkflowTypes_js_1.WorkflowNodeType.DATABASE_QUERY:
                return this.executeDatabaseQueryNode(node, context);
            case WorkflowTypes_js_1.WorkflowNodeType.FILE_OPERATION:
                return this.executeFileOperationNode(node, context);
            case WorkflowTypes_js_1.WorkflowNodeType.RELAY_MESSAGE:
                return this.executeRelayMessageNode(node, context);
            case WorkflowTypes_js_1.WorkflowNodeType.WEBHOOK:
                return this.executeWebhookNode(node, context);
            case WorkflowTypes_js_1.WorkflowNodeType.EMAIL:
            ...
            case WorkflowTypes_js_1.WorkflowNodeType.CODE_GENERATION:
                return this.executeCodeGenerationNode(node, context);
            case WorkflowTypes_js_1.WorkflowNodeType.ANALYSIS:
                return this.executeAnalysisNode(node, context);
            case WorkflowTypes_js_1.WorkflowNodeType.SANDBOX_EXECUTION:
                return this.executeSandboxExecutionNode(node, context);
            case WorkflowTypes_js_1.WorkflowNodeType.CUSTOM:
    ...
    async executeConditionNode(node, context) {
        ...
        try {
            ...
            throw new Error(`Condition evaluation failed: ${(0, errorUtils_js_1.getErrorMessage)(error)}`);
        }
    }
    ...
    async executeAPICallNode(node, context) {
        ...
        try {
            ...
            throw new Error(`API call failed: ${(0, errorUtils_js_1.getErrorMessage)(error)}`);
        }
    }
    async executeDatabaseQueryNode(node, _context) {
        ...
        if (!completeness.complete && config.failOnIncompleteQuery === true) {
            throw new Error(`Database query result incomplete: missing fields ${completeness.missingFields.join(', ')}`);
        }
    ...
    applyQueryDefaults(result, defaultValues) {
    ...
    evaluateQueryCompleteness(result, requiredFields) {
    ...
    createNodeExecution(node, executionId) {
        ...
        return {
            ...
            nodeId: node.id,
            status: WorkflowTypes_js_1.NodeExecutionStatus.PENDING,
            startedAt: new Date(),
            retryCount: 0,
            metadata: {
                nodeType: node.type,
    ...
    async retryNodeExecution(node, context, execution, nodeExecution) {
        ...
        nodeExecution.status = WorkflowTypes_js_1.NodeExecutionStatus.RETRYING;
