...
const WorkflowExecutor_js_1 = require("../executor/WorkflowExecutor.js");
...
const dev_loop_guard_js_1 = require("../utils/dev-loop-guard.js");
class UnifiedWorkflowEngine extends events_1.EventEmitter {
    constructor(config, drizzle /* DrizzleClient */, agentRegistry, heartbeatService, logger, workflowQueue) {
        ...
        this.logger = logger;
        this.executor = new WorkflowExecutor_js_1.WorkflowExecutor({
            maxParallelNodes: 1,
    ...
    async recoverInterruptedExecutions() {
        try {
            ...
            const interruptedExecutions = await this.drizzle.workflowExecution.findMany({
                where: {
                    status: WorkflowTypes_js_1.WorkflowExecutionStatus.RUNNING,
                },
            });
            ...
            for (const dbExecution of interruptedExecutions) {
                try {
                    ...
                    this.logger.error(`❌ Failed to recover execution ${dbExecution.id}: ${(0, errorUtils_js_1.getErrorMessage)(err)}`);
                }
            }
        }
        catch (error) {
            this.logger.error(`❌ Failed to recover interrupted executions: ${(0, errorUtils_js_1.getErrorMessage)(error)}`);
        }
    }
    /**
    ...
    async executeWorkflow(workflowId, input = {}, triggeredBy = 'system', triggerType = 'manual') {
        const devLoopIteration = (0, dev_loop_guard_js_1.assertDevLoopBudget)(`workflow.${workflowId}`, input);
        return TelemetryService_js_1.telemetry.startActiveSpan('executeWorkflow', async (span) => {
            ...
            try {
                ...
                const execution = await this.createExecution(workflow, { ...input, devLoopIteration }, triggeredBy, triggerType);
                ...
                this.logger.error(`❌ Failed to start workflow execution: ${(0, errorUtils_js_1.getErrorMessage)(error)}`);
                throw error;
            }
        });
    }
    ...
    async cancelExecution(executionId, reason = 'User cancelled') {
        ...
        execution.status = WorkflowTypes_js_1.WorkflowExecutionStatus.CANCELLED;
    ...
    async loadWorkflow(workflowId) {
        try {
            ...
            this.logger.error(`Failed to load workflow ${workflowId}: ${(0, errorUtils_js_1.getErrorMessage)(error)}`);
            return null;
        }
    }
    async createExecution(workflow, input, triggeredBy, triggerType) {
        ...
        const execution = {
            ...
            status: WorkflowTypes_js_1.WorkflowExecutionStatus.PENDING,
    ...
    async startExecution(execution, workflow) {
        this.activeExecutions.set(execution.id, execution);
        if (this.workflowQueue) {
            await this.workflowQueue.addStartWorkflowJob({
        ...
        this.emitWorkflowEvent({
            id: `event_${Date.now()}`,
            type: WorkflowTypes_js_1.WorkflowEventType.WORKFLOW_STARTED,
            workflowId: execution.workflowId,
            executionId: execution.id,
            timestamp: new Date(),
    ...
    async runExecutionInProcess(execution, workflow) {
        ...
        execution.status = WorkflowTypes_js_1.WorkflowExecutionStatus.RUNNING;
        ...
        const completedNodeIds = new Set(execution.nodeExecutions
            .filter((nodeExecution) => nodeExecution.status === WorkflowTypes_js_1.NodeExecutionStatus.COMPLETED)
        ...
        while (pendingNodes.length > 0) {
            if (!this.isRunning || execution.status !== WorkflowTypes_js_1.WorkflowExecutionStatus.RUNNING) {
                return;
            }
            if (steps >= maxSteps) {
            ...
            if (nodeExecution.status === WorkflowTypes_js_1.NodeExecutionStatus.COMPLETED) {
                ...
                this.emitWorkflowEvent({
                    ...
                    type: WorkflowTypes_js_1.WorkflowEventType.NODE_COMPLETED,
                    workflowId: execution.workflowId,
                    executionId: execution.id,
                    nodeId: node.id,
                    timestamp: new Date(),
                ...
                for (const nextNode of this.findNextNodes(node, activeWorkflow, execution.context)) {
                    if ((this.nodeAllowsRepeat(nextNode) || !completedNodeIds.has(nextNode.id)) &&
                ...
                execution.status = WorkflowTypes_js_1.WorkflowExecutionStatus.FAILED;
                await this.finalizeExecution(execution);
                return;
            }
        }
        execution.status = WorkflowTypes_js_1.WorkflowExecutionStatus.COMPLETED;
        execution.completedAt = new Date();
    ...
    getStartNodes(workflow) {
        const startNodes = workflow.definition.nodes.filter((node) => node.type === WorkflowTypes_js_1.WorkflowNodeType.START);
        if (startNodes.length > 0) {
            return startNodes;
        }
        const targetNodeIds = new Set(workflow.definition.connections.map((connection) => connection.targetNodeId));
        return workflow.definition.nodes.filter((node) => !targetNodeIds.has(node.id));
    }
    getMaxSequencerSteps(workflow) {
        const configuredMax = workflow.definition.settings?.maxSequencerSteps;
        if (typeof configuredMax === 'number' && configuredMax > 0) {
            return configuredMax;
        }
        return Math.max(workflow.definition.nodes.length * 10, 100);
    }
    getNodeMaxVisits(node) {
        const configuredMax = node.config?.maxVisits;
        return typeof configuredMax === 'number' && configuredMax > 0 ? configuredMax : 1;
    }
    nodeAllowsRepeat(node) {
        return node.config?.allowRepeat === true || node.type === WorkflowTypes_js_1.WorkflowNodeType.LOOP;
    }
    async executeNodeInSequence(node, execution) {
        const startedAt = new Date();
        this.appendExecutionLog(execution, 'info', `Executing node: ${node.name}`, node.id);
        this.emitWorkflowEvent({
            id: `event_${Date.now()}_${this.generateSecureId()}`,
            type: WorkflowTypes_js_1.WorkflowEventType.NODE_STARTED,
            workflowId: execution.workflowId,
            executionId: execution.id,
            nodeId: node.id,
            timestamp: startedAt,
        ...
        try {
            ...
            return {
                ...
                nodeId: node.id,
                status: WorkflowTypes_js_1.NodeExecutionStatus.COMPLETED,
            ...
            const executionError = {
                ...
                message: (0, errorUtils_js_1.getErrorMessage)(error),
            ...
            this.appendExecutionLog(execution, 'error', `Node execution failed: ${(0, errorUtils_js_1.getErrorMessage)(error)}`, node.id);
            this.emitWorkflowEvent({
                id: `event_${Date.now()}_${this.generateSecureId()}`,
                type: WorkflowTypes_js_1.WorkflowEventType.NODE_FAILED,
                workflowId: execution.workflowId,
                executionId: execution.id,
                nodeId: node.id,
                timestamp: completedAt,
            ...
            return {
                id: `node_exec_${Date.now()}_${this.generateSecureId()}`,
                nodeId: node.id,
                status: WorkflowTypes_js_1.NodeExecutionStatus.FAILED,
                input: this.extractNodeExecutionInput(node, execution.context),
                error: executionError,
                startedAt,
                completedAt,
    ...
    extractNodeExecutionInput(node, context) {
        if (!Array.isArray(node.inputs) || node.inputs.length === 0) {
            return {};
        }
        return node.inputs.reduce((input, nodeInput) => {
    ...
    createSerializableNodeOutput(node, output) {
        if (node.type === WorkflowTypes_js_1.WorkflowNodeType.END && output && typeof output === 'object') {
            return {
                ...output,
    ...
    recordSkippedNode(execution, node, reason) {
        const timestamp = new Date();
        execution.nodeExecutions.push({
            id: `node_exec_${Date.now()}_${this.generateSecureId()}`,
            nodeId: node.id,
            status: WorkflowTypes_js_1.NodeExecutionStatus.SKIPPED,
            startedAt: timestamp,
    ...
    appendExecutionLog(execution, level, message, nodeId, metadata = {}) {
        execution.logs.push({
    ...
    async failExecution(execution, error) {
        ...
        this.appendExecutionLog(execution, 'error', `Workflow execution failed: ${(0, errorUtils_js_1.getErrorMessage)(error)}`);
        await this.finalizeExecution(execution);
    }
    stop() {
    ...
    async executeNode(node, context) {
        (0, dev_loop_guard_js_1.assertDevLoopBudget)(`workflow-node.${node.type}`, context);
        return TelemetryService_js_1.telemetry.startActiveSpan('executeNode', async (span) => {
    ...
    evaluateExpression(expression, variables) {
        ...
        try {
            ...
            throw new Error(`Expression evaluation error: ${(0, errorUtils_js_1.getErrorMessage)(error)}`);
        }
    }
        ...
        for (const connection of connections) {
            ...
            if (connection.condition) {
                try {
                    ...
                    this.logger.warn(`Connection condition evaluation failed: ${(0, errorUtils_js_1.getErrorMessage)(error)}`);
                    continue;
                }
            }
    ...
    connectionMatchesSelectedOutput(currentNode, connection, context) {
        if (currentNode.type !== WorkflowTypes_js_1.WorkflowNodeType.CONDITION) {
            return true;
        }
        const nodeOutput = context.temporaryData[currentNode.id] || context.variables[`${currentNode.id}Output`];
    ...
    async finalizeExecution(execution) {
        ...
        const eventType = execution.status === WorkflowTypes_js_1.WorkflowExecutionStatus.COMPLETED
            ? WorkflowTypes_js_1.WorkflowEventType.WORKFLOW_COMPLETED
            : execution.status === WorkflowTypes_js_1.WorkflowExecutionStatus.FAILED
                ? WorkflowTypes_js_1.WorkflowEventType.WORKFLOW_FAILED
                : WorkflowTypes_js_1.WorkflowEventType.WORKFLOW_CANCELLED;
        this.emitWorkflowEvent({
            id: `event_${Date.now()}`,
            type: eventType,
            workflowId: execution.workflowId,
    ...
    async loadExecution(executionId) {
        try {
            ...
            this.logger.error(`Failed to load execution ${executionId}: ${(0, errorUtils_js_1.getErrorMessage)(error)}`);
            return null;
        }
    }
    generateSecureId() {
