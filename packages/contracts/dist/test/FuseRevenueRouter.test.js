...
describe('FuseRevenueRouter Financials', function () {
    let router, agencyRegistry, sftContract;
    let owner, agencyAdmin, investor, affiliate;
    beforeEach(async function () {
        [owner, agencyAdmin, investor, affiliate] = await hardhat_1.ethers.getSigners();
        // Deploy Mocks
        // 1. Registry
        const Registry = await hardhat_1.ethers.getContractFactory('FuseAgencyRegistry');
        agencyRegistry = await Registry.deploy();
        // 2. SFT
        const SFT = await hardhat_1.ethers.getContractFactory('AgentSponsorshipSFT');
        // Mock UD Registry address for SFT constructor
        sftContract = await SFT.deploy(owner.address);
        // 3. Router
        const Router = await hardhat_1.ethers.getContractFactory('FuseRevenueRouter');
    ...
    async function setupAgentShares(agentId, amount) {
