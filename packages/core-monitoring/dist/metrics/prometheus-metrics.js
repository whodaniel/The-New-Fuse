...
export class PrometheusMetrics extends EventEmitter {
     ...
     */
    setDatabaseConnectionPool(database, idle, active, total) {
        if (!this.initialized)
            return;
     ...
     * Set job queue size
     */
    setJobQueueSize(queueName, pending, active, completed, failed) {
        if (!this.initialized)
            return;
     ...
     */
    recordJobProcessing(jobType, duration, success) {
        if (!this.initialized)
            return;
     ...
     */
    setWebSocketConnections(connected, disconnected = 0) {
        if (!this.initialized)
            return;
     ...
     * Set agent count
     */
    setAgentCount(active, inactive, type = 'all') {
        if (!this.initialized)
            return;
     ...
     */
    recordWorkflowExecution(workflowType, success) {
        if (!this.initialized)
            return;
