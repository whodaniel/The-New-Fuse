...
class PerformanceDashboard {
    ...
    addAlert(alert) {
        ...
        const exists = this.alerts.some(a => a.metric === alert.metric &&
    ...
    evaluateFrontendHealth(frontend) {
        ...
        if (poorCount > 2)
            return 'poor';
        if (poorCount > 0 || needsImprovementCount > 3)
            return 'degraded';
        return 'good';
    }
    /**
    ...
    evaluateBackendHealth(backend) {
        if (backend.errors.rate > 0.05 || backend.latency.p95 > 2000)
            return 'poor';
        if (backend.errors.rate > 0.01 || backend.latency.p95 > 1000)
            return 'degraded';
        return 'good';
    }
    /**
    ...
    evaluateDatabaseHealth(database) {
        ...
        const slowQueryRate = database.queries.total > 0
            ? database.queries.slow / database.queries.total
            : 0;
        if (poolUtil > 90 || slowQueryRate > 0.2)
            return 'poor';
        if (poolUtil > 75 || slowQueryRate > 0.1)
            return 'degraded';
        return 'good';
    }
    /**
