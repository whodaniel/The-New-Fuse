...
exports.ErrorFilterTemplate = exports.MonitoringInterceptorTemplate = void 0;
/**
 * Monitoring interceptor implementation
 * This is a template that can be used in NestJS services
 */
class MonitoringInterceptorTemplate {
    /**
     * Intercept method that can be used in NestJS CallHandler
     */
    static async intercept(context, next, services) {
        ...
        try {
            ...
            // Record metrics
            if (services.metrics) {
                services.metrics.recordHttpRequest(request.method, request.route?.path || request.url, response.statusCode, duration);
            }
            return result;
        }
        catch (error) {
            ...
            if (services.metrics) {
                services.metrics.recordHttpRequest(request.method, request.route?.path || request.url, response.statusCode || 500, duration);
            }
...
exports.MonitoringInterceptorTemplate = MonitoringInterceptorTemplate;
/**
 * Error filter template for NestJS
 */
class ErrorFilterTemplate {
    static catch(exception, host, services) {
...
exports.ErrorFilterTemplate = ErrorFilterTemplate;
