...
const EnhancedTimelineView = ({ events, branches, workflows, onEventClick, onCreateBranch, onAddNote, onEventMove, }) => {
    ...
    const handleDrag = (0, react_1.useCallback)((event) => {
        ...
        const node = d3.select(event.sourceEvent.currentTarget); // Use currentTarget
    ...
    const initializeDrag = (0, react_1.useCallback)(() => {
        return d3
            .drag() // Use 'any' for element type to avoid strict typing issues
            .subject(function (d) {
            ...
            const translate = transform
                ? transform.substring(transform.indexOf('(') + 1, transform.indexOf(')')).split(',')
                : ['0', '0'];
            ...
            const node = d3.select(this);
            node.attr('cursor', 'grab').transition().duration(200).attr('filter', null);
            d3.select(svgRef.current).selectAll('.guideline').remove();
        ...
        const zoomBehavior = d3
            ...
            if (event.type === 'mousedown') {
                ...
                return (event.button === 0 &&
                    !event.ctrlKey &&
                    (target === svg.node() || target.classList.contains('grid-background')));
            }
            // Allow touch zoom/pan
            return event.type.startsWith('touch');
        })
            .on('zoom', handleZoom);
        svg.call(zoomBehavior).call(zoomBehavior.transform, d3.zoomIdentity); // Initialize zoom
        // Prevent double-click zoom
        svg.on('dblclick.zoom', null);
        // Prevent default touch behavior like scrolling when interacting with SVG background
        svg.on('touchstart.zoom', (event) => {
            const target = event.target;
            if (target === svg.node() || target.classList.contains('grid-background')) {
                event.preventDefault();
            }
        }, { passive: false });
        svg.on('touchmove.zoom', (event) => {
            const target = event.target;
            if (target === svg.node() || target.classList.contains('grid-background')) {
                event.preventDefault();
            }
    ...
    const timelineRanges = (0, react_1.useMemo)(() => {
        return workflows.map((workflow) => ({
            ...
            color: colors.workflow.bg, // Use ColorValue type assertion
            label: workflow.name,
        }));
    }, [workflows]);
    (0, react_1.useEffect)(() => {
        ...
        Object.entries(colors).forEach(([type, colorDef]) => {
            if (typeof colorDef === 'object' && 'gradient' in colorDef) {
                const color = colorDef; // Type assertion
                const gradient = defs
        ...
        const nodeDataEvents = events.map((e) => ({
            id: e.id,
            type: e.type,
        ...
        try {
            ...
            if (useStratify) {
                ...
                if (!useStratify && rootCandidates.length !== 1) {
                    // Log warning if not already logged by stratify failure
                    console.warn("Data doesn't form a single tree structure or lacks parent IDs. Creating virtual root.");
                }
                const virtualRootData = {
                    id: 'virtual-root',
                    type: 'VIRTUAL',
                ...
                rootNode = d3.hierarchy(virtualRootData, (d) => {
                    if (d.id === 'virtual-root')
                        return rootChildren;
            ...
            rootNode = d3.hierarchy(virtualRootData, (d) => d.id === 'virtual-root'
                ? rootChildren
                : nodeDataEvents.filter((child) => child.parentId === d.id));
        }
        ...
        g.append('g')
            ...
            .join('path')
            .attr('d', (d) => linkGenerator(d)) // Cast d
            .attr('stroke', (d) => {
            // Use D3 type
            const sourceEvent = d.source.data;
            const targetEvent = d.target.data;
            const branchColor = colors.branch; // Type assertion
            ...
            .attr('stroke-width', (d // Use D3 type
        ) => (d.source.data.mergedFrom?.includes(d.target.data.id) ? 3 : 2))
            .attr('fill', 'none')
            .attr('pointer-events', 'none')
            .style('stroke-dasharray', (d // Use D3 type
        ...
        const nodeGroup = g
            ...
            .join('g')
            .attr('class', 'node')
            .attr('transform', (d) => `translate(${d.y},${d.x})`) // Use D3 type, swap x/y for horizontal tree
            .attr('cursor', 'grab')
            // Fix the call method typing by using call directly with a type assertion to bypass strict typing
            .call(initializeDrag()); // Type assertion to avoid TypeScript errors
        const cardWidth = 160;
        const cardHeight = 90;
        ...
        nodeGroup.each(function (d) {
            ...
            const card = node.append('g').attr('class', 'card');
            ...
            if (titleElement) {
                ...
                if (titleElement.getComputedTextLength() > cardWidth - 20) {
                    // Check length
            ...
            card
                ...
                .attr('font-size', '12px')
                .attr('pointer-events', 'none')
                .text((0, date_fns_1.format)(nodeData.timestamp, 'MMM d, yyyy')); // Use nodeData
            ...
            badge
                .append('text')
                .attr('x', 35)
                .attr('y', 13) // Center text in badge
                .attr('text-anchor', 'middle')
                .attr('fill', 'white')
                .attr('font-size', '10px')
        ...
        initializeDrag,
        onEventMove,
    ]); // Added onEventMove dependency
    return ((0, jsx_runtime_1.jsxs)("div", { className: "relative w-full h-full bg-gray-50 rounded-xl shadow-lg overflow-hidden flex flex-col", children: [(0, jsx_runtime_1.jsxs)("div", { className: "h-16 bg-white shadow-sm px-6 flex items-center justify-between z-10 flex-shrink-0", children: [(0, jsx_runtime_1.jsxs)("div", { className: "flex items-center space-x-4", children: [(0, jsx_runtime_1.jsx)("h2", { className: "text-xl font-bold text-gray-800", children: "Timeline View" }), (0, jsx_runtime_1.jsxs)("div", { className: "flex items-center space-x-2", children: [(0, jsx_runtime_1.jsx)("button", { onClick: () => {
                                            const svg = d3.select(svgRef.current);
                                            // Create a temporary zoom behavior just for resetting the transform
                                            const tempZoomBehavior = d3.zoom();
                                            ...
                                            const mainGroup = svg.select('g.main-group');
                                            mainGroup.transition().duration(300).attr('transform', d3.zoomIdentity.toString());
                                        }, className: "px-3 py-1 text-sm bg-gray-100 hover:bg-gray-200 rounded-md transition-colors", children: "Reset Zoom" }), (0, jsx_runtime_1.jsxs)("span", { className: "text-sm text-gray-500", children: [Math.round(zoom * 100), "%"] })] })] }), (0, jsx_runtime_1.jsxs)("div", { className: "flex items-center space-x-4", children: [(0, jsx_runtime_1.jsx)("button", { onClick: () => setShowBranchForm(true), disabled: !selectedEvent, className: "px-4 py-2 bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-lg shadow-lg hover:shadow-xl transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed", children: "Create Branch" }), (0, jsx_runtime_1.jsx)("button", { onClick: () => setShowNoteForm(true), className: "px-4 py-2 bg-gradient-to-r from-purple-500 to-purple-600 text-white rounded-lg shadow-lg hover:shadow-xl transition-all duration-200", children: "Add Note" })] })] }), (0, jsx_runtime_1.jsx)("div", { className: "px-6 py-4 border-b border-gray-200 flex-shrink-0", children: (0, jsx_runtime_1.jsx)(TimelineSlider_1.default, { events: processedEvents, ranges: timelineRanges, onAddNote: onAddNote ? handleAddNote : undefined, 
                    ...
                    onEventClick: onEventClick
                        ...
                        : undefined, onRangeSelect: (_start, _end) => {
                        // Handle range selection if needed
                        console.log('Range selected:', _start, _end);
                    } }) }), (0, jsx_runtime_1.jsxs)("div", { ref: containerRef, className: "flex-1 overflow-hidden relative", children: [' ', (0, jsx_runtime_1.jsx)("svg", { ref: svgRef, className: "absolute top-0 left-0 w-full h-full" }), ' '] }), showBranchForm && selectedEvent && ((0, jsx_runtime_1.jsx)("div", { className: "fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50", children: (0, jsx_runtime_1.jsxs)("div", { className: "bg-white rounded-lg p-6 w-96", children: [(0, jsx_runtime_1.jsxs)("h3", { className: "text-lg font-bold mb-4", children: ["Create New Branch from \"", selectedEvent.data.title, "\""] }), (0, jsx_runtime_1.jsx)("input", { type: "text", value: newBranchName, onChange: (e) => setNewBranchName(e.target.value), placeholder: "Branch name", className: "w-full px-3 py-2 border rounded-lg mb-4" }), (0, jsx_runtime_1.jsxs)("div", { className: "flex justify-end space-x-2", children: [(0, jsx_runtime_1.jsx)("button", { onClick: () => {
                                        setShowBranchForm(false);
                                        setNewBranchName('');
                                    }, className: "px-4 py-2 text-gray-600 hover:text-gray-800", children: "Cancel" }), (0, jsx_runtime_1.jsx)("button", { onClick: () => {
                                        if (newBranchName.trim() && selectedEvent && onCreateBranch) {
                                            onCreateBranch(selectedEvent.id, newBranchName);
                                            setShowBranchForm(false);
                                            setNewBranchName('');
                                        }
                                    }, className: "px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600", children: "Create" })] })] }) })), showNoteForm && ((0, jsx_runtime_1.jsx)("div", { className: "fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50", children: (0, jsx_runtime_1.jsxs)("div", { className: "bg-white rounded-lg p-6 w-96", children: [(0, jsx_runtime_1.jsx)("h3", { className: "text-lg font-bold mb-4", children: "Add Note" }), (0, jsx_runtime_1.jsx)("textarea", { value: noteContent, onChange: (e) => setNoteContent(e.target.value), placeholder: "Enter your note...", className: "w-full px-3 py-2 border rounded-lg mb-4 h-32 resize-none" }), (0, jsx_runtime_1.jsxs)("div", { className: "flex justify-end space-x-2", children: [(0, jsx_runtime_1.jsx)("button", { onClick: () => {
                                        setShowNoteForm(false);
                                        setNoteContent('');
                                    }, className: "px-4 py-2 text-gray-600 hover:text-gray-800", children: "Cancel" }), (0, jsx_runtime_1.jsx)("button", { onClick: () => {
                                        if (noteContent.trim()) {
                                    ...
                                    }, className: "px-4 py-2 bg-purple-500 text-white rounded-lg hover:bg-purple-600", children: "Add Note" })] })] }) })), (0, jsx_runtime_1.jsx)("div", { className: `tooltip-container ${hoveredEvent && hoveredPosition ? 'visible' : 'hidden'}`, style: hoveredPosition
                    ? { left: `${hoveredPosition.x + 10}px`, top: `${hoveredPosition.y + 10}px` }
                    : {}, children: (0, jsx_runtime_1.jsx)("div", { className: "tooltip-content", children: hoveredEvent && events.find((e) => e.id === hoveredEvent)?.data.description }) })] }));
};
exports.EnhancedTimelineView = EnhancedTimelineView;
exports.default = exports.EnhancedTimelineView;
//# sourceMappingURL=EnhancedTimelineView.js.map