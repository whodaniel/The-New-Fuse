...
class ErrorFactory {
     ...
     */
    static createApplicationError(message, code, severity = IErrorHandling_js_1.ErrorSeverity.MEDIUM, category = IErrorHandling_js_1.ErrorCategory.UNKNOWN, retryable = false, metadata, originalError) {
        return new CustomErrors_js_1.ApplicationError(message, code, severity, category, retryable, metadata, originalError);
    }
    /**
    ...
    static fromHttpResponse(statusCode, responseData, endpoint, method) {
        ...
        switch (statusCode) {
            ...
            case 404:
                return new CustomErrors_js_1.NotFoundError(responseData?.resourceType, responseData?.resourceId);
            case 409:
    ...
    static fromError(error, context) {
        ...
        if (errorName.includes('timeout') || errorMessage.includes('timeout')) {
            return new CustomErrors_js_1.TimeoutError(context?.metadata?.endpoint, context?.metadata?.timeout, error);
        }
        // Auth errors
        if (errorName.includes('auth') || errorMessage.includes('auth')) {
            return new CustomErrors_js_1.AuthenticationError(error.message, 2000, context?.metadata, error);
        }
        // Validation errors
        ...
        if (errorName.includes('database') || errorName.includes('sql')) {
            return new CustomErrors_js_1.DatabaseError(error.message, context?.operation, undefined, error);
        }
        // Default to generic system error
        return new CustomErrors_js_1.SystemError(error.message, 5000, IErrorHandling_js_1.ErrorSeverity.HIGH, true, {
            ...context?.metadata,
    ...
    static externalService(serviceName, message, statusCode, originalError) {
        return new CustomErrors_js_1.ExternalServiceError(serviceName, message, statusCode, originalError);
    }
    /**
    ...
    static apiIntegration(provider, endpoint, statusCode, message, originalError) {
        return new CustomErrors_js_1.ApiIntegrationError(provider, endpoint, statusCode, message, originalError);
    }
    /**
