...
Satellite **apps** that extend the product (games, overseers, adapters) live
outside the monorepo in sibling `TNF-Extensions/`, reached via `apps/extensions`
(symlink). Resolve that root with:
...
const root = resolveTnfExtensionsRoot({ repoRoot: process.cwd() });
const apps = listSatelliteAppDirs(root);
```

`ExtensionSystemFactory.createDefault(repoRoot, logger)` sets
`extensionDirectory` to that satellite root when present.
