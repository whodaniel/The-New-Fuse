...
class ExtensionLoader extends events_1.EventEmitter {
     ...
     */
    async performLoad(extensionPath, options, startTime) {
        const warnings = [];
        ...
        this.emitExtensionEvent({
            type: ExtensionTypes_js_1.ExtensionEventType.EXTENSION_LOADED,
     ...
     */
    async loadManifest(extensionPath) {
     ...
     */
    convertPackageJsonToManifest(packageJson) {
        return {
            ...
            type: packageJson.extensionType || ExtensionTypes_js_1.ExtensionType.CUSTOM,
     ...
     * Validate extension
     */
    async validateExtension(manifest, extensionPath) {
        ...
        const dangerousPermissions = manifest.permissions?.filter(p => [ExtensionTypes_js_1.PermissionType.FILE_SYSTEM_WRITE, ExtensionTypes_js_1.PermissionType.SYSTEM_INFO].includes(p));
     ...
     */
    async createExtensionContext(manifest, extensionPath) {
        ...
        return {
            ...
            logger: this.logger.child({ extension: manifest.name })
        };
    }
    /**
     * Resolve extension dependencies
     */
    async resolveDependencies(manifest, context) {
        const dependencies = [
     ...
     */
    async requestPermissions(manifest, options) {
        const permissions = [];
        ...
        for (const permissionName of manifest.permissions) {
            ...
            if (options.permissionOverrides?.[permissionName] !== undefined) {
                ...
                permission.granted = this.shouldAutoGrantPermission(permissionName, manifest);
            }
     ...
     */
    async createExtensionInstance(manifest, context, extensionPath, permissions) {
        const factory = this.extensionFactories.get(manifest.type);
        if (!factory) {
        ...
        extension.status = ExtensionTypes_js_1.ExtensionStatus.LOADING;
        return extension;
    }
    /**
     * Create sandbox for extension
     */
    async createSandbox(extension, _context) {
        ...
        return {
            ...
            resourceLimits: {
                ...
                network: this.hasPermission(extension, ExtensionTypes_js_1.PermissionType.NETWORK_ACCESS),
                filesystem: this.hasPermission(extension, ExtensionTypes_js_1.PermissionType.FILE_SYSTEM_READ)
            },
     ...
     */
    async initializeExtension(extension, context) {
        try {
    ...
    async unloadExtension(extensionId) {
        ...
        try {
            ...
            extension.status = ExtensionTypes_js_1.ExtensionStatus.UNLOADING;
            // Call lifecycle hook
            if (this.implementsLifecycle(extension.instance)) {
            ...
            extension.status = ExtensionTypes_js_1.ExtensionStatus.UNLOADED;
            extension.unloadedAt = new Date();
            // Emit event
            this.emitExtensionEvent({
                type: ExtensionTypes_js_1.ExtensionEventType.EXTENSION_UNLOADED,
                extensionId,
                timestamp: new Date()
            });
     ...
     */
    initializeExtensionFactories() {
        // Initialize built-in factories
        // In a real implementation, these would be separate factory classes
        this.extensionFactories.set(ExtensionTypes_js_1.ExtensionType.NESTJS_MODULE, {
            async create(manifest, context) {
                return {
                    ...
                    status: ExtensionTypes_js_1.ExtensionStatus.UNLOADED,
                    dependencies: [],
                    peerDependencies: [],
            ...
            getType() {
                return ExtensionTypes_js_1.ExtensionType.NESTJS_MODULE;
            }
        });
    ...
    getPermissionDescription(permission) {
        const descriptions = {
            [ExtensionTypes_js_1.PermissionType.FILE_SYSTEM_READ]: 'Read files from the filesystem',
            [ExtensionTypes_js_1.PermissionType.FILE_SYSTEM_WRITE]: 'Write files to the filesystem',
            [ExtensionTypes_js_1.PermissionType.NETWORK_ACCESS]: 'Make network requests',
            [ExtensionTypes_js_1.PermissionType.DATABASE_ACCESS]: 'Access the database',
            [ExtensionTypes_js_1.PermissionType.AGENT_CONTROL]: 'Control and interact with agents',
            [ExtensionTypes_js_1.PermissionType.WORKFLOW_MODIFY]: 'Modify workflows',
            [ExtensionTypes_js_1.PermissionType.SYSTEM_INFO]: 'Access system information',
            [ExtensionTypes_js_1.PermissionType.USER_DATA]: 'Access user data',
            [ExtensionTypes_js_1.PermissionType.SENSITIVE_DATA]: 'Access sensitive data',
            [ExtensionTypes_js_1.PermissionType.EXECUTION_CONTEXT]: 'Access execution context'
        };
        return descriptions[permission] || 'Unknown permission';
    }
    shouldAutoGrantPermission(_permission, _manifest) {
    ...
    hasPermission(extension, permission) {
        return extension.permissions.some(p => p.type === permission && p.granted);
    }
    implementsLifecycle(instance) {
        return instance && (typeof instance.onLoad === 'function' ||
