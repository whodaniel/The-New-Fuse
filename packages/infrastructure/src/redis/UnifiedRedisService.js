...
let UnifiedRedisService = UnifiedRedisService_1 = class UnifiedRedisService {
    ...
    async get(key) {
        return this.executeOperation('get', key, async () => {
            if (this.upstashClient) {
                return this.upstashClient.get(key);
            }
            return this.mainClient.get(key);
        });
    }
    async set(key, value, ttl, mode, ttlUnit = 'EX') {
        return this.executeOperation('set', key, async () => {
            ...
            if (mode || ttlUnit === 'PX') {
                ...
                return this.mainClient.set(...args);
            }
            if (ttl) {
    ...
    async hget(key, field) {
        return this.executeOperation('hget', key, async () => {
            if (this.upstashClient) {
                return this.upstashClient.hget(key, field);
            }
    ...
    async rpop(key) {
        return this.executeOperation('rpop', key, async () => {
            if (this.upstashClient) {
                return this.upstashClient.rpop(key);
            }
    ...
    async lpop(key) {
        return this.executeOperation('lpop', key, async () => {
            if (this.upstashClient) {
                return this.upstashClient.lpop(key);
            }
    ...
    async mget(...keys) {
        return this.executeOperation('mget', undefined, async () => {
            if (this.upstashClient) {
                return this.upstashClient.mget(...keys);
            }
