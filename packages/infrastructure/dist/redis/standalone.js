...
export function describeStandaloneRedisClient(client) {
    return client instanceof Cluster ? 'cluster' : 'standalone';
}
export async function connectStandaloneRedisClient(client) {
...
export function parseRedisUrl(redisUrl) {
    ...
    if (redisUrl) {
        try {
            ...
            console.error('[Standalone-Redis] Failed to parse Redis URL, using defaults for URL component parsing:', error);
