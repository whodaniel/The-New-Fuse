...
async function seed() {
    ...
    for (const wf of workflowData) {
        ...
        if (wf.name === 'Daily Data Sync') {
            ...
            await createExecution(workflowId, 'COMPLETED', new Date(Date.now() - 2 * 60 * 60 * 1000), 1000 * 60 * 5.5); // 5m 30s
            // Failed run 1 day ago
            await createExecution(workflowId, 'FAILED', new Date(Date.now() - 24 * 60 * 60 * 1000), 1000 * 80); // 1m 20s
        }
            ...
            await createExecution(workflowId, 'COMPLETED', new Date(Date.now() - 60 * 60 * 1000), 1000 * 60 * 3.25); // 3m 15s
        }
    }
    console.log('✅ Seeding complete!');
    process.exit(0);
}
async function createExecution(workflowId, status, startTime, durationMs) {
    ...
    const recent = await db.query.workflowExecutions.findFirst({
        where: (ex, { eq, and }) => and(eq(ex.workflowId, workflowId), eq(ex.status, status)),
    });
    if (!recent || true) {
...
//# sourceMappingURL=seed-workflows.js.map