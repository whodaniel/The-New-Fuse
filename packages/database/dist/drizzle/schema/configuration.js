...
export const systemSettings = pgTable('system_settings', {
    ...
    config: jsonb('config').$type().notNull(), // Stores the nested settings object
    updatedAt: timestamp('updated_at').defaultNow().notNull(),
    updatedBy: varchar('updated_by', { length: 255 }),
});
// =============================================================================
...
export const providerApiKeys = pgTable('provider_api_keys', {
    ...
    userProviderUnique: uniqueIndex('provider_api_keys_user_provider_uq').on(table.userId, table.provider),
}));
// =============================================================================
...
export const agentApiGrants = pgTable('agent_api_grants', {
    ...
    provider: varchar('provider', { length: 100 }).notNull(),
    allowedModels: jsonb('allowed_models').$type().default([]).notNull(),
...
// =============================================================================
export const agentManagedAccounts = pgTable('agent_managed_accounts', {
    id: uuid('id').primaryKey().defaultRandom(),
    ...
    ownerProviderLoginUnique: uniqueIndex('agent_managed_accounts_owner_provider_login_uq').on(table.ownerUserId, table.provider, table.loginIdentifier),
}));
export const agentManagedAccountGrants = pgTable('agent_managed_account_grants', {
    ...
    scopes: jsonb('scopes').$type().default([]).notNull(),
    expiresAt: timestamp('expires_at').notNull(),
    revoked: boolean('revoked').default(false).notNull(),
