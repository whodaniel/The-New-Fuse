...
import { agentPromptVersions, optimizationJobs, validationDatasets, workflowTopologies, } from '../schema.js';
export const optimizationJobRepository = {
    ...
    findByUser: async (userId, status, type) => {
        ...
        if (status)
            conditions.push(eq(optimizationJobs.status, status));
        if (type)
            conditions.push(eq(optimizationJobs.type, type));
...
export const agentPromptVersionRepository = {
    ...
    findLatestByAgentId: async (agentId, massStage) => {
        ...
        if (massStage)
            conditions.push(eq(agentPromptVersions.massStage, massStage));
