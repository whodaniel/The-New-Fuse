...
import { agentManagedAccountGrants, agentManagedAccounts } from '../schema.js';
...
export class DrizzleAgentManagedAccountRepository {
    ...
    async findByIdForOwner(ownerUserId, accountId) {
        const [row] = await db
            .select()
            .from(agentManagedAccounts)
            .where(and(eq(agentManagedAccounts.ownerUserId, ownerUserId), eq(agentManagedAccounts.id, accountId)));
        if (!row)
            return null;
    ...
    async listGrantsForAccount(ownerUserId, accountId) {
        return db
            ...
            .where(and(eq(agentManagedAccountGrants.ownerUserId, ownerUserId), eq(agentManagedAccountGrants.accountId, accountId)))
    ...
    async revokeGrant(ownerUserId, grantId) {
        const [row] = await db
            ...
            .where(and(eq(agentManagedAccountGrants.ownerUserId, ownerUserId), eq(agentManagedAccountGrants.id, grantId)))
            .returning();
        return row || null;
    }
    async redeemGrant(input) {
        ...
        const [grant] = await db
            .select()
            .from(agentManagedAccountGrants)
            .where(and(eq(agentManagedAccountGrants.accessTokenHash, tokenHash), eq(agentManagedAccountGrants.granteeAgentId, normalizeValue(input.granteeAgentId)), eq(agentManagedAccountGrants.revoked, false), gte(agentManagedAccountGrants.expiresAt, now)));
        if (!grant)
            return null;
        const [account] = await db
            .select()
            .from(agentManagedAccounts)
            .where(and(eq(agentManagedAccounts.id, grant.accountId), eq(agentManagedAccounts.ownerUserId, grant.ownerUserId)));
        if (!account)
            return null;
        await db
            .update(agentManagedAccountGrants)
