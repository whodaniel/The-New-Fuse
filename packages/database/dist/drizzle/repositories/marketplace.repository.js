...
import { agentNfts, fractionalShares, revenueDistributions, revenueStreams } from '../schema.js';
...
export const revenueStreamRepository = {
    ...
    findWithDistributionsByTime: async (agentNftId, startDate) => {
        const streams = await db.query.revenueStreams.findMany({
            where: and(eq(revenueStreams.agentNFTId, agentNftId), gte(revenueStreams.createdAt, startDate)),
        });
        const results = [];
        for (const stream of streams) {
            const distributions = await db.query.revenueDistributions.findMany({
                where: and(eq(revenueDistributions.revenueStreamId, stream.id), gte(revenueDistributions.createdAt, startDate)),
            });
