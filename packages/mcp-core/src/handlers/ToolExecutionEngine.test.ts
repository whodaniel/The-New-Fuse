...
import { JSONSchema, ToolHandler, ToolResult } from '../interfaces/IMCPTool.js';
  ...
  describe('Timeout Handling', () => {
    it('should timeout long-running executions', async () => {
      ...
      const options: ToolExecutionOptions = {
        ...
        // The resource monitor samples whole-process heap, so the suite-level
        // 64MB limit can fire before the timeout under load. Pin memory high
        // here so this test deterministically exercises the timeout path.
        resourceLimits: { memory: 1073741824 }, // 1GB
      };
    ...
    it('should use default timeout when not specified', async () => {
      const handler = new LongRunningToolHandler(10000); // 10 seconds
      const params = { test: 'data' };
      // Whole-process memory sampling makes the 64MB engine limit load-dependent;
      // pin memory high so the default TIMEOUT path is exercised deterministically.
      const options: ToolExecutionOptions = {
        resourceLimits: { memory: 1073741824 }, // 1GB
      };
