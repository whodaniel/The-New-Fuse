...
describe('MCPErrorHandler', () => {
  ...
  describe('Error Handling', () => {
    it('should handle errors and update statistics', async () => {
      const error = new MCPErrorClass(MCPErrorCode.RESOURCE_NOT_FOUND, 'Resource not found');
  ...
  describe('Statistics', () => {
    it('should track error statistics correctly', async () => {
      ...
      expect(stats.errorsByCategory[ErrorCategory.RESOURCE]).toBe(2);
      expect(stats.errorsByCategory[ErrorCategory.TOOL]).toBe(1);
      ...
      expect(stats.mostCommonError?.code).toBe(MCPErrorCode.RESOURCE_NOT_FOUND);
    ...
    it('should maintain error history with limit', async () => {
      ...
      for (let i = 0; i < 1005; i++) {
        const error = new MCPErrorClass(MCPErrorCode.RATE_LIMIT_EXCEEDED, `Error ${i}`);
        await errorHandler.handleError(error, context);
      }
  ...
  describe('Error Classification', () => {
    it('should correctly classify errors by category', () => {
      const resourceError = new MCPErrorClass(MCPErrorCode.RESOURCE_NOT_FOUND, 'Not found');
      const toolError = new MCPErrorClass(MCPErrorCode.TOOL_EXECUTION_FAILED, 'Tool failed');
      const authError = new MCPErrorClass(MCPErrorCode.AUTHENTICATION_FAILED, 'Auth failed');
      const connectionError = new MCPErrorClass(MCPErrorCode.CONNECTION_TIMEOUT, 'Timeout');

      expect(resourceError.category).toBe(ErrorCategory.RESOURCE);
      expect(toolError.category).toBe(ErrorCategory.TOOL);
    ...
    it('should correctly classify errors by severity', () => {
      const criticalError = new MCPErrorClass(MCPErrorCode.SYSTEM_OVERLOADED, 'Overloaded');
      const highError = new MCPErrorClass(MCPErrorCode.CONNECTION_LOST, 'Connection lost');
      const mediumError = new MCPErrorClass(MCPErrorCode.AUTHENTICATION_FAILED, 'Auth failed');
      const lowError = new MCPErrorClass(MCPErrorCode.RESOURCE_NOT_FOUND, 'Not found');

      expect(criticalError.severity).toBe(ErrorSeverity.HIGH); // System overloaded is high severity
