...
export type VideoJobStatus = 'pending' | 'queued' | 'processing' | 'completed' | 'failed' | 'cancelled' | 'expired';
...
export type VideoGenerationErrorCode = 'INVALID_PARAMS' | 'CONTENT_POLICY_VIOLATION' | 'RATE_LIMITED' | 'QUOTA_EXCEEDED' | 'PROVIDER_ERROR' | 'PROVIDER_UNAVAILABLE' | 'TIMEOUT' | 'CANCELLED' | 'INTERNAL_ERROR' | 'INVALID_API_KEY' | 'INSUFFICIENT_CREDITS' | 'MODEL_NOT_AVAILABLE' | 'UNSUPPORTED_INPUT_TYPE';
...
export type VideoGenerationEventType = 'job.created' | 'job.started' | 'job.progress' | 'job.completed' | 'job.failed' | 'job.cancelled';
