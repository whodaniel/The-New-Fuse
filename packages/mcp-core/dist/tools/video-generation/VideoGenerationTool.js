 ...
 */
const VIDEO_GENERATION_INPUT_SCHEMA = {
    type: 'object',
    properties: {
        prompt: {
            type: 'string',
            description: 'Detailed text description of the video to generate. Be specific about scene, action, style, and mood.',
        },
        provider: {
            type: 'string',
        ...
        model: {
            type: 'string',
            description: 'Specific model to use (provider-dependent). Examples: svd, animatediff, zeroscope',
        },
        inputType: {
            type: 'string',
        ...
        waitForCompletion: {
            type: 'boolean',
            description: 'If true, wait for video generation to complete before returning. If false, return job ID immediately.',
            default: true,
        },
    },
    required: ['prompt'],
 ...
 */
const VIDEO_GENERATION_OUTPUT_SCHEMA = {
    type: 'object',
    properties: {
...
class VideoGenerationHandler {
    ...
    async validate(params) {
        ...
        // Validate provider
        const provider = p.provider || 'replicate';
    ...
    recordExecution(timeMs, success) {
        if (success) {
     ...
     * Get job status
     */
    async getJobStatus(provider, jobId) {
     ...
     * Cancel a job
     */
    async cancelJob(provider, jobId) {
        const p = this.providers.get(provider);
        if (!p) {
            throw new Error(`Provider ${provider} not available`);
        }
 ...
 */
export function createVideoGenerationTool(providerConfigs) {
...
//# sourceMappingURL=VideoGenerationTool.js.map