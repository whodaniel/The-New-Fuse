...
export class ReplicateProvider extends BaseVideoProvider {
    constructor() {
        super(...arguments);
        this.name = 'replicate';
        this.displayName = 'Replicate';
        this.description = 'Run multiple video AI models through a unified API';
        this.pendingJobs = new Map(); // jobId -> replicateId
    }
    getDefaultBaseUrl() {
    ...
    async generateVideo(params) {
        ...
        // Start prediction
        const prediction = await this.apiRequest('/predictions', {
            method: 'POST',
            body: JSON.stringify({
    ...
    async getJobStatus(jobId) {
        ...
        const prediction = await this.apiRequest(`/predictions/${replicateId}`);
    ...
    buildModelInput(params, modelId) {
        const baseInput = {};
        switch (modelId) {
            case 'svd':
            case 'svd-xt':
    ...
    getWidth(aspectRatio, resolution) {
    ...
    getHeight(aspectRatio, resolution) {
    ...
    mapPredictionToJob(jobId, prediction, params) {
        const statusMap = {
            starting: 'pending',
    ...
    createVideoFromUrl(url, index, prediction) {
        return {
...
//# sourceMappingURL=ReplicateProvider.js.map