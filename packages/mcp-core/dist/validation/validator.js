...
export class MCPValidator {
    ...
    compileSchemas() {
        for (const [name, schema] of Object.entries(schemas)) {
            try {
                ...
                throw new MCPErrorClass(MCPErrorCode.SYSTEM_CONFIGURATION_ERROR, `Failed to compile schema '${name}': ${error instanceof Error ? error.message : 'Unknown error'}`, { category: ErrorCategory.VALIDATION, severity: ErrorSeverity.CRITICAL });
            }
        }
    }
    /**
    ...
    addSchema(name, schema) {
        try {
            ...
            throw new MCPErrorClass(MCPErrorCode.SYSTEM_CONFIGURATION_ERROR, `Failed to compile custom schema '${name}': ${error instanceof Error ? error.message : 'Unknown error'}`, { category: ErrorCategory.VALIDATION, severity: ErrorSeverity.HIGH });
        }
    }
    /**
    ...
    validateOrThrow(schemaName, data) {
        ...
        if (!result.valid) {
            throw new MCPErrorClass(MCPErrorCode.MESSAGE_INVALID_FORMAT, `Validation failed for ${schemaName}: ${result.errors.join(', ')}`, {
