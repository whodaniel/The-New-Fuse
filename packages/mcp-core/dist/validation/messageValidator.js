...
export class MessageValidator {
     ...
     */
    static validateRequestParams(method, params) {
     ...
     */
    static validateResponseResult(method, result) {
     ...
     */
    static validateMethodName(method) {
     ...
     */
    static isValidRPCMethod(method) {
     ...
     */
    static convertValidationResult(result) {
        return {
            valid: result.valid,
     ...
     */
    static validateAndNormalize(message) {
        const result = this.validateMessage(message);
        if (!result.valid) {
            throw this.createValidationError(`Message validation failed: ${result.errors.join(', ')}`, { originalMessage: message, errors: result.errors });
        }
