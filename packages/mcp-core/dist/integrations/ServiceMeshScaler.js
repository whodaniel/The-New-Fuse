...
export class ServiceMeshScaler extends EventEmitter {
    ...
    async startScaling() {
        try {
            ...
            this.evaluationInterval = setInterval(() => this.evaluateScaling(), this.config.evaluationInterval * 1000);
            this.isRunning = true;
     ...
     */
    async addService(serviceId, scalingConfig) {
        try {
     ...
     */
    getServiceScalingState(serviceId) {
     ...
     */
    getScalingHistory(serviceId, limit) {
     ...
     */
    async evaluateService(serviceId) {
        const scalingState = this.scalingStates.get(serviceId);
        if (!scalingState) {
            throw new MCPError(MCPErrorCode.RESOURCE_NOT_FOUND, `Service ${serviceId} is not under scaling management`);
        }
     ...
     */
    async scaleService(serviceId, targetInstances, reason) {
        try {
            const scalingState = this.scalingStates.get(serviceId);
            if (!scalingState) {
                throw new MCPError(MCPErrorCode.RESOURCE_NOT_FOUND, `Service ${serviceId} is not under scaling management`);
            ...
            if (targetInstances < scalingState.config.minInstances ||
                ...
                throw new MCPError(MCPErrorCode.INVALID_PARAMS, `Target instances ${targetInstances} is outside allowed range [${scalingState.config.minInstances}, ${scalingState.config.maxInstances}]`);
            }
    ...
    async evaluateScaling() {
        const evaluationPromises = Array.from(this.scalingStates.values()).map(scalingState => this.evaluateAndExecuteScaling(scalingState));
     ...
     */
    async evaluateAndExecuteScaling(scalingState) {
        try {
            // Skip if in cooldown
     ...
     */
    async evaluateServiceScaling(scalingState) {
        // Get current metrics
        ...
        scalingState.metricsHistory = scalingState.metricsHistory.filter(m => m.timestamp.getTime() > retentionCutoff);
        // Get current scaling status
        ...
        const policyResults = scalingState.config.policies?.map((policy) => this.evaluateScalingPolicy(policy, metrics, scalingState)) || [];
        // Calculate scaling recommendation
        const scaleUpVotes = policyResults.filter((r) => r.recommendation === 'scale_up').length;
        const scaleDownVotes = policyResults.filter((r) => r.recommendation === 'scale_down').length;
        const noActionVotes = policyResults.filter((r) => r.recommendation === 'no_action').length;
        let decision;
        if (scaleUpVotes > scaleDownVotes && scaleUpVotes > noActionVotes) {
            // Scale up
            const targetInstances = Math.min(scalingState.status.currentInstances + 1, scalingState.config.maxInstances);
            decision = {
                serviceId: scalingState.serviceId,
                type: 'scale_up',
                currentInstances: scalingState.status.currentInstances,
                recommendedInstances: targetInstances,
                reason: this.buildScalingReason(policyResults.filter((r) => r.recommendation === 'scale_up')),
            ...
            // Scale down
            const targetInstances = Math.max(scalingState.status.currentInstances - 1, scalingState.config.minInstances);
            decision = {
                serviceId: scalingState.serviceId,
                type: 'scale_down',
                currentInstances: scalingState.status.currentInstances,
                recommendedInstances: targetInstances,
                reason: this.buildScalingReason(policyResults.filter((r) => r.recommendation === 'scale_down')),
                influencingMetrics: policyResults
     ...
     */
    evaluateScalingPolicy(policy, metrics, scalingState) {
        let metricValue;
     ...
     */
    async executeScaling(scalingState, decision) {
     ...
     */
    buildScalingReason(policyResults) {
        const reasons = policyResults.map(r => `${r.metric.name}: ${r.metric.value} (threshold: ${r.metric.threshold})`);
        return reasons.join(', ');
    }
    /**
...
//# sourceMappingURL=ServiceMeshScaler.js.map