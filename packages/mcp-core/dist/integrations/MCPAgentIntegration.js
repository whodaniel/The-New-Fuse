 ...
 */
export class MCPAgentIntegration extends EventEmitter {
    constructor(broker, client, config = {}) {
        super();
     ...
     */
    async registerAgentAsMCPService(agent) {
        try {
            ...
            const serviceInfo = {
                ...
                resources: agent.resources.map((resource) => ({
                    ...
                    handler: null, // Will be set by the agent
                })),
    ...
    async unregisterAgent(agentId) {
        try {
            ...
            const collaborations = Array.from(this.activeCollaborations.values()).filter((collab) => collab.participants.includes(agentId));
     ...
     */
    async enableAgentMCPCommunication(agentId) {
        try {
            const endpoint = this.registeredAgents.get(agentId);
            if (!endpoint) {
                throw new MCPErrorClass(MCPErrorCode.RESOURCE_NOT_FOUND, `Agent ${agentId} not found in registered agents`);
            }
     ...
     */
    async disableAgentMCPCommunication(agentId) {
        try {
            const endpoint = this.registeredAgents.get(agentId);
            if (!endpoint) {
                return false;
     ...
     */
    async routeAgentMessage(from, to, message, routing = {}) {
        ...
        while (retryCount <= routingInfo.retryPolicy.maxRetries) {
            try {
                ...
                if (!targetEndpoint) {
                    throw new MCPErrorClass(MCPErrorCode.RESOURCE_NOT_FOUND, `Target agent ${to} not found or not registered`);
                }
                if (targetEndpoint.status !== AgentStatus.ACTIVE) {
                    throw new MCPErrorClass(MCPErrorCode.SERVICE_UNAVAILABLE, `Target agent ${to} is not active (status: ${targetEndpoint.status})`);
                }
                // Create MCP message
                const mcpMessage = {
                    jsonrpc: '2.0',
                    ...
                    meta: {
                        timestamp: new Date(),
                        source: from,
                        priority: routingInfo.priority === 'urgent'
                            ? 'high'
     ...
     */
    async getAgentMCPCapabilities(agentId) {
        try {
            const endpoint = this.registeredAgents.get(agentId);
            if (!endpoint) {
                throw new MCPErrorClass(MCPErrorCode.RESOURCE_NOT_FOUND, `Agent ${agentId} not found in registered agents`);
            ...
            throw new MCPErrorClass(MCPErrorCode.INTERNAL_ERROR, `Failed to get capabilities for agent ${agentId}: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    /**
    ...
    async findAgentsByCapability(capability) {
        ...
        for (const endpoint of this.registeredAgents.values()) {
            const hasCapability = endpoint.capabilities.some((cap) => cap.name === capability || cap.methods.includes(capability));
            if (hasCapability) {
                agents.push({
                    id: endpoint.agentId,
     ...
     */
    async startCollaboration(participants, initiator, purpose) {
     ...
     */
    async endCollaboration(collaborationId) {
     ...
     */
    async getAgentCollaborations(agentId) {
        return Array.from(this.activeCollaborations.values()).filter((collab) => collab.participants.includes(agentId) && collab.status === 'active');
    }
    /**
     * Update agent status
     */
    ...
    validateAgent(agent) {
        ...
        if (!Array.isArray(agent.capabilities) ||
            ...
            throw new MCPErrorClass(MCPErrorCode.INVALID_PARAMS, 'Agent capabilities, resources, and tools must be arrays');
        }
    }
    mapAgentStatusToServiceStatus(status) {
        switch (status) {
    ...
    createDefaultCapabilities(agent) {
        return [
            {
    ...
    calculateAvailability(endpoint) {
        const now = Date.now();
