...
import { TaskExecutionStatus, } from '../interfaces/IMCPWorkflowIntegration.js';
import { MCPErrorClass } from '../types/error.js';
import { MCPCallbackHandler } from './MCPCallbackHandler.js';
import { WorkflowExecutionMonitor } from './WorkflowExecutionMonitor.js';
/**
 * MCP Workflow Integration implementation
 */
export class MCPWorkflowIntegration extends EventEmitter {
    constructor(config) {
        super();
    ...
    async initialize() {
        ...
        try {
            ...
            this.emit('error', new MCPErrorClass(-32603, `Failed to initialize workflow integration: ${err.message}`, {
                cause: err,
            }));
            throw err;
        }
    ...
    async shutdown() {
        ...
        try {
            ...
            const err = error instanceof Error ? error : new Error(String(error));
            this.emit('error', new MCPErrorClass(-32603, `Failed to shutdown workflow integration: ${err.message}`, {
                cause: err,
            }));
            throw err;
        }
    ...
    async executeWorkflowStep(step, context) {
        ...
        try {
            ...
            const mcpResponse = await this.executeWithRetry(mcpRequest, step.mcpService, step.retryPolicy || this.config.defaultRetryPolicy, step.timeout || this.config.defaultTimeout);
     ...
     */
    async delegateTask(task, mcpService) {
     ...
     */
    async trackMCPExecution(executionId) {
     ...
     */
    async handleMCPCallback(callback) {
        try {
            if (this.config.debug) {
     ...
     */
    registerCallbackHandler(executionId, handler) {
     ...
     */
    unregisterCallbackHandler(executionId) {
    ...
    async prepareMCPRequest(step, context) {
    ...
    async executeWithRetry(request, serviceName, retryPolicy, timeoutMs) {
        let lastError = null;
    ...
    async processStepResponse(response, step, context) {
        if (response.error) {
            throw new MCPErrorClass(response.error.code, response.error.message, response.error.data);
        }
        return response.result;
    }
    trackExecution(executionId, status, result, error) {
    ...
    updateExecution(executionId, status, result, error) {
...
//# sourceMappingURL=MCPWorkflowIntegration.js.map