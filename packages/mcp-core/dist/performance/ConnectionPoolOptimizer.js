...
export class OptimizedConnectionPool extends EventEmitter {
    constructor(config, factory, logger) {
        super();
    ...
    async optimizePoolSize() {
        ...
        if (stats.utilizationRate > 0.8 && stats.pendingRequests > 0) {
            const targetSize = Math.min(this.config.maxSize, stats.totalConnections + Math.ceil(stats.pendingRequests / 2));
    ...
    async performHealthChecks() {
        ...
        for (const connection of this.idleConnections) {
            healthCheckPromises.push(connection.isHealthy()
                .then(isHealthy => {
                if (!isHealthy) {
...
export class ConnectionPoolFactory {
     ...
     */
    static createOptimizedPool(factory, config = {}) {
        const defaultConfig = {
            minSize: 5,
            maxSize: 50,
     ...
     */
    static createHighThroughputPool(factory) {
     ...
     */
    static createResourceEfficientPool(factory) {
        return this.createOptimizedPool(factory, {
            minSize: 2,
            maxSize: 20,
