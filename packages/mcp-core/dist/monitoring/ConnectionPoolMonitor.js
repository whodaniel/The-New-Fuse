...
export class ConnectionPoolMonitor {
     ...
     */
    recordConnectionDestroyed(lifetime) {
        const event = {
            type: 'destroyed',
            timestamp: new Date(),
            lifetime
     ...
     */
    recordConnectionAcquired(waitTime) {
        const event = {
            type: 'acquired',
            timestamp: new Date(),
            waitTime
    ...
    getPoolAnalysis(hours = 1) {
        ...
        if (averageWaitTime > 100)
            efficiency -= 20; // High wait time
        if (utilizationRate < 0.3)
            efficiency -= 15; // Low utilization
        if (utilizationRate > 0.9)
            efficiency -= 10; // Over-utilization
        if (this.pendingRequests > this.poolSize * 0.5)
            efficiency -= 25; // Too many pending
    ...
    generatePoolReport(hours = 24) {
        ...
        const recentEvents = this.eventHistory.filter(event => event.timestamp >= new Date(Date.now() - hours * 60 * 60 * 1000));
        const eventCounts = {
    ...
    cleanup() {
        ...
        this.eventHistory.splice(0, this.eventHistory.findIndex(event => event.timestamp >= cutoff));
        ...
        const cleanedRecords = (initialLength - this.eventHistory.length) +
