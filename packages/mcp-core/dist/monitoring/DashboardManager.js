...
export class DashboardManager {
     ...
     */
    generateTimeSeriesData(metrics, timeRange) {
        const now = Date.now();
        const points = 50;
     ...
     */
    generateHeatmapData(metric, timeRange) {
        const buckets = 20;
        const timePoints = 30;
        const now = Date.now();
        const interval = timeRange / timePoints;
        const data = Array.from({ length: timePoints }, (_, t) => Array.from({ length: buckets }, (_, b) => ({
     ...
     */
    generateMetricValue(metric, index) {
