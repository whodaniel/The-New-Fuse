    ...
    async run() {
        ...
        const monitoringInterval = setInterval(() => {
            if (this.stopped)
                return;
            const now = new Date();
            const recentResults = results.filter(r => now.getTime() - r.timestamp.getTime() < 10000 // Last 10 seconds
            );
...
class LoadTestWorker {
     ...
     * Start the worker
     */
    async start(results, errorsByType) {
...
//# sourceMappingURL=LoadTester.js.map