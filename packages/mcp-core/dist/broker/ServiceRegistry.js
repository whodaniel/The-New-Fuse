...
export class ServiceRegistry extends EventEmitter {
    ...
    async start() {
        ...
        switch (this.config.type) {
            ...
            case 'consul':
                ...
                throw new MCPErrorClass(JSONRPCErrorCode.INTERNAL_ERROR, `Storage backend ${this.config.type} not yet implemented`);
            default:
                throw new MCPErrorClass(MCPErrorCode.INVALID_PARAMS, `Unknown storage backend: ${this.config.type}`);
        }
        this.isStarted = true;
    ...
    async discover(query) {
        ...
        // Apply filters
        if (query.name) {
            services = services.filter((service) => service.name.toLowerCase().includes(query.name.toLowerCase()));
        }
        if (query.capability) {
            services = services.filter((service) => service.capabilities.includes(query.capability));
        }
        if (query.resource) {
            services = services.filter((service) => service.resources.some((resource) => resource.name.toLowerCase().includes(query.resource.toLowerCase()) ||
                resource.uri.toLowerCase().includes(query.resource.toLowerCase())));
        }
        if (query.tool) {
            services = services.filter((service) => service.tools.some((tool) => tool.name.toLowerCase().includes(query.tool.toLowerCase())));
        }
        if (query.status) {
            services = services.filter((service) => service.status === query.status);
        }
        if (query.tags && query.tags.length > 0) {
            services = services.filter((service) => service.tags && query.tags.some((tag) => service.tags.includes(tag)));
        }
    ...
    getStatistics() {
        ...
        return {
            ...
            statusCounts,
            oldestService: services.reduce((oldest, service) => !oldest || service.registeredAt < oldest.registeredAt ? service : oldest, null),
            newestService: services.reduce((newest, service) => !newest || service.registeredAt > newest.registeredAt ? service : newest, null),
        };
    }
    /**
     ...
     */
    applyGenericFilter(services, filter) {
     ...
     */
    applySorting(services, sortConfig) {
     ...
     */
    hasCapabilities(service, requiredCapabilities) {
     ...
     */
    checkCapabilityCompatibility(serviceA, serviceB) {
     ...
     */
    async findCompatibleServices(targetService) {
     ...
     */
    async discoverWithCapabilityMatching(query) {
     ...
     */
    hasExactCapabilities(service, requiredCapabilities) {
     ...
     */
    async getServiceRecommendations(currentService, options = {}) {
        const { maxRecommendations = 5, includeCompatible = true, includeSimilar = true, weightByHealth = true, weightByUsage = false, excludeTags = [], includeTags = [], } = options;
        let recommendations = [];
        ...
        // Remove duplicates
        let uniqueRecommendations = recommendations.filter((service, index, self) => index === self.findIndex((s) => s.id === service.id));
        // Apply tag filters
        if (excludeTags.length > 0) {
            uniqueRecommendations = uniqueRecommendations.filter((service) => !service.tags?.some((tag) => excludeTags.includes(tag)));
        }
        if (includeTags.length > 0) {
            uniqueRecommendations = uniqueRecommendations.filter((service) => service.tags?.some((tag) => includeTags.includes(tag)));
        }
