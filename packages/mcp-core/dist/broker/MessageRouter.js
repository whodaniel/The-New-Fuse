...
export class MessageRouter extends EventEmitter {
    constructor(loadBalancer, eventSubscriptionManager, messageQueue) {
        super();
    ...
    async stop() {
        ...
        for (const [requestId, tracker] of this.activeRequests.entries()) {
            ...
            tracker.reject(new MCPErrorClass(JSONRPCErrorCode.INTERNAL_ERROR, `Request ${requestId} cancelled: Message router stopped`));
     ...
     */
    async routeRequest(request, routing) {
        if (!this.isStarted) {
            throw new MCPErrorClass(MCPErrorCode.SERVICE_UNAVAILABLE, 'Message router is not started');
        }
        return new Promise(async (resolve, reject) => {
            if (!this.isStarted) {
                reject(new MCPErrorClass(MCPErrorCode.SERVICE_UNAVAILABLE, // This is correct, router not started is a service issue
            ...
            try {
                ...
                if (error instanceof MCPErrorClass) {
                    ...
                    reject(new MCPErrorClass(JSONRPCErrorCode.INTERNAL_ERROR, `Request routing failed: ${error instanceof Error ? error.message : 'Unknown error'}`));
                }
            }
            finally {
    ...
    async routeNotification(notification) {
        ...
        const matchResults = this.eventSubscriptionManager.findMatchingSubscriptions(notification.method, notification.params);
        ...
        const promises = matchResults.map(async (matchResult) => {
            try {
                ...
                console.error(`Failed to send notification to subscribed service ${matchResult.subscription.serviceId}:`, error);
            }
        });
        await Promise.allSettled(promises);
     ...
     */
    async processQueuedMessages(serviceId) {
        if (!this.isStarted) {
            return;
        }
     ...
     */
    async subscribeToEvents(serviceId, pattern, callback) {
     ...
     */
    async subscribeToEventsAdvanced(serviceId, pattern, patternType, filters, metadata) {
        if (!this.eventSubscriptionManager) {
            throw new MCPErrorClass(JSONRPCErrorCode.METHOD_NOT_FOUND, 'Event subscription is not available - no subscription manager configured');
        }
        return await this.eventSubscriptionManager.subscribe(serviceId, pattern, patternType, filters, metadata);
    }
    /**
     ...
     */
    async unsubscribeService(serviceId) {
        if (!this.eventSubscriptionManager) {
     ...
     */
    async performRouting(request, targetService, routingInfo, tracker) {
            ...
            try {
                ...
                if (!service) {
                    ...
                    if (targetService) {
                        ...
                        throw new MCPErrorClass(MCPErrorCode.SERVICE_UNAVAILABLE, 'No available services for request routing');
                    }
                }
     ...
     */
    async sendRequestToService(request, serviceId, timeout) {
     ...
     */
    async sendNotificationToService(notification, serviceId) {
        // This is a placeholder implementation
     ...
     * Update service distribution metrics
     */
    updateServiceDistribution(serviceId) {
     ...
     */
    updateErrorDistribution(serviceId) {
