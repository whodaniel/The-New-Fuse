...
export class EventSubscriptionManager extends EventEmitter {
    constructor() {
        ...
        this.serviceSubscriptions = new Map(); // serviceId -> subscription IDs
     ...
     */
    async subscribe(serviceId, pattern, patternType = PatternType.EXACT, filters, metadata) {
        if (!this.isStarted) {
            throw new MCPErrorClass(MCPErrorCode.SERVICE_UNAVAILABLE, 'Event subscription manager is not started');
        }
        ...
        if (patternType === PatternType.REGEX || patternType === PatternType.WILDCARD) {
            try {
                const regexPattern = patternType === PatternType.WILDCARD ? this.wildcardToRegex(pattern) : pattern;
    ...
    async unsubscribe(subscriptionId) {
        ...
        if (!subscription) {
            throw new MCPErrorClass(MCPErrorCode.RESOURCE_NOT_FOUND, `Subscription not found: ${subscriptionId}`);
        }
     ...
     */
    findMatchingSubscriptions(eventMethod, eventParams) {
        if (!this.isStarted) {
            return [];
        }
        const results = [];
     ...
     */
    getServiceSubscriptions(serviceId) {
        const subscriptionIds = this.serviceSubscriptions.get(serviceId);
        if (!subscriptionIds) {
            return [];
        }
     ...
     */
    async updateSubscriptionStatus(subscriptionId, active) {
        const subscription = this.subscriptions.get(subscriptionId);
        if (!subscription) {
            throw new MCPErrorClass(MCPErrorCode.RESOURCE_NOT_FOUND, `Subscription not found: ${subscriptionId}`);
        }
    ...
    getStatistics() {
        ...
        const averageMatchesPerSubscription = subscriptions.length > 0 ? totalMatches / subscriptions.length : 0;
     ...
     */
    matchSubscription(subscription, eventMethod, eventParams) {
        const result = {
            subscription,
            matches: false,
            matchedFields: [],
        ...
        const methodMatches = this.matchPattern(eventMethod, subscription.pattern, subscription.patternType, subscription.id);
        if (!methodMatches) {
            return result;
        }
        result.matches = true;
     ...
     */
    matchPattern(value, pattern, patternType, subscriptionId) {
        switch (patternType) {
     ...
     */
    matchFilterValue(actual, expected) {
        if (expected === actual) {
            return true;
        }
    ...
    validatePattern(pattern, patternType) {
        ...
        switch (patternType) {
            ...
            case PatternType.WILDCARD:
                ...
                if (pattern.includes('**')) {
                    throw new MCPErrorClass(MCPErrorCode.INVALID_PARAMS, 'Double wildcards (**) are not supported');
                }
                break;
