...
export class HealthMonitor extends EventEmitter {
     ...
     */
    async addService(serviceId, endpoint) {
        if (!this.config.enabled) {
            return;
        }
        if (this.services.has(serviceId)) {
            throw new MCPErrorClass(MCPErrorCode.INVALID_PARAMS, `Service ${serviceId} is already being monitored`);
        }
        const now = new Date();
        const initialHealth = {
            serviceId,
    ...
    async checkServiceHealth(serviceId) {
        const tracker = this.services.get(serviceId);
        if (!tracker) {
            throw new MCPErrorClass(MCPErrorCode.RESOURCE_NOT_FOUND, `Service not found in health monitor: ${serviceId}`);
        }
    ...
    async performHealthChecks() {
        const promises = Array.from(this.services.keys()).map((serviceId) => this.checkServiceHealth(serviceId).catch((error) => {
     ...
     */
    calculateHealthScore(tracker, responseTime, status) {
        let score = 1.0;
