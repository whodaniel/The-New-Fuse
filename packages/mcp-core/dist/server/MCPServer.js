...
export class MCPServer extends EventEmitter {
    ...
    registerTool(tool, handler) {
        ...
        if (this.tools.has(toolToRegister.name)) {
            throw new MCPErrorClass(JSONRPCErrorCode.INVALID_PARAMS, `Tool with name "${toolToRegister.name}" already registered`);
        }
    ...
    handleResourcesList(params) {
        ...
        if (params?.pattern) {
            ...
            return {
                resources: resources.filter(resource => pattern.test(resource.name) || pattern.test(resource.uri))
            };
        }
        return { resources };
    }
    ...
    async handleResourceSubscribe(params) {
        ...
        if (!resource.handler) {
            throw new MCPErrorClass(MCPErrorCode.RESOURCE_UNAVAILABLE, `Resource handler not found for: ${params.uri}`);
        }
        if (resource.handler.subscribe) {
            await resource.handler.subscribe(params.uri, (data) => {
            ...
            throw new MCPErrorClass(JSONRPCErrorCode.METHOD_NOT_FOUND, `Resource ${params.uri} does not support subscriptions`);
        }
     ...
     */
    async handleResourceUnsubscribe(params) {
        if (!params?.uri) {
            throw new MCPErrorClass(JSONRPCErrorCode.INVALID_PARAMS, 'Resource URI is required');
        }
        const resource = this.resources.get(params.uri);
