...
import { ServiceLevel } from './GracefulDegradation.js';
/**
...
export class FailoverManager extends EventEmitter {
    ...
    async executeWithFailover(operation) {
        ...
        if (availableEndpoints.length === 0) {
            ...
            if (this.degradationManager) {
                this.degradationManager.degradeToLevel(ServiceLevel.OFFLINE, 'No healthy endpoints available');
            }
        ...
        for (const endpoint of availableEndpoints) {
            ...
            try {
                ...
                if (!circuitBreaker) {
                    throw new Error(`No circuit breaker found for endpoint ${endpoint.id}`);
                }
     ...
     */
    markEndpointHealthy(endpointId) {
     ...
     */
    handleEndpointError(endpointId, error) {
        const endpoint = this.endpoints.get(endpointId);
        if (!endpoint)
            return;
        endpoint.errorCount++;
     ...
     * Trigger failover
     */
    triggerFailover(failedEndpointId, error) {
        const availableEndpoints = this.getAvailableEndpoints();
        if (availableEndpoints.length === 0) {
     ...
     */
    async considerFailback(recoveredEndpoint) {
