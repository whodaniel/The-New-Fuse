...
export class ResourceHandler {
     ...
     */
    async list(pattern) {
        throw new MCPErrorClass(MCPErrorCode.RESOURCE_UNAVAILABLE, `List operation not supported for resource handler: ${this.constructor.name}`, {
    ...
    async subscribe(uri, callback) {
        throw new MCPErrorClass(MCPErrorCode.RESOURCE_UNAVAILABLE, `Subscribe operation not supported for resource handler: ${this.constructor.name}`, {
            category: ErrorCategory.RESOURCE,
            severity: ErrorSeverity.LOW,
            retryable: false,
    ...
    async unsubscribe(uri) {
        throw new MCPErrorClass(MCPErrorCode.RESOURCE_UNAVAILABLE, `Unsubscribe operation not supported for resource handler: ${this.constructor.name}`, {
            category: ErrorCategory.RESOURCE,
            severity: ErrorSeverity.LOW,
            retryable: false,
            details: { handlerType: this.constructor.name, uri }
    ...
    validateUri(uri) {
        if (!uri || typeof uri !== 'string') {
            throw new MCPErrorClass(MCPErrorCode.RESOURCE_INVALID_FORMAT, 'Resource URI must be a non-empty string', {
     ...
     */
    createResourceContent(uri, content, options = {}) {
     ...
     */
    handleResourceError(error, operation, uri) {
        ...
        throw new MCPErrorClass(errorCode, `Resource ${operation} failed for URI ${uri}: ${error.message || 'Unknown error'}`, {
            category: ErrorCategory.RESOURCE,
            severity,
...
export class FileResourceHandler extends ResourceHandler {
    constructor(uri, name, basePath, options = {}) {
    ...
    async read(uri, params) {
        ...
        try {
            ...
            if (!filePath.startsWith(this.basePath)) {
                throw new MCPErrorClass(MCPErrorCode.RESOURCE_ACCESS_DENIED, 'Access denied: file path outside allowed directory', {
                    category: ErrorCategory.RESOURCE,
                    severity: ErrorSeverity.HIGH,
                    retryable: false,
...
export class DatabaseResourceHandler extends ResourceHandler {
    constructor(uri, name, connectionString, tableName, options = {}) {
        super(uri, name, options);
