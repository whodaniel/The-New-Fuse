...
export class ToolExecutionEngine extends EventEmitter {
    ...
    async executeToolSecurely(handler, params, permissions, securityContext, options = {}) {
        ...
        try {
            ...
            if (permissions.rateLimit) {
                ...
                if (!allowed) {
                    throw new MCPErrorClass(MCPErrorCode.RATE_LIMIT_EXCEEDED, 'Rate limit exceeded for tool execution', {
            ...
            const result = await this.executeWithSecurityConstraints(handler, params, timeout, resourceLimits, options.sandbox, context, logs);
            // Stop monitoring
     ...
     */
    async executeToolWithLimits(handler, params, options = {}) {
        const executionId = this.generateExecutionId();
        const timeout = options.timeout || this.defaultTimeout;
        const resourceLimits = { ...this.defaultResourceLimits, ...options.resourceLimits };
        const context = {
        ...
        try {
            ...
            const result = await this.executeWithConstraints(handler, params, timeout, resourceLimits, context, logs);
     ...
     */
    async performSecurityChecks(handler, params, permissions, securityContext, logs, executionId) {
        ...
        if (permissions.requiredRoles && permissions.requiredRoles.length > 0) {
            const hasRequiredRole = permissions.requiredRoles.some((role) => securityContext.roles.includes(role));
            if (!hasRequiredRole) {
                ...
                throw new MCPErrorClass(MCPErrorCode.INSUFFICIENT_PERMISSIONS, `Insufficient roles. Required: ${permissions.requiredRoles.join(', ')}`, {
                    category: ErrorCategory.AUTH,
                    severity: ErrorSeverity.MEDIUM,
                    retryable: false,
                    details: {
        ...
        if (permissions.acl && permissions.acl.length > 0) {
            ...
            if (!allowed) {
                ...
                throw new MCPErrorClass(MCPErrorCode.AUTHORIZATION_FAILED, 'ACL permission denied for tool execution', {
                    category: ErrorCategory.AUTH,
                    severity: ErrorSeverity.MEDIUM,
                    retryable: false,
                    details: { executionId, principal: securityContext.principal },
        ...
        return new Promise(async (resolve, reject) => {
            ...
            timeoutHandle = setTimeout(() => {
                if (!completed) {
                    ...
                    reject(new MCPErrorClass(MCPErrorCode.TOOL_TIMEOUT, `Tool execution timed out after ${timeout}ms`, {
                        category: ErrorCategory.TOOL,
                        severity: ErrorSeverity.MEDIUM,
                        retryable: true,
                ...
                if (!completed) {
                    ...
                    if (usage) {
                        ...
                        if (resourceLimits.memory && usage.peakMemory > resourceLimits.memory) {
                            ...
                            reject(new MCPErrorClass(MCPErrorCode.TOOL_RESOURCE_EXHAUSTED, `Memory limit exceeded: ${usage.peakMemory} > ${resourceLimits.memory}`, {
                                category: ErrorCategory.TOOL,
                        ...
                        if (resourceLimits.cpuTime && usage.cpuTime > resourceLimits.cpuTime) {
                            ...
                            reject(new MCPErrorClass(MCPErrorCode.TOOL_RESOURCE_EXHAUSTED, `CPU time limit exceeded: ${usage.cpuTime} > ${resourceLimits.cpuTime}`, {
                                category: ErrorCategory.TOOL,
                                severity: ErrorSeverity.HIGH,
                                retryable: false,
                                details: { executionId: context.executionId, resourceLimits, usage },
     ...
     */
    async validateToolResult(result, handler, logs, executionId) {
        ...
        if (typeof result !== 'object' || result === null) {
            ...
            throw new MCPErrorClass(MCPErrorCode.TOOL_EXECUTION_FAILED, 'Invalid result format: result must be an object', {
                category: ErrorCategory.TOOL,
                severity: ErrorSeverity.MEDIUM,
                retryable: false,
        ...
        if (typeof result.success !== 'boolean') {
            ...
            throw new MCPErrorClass(MCPErrorCode.TOOL_EXECUTION_FAILED, 'Invalid result format: success property must be boolean', {
                category: ErrorCategory.TOOL,
                severity: ErrorSeverity.MEDIUM,
                retryable: false,
     ...
     */
    getToolPerformanceMetrics(toolName) {
     ...
     * Add log entry
     */
    addLog(logs, executionId, level, message, data) {
        logs.push({
...
class ResourceMonitor {
    constructor() {
        this.monitoredExecutions = new Map();
    }
    /**
...
class EnhancedJSONSchemaValidator {
    ...
    async validateObjectProperties(obj, schema, errors) {
        ...
        if (schema.properties) {
            for (const [propName, propSchema] of Object.entries(schema.properties)) {
                if (propName in obj) {
                    const propResult = await this.validate(obj[propName], propSchema);
                    if (!propResult.valid) {
    ...
    async validateArrayItems(arr, schema, errors) {
        const normalized = [...arr];
        if (schema.items) {
            for (let i = 0; i < arr.length; i++) {
                const itemResult = await this.validate(arr[i], schema.items);
                if (!itemResult.valid) {
...
class ToolSandbox {
     ...
     */
    async executeWithMonitoring(fn) {
        const startTime = Date.now();
        try {
            const result = await fn();
            // Check execution time against limits
            const executionTime = Date.now() - startTime;
            if (this.config.resourceLimits.cpuTime &&
...
class ToolPerformanceMonitor {
    constructor() {
        ...
        this.executionTimes = new Map(); // toolName -> executionId -> startTime
        this.executionHistory = new Map(); // toolName -> history
    }
    /**
     * Start monitoring an execution
     */
    startExecution(toolName, executionId) {
     ...
     */
    recordExecution(toolName, executionId, success, resourceUsage) {
