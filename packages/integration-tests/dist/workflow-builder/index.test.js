...
require("./workflow-builder.test");
require("./ui-components.test");
require("./canvas-interactions.test");
require("./validation-execution.test");
describe('Workflow Builder Test Suite', () => {
    let env;
    let testReport;
    beforeAll(async () => {
        env = await (0, test_setup_js_1.getTestEnvironment)(); // Added await here
    ...
    describe('Test Suite Overview', () => {
        ...
        test('should validate test data helpers', async () => {
            // Test agent creation
            const testAgent = await test_setup_js_1.TestHelpers.createTestAgent('ValidationAgent', 'TEST_TYPE');
            expect(testAgent.agentId).toBeDefined();
            // Test workflow creation
            const { workflow, builder } = await test_setup_js_1.TestHelpers.createTestWorkflow('Validation Workflow');
            expect(workflow).toBeDefined();
            expect(builder).toBeDefined();
            // Test extension creation
            const testExtension = await test_setup_js_1.TestHelpers.createTestExtension('validation-extension', 'custom');
    ...
    describe('Integration Verification', () => {
        test('should verify workflow builder integrates with Master Agent Registry', async () => {
            ...
            const agent = await test_setup_js_1.TestHelpers.createTestAgent('IntegrationAgent', 'INTEGRATION_TEST');
        ...
        test('should verify workflow builder integrates with Extension System', async () => {
            ...
            const testExtension = await test_setup_js_1.TestHelpers.createTestExtension('integration-extension', 'workflow_node');
        ...
        test('should verify workflow builder integrates with Workflow Engine', async () => {
            ...
            const { workflow } = await test_setup_js_1.TestHelpers.createTestWorkflow('Integration Test Workflow');
    ...
    describe('Performance Benchmarks', () => {
        test('should benchmark node creation performance', async () => {
            ...
            for (let i = 0; i < nodeCount; i++) {
                builder.addNode('agent_task', `Benchmark Task ${i}`, { x: i * 50, y: 100 }, { task: `Task ${i}`, priority: 'medium' });
            }
        ...
        test('should benchmark validation performance', async () => {
            ...
            const nodes = [];
            for (let i = 0; i < nodeCount; i++) {
                const node = builder.addNode('agent_task', `Validation Task ${i}`, { x: (i % 10) * 100, y: Math.floor(i / 10) * 100 }, { task: `Task ${i}`, priority: 'medium' });
                nodes.push(node);
            }
