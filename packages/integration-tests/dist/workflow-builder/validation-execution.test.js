...
class WorkflowBuilder {
    ...
    addNode(type, name, position, config) {
        const node = {
    ...
    getInlineValidation(nodeId, field) {
        return {
            hasError: false,
            message: 'valid',
            suggestions: []
    ...
    simulateNodeExecution(nodeId, status, data, metrics) {
        // Mock simulation
    }
    ...
    async getAgentSuggestions(nodeId) {
        return [];
    }
    ...
    wouldCreateCircularDependency(fromNodeId, toNodeId) {
...
describe('Workflow Validation and Execution Tests', () => {
    ...
    let builder;
    beforeAll(async () => {
        env = (0, test_setup_js_1.getTestEnvironment)();
    });
    beforeEach(async () => {
    ...
    describe('Comprehensive Workflow Validation', () => {
        test('should validate complete workflow structure', async () => {
            ...
            const taskNode = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Process Data', { x: 200, y: 100 }, {
        ...
        test('should detect missing start node', async () => {
            ...
            expect(validation.errors.some(error => error.type === 'missing_start_node' && error.severity === 'error')).toBe(true);
        });
        test('should detect missing end node', async () => {
            ...
            const validation = builder.validateWorkflow();
            expect(validation.isValid).toBe(false);
            expect(validation.errors.some(error => error.type === 'missing_end_node' && error.severity === 'error')).toBe(true);
        });
        test('should detect disconnected nodes', async () => {
            ...
            const validation = builder.validateWorkflow();
            expect(validation.isValid).toBe(false);
            expect(validation.errors.some(error => error.type === 'disconnected_node' &&
                error.nodeId === isolatedNode.id)).toBe(true);
        });
        ...
        test('should validate node configurations', async () => {
            ...
            const invalidTask = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Invalid Task', { x: 100, y: 100 }, {} // Missing required fields
            );
            const nodeValidation = builder.validateNode(invalidTask.id);
            expect(nodeValidation.isValid).toBe(false);
            expect(nodeValidation.errors.some(error => error.field === 'agentId' && error.type === 'required')).toBe(true);
            expect(nodeValidation.errors.some(error => error.field === 'task' && error.type === 'required')).toBe(true);
        });
        test('should validate condition node logic', async () => {
            // Create condition node with invalid logic
            const invalidCondition = builder.addNode(WorkflowNodeType.CONDITION, 'Invalid Condition', { x: 100, y: 100 }, {
                condition: 'invalid javascript syntax ===',
                conditionType: 'javascript'
            });
            const nodeValidation = builder.validateNode(invalidCondition.id);
            expect(nodeValidation.isValid).toBe(false);
            expect(nodeValidation.errors.some(error => error.type === 'syntax_error' && error.field === 'condition')).toBe(true);
        });
        test('should validate parallel node configuration', async () => {
            // Create parallel node with invalid configuration
            const invalidParallel = builder.addNode(WorkflowNodeType.PARALLEL, 'Invalid Parallel', { x: 100, y: 100 }, {
            ...
            expect(nodeValidation.isValid).toBe(false);
            expect(nodeValidation.errors.some(error => error.field === 'branches' && error.type === 'empty_array')).toBe(true);
            expect(nodeValidation.errors.some(error => error.field === 'maxConcurrency' && error.type === 'invalid_value')).toBe(true);
        });
        test('should validate connection compatibility', async () => {
            const dataNode = builder.addNode(WorkflowNodeType.DATA_TRANSFORM, 'Data Transform', { x: 100, y: 100 });
            const conditionNode = builder.addNode(WorkflowNodeType.CONDITION, 'Condition Check', { x: 200, y: 100 });
            // Valid connection
            const validConnection = builder.addConnection(dataNode.id, 'data_output', conditionNode.id, 'condition_input');
            ...
            const anotherDataNode = builder.addNode(WorkflowNodeType.DATA_TRANSFORM, 'Another Transform', { x: 300, y: 100 });
            const invalidConnection = builder.addConnection(dataNode.id, 'invalid_output', anotherDataNode.id, 'invalid_input');
        ...
        test('should provide validation warnings for performance issues', async () => {
            ...
            const parallelNode = builder.addNode(WorkflowNodeType.PARALLEL, 'Heavy Parallel', { x: 200, y: 100 }, {
            ...
            const taskNodes = [];
            for (let i = 0; i < 20; i++) {
                const taskNode = builder.addNode(WorkflowNodeType.AGENT_TASK, `Task ${i}`, { x: 300, y: i * 50 }, {
            ...
            const validation = builder.validateWorkflow();
            expect(validation.warnings.some(warning => warning.type === 'high_concurrency' && warning.severity === 'warning')).toBe(true);
            expect(validation.warnings.some(warning => warning.type === 'long_execution_time' && warning.severity === 'warning')).toBe(true);
            expect(validation.warnings.some(warning => warning.type === 'complex_workflow' && warning.severity === 'info')).toBe(true);
        });
    });
    describe('Real-Time Validation', () => {
        test('should validate workflow in real-time during editing', async () => {
            const validationEvents = [];
            builder.onValidationChange = (validation) => {
            ...
            // Add start node
            const startNode = builder.addNode(WorkflowNodeType.START, 'Start', { x: 100, y: 100 });
            expect(validationEvents[validationEvents.length - 1]?.errors.some(e => e.type === 'missing_start_node')).toBe(false);
            // Add task node
            const taskNode = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Task', { x: 200, y: 100 }, { task: 'Process data', priority: 'medium' });
        ...
        test('should highlight validation errors on canvas', async () => {
            ...
            const invalidTask = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Invalid Task', { x: 100, y: 100 }, {} // Missing required configuration
            );
        ...
        test('should provide inline validation feedback', async () => {
            ...
            const conditionNode = builder.addNode(WorkflowNodeType.CONDITION, 'Condition', { x: 100, y: 100 }, {
        ...
        test('should validate dependencies and agent availability', async () => {
            // Create test agent
            const agent = await test_setup_js_1.TestHelpers.createTestAgent('TestAgent', 'DATA_PROCESSOR');
            // Create task that references the agent
            const taskNode = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Agent Task', { x: 100, y: 100 }, {
    ...
    describe('Execution Simulation and Monitoring', () => {
        test('should simulate workflow execution', async () => {
            // Create valid workflow
            const startNode = builder.addNode(WorkflowNodeType.START, 'Start', { x: 100, y: 100 });
            const taskNode = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Simulation Task', { x: 200, y: 100 }, {
                agentId: 'test-agent-id',
        ...
        test('should handle execution errors and debugging', async () => {
            ...
            const startNode = builder.addNode(WorkflowNodeType.START, 'Start', { x: 100, y: 100 });
            const errorNode = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Error Task', { x: 200, y: 100 }, {
        ...
        test('should track execution performance metrics', async () => {
            // Create workflow
            const startNode = builder.addNode(WorkflowNodeType.START, 'Start', { x: 100, y: 100 });
            const taskNode = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Performance Task', { x: 200, y: 100 }, {
            ...
            expect(metrics.warnings.some(warning => warning.type === 'slow_execution' && warning.nodeId === taskNode.id)).toBe(true);
        });
        test('should support step-by-step debugging', async () => {
            // Create complex workflow
            const startNode = builder.addNode(WorkflowNodeType.START, 'Start', { x: 100, y: 100 });
            const conditionNode = builder.addNode(WorkflowNodeType.CONDITION, 'Decision', { x: 200, y: 100 }, {
    ...
    describe('Integration with Agent System', () => {
        test('should validate agent capabilities against task requirements', async () => {
            ...
            const agent = await test_setup_js_1.TestHelpers.createTestAgent('SpecializedAgent', 'DATA_PROCESSOR');
            ...
            const compatibleTask = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Compatible Task', { x: 100, y: 100 }, {
                agentId: agent.agentId,
            ...
            const incompatibleTask = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Incompatible Task', { x: 200, y: 100 }, {
                agentId: agent.agentId,
                task: 'Process files',
            ...
            expect(incompatibleValidation.errors.some(error => error.type === 'missing_capability' && error.capability === 'fileOperations')).toBe(true);
        });
        test('should suggest alternative agents for tasks', async () => {
            // Create multiple agents with different capabilities
            const dataAgent = await test_setup_js_1.TestHelpers.createTestAgent('DataAgent', 'DATA_PROCESSOR');
            const fileAgent = await test_setup_js_1.TestHelpers.createTestAgent('FileAgent', 'FILE_PROCESSOR');
            const apiAgent = await test_setup_js_1.TestHelpers.createTestAgent('APIAgent', 'API_CONNECTOR');
            ...
            const taskNode = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Unassigned Task', { x: 100, y: 100 }, {
            ...
            expect(suggestions.some(suggestion => suggestion.agentId === apiAgent.agentId && suggestion.compatibility > 0.8)).toBe(true);
            expect(suggestions.some(suggestion => suggestion.agentId === dataAgent.agentId && suggestion.compatibility < 0.8)).toBe(true);
        });
        test('should validate agent workload and availability', async () => {
            // Create agent
            const agent = await test_setup_js_1.TestHelpers.createTestAgent('BusyAgent', 'WORKER');
            ...
            const taskNode = builder.addNode(WorkflowNodeType.AGENT_TASK, 'New Task', { x: 100, y: 100 }, {
                agentId: agent.agentId,
        ...
        test('should integrate with agent heartbeat monitoring', async () => {
            // Create agent
            const agent = await test_setup_js_1.TestHelpers.createTestAgent('MonitoredAgent', 'MONITORED');
            // Create task for agent
            const taskNode = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Monitored Task', { x: 100, y: 100 }, {
                agentId: agent.agentId,
            ...
            expect(updatedValidation.warnings.some(warning => warning.type === 'agent_offline')).toBe(false);
        });
    });
    describe('Performance Validation', () => {
        test('should validate workflow performance characteristics', async () => {
            ...
            const taskNodes = [];
            for (let i = 0; i < 50; i++) {
                const taskNode = builder.addNode(WorkflowNodeType.AGENT_TASK, `Heavy Task ${i}`, { x: 300, y: i * 20 }, {
            ...
            expect(perfValidation.warnings.some(warning => warning.type === 'high_concurrency')).toBe(true);
            expect(perfValidation.warnings.some(warning => warning.type === 'long_execution_time')).toBe(true);
            expect(perfValidation.warnings.some(warning => warning.type === 'resource_intensive')).toBe(true);
        ...
        test('should suggest performance optimizations', async () => {
            ...
            for (let i = 0; i < 10; i++) {
                const taskNode = builder.addNode(WorkflowNodeType.AGENT_TASK, `Sequential Task ${i}`, { x: (i + 1) * 100, y: 100 }, {
...
//# sourceMappingURL=validation-execution.test.js.map