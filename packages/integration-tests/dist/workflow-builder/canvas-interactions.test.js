...
class WorkflowBuilder {
    ...
    removeNode(id) { this.nodes = this.nodes.filter(n => n.id !== id); }
    ...
    setGridSnap(_enabled, _options) { }
    handleNodeDragMove(_event) { }
    getSnapGuides() {
        return {
    ...
    setAlignmentGuides(_enabled, _options) { }
    getAlignmentGuides() {
        return {
            horizontal: {
                visible: true,
    ...
    updateMarqueeSelection(position) {
    ...
    startLassoSelection(position) {
        this.lassoPath = [position];
    }
    updateLassoSelection(path) {
        this.lassoPath = path;
    }
    endLassoSelection() {
    ...
    setNodeHighlight(_nodeId, _highlight) { }
    setConfiguration(_config) { }
    handleDragMove(_event) { }
    handleDragEnd(event) {
    ...
    calculateConnectionPath(sourceId, sourceHandle, targetId, targetHandle, options) {
        const paths = {
    ...
    setNodeState(_nodeId, _state) { }
    getNodeState(_nodeId) {
        const states = {
    ...
    getConnectionTooltip(connectionId) {
    ...
    setAccessibilityMode(_mode, _enabled) { }
    getNodeStyle(nodeId) {
        return {
            highContrast: true,
            borderWidth: 2,
    ...
    getConnectionStyle(_connectionId) {
        return {
            highContrast: true,
            strokeWidth: 3
        };
...
describe('Canvas Interactions and Visual Feedback', () => {
    let builder;
    beforeAll(async () => {
        (0, test_setup_js_1.getTestEnvironment)();
    });
    beforeEach(async () => {
    ...
    describe('Mouse Interactions', () => {
        ...
        test('should handle shift+click for range selection', async () => {
            ...
            const nodes = [];
            for (let i = 0; i < 5; i++) {
                const node = builder.addNode(WorkflowNodeType.AGENT_TASK, `Task ${i}`, { x: i * 100 + 100, y: 100 });
                nodes.push(node);
            }
            // Select first node
            builder.handleCanvasClick({
        ...
        test('should handle double-click for node editing', async () => {
            const node = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Editable Task', { x: 100, y: 100 }, { task: 'Original task', priority: 'medium' });
    ...
    describe('Performance with Visual Effects', () => {
        test('should maintain performance with many visual elements', async () => {
            ...
            for (let i = 0; i < nodeCount; i++) {
                const node = builder.addNode(WorkflowNodeType.AGENT_TASK, `Task ${i}`, { x: (i % 10) * 100 + 50, y: Math.floor(i / 10) * 100 + 50 }, { task: `Process data ${i}` });
                nodes.push(node);
            ...
            const interactionStart = perf_hooks_1.performance.now();
            // Simulate rapid selection changes
            for (let i = 0; i < 50; i++) {
                builder.selectNode(nodes[i].id);
                builder.clearSelection();
            }
            const interactionEnd = perf_hooks_1.performance.now();
        ...
        test('should handle smooth animation during node drag', async () => {
            ...
            const dragStart = perf_hooks_1.performance.now();
            // Start drag
            ...
            const dragEnd = perf_hooks_1.performance.now();
        ...
        test('should optimize rendering for large canvas areas', async () => {
            ...
            const visibilityStart = perf_hooks_1.performance.now();
            const visibleNodes = builder.getVisibleNodes();
            const visibilityEnd = perf_hooks_1.performance.now();
            ...
            const zoomStart = perf_hooks_1.performance.now();
            for (let i = 0; i < 10; i++) {
                builder.setViewport({
                    x: -500,
                    y: -500,
            ...
            const zoomEnd = perf_hooks_1.performance.now();
        ...
        test('should handle efficient connection path calculation', async () => {
            ...
            const pathStart = perf_hooks_1.performance.now();
            // Calculate connection path with various styles
            const straightPath = builder.calculateConnectionPath(sourceNode.id, 'output', targetNode.id, 'input', { style: 'straight' });
            const bezierPath = builder.calculateConnectionPath(sourceNode.id, 'output', targetNode.id, 'input', { style: 'bezier' });
            const steppedPath = builder.calculateConnectionPath(sourceNode.id, 'output', targetNode.id, 'input', { style: 'stepped' });
            const pathEnd = perf_hooks_1.performance.now();
    ...
    describe('Accessibility and User Experience', () => {
        ...
        test('should provide screen reader announcements', async () => {
            const announcements = [];
            builder.onAccessibilityAnnouncement = (message) => {
        ...
        test('should provide visual indicators for states', async () => {
            const node = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Task Node', { x: 100, y: 100 }, { task: 'Process data' });
        ...
        test('should provide tooltips and contextual help', async () => {
            const node = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Complex Task', { x: 100, y: 100 }, {
...
//# sourceMappingURL=canvas-interactions.test.js.map