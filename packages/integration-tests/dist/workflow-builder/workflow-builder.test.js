...
class WorkflowBuilder {
    constructor(config) {
        ...
        this.gridSnap = { enabled: false, size: 20 };
    ...
    setConfiguration(config) { Object.assign(this.config, config); }
    ...
    updateNode(id, updates) {
        ...
        if (node) {
            ...
            if (updates.position)
                node.position = this.snapToGrid(updates.position);
        }
        return node;
    }
    removeNode(id) {
    ...
    validateNode(id) {
        ...
        if (node && node.type === WorkflowNodeType.AGENT_TASK) {
            if (!node.config.agentId)
                errors.push({ message: 'Agent ID is required' });
            if (!node.config.task)
                errors.push({ message: 'Task is required' });
    ...
    copySelection() {
        ...
        const selectedConnections = this.connections.filter(c => this.selectedNodes.includes(c.source) && this.selectedNodes.includes(c.target));
        return { nodes: selectedNodeObjs, connections: selectedConnections };
    }
    paste(copied, offset) {
        const nodeIdMap = new Map();
        const pastedNodes = copied.nodes.map((node) => {
            const newNode = this.addNode(node.type, node.name, { x: node.position.x + offset.x, y: node.position.y + offset.y }, { ...node.config });
            nodeIdMap.set(node.id, newNode.id);
            return newNode;
        });
        const pastedConnections = copied.connections.map((conn) => {
            return this.addConnection(nodeIdMap.get(conn.source), conn.sourceHandle, nodeIdMap.get(conn.target), conn.targetHandle);
        });
    ...
    replaceTemplateVars(obj, parameters) {
...
describe('Drag and Drop Workflow Builder Tests', () => {
    let env; // Used for test environment
    let builder;
    beforeAll(async () => {
    ...
    describe('Workflow Builder Initialization', () => {
        ...
        test('should load existing workflow into builder', async () => {
            ...
            const { workflow } = await test_setup_js_1.TestHelpers.createTestWorkflow('Load Test Workflow');
            // Created workflow
    ...
    describe('Node Creation and Management', () => {
        ...
        test('should create agent task nodes with configuration', async () => {
            ...
            const agent = await test_setup_js_1.TestHelpers.createTestAgent('TestAgent', 'TASK_PROCESSOR');
            const taskNode = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Process Data', { x: 200, y: 150 }, {
        ...
        test('should create conditional nodes with logic', async () => {
            const conditionNode = builder.addNode(WorkflowNodeType.CONDITION, 'Data Check', { x: 300, y: 200 }, {
        ...
        test('should create parallel execution nodes', async () => {
            const parallelNode = builder.addNode(WorkflowNodeType.PARALLEL, 'Parallel Processing', { x: 400, y: 250 }, {
                branches: [
        ...
        test('should duplicate nodes with new positions', async () => {
            const originalNode = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Original Task', { x: 100, y: 100 }, { task: 'Do something', priority: 'medium' });
    ...
    describe('Connection Management', () => {
        ...
        test('should validate connection compatibility', async () => {
            const dataNode = builder.addNode(WorkflowNodeType.DATA_TRANSFORM, 'Transform Data', { x: 100, y: 100 });
            const conditionNode = builder.addNode(WorkflowNodeType.CONDITION, 'Check Condition', { x: 300, y: 100 });
            // Valid connection
            ...
            const invalidConnection = builder.addConnection(dataNode.id, 'invalid_output', conditionNode.id, 'invalid_input');
    ...
    describe('Canvas Operations', () => {
        test('should handle drag and drop positioning', async () => {
            ...
            const nodes = [];
            for (let i = 0; i < 10; i++) {
                const node = builder.addNode(WorkflowNodeType.AGENT_TASK, `Task ${i}`, { x: i * 150, y: i * 100 }, { task: `Process data ${i}` });
                nodes.push(node);
            }
        ...
        test('should snap nodes to grid when enabled', async () => {
            ...
            const node = builder.addNode(WorkflowNodeType.START, 'Start', { x: 127, y: 143 } // Non-grid-aligned position
            );
    ...
    describe('Validation and Error Handling', () => {
        ...
        test('should detect validation errors', async () => {
            ...
            expect(validation.errors.some(error => error.message.includes('disconnected') || error.message.includes('isolated'))).toBe(true);
        });
        test('should validate node configurations', async () => {
            // Create agent task without required configuration
            const invalidTaskNode = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Invalid Task', { x: 100, y: 100 }, {} // Missing required agentId and task
            );
        ...
        test('should validate connections between incompatible nodes', async () => {
            ...
            const invalidConnection = builder.addConnection(startNode.id, 'invalid_handle', conditionNode.id, 'invalid_input');
            expect(invalidConnection).toBeNull();
            // Create valid connection
            const validConnection = builder.addConnection(startNode.id, 'output', conditionNode.id, 'input');
            expect(validConnection).toBeDefined();
        });
        test('should handle missing agent references', async () => {
            const taskNode = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Task with Missing Agent', { x: 100, y: 100 }, {
                agentId: 'non-existent-agent-id',
                task: 'Do something'
            });
            const validation = builder.validateNode(taskNode.id);
            expect(validation.isValid).toBe(false);
            expect(validation.errors.some(error => error.message.includes('agent') && error.message.includes('not found'))).toBe(true);
        });
    ...
    describe('Performance and Scalability', () => {
        test('should handle large workflows efficiently', async () => {
            ...
            const nodes = [];
            for (let i = 0; i < 100; i++) {
                const node = builder.addNode(WorkflowNodeType.AGENT_TASK, `Task ${i}`, { x: (i % 10) * 150, y: Math.floor(i / 10) * 100 }, { task: `Process data ${i}`, priority: 'medium' });
                nodes.push(node);
            }
        ...
        test('should validate large workflows efficiently', async () => {
            ...
            for (let i = 0; i < nodeCount; i++) {
                const node = builder.addNode(WorkflowNodeType.AGENT_TASK, `Task ${i}`, { x: i * 100, y: 100 }, { task: `Task ${i}`, priority: 'medium' });
                nodes.push(node);
            }
        ...
        test('should handle rapid node creation and deletion', async () => {
            ...
            for (let i = 0; i < operationCount; i++) {
                const node = builder.addNode(WorkflowNodeType.AGENT_TASK, `Temp Task ${i}`, { x: i * 10, y: 100 });
        ...
        test('should maintain performance with complex selection operations', async () => {
            ...
            for (let x = 0; x < gridSize; x++) {
                for (let y = 0; y < gridSize; y++) {
                    const node = builder.addNode(WorkflowNodeType.AGENT_TASK, `Task ${x}-${y}`, { x: x * 150, y: y * 100 });
                    nodes.push(node);
                }
            }
    ...
    describe('Template and Export Operations', () => {
        test('should export workflow as JSON', async () => {
            ...
            const node1 = builder.addNode(WorkflowNodeType.START, 'Start', { x: 100, y: 100 });
            const node2 = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Process Data', { x: 200, y: 100 }, { task: 'Process input data', priority: 'high' });
            const node3 = builder.addNode(WorkflowNodeType.END, 'End', { x: 300, y: 100 });
            builder.addConnection(node1.id, 'output', node2.id, 'input');
            builder.addConnection(node2.id, 'output', node3.id, 'input');
        ...
        test('should import workflow from JSON', async () => {
            ...
            const originalNode = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Original Task', { x: 100, y: 100 }, { task: 'Original task', priority: 'medium' });
            const exported = builder.exportWorkflow('json');
        ...
        test('should create workflow template', async () => {
            ...
            const startNode = builder.addNode(WorkflowNodeType.START, 'Start', { x: 100, y: 100 });
            const taskNode = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Process {{dataType}}', { x: 200, y: 100 }, {
        ...
        test('should instantiate template with parameters', async () => {
            ...
            const startNode = builder.addNode(WorkflowNodeType.START, 'Start', { x: 100, y: 100 });
            const taskNode = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Process {{dataType}}', { x: 200, y: 100 }, { task: 'Process {{dataType}} data', priority: '{{priority}}' });
            builder.addConnection(startNode.id, 'output', taskNode.id, 'input');
            const template = builder.createTemplate({
                name: 'Test Template',
                parameters: [
