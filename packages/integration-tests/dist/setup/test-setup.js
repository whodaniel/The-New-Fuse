...
async function setupTestEnvironment() {
    ...
    // Setup Logger
    const logger = new relay_core_1.Logger('info', testDataDir);
    ...
    const drizzle = {
        ...
        agent: {
            ...
            update: ({ where, data }) => Promise.resolve({
                id: where.id,
                ...data,
                updatedAt: new Date()
            })
    ...
    const heartbeatService = new relay_core_1.HeartbeatMonitoringService({
        intervalMs: 5000,
        timeoutMs: 10000,
        maxRetries: 3,
    ...
    heartbeatService.getAgentHealth = async (agentId) => {
    ...
    heartbeatService.getStagnationStatus = async () => {
        const now = Date.now();
    ...
    heartbeatService.recordActivity = async (agentId, _activityType, _metadata) => {
        const agent = agentState.agents.get(agentId);
        if (agent) {
    ...
    const masterRegistry = new relay_core_1.MasterAgentRegistry(drizzle, logger);
    // Override registerAgent to mock it completely since the real implementation has issues with mock drizzle
    masterRegistry.registerAgent = async (agentData) => {
        ...
        masterRegistry.agentProfiles.set(agentId, profile);
    ...
    masterRegistry.getAgentProfile = (agentId) => {
    ...
    masterRegistry.updateAgentCapabilities = async (agentId, capabilities) => {
        const agent = agentState.agents.get(agentId);
        const profile = masterRegistry.agentProfiles.get(agentId);
        if (agent && profile) {
    ...
    masterRegistry.agentProfiles = new Map();
    masterRegistry.getAllAgentProfiles = () => {
        return Array.from(masterRegistry.agentProfiles.values());
    };
    masterRegistry.updateAgentStatus = async (agentId, status) => {
        const profile = masterRegistry.agentProfiles.get(agentId);
        if (profile) {
    ...
    masterRegistry.getAgentsByType = (type) => {
        return Array.from(masterRegistry.agentProfiles.values())
            .filter((profile) => profile.type === type);
    };
    masterRegistry.executeAgentCapability = async (agentId, capability, _input) => {
        const agent = agentState.agents.get(agentId);
        if (!agent) {
            return { success: false, error: { message: 'Agent not found' } };
        }
        // Find the matching extension
        let matchingExt = Array.from(extensionState.extensions.values()).find(e => e.name.includes(capability) ||
    ...
    masterRegistry.addTaskToAgent = async (agentId, task) => {
        const agent = agentState.agents.get(agentId);
        const profile = masterRegistry.agentProfiles.get(agentId);
        if (agent && profile) {
    ...
    masterRegistry.getSystemHealth = () => ({
        status: 'healthy',
    ...
    let currentWorkflowBeingBuilt = null;
    const workflowSystem = {
        builder: {
            ...
            addConnection: (fromNodeId, fromHandle, toNodeId, toHandle) => ({
                id: `conn-${Date.now()}`,
                source: fromNodeId,
        ...
        engine: {
            executeWorkflow: async (workflowId, input) => {
                ...
                if (workflowDef && workflowDef.definition && workflowDef.definition.nodes) {
                    workflowDef.definition.nodes.forEach(async (node) => {
                        ...
                        if (node.type === 'AGENT_TASK' && node.config) {
                            ...
                            if (!targetAgentId && requiredCapabilities) {
                                const matchingAgent = Array.from(agentState.agents.entries()).find(([_id, agent]) => {
                                    return requiredCapabilities.every((cap) => agent.capabilities && agent.capabilities[cap] === true);
                                });
                                if (matchingAgent) {
                            ...
                            if (!targetAgentId && node.config.agentType) {
                                ...
                                if (matchingAgents.length > 0) {
                                    ...
                                    const agentWithFewestTasks = matchingAgents.reduce((min, [agentId, _agent]) => {
                                        ...
                                        const minProfile = masterRegistry.agentProfiles.get(min[0]);
                            ...
                            if (targetAgentId) {
                                const profile = masterRegistry.agentProfiles.get(targetAgentId);
                                if (profile) {
                                    const todo = {
                                    ...
                                    if (node.config.requiredExtensions && Array.isArray(node.config.requiredExtensions)) {
                                        for (const extensionName of node.config.requiredExtensions) {
                                            try {
                                                await masterRegistry.executeAgentCapability(targetAgentId, extensionName, {
                                                    task: task || node.name,
                                                    nodeId: node.id,
                                                    workflowExecutionId: executionId
                                                });
                        ...
                        if (node.type === 'AGENT_HANDOFF' && node.config) {
                            ...
                            if (fromAgentId && toAgentId) {
                                const fromProfile = masterRegistry.agentProfiles.get(fromAgentId);
                                const toProfile = masterRegistry.agentProfiles.get(toAgentId);
                ...
                if (workflowDef && workflowDef.definition && workflowDef.definition.nodes) {
                    workflowDef.definition.nodes.forEach((node) => {
                ...
                setTimeout(() => {
                    ...
                    if (workflowDef && workflowDef.definition && workflowDef.definition.nodes) {
                        workflowDef.definition.nodes.forEach((node) => {
                            ...
                            const ext = Array.from(extensionState.extensions.values()).find(e => e.type === 'workflow_node' &&
                            ...
                            if (node.type === 'AGENT_TASK' && node.config && node.config.requiredExtensions) {
                                node.config.requiredExtensions.forEach((extName) => {
                                    const capabilityExt = Array.from(extensionState.extensions.values()).find(e => e.type === 'agent_capability' &&
        ...
        repository: {
            ...
            updateWorkflow: (workflowId, workflow) => Promise.resolve({
                id: workflowId,
                ...workflow
            }),
            getWorkflow: (workflowId) => Promise.resolve({
                id: workflowId,
                name: 'Test Workflow',
                definition: { nodes: [], edges: [] }
            }),
            ...
            deleteWorkflow: async (_workflowId) => {
                return true;
            },
            listWorkflows: async () => {
                return [];
        ...
        taskAssignment: {
            ...
            unassignTask: async (taskId, agentId) => {
                const agent = agentState.agents.get(agentId);
                if (agent && agent.tasks) {
                    agent.tasks = agent.tasks.filter((task) => task.id !== taskId);
                    return true;
                }
                return false;
            },
            getTaskAssignments: async (agentId) => {
                const agent = agentState.agents.get(agentId);
                return agent?.tasks || [];
            }
        }
    ...
    const extensionManager = {
        ...
        getExtension: (extensionId) => extensionState.extensions.get(extensionId) || null,
        getAllExtensions: () => Array.from(extensionState.extensions.values()),
        setExtensionConfig: (extensionId, config) => {
            const ext = extensionState.extensions.get(extensionId);
            if (ext) {
        ...
        scanExtensionSecurity: (extensionId) => {
            let permissions = [];
            if (extensionId) {
                const ext = extensionState.extensions.get(extensionId);
        ...
        getExtensionMetrics: (extensionIdOrName) => {
            // Try to find by ID first
            let metrics = extensionState.metrics.get(extensionIdOrName);
            // If not found by ID, try to find by name
            if (!metrics) {
                const extension = Array.from(extensionState.extensions.values()).find(e => e.name.replace('@test/', '') === extensionIdOrName ||
...
exports.TestHelpers = {
     ...
     * Create a test agent
     */
    async createTestAgent(name, type = 'TEST_AGENT') {
        ...
        const agentState = env.agentState;
        if (agentState) {
     ...
     */
    async createTestWorkflow(name, description = 'Test workflow') {
        const env = globalTestEnv;
        if (!env) {
            throw new Error('Test environment not initialized');
        }
     ...
     */
    async createTestExtension(name, type = 'custom') {
        const env = globalTestEnv;
        if (!env) {
            throw new Error('Test environment not initialized');
        }
