...
describe('Performance Benchmarks', () => {
    ...
    describe('Master Agent Registry Performance', () => {
        test('agent registration performance', async () => {
            ...
            for (let i = 0; i < iterations; i++) {
                ...
                void await test_setup_js_1.TestHelpers.createTestAgent(`PerfTestAgent${i}`, 'PERF_TEST');
        ...
        test('agent capability update performance', async () => {
            ...
            const testAgents = await Promise.all(Array.from({ length: 50 }, (_, i) => test_setup_js_1.TestHelpers.createTestAgent(`CapabilityTestAgent${i}`, 'CAPABILITY_TEST')));
        ...
        test('agent profile retrieval performance', async () => {
            // Create agents for testing
            const testAgents = await Promise.all(Array.from({ length: 100 }, (_, i) => test_setup_js_1.TestHelpers.createTestAgent(`RetrievalTestAgent${i}`, 'RETRIEVAL_TEST')));
        ...
        test('concurrent agent operations performance', async () => {
            ...
            const operationPromises = Array.from({ length: concurrentOperations }, async (_, i) => {
                ...
                for (let j = 0; j < operationsPerBatch; j++) {
                    ...
                    // Mix of operations
                    if (j % 3 === 0) {
                        const agent = await test_setup_js_1.TestHelpers.createTestAgent(`ConcurrentAgent${i}_${j}`, 'CONCURRENT_TEST');
    ...
    describe('Workflow Engine Performance', () => {
        test('workflow creation performance', async () => {
            ...
            for (let i = 0; i < iterations; i++) {
                const startTime = perf_hooks_1.performance.now();
                const { workflow, builder } = await test_setup_js_1.TestHelpers.createTestWorkflow(`PerfWorkflow${i}`);
                // Add some complexity
        ...
        test('workflow execution performance', async () => {
            // Create test agents
            const testAgents = await Promise.all(Array.from({ length: 10 }, (_, i) => test_setup_js_1.TestHelpers.createTestAgent(`WorkflowPerfAgent${i}`, 'WORKFLOW_PERF')));
            // Create workflow template
            const { workflow, builder } = await test_setup_js_1.TestHelpers.createTestWorkflow('Performance Test Workflow');
        ...
        test('concurrent workflow execution performance', async () => {
            // Create agent pool
            const agentPool = await Promise.all(Array.from({ length: 5 }, (_, i) => test_setup_js_1.TestHelpers.createTestAgent(`ConcurrentWorkflowAgent${i}`, 'CONCURRENT_WORKFLOW')));
            // Create multiple workflow templates
            const workflows = await Promise.all(Array.from({ length: 10 }, async (_, i) => {
                const { workflow, builder } = await test_setup_js_1.TestHelpers.createTestWorkflow(`ConcurrentWorkflow${i}`);
                const taskNode = builder.addNode(WorkflowNodeType.AGENT_TASK, `Concurrent Task ${i}`, { x: 200, y: 100 }, {
            ...
            const startTime = perf_hooks_1.performance.now();
            const executionPromises = Array.from({ length: concurrentExecutions }, (_, i) => env.workflowEngine.engine.executeWorkflow(workflows[i % workflows.length].id, { data: test_setup_js_1.TestHelpers.generateTestData(5), executionIndex: i }));
    ...
    describe('Extension System Performance', () => {
        test('extension loading performance', async () => {
            ...
            const times = [];
            for (let i = 0; i < iterations; i++) {
                const testExtension = await test_setup_js_1.TestHelpers.createTestExtension(`LoadPerfExtension${i}`, 'custom');
                const startTime = perf_hooks_1.performance.now();
        ...
        test('extension execution performance', async () => {
            ...
            const testExtension = await test_setup_js_1.TestHelpers.createTestExtension('ExecutionPerfExtension', 'agent_capability');
            const loadResult = await env.extensionManager.loadExtension(testExtension.extensionDir);
            await env.extensionManager.activateExtension(loadResult.extension.id);
            // Create test agent
            const agent = await test_setup_js_1.TestHelpers.createTestAgent('ExtensionPerfAgent', 'EXTENSION_PERF');
            ...
            for (let i = 0; i < iterations; i++) {
                const startTime = perf_hooks_1.performance.now();
                const result = await env.agentRegistry.executeAgentCapability(agent.agentId, 'ExecutionPerfExtension', { data: `test-data-${i}` });
                const endTime = perf_hooks_1.performance.now();
                times.push(endTime - startTime);
        ...
        test('concurrent extension usage performance', async () => {
            ...
            const extensions = await Promise.all([
                test_setup_js_1.TestHelpers.createTestExtension('ConcurrentExt1', 'agent_capability'),
                test_setup_js_1.TestHelpers.createTestExtension('ConcurrentExt2', 'agent_capability'),
                test_setup_js_1.TestHelpers.createTestExtension('ConcurrentExt3', 'agent_capability')
            ]);
            // Load all extensions
            ...
            const agents = await Promise.all([
                test_setup_js_1.TestHelpers.createTestAgent('ConcurrentAgent1', 'CONCURRENT_EXT'),
                test_setup_js_1.TestHelpers.createTestAgent('ConcurrentAgent2', 'CONCURRENT_EXT'),
                test_setup_js_1.TestHelpers.createTestAgent('ConcurrentAgent3', 'CONCURRENT_EXT')
            ]);
            // Wait for extensions
            await test_setup_js_1.TestHelpers.waitForCondition(async () => {
                const profiles = await Promise.all(agents.map(agent => env.agentRegistry.getAgentProfile(agent.agentId)));
            ...
            const executionPromises = Array.from({ length: concurrentExecutions }, (_, i) => {
                ...
                return env.agentRegistry.executeAgentCapability(agent.agentId, extension.name, { data: `concurrent-test-${i}` });
            });
    ...
    describe('Integrated System Performance', () => {
        test('end-to-end system performance', async () => {
            ...
            const extension = await test_setup_js_1.TestHelpers.createTestExtension('E2EPerfExtension', 'workflow_node');
            const loadResult = await env.extensionManager.loadExtension(extension.extensionDir);
            await env.extensionManager.activateExtension(loadResult.extension.id);
            const agent = await test_setup_js_1.TestHelpers.createTestAgent('E2EPerfAgent', 'E2E_PERF');
            ...
            const { workflow, builder } = await test_setup_js_1.TestHelpers.createTestWorkflow('E2E Performance Test');
            const agentTask = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Agent Task', { x: 150, y: 100 }, {
                agentId: agent.agentId,
                task: 'Process data with agent',
                priority: 'high',
                expectedDuration: 3
            });
            const extensionNode = builder.addNode('E2EPerfExtension', 'Extension Processing', { x: 300, y: 100 }, {
                processingMode: 'fast'
            });
            ...
            for (let i = 0; i < iterations; i++) {
                const startTime = perf_hooks_1.performance.now();
                const executionId = await env.workflowEngine.engine.executeWorkflow(savedWorkflow.id, { data: test_setup_js_1.TestHelpers.generateTestData(20) });
                await test_setup_js_1.TestHelpers.waitForCondition(async () => {
                    const execution = await env.workflowEngine.engine.getExecutionStatus(executionId);
        ...
        test('memory usage benchmark', async () => {
            ...
            void await Promise.all(Array.from({ length: 50 }, (_, i) => test_setup_js_1.TestHelpers.createTestAgent(`MemoryTestAgent${i}`, 'MEMORY_TEST')));
            const extensions = await Promise.all([
                test_setup_js_1.TestHelpers.createTestExtension('MemoryExt1', 'agent_capability'),
                test_setup_js_1.TestHelpers.createTestExtension('MemoryExt2', 'workflow_node'),
                test_setup_js_1.TestHelpers.createTestExtension('MemoryExt3', 'custom')
            ]);
            await Promise.all(extensions.map(ext => env.extensionManager.loadExtension(ext.extensionDir)));
            void await Promise.all(Array.from({ length: 20 }, async (_, i) => {
                const { workflow } = await test_setup_js_1.TestHelpers.createTestWorkflow(`MemoryWorkflow${i}`);
                return await env.workflowEngine.repository.createWorkflow(workflow);
            }));
    ...
    // Helper functions
    function calculateBenchmarkResult(name, iterations, times) {
    ...
    function addBenchmarkResult(suiteName, result) {
