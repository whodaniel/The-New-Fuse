...
describe('End-to-End Complete Scenarios', () => {
    ...
    describe('Data Processing Pipeline Scenario', () => {
        test('should execute complete data processing pipeline with agents and extensions', async () => {
            ...
            const extensions = await Promise.all([
                test_setup_js_1.TestHelpers.createTestExtension('data-validator', 'workflow_node'),
                test_setup_js_1.TestHelpers.createTestExtension('data-cleaner', 'agent_capability'),
                test_setup_js_1.TestHelpers.createTestExtension('data-analyzer', 'workflow_node'),
                test_setup_js_1.TestHelpers.createTestExtension('report-generator', 'agent_capability')
            ]);
            ...
            const dataCleaningAgent = await test_setup_js_1.TestHelpers.createTestAgent('DataCleaner', 'DATA_PROCESSOR');
            const reportingAgent = await test_setup_js_1.TestHelpers.createTestAgent('ReportGenerator', 'ANALYST');
            ...
            const { workflow, builder } = await test_setup_js_1.TestHelpers.createTestWorkflow('Complete Data Processing Pipeline', 'End-to-end data processing with validation, cleaning, analysis, and reporting');
            // Build the pipeline workflow
            const dataValidatorNode = builder.addNode('data-validator', 'Validate Input Data', { x: 150, y: 100 }, {
                validationRules: ['not_null', 'format_check', 'range_validation'],
                strictMode: true
            });
            const dataCleaningTask = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Clean Data', { x: 300, y: 100 }, {
            ...
            const dataAnalyzerNode = builder.addNode('data-analyzer', 'Analyze Data', { x: 450, y: 100 }, {
                analysisType: 'comprehensive',
                includeStatistics: true,
                generateInsights: true
            });
            const reportGenerationTask = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Generate Report', { x: 600, y: 100 }, {
            ...
            const testDataset = test_setup_js_1.TestHelpers.generateTestData(100);
    ...
    describe('Multi-Agent Collaboration Scenario', () => {
        test('should coordinate multiple agents with extensions in complex workflow', async () => {
            ...
            const extensions = await Promise.all([
                test_setup_js_1.TestHelpers.createTestExtension('task-coordinator', 'agent_capability'),
                test_setup_js_1.TestHelpers.createTestExtension('message-router', 'workflow_node'),
                test_setup_js_1.TestHelpers.createTestExtension('progress-tracker', 'agent_capability'),
                test_setup_js_1.TestHelpers.createTestExtension('conflict-resolver', 'workflow_node')
            ]);
            // Load extensions
            const loadResults = await Promise.all(extensions.map(ext => env.extensionManager.loadExtension(ext.extensionDir)));
            await Promise.all(loadResults.map(result => env.extensionManager.activateExtension(result.extension.id)));
            // Create team of specialized agents
            const teamLeader = await test_setup_js_1.TestHelpers.createTestAgent('TeamLeader', 'COORDINATOR');
            const developer1 = await test_setup_js_1.TestHelpers.createTestAgent('Developer1', 'DEVELOPER');
            const developer2 = await test_setup_js_1.TestHelpers.createTestAgent('Developer2', 'DEVELOPER');
            const tester = await test_setup_js_1.TestHelpers.createTestAgent('Tester', 'QA_TESTER');
            const reviewer = await test_setup_js_1.TestHelpers.createTestAgent('Reviewer', 'REVIEWER');
            ...
            await test_setup_js_1.TestHelpers.waitForCondition(async () => {
                const profiles = await Promise.all(agentConfigs.map(({ agent }) => env.agentRegistry.getAgentProfile(agent.agentId)));
                return profiles.every(profile => profile?.availableExtensions?.length >= 2);
            });
            // Create collaborative software development workflow
            const { workflow, builder } = await test_setup_js_1.TestHelpers.createTestWorkflow('Collaborative Software Development', 'Multi-agent workflow for feature development with reviews and testing');
            // Build collaboration workflow
            const coordinationNode = builder.addNode(WorkflowNodeType.AGENT_COORDINATION, 'Team Coordination', { x: 150, y: 100 }, {
            ...
            const messageRoutingNode = builder.addNode('message-router', 'Route Communications', { x: 300, y: 50 }, {
                routingStrategy: 'skill_based',
                enableBroadcast: true,
                messageFiltering: true
            });
            const developmentTask1 = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Feature Development - Part 1', { x: 200, y: 200 }, {
            ...
            const developmentTask2 = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Feature Development - Part 2', { x: 350, y: 200 }, {
            ...
            const conflictResolverNode = builder.addNode('conflict-resolver', 'Resolve Integration Conflicts', { x: 500, y: 150 }, {
                strategy: 'automated_merge',
                requireManualReview: true,
                notifyStakeholders: true
            });
            const testingTask = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Feature Testing', { x: 650, y: 100 }, {
            ...
            const reviewTask = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Code Review', { x: 800, y: 100 }, {
            ...
            const agentProfiles = await Promise.all(agentConfigs.map(({ agent }) => env.agentRegistry.getAgentProfile(agent.agentId)));
    ...
    describe('Error Recovery and Resilience Scenario', () => {
        test('should handle failures and recover gracefully in complex multi-system scenario', async () => {
            ...
            const extensions = await Promise.all([
                test_setup_js_1.TestHelpers.createTestExtension('unreliable-processor', 'workflow_node'),
                test_setup_js_1.TestHelpers.createTestExtension('backup-processor', 'workflow_node'),
                test_setup_js_1.TestHelpers.createTestExtension('error-handler', 'agent_capability'),
                test_setup_js_1.TestHelpers.createTestExtension('recovery-coordinator', 'agent_capability')
            ]);
...
`;
            ...
            const primaryAgent = await test_setup_js_1.TestHelpers.createTestAgent('PrimaryProcessor', 'PRIMARY');
            const backupAgent = await test_setup_js_1.TestHelpers.createTestAgent('BackupProcessor', 'BACKUP');
            const monitorAgent = await test_setup_js_1.TestHelpers.createTestAgent('SystemMonitor', 'MONITOR');
            ...
            const { workflow, builder } = await test_setup_js_1.TestHelpers.createTestWorkflow('Resilient Processing Pipeline', 'Error-prone workflow with recovery mechanisms');
            // Build resilient workflow
            const monitoringTask = builder.addNode(WorkflowNodeType.AGENT_TASK, 'System Monitoring', { x: 100, y: 50 }, {
            ...
            const unreliableNode = builder.addNode('unreliable-processor', 'Primary Processing (Unreliable)', { x: 200, y: 100 }, {
                retryAttempts: 3,
                timeoutMs: 10000,
                fallbackEnabled: true
            });
            const errorDetectionTask = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Error Detection', { x: 350, y: 50 }, {
            ...
            const backupNode = builder.addNode('backup-processor', 'Backup Processing', { x: 500, y: 150 }, {
                activateOnFailure: true,
                preserveContext: true
            });
            const recoveryTask = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Recovery Coordination', { x: 650, y: 100 }, {
            ...
            for (let i = 0; i < 5; i++) {
                const executionPromise = env.workflowEngine.engine.executeWorkflow(savedWorkflow.id, {
                    data: test_setup_js_1.TestHelpers.generateTestData(20),
                    executionIndex: i,
                    expectFailures: true
                });
            ...
            await test_setup_js_1.TestHelpers.waitForCondition(async () => {
                const executions = await Promise.all(executionIds.map(id => env.workflowEngine.engine.getExecutionStatus(id)));
                return executions.every(exec => exec?.status === WorkflowExecutionStatus.COMPLETED ||
    ...
    describe('Performance and Scalability Scenario', () => {
        test('should handle high-load scenario with multiple workflows and agents', async () => {
            ...
            const extensions = await Promise.all([
                test_setup_js_1.TestHelpers.createTestExtension('high-throughput-processor', 'workflow_node'),
                test_setup_js_1.TestHelpers.createTestExtension('load-balancer', 'agent_capability'),
                test_setup_js_1.TestHelpers.createTestExtension('performance-monitor', 'agent_capability'),
                test_setup_js_1.TestHelpers.createTestExtension('resource-optimizer', 'workflow_node')
            ]);
            // Load extensions
            const loadResults = await Promise.all(extensions.map(ext => env.extensionManager.loadExtension(ext.extensionDir)));
            await Promise.all(loadResults.map(result => env.extensionManager.activateExtension(result.extension.id)));
            // Create agent pool for high-load processing
            const agentPool = await Promise.all(Array.from({ length: 10 }, (_, i) => test_setup_js_1.TestHelpers.createTestAgent(`WorkerAgent${i}`, 'WORKER')));
            const loadBalancer = await test_setup_js_1.TestHelpers.createTestAgent('LoadBalancer', 'LOAD_BALANCER');
            const performanceMonitor = await test_setup_js_1.TestHelpers.createTestAgent('PerformanceMonitor', 'MONITOR');
            ...
            const { workflow, builder } = await test_setup_js_1.TestHelpers.createTestWorkflow('High-Performance Processing Template', 'Scalable workflow for high-load processing');
            // Build high-performance workflow
            const performanceMonitoringTask = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Performance Monitoring', { x: 100, y: 50 }, {
            ...
            const loadBalancingTask = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Load Balancing', { x: 200, y: 100 }, {
            ...
            const resourceOptimizerNode = builder.addNode('resource-optimizer', 'Resource Optimization', { x: 350, y: 50 }, {
                optimizationStrategy: 'dynamic',
                resourceThrottling: true,
                memoryManagement: true
            });
            const highThroughputNode = builder.addNode('high-throughput-processor', 'High Throughput Processing', { x: 500, y: 100 }, {
                batchSize: 100,
                parallelism: 5,
                optimization: 'speed'
            });
            const coordinationNode = builder.addNode(WorkflowNodeType.AGENT_COORDINATION, 'Worker Coordination', { x: 650, y: 100 }, {
            ...
            const executionPromises = Array.from({ length: concurrentExecutions }, (_, i) => env.workflowEngine.engine.executeWorkflow(savedWorkflow.id, {
                data: test_setup_js_1.TestHelpers.generateTestData(datasetSize),
            ...
            const agentProfiles = await Promise.all(agentPool.map(agent => env.agentRegistry.getAgentProfile(agent.agentId)));
