...
describe('Master Agent Registry + Workflow Engine Integration', () => {
    ...
    describe('Agent Task Assignment', () => {
        test('should assign workflow tasks to agents', async () => {
            // Create test agents
            const dataAgent = await test_setup_js_1.TestHelpers.createTestAgent('DataProcessor', 'DATA_PROCESSOR');
            const analysisAgent = await test_setup_js_1.TestHelpers.createTestAgent('DataAnalyzer', 'ANALYST');
            // Create workflow with agent tasks
            const { workflow, builder } = await test_setup_js_1.TestHelpers.createTestWorkflow('Agent Task Test');
            // Add agent task nodes
            const processTaskNode = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Process Data', { x: 200, y: 100 }, {
            ...
            const analyzeTaskNode = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Analyze Data', { x: 350, y: 100 }, {
            ...
            const executionId = await env.workflowEngine.engine.executeWorkflow(savedWorkflow.id, {
                data: test_setup_js_1.TestHelpers.generateTestData(50),
            });
        ...
        test('should handle agent capability matching', async () => {
            ...
            const fileAgent = await test_setup_js_1.TestHelpers.createTestAgent('FileProcessor', 'FILE_PROCESSOR');
            const apiAgent = await test_setup_js_1.TestHelpers.createTestAgent('APIConnector', 'API_CONNECTOR');
            ...
            const { workflow, builder } = await test_setup_js_1.TestHelpers.createTestWorkflow('Capability Matching Test');
            const fileTaskNode = builder.addNode(WorkflowNodeType.AGENT_TASK, 'File Processing', { x: 200, y: 100 }, {
                task: 'Read and process files',
                requiredCapabilities: ['fileOperations'],
                priority: 'medium',
                expectedDuration: 3,
            });
            const apiTaskNode = builder.addNode(WorkflowNodeType.AGENT_TASK, 'API Integration', { x: 350, y: 100 }, {
            ...
            const fileTask = fileAgentProfile.todoList.find((todo) => todo.content.includes('Read and process files'));
    ...
    describe('Agent Handoffs', () => {
        test('should execute agent handoffs in workflows', async () => {
            ...
            const primaryAgent = await test_setup_js_1.TestHelpers.createTestAgent('PrimaryAgent', 'PRIMARY');
            const secondaryAgent = await test_setup_js_1.TestHelpers.createTestAgent('SecondaryAgent', 'SECONDARY');
            // Create workflow with handoff
            const { workflow, builder } = await test_setup_js_1.TestHelpers.createTestWorkflow('Agent Handoff Test');
            const primaryTaskNode = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Primary Task', { x: 200, y: 100 }, {
                agentId: primaryAgent.agentId,
                task: 'Initial data processing',
                priority: 'high',
                expectedDuration: 3,
            });
            const handoffNode = builder.addNode(WorkflowNodeType.AGENT_HANDOFF, 'Handoff to Secondary', { x: 350, y: 100 }, {
            ...
            const secondaryTaskNode = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Secondary Task', { x: 500, y: 100 }, {
    ...
    describe('Agent Coordination', () => {
        test('should coordinate multiple agents in parallel workflows', async () => {
            ...
            const agents = await Promise.all([
                test_setup_js_1.TestHelpers.createTestAgent('Agent1', 'PROCESSOR'),
                test_setup_js_1.TestHelpers.createTestAgent('Agent2', 'PROCESSOR'),
                test_setup_js_1.TestHelpers.createTestAgent('Agent3', 'PROCESSOR'),
            ]);
            // Create workflow with parallel agent coordination
            const { workflow, builder } = await test_setup_js_1.TestHelpers.createTestWorkflow('Agent Coordination Test');
            const coordinationNode = builder.addNode(WorkflowNodeType.AGENT_COORDINATION, 'Parallel Processing', { x: 200, y: 100 }, {
            ...
            // Execute workflow
            const savedWorkflow = await env.workflowEngine.repository.createWorkflow(workflow);
            const testData = test_setup_js_1.TestHelpers.generateTestData(300); // Large dataset for parallel processing
    ...
    describe('Heartbeat Integration', () => {
        test('should monitor agent activity during workflow execution', async () => {
            ...
            const { workflow, builder } = await test_setup_js_1.TestHelpers.createTestWorkflow('Heartbeat Monitor Test');
            const longTaskNode = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Long Running Task', { x: 200, y: 100 }, {
            ...
            const savedWorkflow = await env.workflowEngine.repository.createWorkflow(workflow);
            const executionId = await env.workflowEngine.engine.executeWorkflow(savedWorkflow.id, {
                largeDataset: test_setup_js_1.TestHelpers.generateTestData(1000),
            });
        ...
        test('should detect and handle agent stagnation during workflow execution', async () => {
            // Create test agent
            const agent = await test_setup_js_1.TestHelpers.createTestAgent('StagnantAgent', 'STAGNANT_TEST');
            // Create workflow
            const { workflow, builder } = await test_setup_js_1.TestHelpers.createTestWorkflow('Stagnation Detection Test');
            const taskNode = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Task That Will Stagnate', { x: 200, y: 100 }, {
                agentId: agent.agentId,
    ...
    describe('Error Handling and Recovery', () => {
        test('should handle agent failures gracefully in workflows', async () => {
            // Create test agents
            const unreliableAgent = await test_setup_js_1.TestHelpers.createTestAgent('UnreliableAgent', 'UNRELIABLE');
            const backupAgent = await test_setup_js_1.TestHelpers.createTestAgent('BackupAgent', 'BACKUP');
            // Create workflow with error handling
            const { workflow, builder } = await test_setup_js_1.TestHelpers.createTestWorkflow('Error Handling Test');
            const primaryTaskNode = builder.addNode(WorkflowNodeType.AGENT_TASK, 'Primary Task (May Fail)', { x: 200, y: 100 }, {
    ...
    describe('Performance and Scalability', () => {
        test('should handle multiple concurrent workflows with agent task assignment', async () => {
            ...
            const agentPool = await Promise.all(Array.from({ length: 5 }, (_, i) => test_setup_js_1.TestHelpers.createTestAgent(`PoolAgent${i}`, 'POOL_AGENT')));
            // Create multiple workflows
            const workflows = await Promise.all(Array.from({ length: 10 }, async (_, i) => {
                const { workflow, builder } = await test_setup_js_1.TestHelpers.createTestWorkflow(`Concurrent Workflow ${i}`);
            ...
            const executionPromises = workflows.map((wf) => env.workflowEngine.engine.executeWorkflow(wf.id, { data: test_setup_js_1.TestHelpers.generateTestData(10) }));
            const executionIds = await Promise.all(executionPromises);
            // Wait for all executions to start
            await test_setup_js_1.TestHelpers.waitForCondition(async () => {
                const agentProfiles = await Promise.all(agentPool.map((a) => env.agentRegistry.getAgentProfile(a.agentId)));
