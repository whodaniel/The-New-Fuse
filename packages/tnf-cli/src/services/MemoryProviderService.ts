...
export class MemoryProviderService {
  ...
  constructor() {
    this.hermesHome = process.env.HERMES_HOME || path.join(os.homedir(), '.hermes');
    this.configPath = path.join(this.hermesHome, 'memory_provider.json');
  }
  ...
  async reset(): Promise<void> {
    const memoryPath = path.join(this.hermesHome, 'memories');
    const userPath = path.join(this.hermesHome, 'USER.md');
    const memoryPathFile = path.join(this.hermesHome, 'MEMORY.md');
