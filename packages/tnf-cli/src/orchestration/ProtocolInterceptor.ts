...
export class ProtocolInterceptor {
  ...
  getStateSummary(): Record<string, unknown> {
    ...
    return {
      ...
      handoff: handoffPayload
        ? { id: handoffPayload.handoffId, status: handoffPayload.continuation?.priority ?? 'active' }
        : null,
      disclosure: {
  ...
  enforceTurnZero(): void {
    ...
    if (missing.length > 0) {
      console.warn(
        chalk.yellow(`[ProtocolInterceptor] Turn Zero enforcement: ${missing.length} file(s) missing:`)
      );
