...
function postVoiceActivate(port) {
    const result = spawnSync('curl', ['-fsS', '--max-time', '2', '-X', 'POST', `http://127.0.0.1:${port}/activate`], {
        encoding: 'utf8',
        env: process.env,
    });
    if (result.status !== 0) {
...
masterClock
    ...
    try {
        ...
        console.log(chalk.cyan(`☁️ Starting cloud master clock on Railway service ${options.service}`));
...
superCycle
    ...
    try {
        ...
        console.log(chalk.cyan(`☁️ Sending super-cycle event via cloud service ${options.service}`));
...
import { generateCompletion, getInstallInstructions } from './services/CompletionService';
// ACP command
