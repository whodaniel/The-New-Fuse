...
export class StoryService {
    constructor() {
        ...
        if (!supabaseKey) {
            throw new Error('StoryService requires SUPABASE_ANON_KEY, SUPABASE_SERVICE_ROLE_KEY, or STORY_SUPABASE_KEY env var. ' +
        ...
        this.defaultOwnerPrincipalId = this.resolveOwnerPrincipalId(process.env.STORY_OWNER_PRINCIPAL_ID || process.env.TNF_OWNER_PRINCIPAL_ID || 'daniel');
    ...
    async listSessions(ownerPrincipalId) {
        ...
        if (error)
            throw this.wrapSupabaseError('list story sessions', error);
        return data || [];
    }
    async getActiveSession(ownerPrincipalId) {
    ...
    async createSession(params) {
        ...
        const { data, error } = await this.supabase
            ...
            .single();
        if (error)
            throw this.wrapSupabaseError('create story session', error);
        return data;
    }
    getQuestions() {
        return [
    ...
    async getCapturedQuestionIds(sessionId) {
        const { data, error } = await this.supabase
            .from('timeline_events')
            .select('tags')
            .eq('session_id', sessionId);
        if (error)
            throw this.wrapSupabaseError('fetch captured questions', error);
        const ids = [];
    ...
    async listTimelineEvents(ownerPrincipalId) {
        const resolvedOwner = this.resolveOwnerPrincipalId(ownerPrincipalId);
        ...
        if (error)
            throw this.wrapSupabaseError('list story timeline events', error);
        return data || [];
    }
    async doctor() {
        const supabaseUrl = process.env.SUPABASE_URL || 'https://wslydgtgindrywldatbv.supabase.co';
    ...
    async saveCapture(params) {
        ...
        if (!session) {
            throw new Error(`Story session "${params.sessionId}" was not found for owner "${resolvedOwner}".`);
        }
        ...
        const { data, error } = await this.supabase
            ...
            .single();
        if (error)
            throw this.wrapSupabaseError('capture story timeline event', error);
        return data;
    }
    async getSessionById(sessionId, ownerPrincipalId) {
        const { data, error } = await this.supabase
            .from('story_sessions')
            .select('*')
            .eq('id', sessionId)
            ...
            .maybeSingle();
        if (error)
            throw this.wrapSupabaseError('validate story session ownership', error);
        return data || null;
    }
    resolveOwnerPrincipalId(ownerPrincipalId) {
    ...
    wrapSupabaseError(action, error) {
        const message = this.extractErrorMessage(error);
        if (this.isPermissionError(error)) {
            const authHint = this.authMode === 'service-role'
