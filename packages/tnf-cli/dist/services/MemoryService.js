...
export class MemoryService {
    ...
    static { this.MAX_CONTEXT_MEMORIES = 20; }
    static { this.MAX_CONTEXT_CHARS = 8000; }
    async query(query, categoryFilter) {
        ...
        const loadedMemories = await Promise.all(topMemories.map(async (m) => {
            const content = await fs.readFile(path.join(this.memoryTreePath, m.category, m.name), 'utf8');
            return { ...m, content };
        }));
        let context = '';
        for (const m of loadedMemories) {
            const entry = `\n--- Memory (${m.category}): ${m.name} (relevance: ${m.score}) ---\n${m.content}\n`;
            if (context.length + entry.length > MemoryService.MAX_CONTEXT_CHARS)
                break;
            context += entry;
        }
        const systemPrompt = `
