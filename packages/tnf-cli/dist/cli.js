import { PackageReconnectHub } from '@the-new-fuse/tnf-core';
...
import { findSlashCommand, formatPromptSlashCommand, getAllSlashCommands, parseSlashCommand, renderSlashCommandDetail, renderSlashCommandList, } from './slashCommands.js';
...
function loadLocalEnv(rootDir) {
    ...
    for (const envFile of LOCAL_ENV_FILES) {
        ...
        for (const rawLine of fs.readFileSync(envPath, 'utf8').split(/\r?\n/)) {
            ...
            if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(key) || exportedKeys.has(key))
                continue;
...
async function runTnfCliEntrypoint(args) {
...
async function runTurnZeroOnboardSurface(options = {}) {
    const args = ['scripts/tnf-onboard.cjs', '--runtime-timeout-ms', '1000'];
    if (options.repair)
        args.push('--repair');
    await runCommand('node', args);
}
async function ensureTurnZeroForAgentEntrypoint() {
    if (isTruthyEnv(process.env.TNF_SKIP_TURN_ZERO_ONBOARD)) {
        console.warn(chalk.yellow('[TNF Harness] Skipping Turn Zero onboarding because TNF_SKIP_TURN_ZERO_ONBOARD is set.'));
        return;
    }
...
function resolveProvidedSuperAdminToken(options) {
    const candidates = [
...
function requireSuperAdmin(options, commandLabel) {
    ...
    if (provided.token !== expected) {
        throw new Error(`Super Admin authentication failed for '${commandLabel}'. Token from ${provided.source || 'unknown source'} does not match ${SUPER_ADMIN_ENV_KEY}.`);
    }
...
async function readVoiceBridgeJson(pathname, method = 'GET', port = 50005) {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(`Voice Bridge API call failed for ${method} ${pathname} on 127.0.0.1:${port} (HTTP ${response.status}).`);
        }
        ...
        if (err.name === 'TimeoutError' || err.code === 'ECONNREFUSED') {
            throw new Error(`Voice Bridge API call failed for ${method} ${pathname} on 127.0.0.1:${port}. Is voice server running?`);
        }
        throw err;
    }
}
...
function resolveVoiceBridgeStateDir() {
    ...
    if (repoRoot && fs.existsSync(repoRoot))
        return path.join(repoRoot, '.voicebridge');
...
async function postVoiceSend(port, text) {
...
async function postVoiceActivate(port) {
    try {
...
function readVoiceProfileLastInput(profileInput) {
    ...
    if (!fs.existsSync(tsPath) || !fs.existsSync(textPath))
        return null;
    let ts = Number.NaN;
    let text = '';
    try {
...
function findProfilePythonWorkerPids(profileInput, scriptName) {
    ...
    return parseProcessTable()
        ...
        if (!cmd.toLowerCase().includes(scriptName.toLowerCase()))
            return false;
        const argv0 = (cmd.trim().split(/\s+/)[0] || '').toLowerCase();
        if (!path.basename(argv0).includes('python'))
            return false;
...
const SELF_IMPROVEMENT_ARTIFACTS = {
...
function discoverFileScripts() {
    const out = [];
    const roots = [path.join(repoRoot, 'scripts'), path.join(repoRoot, 'tools')].filter((p) => fs.existsSync(p));
    const walk = (dir) => {
        let entries = [];
        try {
...
async function runFastHarnessProtocolGate(label) {
...
function resolveControlPlaneProvider(options = {}, envKeys = []) {
    if (options.local)
        return 'local';
    const envCandidate = envKeys
        .map((key) => normalizeToken(process.env[key]))
        .find((value) => Boolean(value));
    const candidate = normalizeToken(options.provider) ??
        envCandidate ??
    ...
    throw new Error(`Unsupported provider '${candidate}'. Supported providers: local, cloud_runtime.`);
}
function assertCloudRuntimeAvailable(commandLabel) {
    if (findExecutableOnPath('cloud_runtime'))
        return;
    throw new Error(`CloudRuntime CLI is required for '${commandLabel}' when provider is cloud_runtime. Install CloudRuntime CLI or pass --provider local.`);
}
function resolveSelfImprovementBaseUrl(input) {
...
function resolveSelfImprovementApiUrl(input) {
    return (normalizeToken(input) ??
...
function parsePositiveIntegerOption(input, fallback, label) {
...
function parseNonNegativeIntegerOption(input, fallback, label) {
    if (typeof input === 'undefined')
        return fallback;
    const parsed = Number.parseInt(input, 10);
    if (!Number.isFinite(parsed) || parsed < 0) {
...
function collectSelfImprovementArtifactStatus() {
    const tracked = [
...
function assertExpectedArtifacts(expectedPaths, startedAtMs) {
    const missing = [];
    const stale = [];
...
function appendSelfImprovementRunLog(note) {
...
function buildSelfImprovementRunCliArgs(options) {
    ...
    if (options.maxExternal)
        args.push('--max-external', options.maxExternal);
    if (options.skipBuild)
        args.push('--skip-build');
    if (options.skipLiveLinks)
        args.push('--skip-live-links');
    if (options.skipSemantic)
        args.push('--skip-semantic');
    if (options.skipAuth)
        args.push('--skip-auth');
    if (options.skipScorecard)
        args.push('--skip-scorecard');
    if (options.skipMermaid)
        args.push('--skip-mermaid');
    if (options.note)
        args.push('--note', options.note);
    if (options.superAdminToken)
        args.push('--super-admin-token', options.superAdminToken);
    return args;
}
function buildSelfImprovementStatusCliArgs(options) {
...
function writeFullAutoState(state) {
    ensureParentDir(FULL_AUTO_STATE_PATH);
    fs.writeFileSync(FULL_AUTO_STATE_PATH, JSON.stringify(state, null, 2), 'utf8');
}
function readFullAutoState() {
    if (!fs.existsSync(FULL_AUTO_STATE_PATH))
        return null;
    try {
        return JSON.parse(fs.readFileSync(FULL_AUTO_STATE_PATH, 'utf8'));
    }
    catch {
        return null;
    }
...
const HOOK_RUN_LOG_PATH = normalizeToken(process.env.TNF_HOOKS_RUN_LOG) ||
    (process.env.HOME
...
function toHookRecord(value) {
    return isPlainObject(value) ? value : null;
}
function addHookDiagnostic(diagnostics, level, code, message, path) {
...
function pickFirstString(root, candidates) {
...
function collectHookFilesInDir(rootDir) {
...
function resolveHookRegistryDirs() {
    const cwd = process.cwd();
    const envRaw = normalizeToken(process.env.TNF_HOOKS_REGISTRY_DIRS) ??
...
async function parseYamlContent(raw, filePath) {
    ...
    try {
        ...
        throw new HookCliError(`YAML parsing unavailable for ${filePath}. Install js-yaml. (${error?.message || error})`, HOOKS_EXIT_CODES.VALIDATION_FAILURE);
    }
    const loadFn = typeof yamlModule?.load === 'function'
        ? yamlModule.load
        : typeof yamlModule?.default?.load === 'function'
            ? yamlModule.default.load
            : null;
    if (!loadFn) {
        throw new HookCliError(`YAML parser is not available for ${filePath}.`, HOOKS_EXIT_CODES.VALIDATION_FAILURE);
    }
    try {
        return loadFn(raw);
    }
    catch (error) {
        throw new HookCliError(`Invalid YAML in ${filePath}: ${error?.message || error}`, HOOKS_EXIT_CODES.VALIDATION_FAILURE);
    }
}
async function parseJsonOrYamlFile(filePath) {
    ...
    if (ext === '.json') {
        try {
            ...
            throw new HookCliError(`Invalid JSON in ${filePath}: ${error?.message || error}`, HOOKS_EXIT_CODES.VALIDATION_FAILURE);
        }
    }
    if (ext === '.yaml' || ext === '.yml') {
...
async function findHookChainFileByName(chainName) {
...
function validateHookChainDefinition(chainInput) {
    const diagnostics = [];
    const chain = toHookRecord(chainInput);
    if (!chain) {
        addHookDiagnostic(diagnostics, 'error', 'CHAIN_NOT_OBJECT', 'HookChain definition must be an object.');
        return diagnostics;
    }
    if (chain.apiVersion !== 'tnf.hooks/v2') {
        addHookDiagnostic(diagnostics, 'error', 'INVALID_API_VERSION', "apiVersion must equal 'tnf.hooks/v2'.", 'apiVersion');
    }
    ...
    const metadata = toHookRecord(chain.metadata);
    if (!metadata) {
        addHookDiagnostic(diagnostics, 'error', 'MISSING_METADATA', 'metadata is required and must be an object.', 'metadata');
    }
    else {
        if (typeof metadata.name !== 'string' || !HOOK_CHAIN_NAME_PATTERN.test(metadata.name)) {
            addHookDiagnostic(diagnostics, 'error', 'INVALID_METADATA_NAME', 'metadata.name must match ^[a-z0-9]([a-z0-9_-]{1,62}[a-z0-9])?$', 'metadata.name');
        }
        if (!Number.isInteger(metadata.version) || metadata.version < 1) {
            addHookDiagnostic(diagnostics, 'error', 'INVALID_METADATA_VERSION', 'metadata.version must be an integer >= 1.', 'metadata.version');
        }
        if (typeof metadata.owner !== 'string' || metadata.owner.trim().length === 0) {
            addHookDiagnostic(diagnostics, 'error', 'INVALID_METADATA_OWNER', 'metadata.owner must be a non-empty string.', 'metadata.owner');
        }
    }
    const spec = toHookRecord(chain.spec);
    if (!spec) {
        addHookDiagnostic(diagnostics, 'error', 'MISSING_SPEC', 'spec is required and must be an object.', 'spec');
        return diagnostics;
    }
    const trigger = toHookRecord(spec.trigger);
    if (!trigger) {
        addHookDiagnostic(diagnostics, 'error', 'MISSING_TRIGGER', 'spec.trigger is required and must be an object.', 'spec.trigger');
    }
    else {
        if (typeof trigger.event !== 'string' || !HOOK_EVENT_PATTERN.test(trigger.event)) {
            addHookDiagnostic(diagnostics, 'error', 'INVALID_TRIGGER_EVENT', 'spec.trigger.event must match ^[a-z0-9]+(\\.[a-z0-9_]+)+$', 'spec.trigger.event');
        }
        if (trigger.mode !== 'async' && trigger.mode !== 'sync_gate') {
            addHookDiagnostic(diagnostics, 'error', 'INVALID_TRIGGER_MODE', "spec.trigger.mode must be 'async' or 'sync_gate'.", 'spec.trigger.mode');
        }
        if (typeof trigger.match !== 'undefined') {
            const match = toHookRecord(trigger.match);
            if (!match) {
                addHookDiagnostic(diagnostics, 'error', 'INVALID_TRIGGER_MATCH', 'spec.trigger.match must be an object when provided.', 'spec.trigger.match');
            }
            else {
                if (typeof match.path_regex !== 'undefined' &&
                    (typeof match.path_regex !== 'string' || match.path_regex.trim().length === 0)) {
                    addHookDiagnostic(diagnostics, 'error', 'INVALID_PATH_REGEX', 'spec.trigger.match.path_regex must be a non-empty string.', 'spec.trigger.match.path_regex');
                }
                if (typeof match.command_regex !== 'undefined' &&
                    (typeof match.command_regex !== 'string' || match.command_regex.trim().length === 0)) {
                    addHookDiagnostic(diagnostics, 'error', 'INVALID_COMMAND_REGEX', 'spec.trigger.match.command_regex must be a non-empty string.', 'spec.trigger.match.command_regex');
                }
                if (typeof match.source_in !== 'undefined') {
                    if (!Array.isArray(match.source_in) || match.source_in.length === 0) {
                        addHookDiagnostic(diagnostics, 'error', 'INVALID_SOURCE_IN', 'spec.trigger.match.source_in must be a non-empty array when provided.', 'spec.trigger.match.source_in');
                    }
                    else {
                        for (let i = 0; i < match.source_in.length; i += 1) {
                            if (typeof match.source_in[i] !== 'string' ||
                                match.source_in[i].trim().length === 0) {
                                addHookDiagnostic(diagnostics, 'error', 'INVALID_SOURCE_IN_ITEM', 'spec.trigger.match.source_in items must be non-empty strings.', `spec.trigger.match.source_in.${i}`);
                            }
                        }
                    }
                }
        ...
        if (typeof trigger.dedupe !== 'undefined') {
            ...
            if (!dedupe) {
                addHookDiagnostic(diagnostics, 'error', 'INVALID_TRIGGER_DEDUPE', 'spec.trigger.dedupe must be an object when provided.', 'spec.trigger.dedupe');
            }
            else {
                if (typeof dedupe.key !== 'string' || dedupe.key.trim().length === 0) {
                    addHookDiagnostic(diagnostics, 'error', 'INVALID_DEDUPE_KEY', 'spec.trigger.dedupe.key must be a non-empty string.', 'spec.trigger.dedupe.key');
                }
                if (!Number.isInteger(dedupe.window_ms) || dedupe.window_ms < 0) {
                    addHookDiagnostic(diagnostics, 'error', 'INVALID_DEDUPE_WINDOW', 'spec.trigger.dedupe.window_ms must be an integer >= 0.', 'spec.trigger.dedupe.window_ms');
                }
            }
        }
    }
    ...
    if (!execution) {
        addHookDiagnostic(diagnostics, 'error', 'MISSING_EXECUTION', 'spec.execution is required and must be an object.', 'spec.execution');
    }
    else {
        if (!Number.isInteger(execution.max_run_time_ms) || execution.max_run_time_ms < 1) {
            addHookDiagnostic(diagnostics, 'error', 'INVALID_MAX_RUN_TIME', 'spec.execution.max_run_time_ms must be an integer >= 1.', 'spec.execution.max_run_time_ms');
        }
        const concurrency = execution.concurrency;
        if (!['unbounded', 'single_per_key', 'fixed'].includes(String(concurrency || ''))) {
            addHookDiagnostic(diagnostics, 'error', 'INVALID_CONCURRENCY', "spec.execution.concurrency must be 'unbounded', 'single_per_key', or 'fixed'.", 'spec.execution.concurrency');
        }
        if (execution.concurrency === 'fixed') {
            if (!Number.isInteger(execution.fixed_concurrency) ||
                execution.fixed_concurrency < 1) {
                addHookDiagnostic(diagnostics, 'error', 'INVALID_FIXED_CONCURRENCY', 'spec.execution.fixed_concurrency must be an integer >= 1 when concurrency=fixed.', 'spec.execution.fixed_concurrency');
            }
        }
        if (!['fail', 'continue'].includes(String(execution.on_chain_error || ''))) {
            addHookDiagnostic(diagnostics, 'error', 'INVALID_CHAIN_ERROR_POLICY', "spec.execution.on_chain_error must be 'fail' or 'continue'.", 'spec.execution.on_chain_error');
        }
    }
    const context = toHookRecord(spec.context);
    if (!context) {
        addHookDiagnostic(diagnostics, 'error', 'MISSING_CONTEXT', 'spec.context is required and must be an object.', 'spec.context');
    }
    else {
        if (!['immutable', 'mutable'].includes(String(context.model || ''))) {
            addHookDiagnostic(diagnostics, 'error', 'INVALID_CONTEXT_MODEL', "spec.context.model must be 'immutable' or 'mutable'.", 'spec.context.model');
        }
        if (typeof context.write_root !== 'string' ||
            !/^[a-zA-Z0-9_.-]+$/.test(context.write_root || '')) {
            addHookDiagnostic(diagnostics, 'error', 'INVALID_CONTEXT_WRITE_ROOT', 'spec.context.write_root must match ^[a-zA-Z0-9_.-]+$', 'spec.context.write_root');
        }
    }
    const steps = Array.isArray(spec.steps) ? spec.steps : null;
    if (!steps || steps.length === 0) {
        addHookDiagnostic(diagnostics, 'error', 'MISSING_STEPS', 'spec.steps must be a non-empty array.', 'spec.steps');
    }
    else {
        ...
        for (let i = 0; i < steps.length; i += 1) {
            ...
            if (!step) {
                addHookDiagnostic(diagnostics, 'error', 'INVALID_STEP', 'Each step must be an object.', stepPath);
                continue;
            }
            const stepId = step.id;
            if (typeof stepId !== 'string' || !HOOK_CHAIN_NAME_PATTERN.test(stepId)) {
                addHookDiagnostic(diagnostics, 'error', 'INVALID_STEP_ID', 'step.id must match ^[a-z0-9]([a-z0-9_-]{1,62}[a-z0-9])?$', `${stepPath}.id`);
            }
            else if (seenStepIds.has(stepId)) {
                addHookDiagnostic(diagnostics, 'error', 'DUPLICATE_STEP_ID', `Duplicate step id '${stepId}'.`, `${stepPath}.id`);
            }
            else {
                seenStepIds.add(stepId);
            }
            if (typeof step.runner !== 'string' ||
                !['shell', 'node', 'agent', 'mcp', 'webhook'].includes(step.runner)) {
                addHookDiagnostic(diagnostics, 'error', 'INVALID_STEP_RUNNER', 'step.runner must be one of: shell, node, agent, mcp, webhook.', `${stepPath}.runner`);
            }
            if (!Number.isInteger(step.timeout_ms) || step.timeout_ms < 1) {
                addHookDiagnostic(diagnostics, 'error', 'INVALID_STEP_TIMEOUT', 'step.timeout_ms must be an integer >= 1.', `${stepPath}.timeout_ms`);
            }
            if (typeof step.if !== 'string' || step.if.trim().length === 0) {
                addHookDiagnostic(diagnostics, 'error', 'INVALID_STEP_CONDITION', 'step.if must be a non-empty string.', `${stepPath}.if`);
            }
            if (typeof step.on_failure !== 'string' ||
                !['stop', 'continue', 'branch'].includes(step.on_failure)) {
                addHookDiagnostic(diagnostics, 'error', 'INVALID_ON_FAILURE', 'step.on_failure must be one of: stop, continue, branch.', `${stepPath}.on_failure`);
            }
            if (step.runner === 'shell') {
                if (typeof step.command !== 'string' || step.command.trim().length === 0) {
                    addHookDiagnostic(diagnostics, 'error', 'MISSING_SHELL_COMMAND', 'shell runner requires non-empty command.', `${stepPath}.command`);
                }
            }
            else if (step.runner === 'node') {
                if (typeof step.module !== 'string' || step.module.trim().length === 0) {
                    addHookDiagnostic(diagnostics, 'error', 'MISSING_NODE_MODULE', 'node runner requires module.', `${stepPath}.module`);
                }
                if (typeof step.function !== 'string' || step.function.trim().length === 0) {
                    addHookDiagnostic(diagnostics, 'error', 'MISSING_NODE_FUNCTION', 'node runner requires function.', `${stepPath}.function`);
                }
            }
            else if (step.runner === 'agent') {
                const selector = toHookRecord(step.agent_selector);
                if (!selector) {
                    addHookDiagnostic(diagnostics, 'error', 'MISSING_AGENT_SELECTOR', 'agent runner requires agent_selector.', `${stepPath}.agent_selector`);
                }
                else {
                    if (typeof selector.type !== 'string' ||
                        !['id', 'role', 'capability'].includes(selector.type)) {
                        addHookDiagnostic(diagnostics, 'error', 'INVALID_AGENT_SELECTOR_TYPE', 'agent_selector.type must be one of: id, role, capability.', `${stepPath}.agent_selector.type`);
                    }
                    if (typeof selector.value !== 'string' || selector.value.trim().length === 0) {
                        addHookDiagnostic(diagnostics, 'error', 'INVALID_AGENT_SELECTOR_VALUE', 'agent_selector.value must be a non-empty string.', `${stepPath}.agent_selector.value`);
                    }
                }
                if (typeof step.prompt !== 'string' || step.prompt.trim().length === 0) {
                    addHookDiagnostic(diagnostics, 'error', 'MISSING_AGENT_PROMPT', 'agent runner requires prompt.', `${stepPath}.prompt`);
                }
            }
            else if (step.runner === 'mcp') {
                if (typeof step.tool !== 'string' || step.tool.trim().length === 0) {
                    addHookDiagnostic(diagnostics, 'error', 'MISSING_MCP_TOOL', 'mcp runner requires tool.', `${stepPath}.tool`);
                }
            }
            else if (step.runner === 'webhook') {
                if (typeof step.url !== 'string' || step.url.trim().length === 0) {
                    addHookDiagnostic(diagnostics, 'error', 'MISSING_WEBHOOK_URL', 'webhook runner requires url.', `${stepPath}.url`);
                }
                if (typeof step.method !== 'string' ||
                    !['GET', 'POST', 'PUT', 'PATCH', 'DELETE'].includes(step.method)) {
                    addHookDiagnostic(diagnostics, 'error', 'INVALID_WEBHOOK_METHOD', 'webhook runner method must be one of: GET, POST, PUT, PATCH, DELETE.', `${stepPath}.method`);
                }
            }
            if (typeof step.retry !== 'undefined') {
                const retry = toHookRecord(step.retry);
                if (!retry) {
                    addHookDiagnostic(diagnostics, 'error', 'INVALID_RETRY', 'step.retry must be an object when provided.', `${stepPath}.retry`);
                }
                else {
                    if (!Number.isInteger(retry.max_attempts) || retry.max_attempts < 0) {
                        addHookDiagnostic(diagnostics, 'error', 'INVALID_RETRY_MAX_ATTEMPTS', 'retry.max_attempts must be an integer >= 0.', `${stepPath}.retry.max_attempts`);
                    }
                    if (typeof retry.backoff_ms !== 'undefined' &&
                        (!Number.isInteger(retry.backoff_ms) || retry.backoff_ms < 0)) {
                        addHookDiagnostic(diagnostics, 'error', 'INVALID_RETRY_BACKOFF', 'retry.backoff_ms must be an integer >= 0 when provided.', `${stepPath}.retry.backoff_ms`);
                    }
                }
            }
        }
    ...
    if (!security) {
        addHookDiagnostic(diagnostics, 'error', 'MISSING_SECURITY', 'spec.security is required and must be an object.', 'spec.security');
    }
    else {
        if (typeof security.policy_pack !== 'string' || security.policy_pack.trim().length === 0) {
            addHookDiagnostic(diagnostics, 'error', 'INVALID_POLICY_PACK', 'spec.security.policy_pack must be a non-empty string.', 'spec.security.policy_pack');
        }
        if (typeof security.approval_policy !== 'string' ||
            !['none', 'on_high_risk', 'always'].includes(security.approval_policy)) {
            addHookDiagnostic(diagnostics, 'error', 'INVALID_APPROVAL_POLICY', 'spec.security.approval_policy must be one of: none, on_high_risk, always.', 'spec.security.approval_policy');
        }
    }
    return diagnostics;
}
...
function evaluateHookTriggerMatch(chain, event, diagnostics) {
    const spec = toHookRecord(chain.spec);
    const trigger = toHookRecord(spec?.trigger);
    ...
    if (expectedEvent && receivedEvent && expectedEvent !== receivedEvent) {
        matched = false;
        addHookDiagnostic(diagnostics, 'warning', 'TRIGGER_EVENT_MISMATCH', `Trigger event '${expectedEvent}' does not match fixture event '${receivedEvent}'.`, 'spec.trigger.event');
    }
    else if (expectedEvent && !receivedEvent) {
        matched = false;
        addHookDiagnostic(diagnostics, 'warning', 'EVENT_TYPE_MISSING', 'Event fixture has no detectable event type.', 'event');
    }
    const match = toHookRecord(trigger?.match);
    if (!match) {
    ...
    if (typeof match.path_regex === 'string') {
        ...
        if (!pathCandidate) {
            matched = false;
            addHookDiagnostic(diagnostics, 'warning', 'MATCH_PATH_MISSING', 'Trigger defines path_regex but event fixture has no filepath/path value.', 'spec.trigger.match.path_regex');
        }
        else {
            try {
                if (!new RegExp(match.path_regex).test(pathCandidate)) {
                    matched = false;
                    addHookDiagnostic(diagnostics, 'warning', 'MATCH_PATH_REGEX_MISS', `Event path '${pathCandidate}' does not match path_regex.`, 'spec.trigger.match.path_regex');
                }
            }
            catch (error) {
                matched = false;
                addHookDiagnostic(diagnostics, 'error', 'INVALID_MATCH_PATH_REGEX', `Invalid path_regex: ${error?.message || error}`, 'spec.trigger.match.path_regex');
            }
        }
    }
    if (typeof match.command_regex === 'string') {
        ...
        if (!commandCandidate) {
            matched = false;
            addHookDiagnostic(diagnostics, 'warning', 'MATCH_COMMAND_MISSING', 'Trigger defines command_regex but event fixture has no command value.', 'spec.trigger.match.command_regex');
        }
        else {
            try {
                if (!new RegExp(match.command_regex).test(commandCandidate)) {
                    matched = false;
                    addHookDiagnostic(diagnostics, 'warning', 'MATCH_COMMAND_REGEX_MISS', `Event command does not match command_regex.`, 'spec.trigger.match.command_regex');
                }
            }
            catch (error) {
                matched = false;
                addHookDiagnostic(diagnostics, 'error', 'INVALID_MATCH_COMMAND_REGEX', `Invalid command_regex: ${error?.message || error}`, 'spec.trigger.match.command_regex');
            }
        }
    }
    if (Array.isArray(match.source_in) && match.source_in.length > 0) {
        ...
        if (!sourceCandidate) {
            matched = false;
            addHookDiagnostic(diagnostics, 'warning', 'MATCH_SOURCE_MISSING', 'Trigger defines source_in but event fixture has no source value.', 'spec.trigger.match.source_in');
        }
        else if (!match.source_in.includes(sourceCandidate)) {
            matched = false;
            addHookDiagnostic(diagnostics, 'warning', 'MATCH_SOURCE_MISS', `Event source '${sourceCandidate}' is not allowed by source_in.`, 'spec.trigger.match.source_in');
        }
    }
    return { matched, expectedEvent, receivedEvent };
}
function evaluateHookCondition(rawCondition, stepState) {
    ...
    const explicitComparison = condition.match(/^steps\.([a-z0-9]([a-z0-9_-]{1,62}[a-z0-9])?)\.success\s*(==|!=)\s*(true|false)$/i);
    ...
    const implicitBoolean = condition.match(/^(!)?steps\.([a-z0-9]([a-z0-9_-]{1,62}[a-z0-9])?)\.success$/i);
    if (implicitBoolean) {
...
function resolveTemplateValue(scope, expression) {
...
function collectUnresolvedTemplates(step, scope) {
    const leaves = [];
...
function buildHookStepPlan(chain, event, triggerMatched, diagnostics) {
    const spec = toHookRecord(chain.spec);
    ...
    for (let i = 0; i < steps.length; i += 1) {
        ...
        if (!conditionResult.supported) {
            addHookDiagnostic(diagnostics, 'warning', 'UNSUPPORTED_CONDITION', `Step '${stepId}' condition could not be evaluated: ${conditionResult.reason}`, `spec.steps.${i}.if`);
        }
        let willRun = conditionResult.supported && conditionResult.value;
        let reason = willRun ? 'condition true' : conditionResult.reason;
        const templateScope = {
            event,
            steps: Object.fromEntries(Object.entries(stepState).map(([id, state]) => [id, { success: state.success }])),
        };
        ...
        if (unresolved.length > 0) {
            ...
            for (const miss of unresolved) {
                addHookDiagnostic(diagnostics, 'warning', 'UNRESOLVED_TEMPLATE', `Step '${stepId}' has unresolved template '{{${miss.expression}}}' in ${miss.field}.`, `spec.steps.${i}.${miss.field}`);
            }
        }
        plan.push({
            step: stepId,
...
function mapHookRunnerToWorkflowNodeType(runner) {
    switch (runner) {
        case 'shell':
...
function buildWorkflowProjection(chain, plan) {
    const spec = toHookRecord(chain.spec) || {};
    const trigger = toHookRecord(spec.trigger) || {};
    const execution = toHookRecord(spec.execution) || {};
    ...
    for (let i = 0; i < stepList.length; i += 1) {
        ...
        const nodeType = mapHookRunnerToWorkflowNodeType(typeof step.runner === 'string' ? step.runner : 'custom');
        ...
        nodes.push({
            ...
            conditions: typeof step.if === 'string' && step.if.trim().length > 0
                ? [{ id: `cond_${stepId}`, expression: step.if, outputId: 'success' }]
                : [],
            retry: toHookRecord(step.retry) && typeof toHookRecord(step.retry)?.max_attempts === 'number'
                ? {
                    enabled: toHookRecord(step.retry)?.max_attempts > 0,
    ...
    return {
        ...
        settings: {
            parallel: false,
            maxConcurrentExecutions: execution.concurrency === 'fixed' && typeof execution.fixed_concurrency === 'number'
...
function formatHookDiagnostics(diagnostics, level) {
    return diagnostics
...
function parseHookDurationMs(raw) {
    if (!raw)
        return null;
...
function createHookRunId(prefix = 'run') {
...
function readHookRunRecords(filePath = HOOK_RUN_LOG_PATH) {
    if (!fs.existsSync(filePath))
        return [];
    const raw = fs.readFileSync(filePath, 'utf8');
    const records = [];
...
function filterHookRunRecords(records, options) {
    ...
    return records
        .filter((record) => {
        if (run && record.run_id !== run)
            return false;
        if (chain && String(record.chain || '') !== chain)
            return false;
        if (status && String(record.status || '').toLowerCase() !== status)
            return false;
        if (tenant && String(record.tenant || '') !== tenant)
            return false;
        if (traceId && String(record.trace_id || '') !== traceId)
            return false;
        if (sinceCutoff != null) {
            const stamp = Date.parse(String(record.started_at || record.queued_at || ''));
            if (!Number.isFinite(stamp) || stamp < sinceCutoff)
                return false;
        }
        if (step) {
...
function printHookLogsSummary(payload) {
    ...
    for (const record of records) {
        ...
        const statusText = status === 'completed'
            ...
            : status === 'failed' || status === 'blocked'
                ? chalk.red(status)
                : chalk.yellow(status);
        console.log(`- ${chalk.cyan(record.run_id)} ${statusText} ${chalk.dim(String(record.started_at || record.queued_at || ''))}`);
        console.log(`  chain=${String(record.chain || 'unknown')} event=${String(record.trigger_event || 'unknown')} trace=${String(record.trace_id || 'none')}`);
        const steps = Array.isArray(record.steps) ? record.steps : [];
        if (steps.length > 0) {
...
function deriveHookDecisionReason(record) {
    const status = String(record.status || 'unknown').toLowerCase();
    if (status === 'blocked')
        return 'REQUIRE_APPROVAL_OR_POLICY_BLOCK';
    if (status === 'failed')
        return 'HOOK_RUN_FAILED';
    if (status === 'cancelled')
        return 'HOOK_RUN_CANCELLED';
    if (status === 'completed')
        return 'HOOK_RUN_COMPLETED';
    if (status === 'dry_run')
        return 'DRY_RUN_NO_SIDE_EFFECTS';
    if (status === 'queued')
        return 'REPLAY_QUEUED';
    return 'STATUS_UNKNOWN';
}
function buildHookExplainPayload(record, options = {}) {
        ...
        : [
            {
                ...
                decision: String(record.status || '').toLowerCase() === 'blocked'
...
function printHookExplainSummary(payload) {
    ...
    console.log(`Status: ${chalk.yellow(String(summary.final_status || 'unknown'))} (${String(summary.reason || 'unknown')})`);
    if (gates.length > 0) {
        console.log(chalk.bold('\nGate Decisions:'));
        for (const gate of gates) {
            console.log(`- ${String(gate.gate || 'gate')}: ${chalk.cyan(String(gate.decision || 'unknown'))} ${chalk.dim(String(gate.reason || ''))}`);
        }
    }
    if (steps.length > 0) {
        console.log(chalk.bold('\nSteps:'));
        for (const step of steps) {
            console.log(`- ${String(step.id || 'unknown')}: ${String(step.status || 'unknown')} ${chalk.dim(String(step.reason || ''))}`);
        }
    }
    console.log('');
}
function printHookTestSummary(payload) {
    ...
    console.log(`Event: ${chalk.cyan(String(event.received_event || 'unknown'))} (expected ${chalk.cyan(String(event.expected_event || 'unknown'))})`);
    console.log(`Trigger matched: ${event.matched === true ? chalk.green('yes') : chalk.yellow('no')}`);
    console.log(`Compiled: ${chalk.cyan(String(compiled.node_count || 0))} nodes, ${chalk.cyan(String(compiled.edge_count || 0))} edges`);
    console.log(`Result: ${valid
        ? warnings.length > 0
            ? chalk.yellow('VALID_WITH_WARNINGS')
            : chalk.green('VALID')
        : chalk.red('INVALID')} (exit ${chalk.cyan(String(exitCode))})`);
    if (errors.length > 0) {
    ...
    if (plan.length > 0) {
        ...
        for (const step of plan) {
            ...
            console.log(`- ${stepName}: ${willRun ? chalk.green('will_run=true') : chalk.yellow('will_run=false')}${chalk.dim(reason)}`);
        }
    }
    console.log('');
}
...
// =============================================================================
export const PLATFORM_TAXONOMY = [
...
function buildTraitGroups() {
    ...
    let discoveredWorkerActions = [];
    let discoveredPlatforms = [];
    let discoveredVendors = [];
    let discoveredDaccRoles = [];
...
async function printCommandMenu(options = {}) {
    ...
    console.log(chalk.bold('\nTNF Command Menu\n'));
    console.log(chalk.dim(`CLI paths: ${allPaths.length} | tnf package scripts: ${tnfRootScripts.length} | total root scripts: ${rootScripts.length}`));
...
function findFullAutoStartProcesses() {
...
function buildFullAutoStartArgs(options) {
    const args = ['full-auto', 'start'];
    if (options.intervalMinutes)
        args.push('--interval-minutes', options.intervalMinutes);
    if (options.maxCycles)
        args.push('--max-cycles', options.maxCycles);
    if (options.baseUrl)
        args.push('--base-url', options.baseUrl);
    if (options.apiUrl)
        args.push('--api-url', options.apiUrl);
 ...
 */
function resolvePassthroughCommand(cliName) {
    ...
    if (cliName === 'hermes') {
        ...
        if (isExecutableFile(venvHermes))
            return venvHermes;
    }
...
function isCursorPassthroughArgv(argv) {
    const subcommand = argv[2];
...
function buildBootPlan(nonInteractive) {
    return [
        {
...
function printBootPlan(name, nonInteractive) {
    console.log(chalk.bold.cyan(`\nTNF Boot Plan: ${name}\n`));
...
function printSlashText(text) {
    console.log('');
    console.log(text);
    console.log('');
}
function printSlashCommandList() {
    printSlashText(renderSlashCommandList(invocationCwd));
}
function printSlashCommandDetail(command) {
    printSlashText(renderSlashCommandDetail(command));
}
function createSlashCompleter(projectRoot) {
    return (line) => {
...
function getSlashCommandMatches(projectRoot, line) {
...
function renderSlashCommandDropdown(projectRoot, state, limit = 14) {
...
function setReadlineLine(rl, line) {
    rl.line = line;
    rl.cursor = line.length;
    rl._refreshLine?.();
}
function resolveSlashDropdownInput(input, state) {
...
function attachSlashCommandDropdown(rl, projectRoot) {
    const state = {
        visible: false,
        query: '',
        selectedIndex: 0,
    ...
    const onKeypress = (_value, key) => {
        ...
        setReadlineLine(rl, ['up', 'down'].includes(keyName) && state.selectedCommand
            ? `/${state.selectedCommand}`
            : currentLine);
    };
...
async function runSlashCliCommand(command, args) {
...
function setInteractiveModel(client, modelName) {
...
function estimateSessionCost(client, tokenCount) {
    const providers = client?.getProviderCatalog?.() || [];
    const provider = providers.find((candidate) => candidate.id === client?.providerName || candidate.model === client?.model);
    if (typeof provider?.costPerMtokens !== 'number')
        return null;
    return (tokenCount / 1_000_000) * provider.costPerMtokens;
}
function printSessionCost(context) {
...
async function handleOneShotSlashInput(input) {
...
async function handleInteractiveSlashCommand(input, context) {
    const parsed = parseSlashCommand(input);
    if (!parsed)
    ...
    if (command.name === 'autonomous' || command.aliases?.includes('auto')) {
        ...
        console.log(`  Autonomous shell execution: ${context.autonomousMode ? chalk.green('ON') : chalk.yellow('OFF')}`);
        return { handled: true };
    }
    if (command.mode === 'cli') {
        await runSlashCliCommand(command, parsed.args);
        ...
        const steps = [
            ...
            },
            {
                ...
                action: async () => {
                    ...
                    if (options.strictGates)
                        args.push('--require-doctor');
                    if (options.nonInteractive || !findExecutableOnPath('openclaw')) {
                        args.push('--no-launch');
                        if (!findExecutableOnPath('openclaw')) {
                            console.log(chalk.dim('   OpenClaw CLI not found; provisioning MCP config without launching client'));
                        }
                    }
                    await runCommand('node', args);
                },
...
program
    ...
    .argument('[role]', `Agent role (${AGENT_ROLE_TRAITS.join(', ')})`, process.env.AGENT_ROLE || 'worker')
    .argument('[platform]', `Agent platform (${PLATFORM_TAXONOMY.join(', ')})`, process.env.AGENT_PLATFORM || 'vscode')
    ...
    .option('--worker-action <action>', 'Worker action primitive (e.g. code_generation, cli_coder, orchestrator)')
    ...
    try {
        ...
        if (!AGENT_ROLE_TRAITS.includes(role)) {
            console.error(chalk.yellow(`⚠ role '${role}' is not in the canonical DACC-v1 role traits ` +
        ...
        if (!PLATFORM_TAXONOMY.includes(platform)) {
            console.error(chalk.yellow(`⚠ platform '${platform}' is not in PLATFORM_TAXONOMY ` +
...
program
    ...
    try {
        ...
        if (options.runtimeTimeoutMs)
            args.push('--runtime-timeout-ms', options.runtimeTimeoutMs);
        await runCommand('node', args);
    }
    catch (err) {
        console.error(chalk.red(`Error: ${err.message}`));
...
protocol
    ...
    const turnZeroOk = state.turnZero.missing === 0;
    const livingSynced = state.livingState.synchronized;
    const disclosureReady = state.disclosure.ready;
    console.log(chalk.bold.cyan('\n[TNF Protocol Health]\n'));
    ...
    console.log(`Directives: ${chalk.cyan(`${state.directives.pending} pending`)}`);
...
program
    ...
    try {
        ...
        if (options.reduce)
            args.push('--reduce', options.reduce);
        if (options.concurrency)
            args.push('--concurrency', options.concurrency);
        if (options.redis)
            args.push('--redis', options.redis);
        if (options.localFallback === false)
            args.push('--no-local-fallback');
        if (options.demo)
            args.push('--demo');
        await runCommand('node', args);
    }
...
handoff
    ...
    try {
        ...
        if (options.projectIds)
            args.push('--project-ids', options.projectIds);
        if (options.scope)
            args.push('--scope', options.scope);
        if (options.summary)
            args.push('--summary', options.summary);
        if (options.nextActions)
            args.push('--next-actions', options.nextActions);
        if (options.resumeChecklist)
            args.push('--resume-checklist', options.resumeChecklist);
        if (options.autoVerify)
            args.push('--auto-verify');
        await runCommand('node', args);
    }
    catch (err) {
        console.error(chalk.red(`Error: ${err.message}`));
...
state
    ...
    try {
        ...
        const payload = {
            turnZeroMandatePresent: fs.existsSync(path.join(repoRoot, 'docs/protocols/TURN_ZERO_MANDATE.md')),
        ...
        console.log(chalk.bold('\nTNF Harness State\n'));
        console.log(`Turn Zero mandate: ${payload.turnZeroMandatePresent ? chalk.green('present') : chalk.red('missing')}`);
        console.log(`Repo handoff: ${payload.handoff?.handoff_id || chalk.yellow('unavailable')} (${payload.handoff?.created_at || 'unknown'})`);
        console.log(`Home handoff: ${payload.homeHandoff?.handoff_id || payload.homeHandoff?.session || chalk.yellow('unavailable')} (${payload.homeHandoff?.created_at || payload.homeHandoff?.generatedAt || 'unknown'})`);
        if (handoffDivergence) {
            console.log(chalk.yellow(`Handoff divergence: ${handoffDivergence}`));
        }
        console.log(`MCP servers: ${payload.mcpServers.length ? payload.mcpServers.join(', ') : 'unavailable'}`);
        console.log(chalk.bold('\nLiving State\n'));
...
mcp
    ...
    try {
        if ((options.from || 'repo') !== 'repo') {
            throw new Error("Only '--from repo' is currently supported to protect the canonical source of truth.");
        }
        ...
        console.log(chalk.green(`✅ Synced ${result.imported} MCP server(s) from ${path.relative(repoRoot, result.configPath)} to the user-local TNF MCP config`));
    }
    catch (err) {
        console.error(chalk.red(`Error: ${err.message}`));
        process.exit(1);
...
marketplace
    ...
    try {
        ...
        let whereClauses = ["publication_status = 'published'"];
        if (options.kind)
            whereClauses.push(`kind = '${options.kind.replace(/'/g, "''")}'`);
        if (options.category)
...
function resolveMasterClockLogDir() {
    return (normalizeToken(process.env.LOG_DIR) ??
        path.join(process.env.HOME || '/tmp', '.tnf-master-clock'));
}
function resolveLatestMasterClockLogPath(logDir) {
...
masterClock
    ...
    .option('--service <name>', 'CloudRuntime service name for master clock (used when --provider cloud_runtime)', 'tnf-master-clock')
    .option('--super-admin-token <token>', 'Super Admin authentication token (can also be set via TNF_SUPER_ADMIN_INPUT_TOKEN env var)')
    .action(async (options) => {
    try {
...
masterClock
    ...
    try {
        ...
        if (provider === 'local') {
            ...
            if (!logPath) {
                throw new Error(`No local master-clock log file found in ${logDir}. Start master-clock first or set LOG_DIR.`);
            }
...
const compatOpenClaw = compat
    ...
    try {
        ...
        console.log(`   Total OpenClaw top-level commands: ${chalk.cyan(String(report.totalOpenClawTopLevelCommands))}`);
        console.log(`   Direct TNF paths: ${chalk.green(String(report.implicitCommands))}`);
        console.log(`   Explicit namespace only: ${chalk.yellow(String(report.explicitOnlyCommands))}`);
...
library
    ...
    try {
        ...
        const mapPath = path.join(repoRoot, 'docs/protocols/storage/tnf-virtual-library-surface-map.json');
        if (!fs.existsSync(mapPath)) {
            throw new Error(`Surface map not found at ${mapPath}. Run 'tnf library audit' to generate it.`);
        }
        ...
        console.log(`   Drift: head=${chalk.yellow(String(!!drift.head_mismatch))} branch=${chalk.yellow(String(!!drift.branch_mismatch))} remote=${chalk.yellow(String(!!drift.remote_mismatch))}`);
...
selfImprovement
    ...
    try {
        ...
        if (!options.skipScorecard) {
            ...
            expectedArtifacts.push(SELF_IMPROVEMENT_ARTIFACTS.scorecardJson, SELF_IMPROVEMENT_ARTIFACTS.scorecardMd);
        }
        if (!options.skipMermaid) {
        ...
        const runNote = normalizeToken(options.note) ||
    ...
    try {
        ...
        if (options.json) {
            ...
            if (payload.scorecard) {
                console.log(`Scorecard: ${payload.scorecard.passed ? chalk.green('PASS') : chalk.red('FAIL')} (${chalk.dim(payload.scorecard.generatedAt || 'unknown')})`);
            }
            else {
...
fullAuto
    ...
    .option('--targets <list>', 'Comma-separated targets: codex,claude,gemini,opencode,kilo,augment,tnf,hermes,project,all', 'all')
...
fullAuto
    ...
    try {
        ...
        const intervalMinutes = parsePositiveIntegerOption(options.intervalMinutes, DEFAULT_FULL_AUTO_INTERVAL_MINUTES, '--interval-minutes');
        ...
        while (maxCycles === 0 || cycle < maxCycles) {
            ...
            try {
                ...
                completedCycles += 1;
                console.log(chalk.green(`[full-auto] cycle ${cycle} completed in ${Math.round(event.durationMs / 1000)}s`));
            }
            catch (err) {
                const finishedAt = new Date();
                event = {
            ...
            console.log(chalk.dim(`[full-auto] sleeping ${intervalMinutes} minute(s) before next cycle...`));
    ...
    try {
        ...
        const child = spawn(process.execPath, [...process.execArgv, cliEntryPath, ...args], {
            ...
            env: {
                ...
                [SUPER_ADMIN_INPUT_ENV_KEY]: process.env[SUPER_ADMIN_INPUT_ENV_KEY] ||
...
fullAutoDaemon
    ...
    try {
        ...
        if (state) {
            ...
            console.log(`Failed cycles: ${state.failedCycles > 0 ? chalk.yellow(String(state.failedCycles)) : chalk.green('0')}`);
        }
        if (lastRun) {
            console.log(`Last cycle: cycle=${lastRun.cycle} ok=${lastRun.ok} durationMs=${lastRun.durationMs}`);
        }
                ...
                action: async () => {
                    try {
                        ...
                        if (output.includes('heartbeat')) {
                            ...
                            const tnfScript = path.join(repoRoot, 'scripts', 'agents', 'tnf-heartbeat-selfwake.py');
                            await runCommand('hermes', [
                                'cronjob',
                                'action=create',
                                '--schedule',
...
voiceBridgeCommand
    ...
    try {
        ...
        if (preStop && preStop.stopped.length > 0) {
            console.log(chalk.dim(`Stopped existing profile processes: ${preStop.stopped.join(', ')}`));
        }
        if (serverReady) {
            console.log(chalk.green(`Server reachable at http://127.0.0.1:${port}`));
        }
        else {
            console.log(chalk.yellow(`Server not reachable yet on 127.0.0.1:${port} (startup still warming or failed).`));
        }
        console.log(chalk.dim(`Use 'tnf voice down --profile ${profile}' to stop this profile runtime.`));
    }
    catch (err) {
        console.error(chalk.red(`Error: ${err.message}`));
        process.exit(1);
...
voiceBridgeCommand
    ...
    try {
        ...
        if (mainInterferencePids.length > 0) {
            if (options.keepMain) {
                console.log(chalk.yellow(`⚠️ main-profile runtime still active (${mainInterferencePids.join(', ')}); overlap risk remains because --keep-main was set.`));
            }
            else {
                ...
                console.log(chalk.yellow(`Isolated relay pair by stopping main-profile runtime pids: ${stoppedList}`));
            }
        }
        const directions = [
            {
        ...
        console.log(chalk.bold('\nVoice Relay'));
        console.log(`Path: ${chalk.cyan(fromProfile)}:${fromPort} -> ${chalk.cyan(toProfile)}:${toPort}`);
        console.log(`Bidirectional: ${options.bidirectional ? chalk.green('ON') : chalk.yellow('OFF')}`);
        console.log(`Source signal: /tmp/voice_last_user_input_* (profile-scoped)`);
        console.log(`Endpoints: from=${fromReady ? chalk.green('UP') : chalk.yellow('DOWN')} | to=${toReady ? chalk.green('UP') : chalk.yellow('DOWN')}`);
        console.log(chalk.dim('Loop guards active: msg_id + ACK + hash dedupe'));
        console.log(chalk.dim(`Heartbeat: ${heartbeatMs > 0
            ...
            if (heartbeatMs > 0 && now - lastHeartbeatAt >= heartbeatMs) {
                ...
                if (shouldLogHeartbeat) {
                    ...
                    const healChunk = heartbeatHeal
                        ...
                        : '';
                    console.log(chalk.dim(`HB ${new Date(now).toISOString()} ${statusChunk}${healChunk}`));
                }
            }
            for (const direction of directions) {
                ...
                if (ackCandidate && now - ackCandidate.at <= ackWindowMs) {
                    if (!ackedMsgIds.has(ackCandidate.msgId)) {
                        ...
                        direction.acked += 1;
                        console.log(chalk.dim(`ACK ${ackCandidate.msgId} (${direction.fromProfile} observed relay-return hash)`));
                    }
                ...
                if (!sendResult.ok) {
                    ...
                    console.log(chalk.yellow(`SEND_FAIL ${msgId} ${direction.id} (${sendResult.error || 'unknown send error'})`));
                    continue;
                }
        ...
        for (const item of summary) {
            console.log(`${chalk.cyan(item.path)} forwarded=${item.forwarded} acked=${item.acked} ` +
...
voiceBridgeCommand
    ...
    try {
        ...
        const payload = (await readVoiceBridgeJson('/activate', 'POST', port));
        ...
        if (started.length === 0) {
            ...
            console.log(chalk.green(`✅ Voice Bridge activate succeeded. Started: ${started.join(', ')}`));
        }
    }
    catch (err) {
        console.error(chalk.red(`Error: ${err.message}`));
...
voiceBridgeCommand
    ...
    try {
        ...
        let kwsState = null;
        if (serverReachable) {
            micState = (await readVoiceBridgeJson('/mic_state', 'GET', port));
            kwsState = (await readVoiceBridgeJson('/kws_state', 'GET', port));
        }
        const payload = {
            profile,
            port,
        ...
        console.log(`Server: ${serverReachable ? chalk.green('UP') : chalk.red('DOWN')} (127.0.0.1:${port})`);
        if (serverReachable) {
            ...
            console.log(chalk.dim(`Run \`tnf voice start --profile ${profile}\` to bring up Voice Bridge for this profile.`));
        }
        console.log('\nCommands:');
...
voiceProtocolCommand
    ...
    try {
        ...
        console.log(`Pair: ${chalk.cyan(fromProfile)}:${fromPort} <-> ${chalk.cyan(toProfile)}:${toPort}`);
        console.log(`Relay: ${relayPids.length > 0
            ? chalk.green(`RUNNING (${relayPids.join(', ')})`)
            : chalk.yellow('NOT RUNNING')}`);
        if (relayHeartbeat) {
            console.log(`Heartbeat: ${chalk.dim(relayHeartbeat.line)} (age ${formatAgeMs(relayHeartbeat.ageMs)})`);
        }
        else {
            console.log(`Heartbeat: ${chalk.yellow('none observed yet')} (${chalk.dim(relayLog)})`);
        }
        if (mainInterferencePids.length > 0) {
            console.log(chalk.yellow(`Interference: main-profile runtime active (${mainInterferencePids.join(', ')}). Run 'tnf voice down --profile main' to isolate this pair.`));
        }
        const printProfile = (snapshot) => {
            ...
            console.log(`last_user_input: ${chalk.cyan(userAge)} ${snapshot.lastUserInput
                ? chalk.dim(clipProtocolText(snapshot.lastUserInput.text))
                : chalk.dim('n/a')}`);
            console.log(`last_assistant_output: ${chalk.cyan(outAge)} ${snapshot.lastAssistantOutput
    ...
    try {
        ...
        while (running) {
            ...
            if (options.json) {
                ...
                console.log(`${chalk.dim(nowIso)} relay:${relayPids.length > 0 ? chalk.green('up') : chalk.red('down')} ` +
...
hooks
    ...
    try {
        ...
        if (hasChainName === hasChainFile) {
            throw new HookCliError("Provide exactly one of '--chain <name>' or '--file <path>'.", HOOKS_EXIT_CODES.INVALID_ARGUMENTS);
        }
        if (!options.event || options.event.trim().length === 0) {
            throw new HookCliError("Missing required '--event <path>' fixture.", HOOKS_EXIT_CODES.INVALID_ARGUMENTS);
        }
        ...
        if (!fs.existsSync(eventPath) || !fs.statSync(eventPath).isFile()) {
            throw new HookCliError(`Event fixture not found: ${eventPath}`, HOOKS_EXIT_CODES.RESOURCE_NOT_FOUND);
        }
        const parsedEvent = await parseJsonOrYamlFile(eventPath);
        const event = toHookRecord(parsedEvent);
        if (!event) {
            throw new HookCliError(`Event fixture must parse to an object: ${eventPath}`, HOOKS_EXIT_CODES.VALIDATION_FAILURE);
        }
        let chainPath = '';
        if (hasChainFile && options.file) {
            ...
            if (!fs.existsSync(chainPath) || !fs.statSync(chainPath).isFile()) {
                throw new HookCliError(`HookChain file not found: ${chainPath}`, HOOKS_EXIT_CODES.RESOURCE_NOT_FOUND);
            }
        }
        else if (hasChainName && options.chain) {
            const discovered = await findHookChainFileByName(options.chain.trim());
            if (!discovered) {
                throw new HookCliError(`HookChain '${options.chain.trim()}' was not found in registry dirs: ${resolveHookRegistryDirs().join(', ')}`, HOOKS_EXIT_CODES.RESOURCE_NOT_FOUND);
            }
        ...
        if (!chain) {
            addHookDiagnostic(diagnostics, 'error', 'CHAIN_NOT_OBJECT', 'Parsed HookChain definition must be an object.');
        }
        ...
        let exitCode = HOOKS_EXIT_CODES.SUCCESS;
        if (!valid) {
        ...
        const metadata = toHookRecord(chain?.metadata);
        const chainName = typeof metadata?.name === 'string'
            ? metadata.name
        ...
        const exitCode = error instanceof HookCliError ? error.exitCode : HOOKS_EXIT_CODES.EXECUTION_FAILURE;
...
hooks
    ...
    .option('--status <status>', 'Filter by queued|running|completed|failed|blocked|cancelled|dry_run')
    ...
    try {
        if (options.status && !normalizeHookStatus(options.status)) {
            throw new HookCliError(`Invalid --status '${options.status}'. Use one of: ${Array.from(HOOK_RUN_STATUSES).join(', ')}`, HOOKS_EXIT_CODES.INVALID_ARGUMENTS);
        }
        if (options.since && parseHookDurationMs(options.since) == null) {
            throw new HookCliError('Invalid --since duration. Use a number plus ms, s, m, h, or d.', HOOKS_EXIT_CODES.INVALID_ARGUMENTS);
        }
...
hooks
    ...
    .option('--from-step <id>', 'Restart at specific step (default: first failed/blocked/skipped step)')
    ...
    try {
        ...
        if (!source) {
            throw new HookCliError(`Hook run not found: ${options.run}`, HOOKS_EXIT_CODES.RESOURCE_NOT_FOUND);
        }
        const status = String(source.status || '').toLowerCase();
        const replayable = ['failed', 'blocked', 'cancelled', 'dry_run'].includes(status);
        if (!replayable && !options.force) {
            throw new HookCliError(`Run ${source.run_id} has status '${status || 'unknown'}'. Use --force to queue a replay anyway.`, HOOKS_EXIT_CODES.AUTHORIZATION_DENIED);
        }
        let eventPayload = source.event || source.original_event || null;
        let eventOverridePath = null;
        if (options.eventOverride) {
            ...
            if (!fs.existsSync(eventOverridePath) || !fs.statSync(eventOverridePath).isFile()) {
                throw new HookCliError(`Event override not found: ${eventOverridePath}`, HOOKS_EXIT_CODES.RESOURCE_NOT_FOUND);
            }
        ...
        const failedStep = steps
            .map((entry) => toHookRecord(entry))
            .find((entry) => ['failed', 'blocked', 'skipped'].includes(String(entry?.status || '').toLowerCase()));
        const fromStep = normalizeToken(options.fromStep) ||
        ...
        if (fromStep) {
            ...
            if (steps.length > 0 && !hasStep) {
                throw new HookCliError(`Step '${fromStep}' was not found in run ${source.run_id}.`, HOOKS_EXIT_CODES.RESOURCE_NOT_FOUND);
            }
        }
        ...
        const replayRecord = {
            ...
            steps: steps.map((entry) => {
                ...
                return {
                    ...stepRecord,
                    status: fromStep && String(stepRecord.id || stepRecord.step || '') !== fromStep
                        ? 'carried_forward'
                        : 'queued',
                };
            }),
...
agentsBank
    .command('reconcile')
    .description('Restore/sync agent banks and provision imported Claude agent definitions across runtime homes')
    .option('--targets <list>', `Comma-separated targets (${PLATFORM_TAXONOMY.join(', ')}, all)`, 'all')
    .option('--dry-run', 'Preview changes without writing files')
    .option('--json', 'Output machine-readable JSON summary')
    ...
    try {
        ...
        if (options.skipImportedSync)
            args.push('--skip-imported-sync');
        if (options.skipProvision)
            args.push('--skip-provision');
        await runCommand('node', args);
    }
    catch (err) {
        console.error(chalk.red(`Error: ${err.message}`));
...
const aliveCommand = program
    .command('alive')
    .description('Activate TNF persistent stack (daemon + heartbeat sentinel) so it stays running autonomously');
aliveCommand
    .command('up')
    ...
    .option('--interval <seconds>', 'Autonomous think interval in seconds', '120')
    .option('--no-bridge', 'Skip bridge (deprecated alias; bridge is wired separately via `tnf bridge`)')
    ...
    try {
        ...
        if (!fs.existsSync(tnfVenv) && pythonBin === 'python3') {
            ...
            console.log(chalk.dim('[2/4] venv: ' + (fs.existsSync(tnfVenv) ? tnfVenv : 'system python3 (no venv)')));
        }
        ...
        if (options.model)
            daemonArgs.push('--model', options.model);
        if (options.interval)
        ...
        try {
            ...
            if (matches.length) {
                ...
                for (const m of matches.slice(0, 3))
                    console.log(chalk.dim('   ' + m.trim()));
            }
            else {
                console.log(chalk.red('❌ tnf-agent-daemon NOT running (yet — it may still be starting or failed; check log)'));
            }
        }
        catch {
        ...
        try {
            ...
            console.log(chalk.dim('   (Redis status post skipped: ' +
...
aliveCommand
    ...
    console.log(chalk.bold('\n=== tnf alive status ===\n'));
    console.log(result.daemon.running
    ...
    if (result.daemon.pids.length) {
        for (const p of result.daemon.pids.slice(0, 3))
            console.log(chalk.dim('   ' + p));
    }
    console.log(result.bridge.running
        ? chalk.green(`✅ A2A Bridge: running`)
        : chalk.dim('⚪ A2A Bridge: not running (start with `tnf bridge start`)'));
    console.log(result.heartbeat_cron.installed
...
aliveCommand
    ...
    console.log(chalk.dim('   Note: heartbeat cron (if installed) still fires every 5 min to auto-restart.'));
...
const bridgeCommand = program
    .command('bridge')
    .description('Control the TNF A2A bridge (inter-runtime bus translator) — start/stop/status/test');
bridgeCommand
    .command('start')
...
heartbeatCommand
    ...
    try {
        ...
        const match = out.match(/(?:id|job)[":= ]+"?([a-f0-9-]{20,})"?\s+[\s\S]*?TNF Heartbeat Self-Wake/i);
        if (!match) {
...
reports
    ...
    try {
        ...
        for (const [prefix, count] of Object.entries(counts).sort()) {
            ...
            console.log(`   ${chalk.green(prefix)}: ${chalk.bold(String(count))} files` +
    ...
    try {
        ...
        if (options.dryRun) {
            ...
            for (const prefix of prefixes) {
                ...
                console.log(`   ${chalk.green(prefix)}: ${chalk.red(String(wouldPrune))} pruned, ${chalk.cyan(String(Math.max(0, files.length - wouldPrune)))} kept`);
            }
            console.log('');
            return;
        }
...
reports
    ...
    try {
        ...
        if (!fs.existsSync(summaryPath)) {
            console.log(chalk.yellow('No rolling summary found. Run `tnf reports prune` to generate one.'));
            process.exit(0);
        }
        ...
        for (const [type, data] of Object.entries(summary.types || {})) {
            ...
            if (data.latestReport) {
                console.log(`     Latest: ${chalk.dim(data.latestReport.file)} (${data.latestReport.status})`);
            }
            console.log('');
        }
    }
    ...
    try {
        ...
        for (const file of files) {
            try {
                ...
                console.log(`   ${chalk.dim(ts)}  ${scoreColor(bar)} ${scoreColor.bold(String(score) + '%')} ${chalk.dim(status)}`);
            }
            catch {
                /* skip corrupt */
            }
...
function buildPassthroughEnv(cliName) {
    const env = {};
...
async function runPassthrough(cliName, args = []) {
    ...
    if (isHermesUpdate) {
        ...
        if (preflightCleanup.removed.length > 0) {
            console.log(chalk.yellow(`↺ Removed ${preflightCleanup.removed.length} stale Hermes git lock file(s) before update.`));
        }
    }
    try {
        ...
        if (isHermesUpdate) {
            ...
            if (retryCleanup.removed.length > 0) {
                console.log(chalk.yellow(`↺ Removed ${retryCleanup.removed.length} stale Hermes git lock file(s); retrying update once.`));
                try {
                    await runCommand(resolvedCmd, forwardedArgs, { env: passthroughEnv });
                    return;
                }
...
function isLockFileInUse(lockPath) {
...
function collectGitLockFiles(rootDir, out) {
    let entries = [];
    try {
...
import { generateCompletion, getInstallInstructions, } from './services/CompletionService.js';
...
function buildAcpExternalAgentPlan(agent, command, args, options) {
...
acp
    ...
    .action(async (grokArgs, options) => {
    try {
        const forwardedArgs = normalizeForwardedArgs(grokArgs || []);
        const plan = buildAcpExternalAgentPlan('grok', options.command || 'grok', forwardedArgs.length ? forwardedArgs : ['agent', 'test', '-o'], options);
        if (options.dryRun) {
            console.log(JSON.stringify({
                protocol: 'ACP',
                agent: plan.agent,
...
debug
    ...
    try {
        ...
        if (options.json) {
            ...
            if (lsp.path)
                console.log(`  Path: ${chalk.dim(lsp.path)}`);
            if (lsp.version)
                console.log(`  Version: ${chalk.dim(lsp.version)}`);
            if (lsp.error)
                console.log(`  Error: ${chalk.red(lsp.error)}`);
            console.log('');
        }
    }
    catch (err) {
...
debug
    ...
    try {
        ...
        if (options.json) {
            ...
            if (rg.path)
                console.log(`  Path: ${chalk.dim(rg.path)}`);
            if (rg.version)
                console.log(`  Version: ${chalk.dim(rg.version)}`);
            console.log('');
        }
    }
    catch (err) {
    ...
    try {
        ...
        if (options.json) {
            ...
            if (info.exists) {
                ...
                if (info.modified)
                    console.log(`  Modified: ${chalk.dim(info.modified)}`);
                if (info.permissions)
                    console.log(`  Permissions: ${info.permissions}`);
            }
            if (info.error)
                console.log(`  Error: ${chalk.red(info.error)}`);
            console.log('');
        }
    }
    catch (err) {
    ...
    try {
        if (options.path) {
            ...
            if (options.json) {
                ...
                if (config.$schema)
                    console.log(` Schema: ${chalk.dim(config.$schema)}`);
                if (config.model)
                    console.log(` Model: ${chalk.cyan(config.model)}`);
                if (config.provider)
                    console.log(` Provider: ${chalk.cyan(config.provider)}`);
                if (config.apiBaseUrl)
                    console.log(` API Base: ${chalk.dim(config.apiBaseUrl)}`);
...
permissionCmd
    ...
    .action((category, pattern, action, options) => {
    try {
        ...
        if (category === 'bash')
            permService.addBashRule(pattern, action, scope);
        else if (category === 'read')
            permService.addReadRule(pattern, action, scope);
        else
            permService.addExternalDirectoryRule(pattern, action, scope);
...
permissionCmd
    ...
    .option('--scope <scope>', 'Scope: global or project', 'global')
    .action((category, pattern, options) => {
    try {
        const permService = new PermissionService(undefined, process.cwd());
        const scope = options.scope;
        let removed = false;
        if (category === 'bash')
            removed = permService.removeBashRule(pattern, scope);
        else if (category === 'read')
            removed = permService.removeReadRule(pattern, scope);
...
permissionCmd
    ...
    try {
        ...
        if (options.type === 'bash')
            result = permService.checkBashCommand(command);
        else if (options.type === 'read')
            result = permService.checkReadPath(command);
        else
            result = permService.checkExternalDirectory(command);
        if (result.allowed) {
...
const PROJECT_SCAFFOLD_KINDS = new Set([
    'command',
    'agent',
    'skill',
    'workflow',
...
function parseProjectScaffoldKind(kind) {
    const normalized = kind.trim().toLowerCase().replace(/_/g, '-');
    if (normalized === 'mcpserver')
        return 'mcp-server';
    if (PROJECT_SCAFFOLD_KINDS.has(normalized)) {
        return normalized;
    }
    throw new Error(`Invalid scaffold kind '${kind}'. Expected one of: ${Array.from(PROJECT_SCAFFOLD_KINDS).join(', ')}.`);
}
function printProjectScaffoldResult(result) {
...
goals
    ...
    try {
        ...
        for (const g of list) {
            ...
            console.log(`  ${status} | ${pColor(g.priority.padEnd(8))} | ${chalk.bold(g.title)} ${progress}`);
            if (g.description)
...
feedback
    ...
    try {
        ...
        const allFeedback = (await resp.json());
        if (options.json) {
...
function checkExtensionExists(appDir) {
    return fs.existsSync(path.join(repoRoot, appDir));
}
function getExtensionVersion(appDir) {
...
extensionCmd
    ...
    try {
        ...
        if (options.json) {
            console.log(JSON.stringify(entries.map((ext) => ({
                ...ext,
...
function printPackageProbeTable(results) {
...
packagesCommand
    ...
    try {
        ...
        const summary = {
            ...
            manifestReady: results.filter((result) => result.hasMainField &&
                result.hasTypesField &&
                result.hasExportsField &&
                result.hasBuildScript &&
                result.hasTestScript).length,
        ...
        console.log(`Packages=${summary.packageCount} manifest-ready=${summary.manifestReady} entry-ready=${summary.entryReady} resolved=${summary.resolved}`);
        if (options.runtime) {
            console.log(`Runtime load: success=${summary.runtimeLoadSucceeded}/${summary.runtimeLoadAttempted}`);
        }
        console.log('');
...
packagesCommand
    ...
    try {
        ...
        console.log(`Resolved:    ${result.resolvedFromWorkspace ? chalk.green(result.resolvedPath || 'yes') : chalk.yellow('no')}`);
        if (options.runtime) {
            console.log(`Runtime:     ${result.loadAttempted
...
function createNotesService(options) {
...
function parsePositiveIntOption(raw, fallback, label) {
    if (!raw)
        return fallback;
...
toolsCommand
    ...
    for (const tool of toolsets) {
        console.log(`- ${tool.enabled ? chalk.green('[ON]') : chalk.red('[OFF]')} ${chalk.cyan(tool.name)}: ${tool.description} (${tool.source || 'builtin'})`);
    }
});
...
pluginsCommand
    ...
    for (const plugin of plugins) {
        console.log(`- ${chalk.cyan(plugin.name)} (v${plugin.version}): ${plugin.description} [${plugin.category}]`);
    }
});
...
cronCommand
    ...
    for (const job of jobs) {
        console.log(`- ${job.enabled ? chalk.green('[ON]') : chalk.red('[OFF]')} ${chalk.cyan(job.name)} (${job.schedule}) -> ${job.command}`);
    }
});
...
webhookCommand
    ...
    for (const webhook of webhooks) {
        console.log(`- ${webhook.active ? chalk.green('[ON]') : chalk.red('[OFF]')} ${chalk.cyan(webhook.event)}: ${webhook.url}`);
    }
});
...
memoryCommand
    ...
    for (const provider of providers) {
        console.log(`- ${provider.enabled ? chalk.green('[ON]') : chalk.red('[OFF]')} ${chalk.cyan(provider.name)} [${provider.type}]`);
    }
});
async function loadTnfSystemPrompt() {
    const promptPath = path.join(repoRoot, '.agent/SYSTEM_PROMPT.md');
    const fallbackPrompt = 'You are the TNF Orchestrator — the central agent at the heart of The New Fuse network. You coordinate sub-agents, maintain system health, and drive the network forward.';
...
function readTextFileIfPresent(relativePath, maxChars = 1600) {
...
function readJsonFileIfPresent(relativePath) {
...
function readAbsoluteJsonFileIfPresent(absolutePath) {
...
function getHomeHandoffPath() {
    return path.join(os.homedir(), '.tnf', 'handoff-current.json');
}
function syncHomeHandoffCache() {
...
function getHandoffDivergence(repoHandoff, homeHandoff) {
    if (!repoHandoff || !homeHandoff)
        return null;
...
function summarizeHandoffPacket(handoff, source) {
    if (!handoff)
...
function loadTnfInteractiveContextPack() {
    ...
    const homeMemory = readAbsoluteTextFileIfPresent(path.join(os.homedir(), '.tnf', 'MEMORY.md'), 900);
...
function extractInteractiveBashBlocks(text) {
    const blocks = [];
...
function wantsAutonomousExecution(input) {
    const lower = input.toLowerCase();
    return /\b(autonomous|autonomously|do all|execute all|run all|just do it|do your best|go ahead)\b/.test(lower);
}
function resolveAutonomousModeToggle(args) {
    const token = args.join(' ').trim().toLowerCase();
    if (!token)
        return null;
    if (['on', 'true', '1', 'yes', 'enable'].includes(token))
        return true;
    if (['off', 'false', '0', 'no', 'disable'].includes(token))
        return false;
    return null;
}
async function executeInteractiveBash(script) {
...
async function runInteractiveBashBlocks(blocks, messages) {
    if (!blocks.length)
        return;
...
function readAbsoluteTextFileIfPresent(absolutePath, maxChars = 1600) {
    try {
        if (!fs.existsSync(absolutePath))
            return null;
        return fs.readFileSync(absolutePath, 'utf8').slice(0, maxChars).trim();
...
function countRuntimeField(value, fallback) {
    if (Array.isArray(value))
...
function getMcpServerNames(runtimeState) {
...
function startProcessingIndicator(label = 'Processing') {
    if (spinnerActive)
        return;
    spinnerActive = true;
    spinnerFrame = 0;
    ...
    const write = (frame) => process.stderr.write(`\r${chalk.cyan(frame)} ${chalk.dim(label)}... `);
...
function stopProcessingIndicator(success = true) {
    if (!spinnerActive)
        return;
    spinnerActive = false;
    if (spinnerHandle) {
...
async function startInteractiveAgent() {
    ...
    const availableCount = catalog.filter((p) => p.hasKey).length;
    console.log(chalk.cyan('║') +
        chalk.dim(' Fallbacks: ') +
        chalk.white(`${availableCount} providers available`) +
        chalk.cyan(' ║'));
    console.log(chalk.cyan('╚══════════════════════════════════════════════╝'));
    console.log(chalk.dim(' Type /help for commands, /exit to quit, /clear to clear history, /autonomous on for shell execution\n'));
    ...
    const ask = (prompt) => new Promise((resolve, reject) => {
        if (rlClosed)
            return reject(new Error('stdin closed'));
        ...
        if (timeoutSec > 0) {
            ...
            const timer = setTimeout(() => {
                ...
                resolve(process.env.TNF_STALL_DEFENSE_PROMPT ||
    ...
    while (true) {
        ...
        const promptWithModel = process.env.TNF_SHOW_MODEL_IN_PROMPT !== '0'
...
function normalizeEntrypointArgv(argv) {
    if (argv[2] !== '--')
        return argv;
