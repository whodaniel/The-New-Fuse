...
function defaultProviderTimeoutMs() {
    return (parsePositiveIntegerEnv('TNF_LLM_TIMEOUT_MS') ??
...
export class LLMClient {
    ...
    static async create(role = 'worker') {
    ...
    async resolveProvider() {
        ...
        const sortedProviders = [...this.providers].sort((a, b) => (a.priority ?? 99) - (b.priority ?? 99));
        for (const provider of sortedProviders) {
            const key = this.resolveApiKey(provider);
            if (!key)
                continue;
            // Quick liveness check — skip if marked dead
            if (provider.note && /402|410|exhausted|gone/i.test(provider.note))
                continue;
    ...
    detectProviderFromUrl(url) {
        ...
        if (url.includes('groq.com'))
            return 'groq';
        if (url.includes('generativelanguage.googleapis'))
            return 'gemini';
    ...
    async _tryFallbacks(messages, options, primaryError) {
        ...
        for (const provider of sorted) {
            ...
            if (tried.has(providerAttemptKey))
                continue;
