...
const STANDARD_SLASH_COMMANDS = [
    ...
    },
    {
        ...
        source: 'standard',
        mode: 'prompt',
        prompt: 'Review the current TNF workspace with a bug-first engineering stance. Prioritize correctness, regressions, missing verification, and public-distribution readiness. If you cannot inspect local files from this runtime, say exactly what command or tool-enabled agent should run next.',
    },
    {
        name: 'apply',
        ...
        source: 'standard',
        mode: 'prompt',
        prompt: 'Apply the following requested change to the TNF workspace. Inspect state first, keep edits scoped, and verify the result. Instruction:',
    },
    {
        name: 'new',
        ...
        source: 'standard',
        mode: 'prompt',
        prompt: 'Start a new conversation. Clear the current context, chat history, and any temporary state. Begin fresh with the system prompt.',
    },
];
const TNF_SLASH_COMMANDS = [
    {
        name: 'commands',
        aliases: ['slash'],
...
export function normalizeSlashName(name) {
    return name.replace(/^\//, '').trim().toLowerCase();
}
export function getStandardSlashCommands() {
    return [...STANDARD_SLASH_COMMANDS, ...TNF_SLASH_COMMANDS];
}
export function getProjectSlashCommands(projectRoot) {
    const project = new ProjectConfigService(projectRoot);
    return project.getCommands().map(projectCommandToSlashCommand);
}
export function getAllSlashCommands(projectRoot) {
    return [...getStandardSlashCommands(), ...getProjectSlashCommands(projectRoot)];
}
export function findSlashCommand(name, projectRoot) {
    const normalized = normalizeSlashName(name);
    return (getAllSlashCommands(projectRoot).find((command) => command.name === normalized ||
        command.aliases?.some((alias) => normalizeSlashName(alias) === normalized)) || null);
}
export function renderSlashCommandList(projectRoot) {
    const commands = getAllSlashCommands(projectRoot);
    const groups = ['standard', 'tnf', 'project'];
...
export function renderSlashCommandDetail(command) {
...
export function formatPromptSlashCommand(command, args) {
