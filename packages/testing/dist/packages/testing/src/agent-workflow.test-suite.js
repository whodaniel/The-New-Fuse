...
exports.AgentWorkflowTestSuite = void 0;
...
(function (A2APriority) {
    A2APriority[A2APriority["LOW"] = 1] = "LOW";
    A2APriority[A2APriority["MEDIUM"] = 2] = "MEDIUM";
    A2APriority[A2APriority["HIGH"] = 3] = "HIGH";
})(A2APriority || (A2APriority = {}));
class AgentWorkflowTestSuite {
    constructor(app, queueService, cacheService, websocketService, a2aService) {
        // Test data storage
        this.testAgents = new Map();
        this.testWorkflows = new Map();
        ...
        for (const scenario of testScenarios) {
            ...
            try {
                ...
                try {
                    ...
                    console.log(`⚠️ Cleanup failed for ${scenario.name}: ${cleanupError?.message || 'Unknown error'}`);
                }
            }
        }
        ...
        return [
            ...
            },
            {
                ...
                execute: async () => {
                    ...
                    const submitPromises = parallelTasks.map(task => this.queueService.addJob(JobType.TASK_PROCESSING, {
                        id: task.id,
                        type: 'parallel_task',
                        payload: {
                            taskId: task.id,
                ...
                verify: async () => {
                    ...
                    const workflowTasks = Array.from(this.testTasks.values())
                    ...
                    if (maxStartTime - minStartTime > 5000) { // 5 second window
            ...
            },
            {
                ...
                execute: async () => {
                    const coordinator = Array.from(this.testAgents.values())
                        .find(agent => agent.type === 'coordinator');
                    const workers = Array.from(this.testAgents.values())
                    ...
                    if (broadcastResponses.length !== workers.length) {
                        throw new Error(`Expected ${workers.length} broadcast responses, got ${broadcastResponses.length}`);
                    }
                },
                verify: async () => {
                    // Verify A2A metrics
                    ...
                    if (metrics.onlineAgents !== this.testAgents.size) {
                        throw new Error(`Expected ${this.testAgents.size} online agents, found ${metrics.onlineAgents}`);
                    }
                },
                cleanup: async () => {
                    await this.cleanupTestData();
    ...
    // Helper methods
    createTestAgent(type, suffix) {
        const id = `test_${type}_${suffix || (0, uuid_1.v4)().slice(0, 8)}`;
        const capabilityMap = {
    ...
    createTestWorkflow(type, suffix) {
        const id = `test_workflow_${type}_${suffix || (0, uuid_1.v4)().slice(0, 8)}`;
        const stepTemplates = {
            simple_linear: [
            ...
            parallel_execution: [],
            error_recovery: [
                { name: 'Failing Step', type: 'task', dependencies: [] },
            ],
            load_test: [
        ...
        const steps = (stepTemplates[type] || []).map((template, index) => ({
            id: `step_${index}`,
            name: template.name,
            type: template.type,
    ...
    createTestTask(workflowId, stepId) {
        return {
            id: `test_task_${(0, uuid_1.v4)().slice(0, 8)}`,
            workflowId,
            stepId,
            type: 'test_task',
            priority: 'medium',
    ...
    async executeLoadTestWorkflow(workflow) {
    ...
    async runSingleTest(testName) {
        ...
        try {
            ...
            await scenario.cleanup();
            return { status: 'FAILED', scenario: scenario.name, error: error?.message || 'Unknown error' };
        }
        finally {
            await this.cleanupTestEnvironment();
        }
...
exports.AgentWorkflowTestSuite = AgentWorkflowTestSuite;
//# sourceMappingURL=agent-workflow.test-suite.js.map