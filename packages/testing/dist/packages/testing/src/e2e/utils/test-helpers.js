...
class TestHelpers {
    ...
    async createTestWorkflowData(options = {}) {
        ...
        const response = await this.page.request.post(`${test_config_1.config.apiUrl}/api/workflows`, {
            data: {
                name,
                nodes,
                edges,
    ...
    async cleanupTestData() {
        // Clean up workflows
        await this.page.request.delete(`${test_config_1.config.apiUrl}/api/workflows/test-*`);
        // Clean up test users except admin
        const testUsers = await this.page.request.get(`${test_config_1.config.apiUrl}/api/users/test-*`);
        const users = await testUsers.json();
        for (const user of users) {
            if (user.username !== test_config_1.config.userPool.admin.username) {
                await this.page.request.delete(`${test_config_1.config.apiUrl}/api/users/${user.id}`);
            }
        }
    }
    async createTestUser(options = {}) {
        const timestamp = Date.now();
        const { username = `testuser-${timestamp}`, email = `test-${timestamp}@example.com`, isAdmin = false } = options;
        const response = await this.page.request.post(`${test_config_1.config.apiUrl}/api/users`, {
            data: {
                username,
                email,
