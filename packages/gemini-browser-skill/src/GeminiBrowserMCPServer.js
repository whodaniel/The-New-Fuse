...
exports.geminiBrowserMCP = exports.GeminiBrowserMCPServer = void 0;
const GeminiBrowserAutomation_js_1 = require('./GeminiBrowserAutomation.js');
class GeminiBrowserMCPServer {
  constructor() {
    this.tools = [
      {
  ...
  async handlePrompt(args) {
    ...
    const response = await GeminiBrowserAutomation_js_1.geminiBrowser.prompt(request);
    if (!response.success) {
      return {
        content: [
          {
   ...
   */
  handleStatus() {
    const isReady = GeminiBrowserAutomation_js_1.geminiBrowser.isReady();
    return {
      content: [
        {
          type: 'text',
  ...
  async handleInitialize() {
    const success = await GeminiBrowserAutomation_js_1.geminiBrowser.initialize();
    return {
      content: [
        {
          type: 'text',
   ...
   */
  async handleClose() {
    await GeminiBrowserAutomation_js_1.geminiBrowser.close();
    return {
      content: [
        {
          type: 'text',
