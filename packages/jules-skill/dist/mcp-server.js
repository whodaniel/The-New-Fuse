...
import { CallToolRequestSchema, ListResourcesRequestSchema, ListToolsRequestSchema, ReadResourceRequestSchema, } from '@modelcontextprotocol/sdk/types';
...
const TOOLS = [
    ...
    {
        name: 'jules_teleport',
        description: 'Teleport to a Jules session - clones the repository, checks out the branch, and applies the patch.',
        inputSchema: {
            type: 'object',
            properties: {
                session_id: {
    ...
    },
    {
        ...
        description: 'Submit multiple tasks to Jules in parallel. Use this for bulk operations like creating multiple repositories or implementing multiple features.',
        inputSchema: {
            type: 'object',
            properties: {
                tasks: {
    ...
    try {
        switch (name) {
            ...
            case 'jules_get_session': {
                const session = await julesClient.getSession(args.session_id);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'jules_teleport': {
                const result = await julesClient.teleport(args.session_id);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'jules_submit_template': {
                const template = await getTemplate(args.template_id);
                if (!template) {
                    return {
                        content: [
                            {
...
function parseTemplateFile(filename, content) {
