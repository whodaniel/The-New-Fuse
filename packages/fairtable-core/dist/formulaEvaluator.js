...
const tokenize = (formula) => {
    ...
    while ((match = tokenRegex.exec(formula)) !== null) {
        if (match[1])
            tokens.push(match[1]); // Field reference {Field Name}
        else if (match[2])
            tokens.push(parseFloat(match[2])); // Number
        else if (match[3])
            tokens.push(match[3]); // Operator
        else if (match[4])
            tokens.push(match[4].slice(1, -1)); // String literal (remove quotes)
    }
    return tokens;
};
const getColumnValue = (fieldName, rowData, columns, rows, // all rows in the current table for potential lookups (not used in this simple version)
allTables // all tables for potential cross-table lookups (not used in this simple version)
) => {
    ...
    switch (targetColumn.type) {
        ...
        case DataType.LAST_MODIFIED_TIME:
            ...
            if (value instanceof Date)
                return value.getTime(); // Already a Date object
            if (typeof value === 'string') {
        ...
        case DataType.LINKED_RECORD:
            if (Array.isArray(value) && value.length > 0 && targetColumn.linkedTableId && allTables) {
                ...
                return value.join(', '); // Fallback to IDs if lookup fails
            }
            return null;
...
export const evaluateFormula = (formulaString, currentRow, columns, currentTableRows, allTables) => {
    if (!formulaString)
        return { value: null };
    ...
    const RPNQueue = []; // Using as a simple evaluation stack/queue
    const operatorStack = []; // Not a full Shunting-yard, basic op handling
    ...
    for (const token of tokens) {
        if (typeof token === 'string' && token.startsWith('{') && token.endsWith('}')) {
            ...
            if (typeof colValResult === 'object' && colValResult !== null && 'error' in colValResult) {
                return { value: null, error: colValResult.error };
            }
            RPNQueue.push(colValResult);
        }
    ...
    for (const item of RPNQueue) {
        if (typeof item === 'string' && ['+', '-', '*', '/', '&'].includes(item)) {
            ...
            if (a_raw === undefined || b_raw === undefined)
                return { value: null, error: "Syntax error in formula." };
            let valA = a_raw;
            let valB = b_raw;
            if (item === '&') { // String concatenation
                ...
                if (isNaN(valA) || isNaN(valB)) {
                    ...
                    if ((isNaN(valA) && a_raw !== null && a_raw !== undefined) ||
                        (isNaN(valB) && b_raw !== null && b_raw !== undefined)) {
    ...
    if (evalStack.length !== 1)
        return { value: null, error: "Invalid formula structure." };
