...
import { Environment } from '../types/featureFlags.js';
...
// Instance methods
FeatureFlagSchema.methods.isEnabledForEnvironment = function (environment) {
    if (!this.enabled)
        return false;
...
FeatureFlagSchema.methods.evaluateForUser = function (userId, userAttributes) {
    if (!this.enabled)
        return false;
...
FeatureFlagSchema.methods.hashUser = function (userId) {
    const crypto = require('crypto');
    const hash = crypto.createHash('md5').update(`${this.name}:${userId}`).digest('hex');
    return parseInt(hash.substring(0, 8), 16);
};
FeatureFlagSchema.methods.evaluateRules = function (rules, userAttributes) {
...
// Static methods
FeatureFlagSchema.statics.findByEnvironment = function (environment) {
    return this.find({
        $or: [
...
FeatureFlagSchema.statics.findEnabledFeatures = function (environment) {
