...
export class ContextAwareOrchestrator {
     ...
     */
    constructor(vectorMemory, skillDiscovery) {
     ...
     */
    async planTaskWithContext(task) {
        try {
     ...
     */
    synthesizeApproach(successfulPatterns) {
        if (successfulPatterns.length === 0) {
            return 'No relevant past experience found. A novel approach is required.';
        }
        const keyElements = successfulPatterns.map((p) => p.taskDescription.split(' ').slice(0, 5).join(' ') + '...');
        return `Based on past successes with tasks like "${keyElements.join(', ')}", the recommended approach involves leveraging similar strategies.`;
    }
    /**
     ...
     */
    calculateConfidence(successfulPatterns, totalSimilarTasks) {
        ...
        const averageRelevance = successfulPatterns.reduce((sum, p) => sum + p.relevance, 0) /
...
//# sourceMappingURL=ContextAwareOrchestrator.js.map