...
import { TaskStatus } from '../types/core.js';
...
let AgentSwarmOrchestrationService = AgentSwarmOrchestrationService_1 = class AgentSwarmOrchestrationService extends EventEmitter {
    constructor() {
        super();
        this.logger = new Logger(AgentSwarmOrchestrationService_1.name);
    ...
    async getSwarmStatus() {
        ...
        return {
            ...
            onlineAgents: agents.filter((a) => a.status === AgentStatus.IDLE || a.status === AgentStatus.BUSY).length,
        ...
        while (this.isProcessing && this.state === ServiceState.RUNNING) {
            ...
            for (const taskId of readyTasks) {
                ...
                if (task && task.status === TaskStatus.PENDING) {
                    ...
                    const execution = Array.from(this.executions.values()).find((exec) => exec.tasks.includes(taskId));
