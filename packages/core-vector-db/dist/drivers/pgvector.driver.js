...
let PgVectorDriver = PgVectorDriver_1 = class PgVectorDriver {
    constructor(config) {
        this.config = config;
        this.logger = new Logger(PgVectorDriver_1.name);
        const normalizedHost = config.host && config.host.startsWith('http') ? new URL(config.host).hostname : config.host;
        this.pool = new Pool({
...
PgVectorDriver = PgVectorDriver_1 = __decorate([
    Injectable(),
    __param(0, Inject('VECTOR_DB_CONFIG')),
    __metadata("design:paramtypes", [Object])
], PgVectorDriver);
export { PgVectorDriver };
