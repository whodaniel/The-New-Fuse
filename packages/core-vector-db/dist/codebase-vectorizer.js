...
export class CodebaseVectorizer {
     ...
     */
    async vectorizeCodebase(rootPath) {
     ...
     */
    async extractCodeEntities(content, filePath, extension) {
        const entities = [];
     ...
     */
    extractBlock(lines, startLine, maxLines = 100) {
    ...
    async storeEntities(entities) {
        ...
        for (const entity of entities) {
            ...
            // Insert new entity
            const result = await query(`INSERT INTO code_entities 
    ...
    async generateEmbeddings(entities) {
        ...
        for (let i = 0; i < entities.length; i += this.batchSize) {
            ...
            // Get entity contents
            const entityData = await query(`SELECT id, entity_name, content FROM code_entities WHERE id = ANY($1::bigint[])`, [batch.map((e) => e.id)]);
            if (entityData.rows.length === 0)
                continue;
            // Prepare texts for embedding
            const texts = entityData.rows.map((e) => `${e.entity_name}\n\n${e.content.substring(0, 8000)}`);
            try {
                ...
                for (let j = 0; j < entityData.rows.length; j++) {
                    ...
                    await query(`INSERT INTO code_embeddings (entity_id, embedding, model)
            VALUES ($1, $2, $3)
            ON CONFLICT (entity_id) DO UPDATE SET embedding = $2, model = $3`, [entity.id, JSON.stringify(embedding), this.embeddingModel]);
                }
     ...
     */
    async extractRelationships(_entities) {
        console.log('🔗 Extracting code relationships...');
        // Get all entities with their content
        const allEntitiesResult = await query(`SELECT id, file_path, entity_name, entity_type, content FROM code_entities`);
        ...
        for (const entity of allEntities) {
            ...
            while ((match = importRegex.exec(entity.content)) !== null) {
                ...
                const importedEntity = allEntities.find((e) => e.file_path.includes(importPath));
                if (importedEntity) {
                    relationships.push({
            ...
            while ((match = callRegex.exec(entity.content)) !== null) {
                ...
                const calledEntity = allEntities.find((e) => e.entity_name === calledFunction &&
        ...
        for (const rel of relationships) {
            await query(`INSERT INTO code_relationships (from_entity_id, to_entity_id, relationship_type, metadata)
        VALUES ($1, $2, $3, $4)
        ON CONFLICT DO NOTHING`, [rel.fromEntityId, rel.toEntityId, rel.relationshipType, JSON.stringify(rel.metadata || {})]);
        }
     ...
     */
    async createSnapshot(totalEntities) {
        ...
        await query(`INSERT INTO codebase_snapshots (git_commit_hash, total_entities, total_files, metadata)
      VALUES ($1, $2, $3, $4)`, [gitHash, totalEntities, count, JSON.stringify({ timestamp: new Date().toISOString() })]);
    }
    /**
     * Cleanup
     */
...
//# sourceMappingURL=codebase-vectorizer.js.map