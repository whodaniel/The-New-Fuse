...
let VectorDatabaseService = VectorDatabaseService_1 = class VectorDatabaseService {
    constructor(dbConfig, embeddingConfig) {
        ...
        this.logger = new Logger(VectorDatabaseService_1.name);
    }
    async onModuleInit() {
        if (!this.dbConfig?.provider || !this.embeddingConfig?.provider) {
            this.logger.warn('Vector DB configuration is incomplete. Skipping vector provider initialization for this runtime.');
            return;
        }
...
VectorDatabaseService = VectorDatabaseService_1 = __decorate([
    Injectable(),
