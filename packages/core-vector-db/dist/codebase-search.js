...
export class CodebaseSearch {
     ...
     */
    async semanticSearch(searchQuery, limit = 10) {
      ...
      LIMIT $2`, [JSON.stringify(queryEmbedding), limit]);
        // 3. Log analytics
     ...
     */
    async findSimilarCode(filePath, entityName, threshold = 0.9) {
        // Get the entity's embedding
        const entityResult = await query(`SELECT ce.id, cem.embedding
      FROM code_entities ce
      JOIN code_embeddings cem ON ce.id = cem.entity_id
      WHERE ce.file_path = $1 AND ce.entity_name = $2
      LIMIT 1`, [filePath, entityName]);
      ...
      ORDER BY cem.embedding <=> $1::vector
      LIMIT 20`, [sourceEmbedding, sourceId, threshold]);
        return results.rows;
    }
    /**
     * Find code that imports/uses a specific entity (knowledge graph traversal)
     */
    async findUsages(filePath, entityName) {
        const entityResult = await query(`SELECT id FROM code_entities 
      ...
      LIMIT 1`, [filePath, entityName]);
        ...
        const usages = await query(`SELECT 
        ce.id,
        ce.file_path as "filePath",
        ce.entity_type as "entityType",
        ce.entity_name as "entityName",
      ...
      WHERE cr.to_entity_id = $1`, [entityId]);
     ...
     */
    async findDependencies(filePath, entityName) {
        const entityResult = await query(`SELECT id FROM code_entities 
      WHERE file_path = $1 AND entity_name = $2
      LIMIT 1`, [filePath, entityName]);
        if (entityResult.rows.length === 0) {
        ...
        const entityId = entityResult.rows[0].id;
        const dependencies = await query(`SELECT 
        ce.id,
        ce.file_path as "filePath",
        ce.entity_type as "entityType",
        ce.entity_name as "entityName",
      ...
      WHERE cr.from_entity_id = $1`, [entityId]);
     ...
     */
    async hybridSearch(searchQuery, filters = {}, limit = 10) {
        const queryEmbedding = await this.generateQueryEmbedding(searchQuery);
     ...
     */
    async findCodeClusters(minSimilarity = 0.85) {
        // Get all embeddings
        const allEmbeddings = await query(`SELECT 
        ce.id,
        ce.file_path as "filePath",
        ce.entity_type as "entityType",
        ce.entity_name as "entityName",
      ...
      WHERE ce.entity_type IN ('function', 'class', 'method')`);
        ...
        for (const entity of allEmbeddings.rows) {
            ...
            const similar = await query(`SELECT 
          ce.id,
          ce.file_path as "filePath",
          ce.entity_type as "entityType",
          ce.entity_name as "entityName",
        ...
        WHERE ce.id != $2
          AND 1 - (cem.embedding <=> $1::vector) > $3
        ORDER BY cem.embedding <=> $1::vector`, [entity.embedding, entity.id, minSimilarity]);
            if (similar.rows.length > 0) {
                clusters.push([entity, ...similar.rows]);
     ...
     */
    async logSearch(searchQuery, queryType, resultCount, queryTimeMs) {
        try {
            await query(`INSERT INTO search_analytics (query_text, query_type, results_count, query_time_ms)
        VALUES ($1, $2, $3, $4)`, [searchQuery, queryType, resultCount, queryTimeMs]);
        }
        catch (error) {
