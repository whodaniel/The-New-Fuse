...
const Toast = React.forwardRef(
  ({ title, description, variant = 'default', duration = 3000, action, ...props }, ref) => {
    ...
    const variantStyles = {
      ...
      warning:
        'border-yellow-500 bg-yellow-50 text-yellow-800 dark:border-yellow-700 dark:bg-yellow-900 dark:text-yellow-100',
      destructive:
        'border-red-500 bg-red-50 text-red-800 dark:border-red-700 dark:bg-red-900 dark:text-red-100',
    };
    ...
    return _jsxs('div', {
      ...
      className: cn(
        ...
        variantStyles[toastVariant] // Type assertion to help TypeScript
      ),
      ...props,
      children: [
        title && _jsx('div', { className: 'font-medium', children: title }),
        description && _jsx('div', { className: 'mt-1 text-sm', children: description }),
        action && _jsx('div', { className: 'mt-2 flex justify-end', children: action }),
      ],
    });
  }
);
...
const Toaster = () => {
  return _jsx('div', {
    id: 'toast-container',
    className: 'fixed top-0 right-0 z-50 p-4 space-y-4',
  });
};
