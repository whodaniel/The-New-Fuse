...
const Alert = React.forwardRef(
  ({ className, variant = 'default', icon, children, ...props }, ref) => {
    const variantClasses = {
      ...
      warning:
        'border-yellow-500/50 text-yellow-600 dark:border-yellow-500 dark:text-yellow-500 [&>svg]:text-yellow-600',
      success:
        'border-green-500/50 text-green-600 dark:border-green-500 dark:text-green-500 [&>svg]:text-green-600',
    ...
    return _jsxs('div', {
      ...
      ...props,
      children: [
        icon && _jsx('span', { className: 'mr-2 inline-block', children: icon }),
        children,
      ],
    });
  }
