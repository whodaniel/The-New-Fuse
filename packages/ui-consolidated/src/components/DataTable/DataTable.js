...
function DataTable({ data, columns, className = '', onRowClick }) {
  return _jsx('div', {
    ...
    children: _jsxs('table', {
      className: 'min-w-full divide-y divide-border/50',
      children: [
        _jsx('thead', {
            ...
            children: columns.map((column, idx) =>
              _jsx(
                'th',
                {
                  scope: 'col',
                  className:
                    'px-3 py-2 text-left text-xs font-semibold uppercase tracking-wide text-muted-foreground/80',
        ...
        _jsx('tbody', {
          className: 'bg-transparent divide-y divide-border/40',
          children: data.map((item, rowIdx) =>
            _jsx(
              'tr',
              {
