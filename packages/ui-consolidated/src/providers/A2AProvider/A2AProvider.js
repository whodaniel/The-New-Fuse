...
export function A2AProvider({
  ...
  return _jsx(A2AContext.Provider, { value: contextValue, children: children });
}
...
export function useA2AAgents() {
  ...
  const findAgentsByCapability = (capability) => {
    return agents.filter((agent) => agent.capabilities.some((cap) => cap === capability));
  };
  const findAgentsByType = (type) => {
