...
const server = new Server({ name: 'mcp-cloud-redis-bridge', version: '1.0.0' }, { capabilities: { tools: {} } });
const redisClient = new CloudRedisClient();
let authorizedIdentity = process.env.LOCAL_SUBDIRECTOR_NFT_ID && process.env.LOCAL_SUBDIRECTOR_WALLET_ADDRESS
    ? {
...
const BroadcastPromptSchema = z.object({
    ...
    invokerAgentId: z
        .string()
        .describe("The ID of the agent invoking this tool (must be 'sub-director' or 'orchestration-agent')"),
});
    ...
    return {
        tools: [
                ...
                inputSchema: {
                    ...
                    properties: {
                        ...
                        signing_private_key_pem: {
                            type: 'string',
                            description: 'Optional: PKCS8 PEM for Ed25519',
                        },
                        encryption_private_key_pem: {
                            type: 'string',
                            description: 'Optional: PKCS8 PEM for X25519',
                        },
                    },
            ...
            },
            {
                ...
                description: 'Get the WebSocket URL for the interactive cloud terminal (PTY).',
                inputSchema: { type: 'object', properties: {} },
            },
        ],
    };
    ...
    try {
        switch (name) {
            ...
            case 'verify_master_clock_signal': {
                ...
                const plaintext = SecurityService.verifyAndDecryptSignal(envelope, authorizedIdentity.encryption_private_key_pem);
                return {
                    content: [
                        {
                            type: 'text',
                            text: `Signal Verified & Decrypted:\n${JSON.stringify(plaintext, null, 2)}`,
                        },
                    ],
                };
            }
            case 'read_super_cycle_state': {
                ...
                return {
                    content: [
                        { type: 'text', text: `Super Cycle State:\n${JSON.stringify(state, null, 2)}` },
                    ],
                };
            }
            ...
            case 'get_terminal_access': {
                const publicUrl = process.env.CLOUD_RUNTIME_PUBLIC_DOMAIN || 'tnf-cloud-sandbox-production.thenewfuse.com';
                const protocol = publicUrl.includes('localhost') ? 'ws' : 'wss';
                return {
                    content: [
                        {
                            type: 'text',
                            text: `Cloud Terminal WebSocket URL: ${protocol}://${publicUrl}/ws/terminal\nInstructions: Use a WebSocket client to connect. You can now use Cline CLI for Nvidia NIM API access.`,
                        },
                    ],
                };
            }
