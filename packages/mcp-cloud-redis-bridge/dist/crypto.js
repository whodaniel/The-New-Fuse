import { createDecipheriv, createPrivateKey, createPublicKey, diffieHellman, generateKeyPairSync, hkdfSync, sign, verify, } from 'node:crypto';
/**
...
export class SecurityService {
     ...
     */
    static verifyAndDecryptSignal(signal, nodeEncryptionPrivateKeyPem) {
        ...
        const signatureValid = verify(null, Buffer.from(stableStringify(signable)), createPublicKey(payload.signing_public_key_pem), Buffer.from(payload.signature_b64, 'base64'));
        if (!signatureValid) {
        ...
        const key = Buffer.from(hkdfSync('sha256', sharedSecret, Buffer.from(payload.salt_b64, 'base64'), Buffer.from(`tnf-master-clock:${payload.signal_id}`), 32));
     ...
     */
    static createNodeAckSignature(request, nodeSigningPrivateKeyPem) {
        const signable = {
        ...
        return sign(null, Buffer.from(stableStringify(signable)), createPrivateKey(nodeSigningPrivateKeyPem)).toString('base64');
    }
    /**
