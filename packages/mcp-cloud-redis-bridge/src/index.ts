...
let authorizedIdentity: {
  ...
  process.env.LOCAL_SUBDIRECTOR_NFT_ID && process.env.LOCAL_SUBDIRECTOR_WALLET_ADDRESS
    ? {
...
const BroadcastPromptSchema = z.object({
  ...
  invokerAgentId: z
    .string()
    .describe(
      "The ID of the agent invoking this tool (must be 'sub-director' or 'orchestration-agent')"
    ),
});
    ...
    tools: [
      {
        ...
        description:
          "Set the Local Director's NFT identity and private keys for secure communication.",
        inputSchema: {
          type: 'object',
          properties: {
      ...
      },
      {
        ...
        description:
          'Generate a fresh NFT-compatible identity (keys + placeholder NFT ID) for a new node.',
        inputSchema: { type: 'object', properties: {} },
      },
      {
