...
class TemplateValidator {
        ...
        try {
            ...
            for (const rule of this.rules) {
                try {
                    ...
                    issues.push({
                        rule: rule.name,
                        severity: infrastructure_js_1.ErrorSeverity.ERROR,
            ...
            // Categorize issues
            const errors = issues
                .filter(i => i.severity === infrastructure_js_1.ErrorSeverity.ERROR)
                .map(i => ({
                code: i.rule,
                message: i.message,
                path: i.path,
            ...
            const warnings = issues
                .filter(i => i.severity === infrastructure_js_1.ErrorSeverity.WARNING)
                .map(i => ({
                code: i.rule,
                message: i.message,
                path: i.path,
            ...
            const suggestions = issues
                .filter(i => i.severity === infrastructure_js_1.ErrorSeverity.INFO && i.suggestion)
                .map(i => ({
            ...
            const mappedWarnings = warnings.map(warn => ({
                ...
                severity: infrastructure_js_1.ErrorSeverity.WARNING
            }));
            const mappedSuggestions = suggestions.map(sugg => ({
                ...sugg,
                rule: sugg.type || 'unknown',
                severity: infrastructure_js_1.ErrorSeverity.INFO,
    ...
    getSuggestionType(ruleName) {
        if (ruleName.includes('security'))
            return infrastructure_js_1.SuggestionType.SECURITY;
        if (ruleName.includes('cost'))
            return infrastructure_js_1.SuggestionType.COST;
        if (ruleName.includes('performance'))
            return infrastructure_js_1.SuggestionType.PERFORMANCE;
        return infrastructure_js_1.SuggestionType.OPTIMIZATION;
    }
}
...
class RequiredFieldsRule {
    constructor() {
        this.name = 'required-fields';
        this.description = 'Validates that all required fields are present';
        this.severity = infrastructure_js_1.ErrorSeverity.ERROR;
    }
    validate(template) {
        const issues = [];
...
class ResourceNamingRule {
    constructor() {
        this.name = 'resource-naming';
        this.description = 'Validates resource naming conventions';
        this.severity = infrastructure_js_1.ErrorSeverity.WARNING;
    }
    validate(template) {
        const issues = [];
...
class DependencyValidationRule {
    constructor() {
        this.name = 'dependency-validation';
        this.description = 'Validates resource dependencies';
        this.severity = infrastructure_js_1.ErrorSeverity.ERROR;
    }
    validate(template) {
        const issues = [];
    ...
    findCircularDependencies(resources) {
        const cycles = [];
        const visited = new Set();
        const recursionStack = new Set();
        const resourceMap = new Map(resources.map(r => [r.name, r]));
        const dfs = (resourceName, path) => {
...
class VariableValidationRule {
    constructor() {
        this.name = 'variable-validation';
        this.description = 'Validates template variables';
        this.severity = infrastructure_js_1.ErrorSeverity.ERROR;
    }
    validate(template) {
        const issues = [];
    ...
    validateVariableType(value, type) {
        switch (type) {
            case infrastructure_js_1.VariableType.STRING:
                return typeof value === 'string';
            case infrastructure_js_1.VariableType.NUMBER:
                return typeof value === 'number';
            case infrastructure_js_1.VariableType.BOOLEAN:
                return typeof value === 'boolean';
            case infrastructure_js_1.VariableType.LIST:
                return Array.isArray(value);
            case infrastructure_js_1.VariableType.MAP:
            case infrastructure_js_1.VariableType.OBJECT:
...
class SecurityValidationRule {
    constructor() {
        this.name = 'security-validation';
        this.description = 'Validates security best practices';
        this.severity = infrastructure_js_1.ErrorSeverity.WARNING;
    }
    validate(template) {
        const issues = [];
        template.resources.forEach((resource, index) => {
            ...
            if (resource.type === infrastructure_js_1.ResourceType.FIREWALL_RULE) {
                ...
                sourceRanges.forEach((range, rangeIndex) => {
                    if (range === '0.0.0.0/0' && resource.properties.allowed) {
                        const allowedPorts = resource.properties.allowed.flatMap((a) => a.ports || []);
            ...
            if (resource.type === infrastructure_js_1.ResourceType.CLOUD_STORAGE) {
...
class PerformanceValidationRule {
    constructor() {
        ...
        this.description = 'Validates performance best practices';
        this.severity = infrastructure_js_1.ErrorSeverity.INFO;
    }
    validate(template) {
        const issues = [];
        template.resources.forEach((resource, index) => {
            // Check for undersized compute resources
            if (resource.type === infrastructure_js_1.ResourceType.COMPUTE_ENGINE) {
            ...
            if (resource.type === infrastructure_js_1.ResourceType.LOAD_BALANCER) {
...
class CostOptimizationRule {
    constructor() {
        ...
        this.description = 'Suggests cost optimization opportunities';
        this.severity = infrastructure_js_1.ErrorSeverity.INFO;
    }
    validate(template) {
        const issues = [];
        template.resources.forEach((resource, index) => {
            ...
            if (resource.type === infrastructure_js_1.ResourceType.COMPUTE_ENGINE && !resource.properties.autoScaling) {
                issues.push({
                    rule: this.name,
                    severity: this.severity,
...
class BestPracticesRule {
    constructor() {
        this.name = 'best-practices';
        this.description = 'Validates general best practices';
        this.severity = infrastructure_js_1.ErrorSeverity.INFO;
    }
    validate(template) {
        const issues = [];
