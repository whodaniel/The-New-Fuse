...
class EnvironmentManager {
    ...
    async createEnvironment(name, type, projectId, configuration = {}) {
        ...
        const environment = {
            ...
            metadata: {
                ...
                compliance: {
                    dataClassification: type === infrastructure_js_1.EnvironmentType.PRODUCTION ? 'confidential' : 'internal',
    ...
    async promoteEnvironment(promotion) {
    ...
    generateEnvironmentId(name, type) {
        return `env-${type}-${name}-${Date.now()}`;
    }
    mergeWithDefaults(config, type) {
        const defaults = {
            networking: {
                vpcName: `vpc-${type}`,
            ...
            security: {
                enableOSLogin: true,
                requireSSL: type === infrastructure_js_1.EnvironmentType.PRODUCTION,
                enableAuditLogs: type === infrastructure_js_1.EnvironmentType.PRODUCTION,
                iamPolicies: []
            },
            monitoring: {
            ...
            backup: {
                enableAutomaticBackups: type === infrastructure_js_1.EnvironmentType.PRODUCTION,
                retentionDays: type === infrastructure_js_1.EnvironmentType.PRODUCTION ? 30 : 7,
                backupSchedule: '0 2 * * *', // Daily at 2 AM
                crossRegionBackup: type === infrastructure_js_1.EnvironmentType.PRODUCTION
            },
            scaling: {
                enableAutoScaling: true,
                minInstances: type === infrastructure_js_1.EnvironmentType.PRODUCTION ? 2 : 1,
                maxInstances: type === infrastructure_js_1.EnvironmentType.PRODUCTION ? 10 : 3,
    ...
    generateEnvironmentTemplate(environment) {
        return {
            ...
            resources: [
                {
                    ...
                    lifecycle: {
                        ...
                        preventDestroy: environment.type === infrastructure_js_1.EnvironmentType.PRODUCTION,
                        ignoreChanges: [],
                        replaceTriggeredBy: []
                    },
                    tags: {
    ...
    configurationToVariables(config) {
        const variables = {};
        if (config.scaling) {
    ...
    async runValidationCheck(check, _environment) {
    ...
    async performBlueGreenPromotion(_source, _target) {
        // Mock blue-green promotion implementation
    }
    async performCanaryPromotion(_source, _target) {
        // Mock canary promotion implementation
    }
    async performRollingPromotion(_source, _target) {
