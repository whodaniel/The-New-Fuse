...
class InfrastructureManager {
    ...
    async provisionInfrastructure(template) {
        ...
        try {
            ...
            const infrastructureState = {
                ...
                resources: [],
                status: infrastructure_js_1.InfrastructureStatus.PROVISIONING,
                createdAt: new Date(),
                updatedAt: new Date(),
                metadata: {
            ...
            // Provision resources
            const provisionResults = await this.resourceProvisioner.provisionResources(parsedTemplate.resources, infrastructureState.id);
            // Update state with provisioned resources
            infrastructureState.resources = provisionResults.map(result => ({
                id: result.resourceId || '',
                name: result.resourceName,
                type: result.resourceType,
                state: result.success ? infrastructure_js_1.ResourceState.CREATED : infrastructure_js_1.ResourceState.ERROR,
                properties: {},
            ...
            infrastructureState.status = provisionResults.every(r => r.success)
                ? infrastructure_js_1.InfrastructureStatus.PROVISIONED
                : infrastructure_js_1.InfrastructureStatus.ERROR;
            ...
            await this.metricsCollector.recordProvisioningMetrics({
                ...
                success: infrastructureState.status === infrastructure_js_1.InfrastructureStatus.PROVISIONED
            });
            return {
                success: infrastructureState.status === infrastructure_js_1.InfrastructureStatus.PROVISIONED,
                infrastructureId: infrastructureState.id,
    ...
    async updateInfrastructure(update) {
        const startTime = Date.now();
        try {
            // Get current state
    ...
    async destroyInfrastructure(resourceId) {
        const startTime = Date.now();
        try {
            // Get current state
            ...
            try {
                ...
                currentState.status = infrastructure_js_1.InfrastructureStatus.DESTROYING;
                ...
                // Update final state
                currentState.status = infrastructure_js_1.InfrastructureStatus.DESTROYED;
                currentState.updatedAt = new Date();
                await this.stateManager.saveState(currentState);
                return {
                    success: true,
    ...
    async applyChanges(plan) {
        ...
        try {
            ...
            // Update state
            currentState.status = infrastructure_js_1.InfrastructureStatus.PROVISIONED;
            currentState.updatedAt = new Date();
            await this.stateManager.saveState(currentState);
            return {
    ...
    async getInfrastructureState(infrastructureId) {
    ...
    async listInfrastructure(filters) {
        return await this.stateManager.listStates(filters);
    }
    async importInfrastructure(importConfig) {
        const startTime = Date.now();
        try {
            ...
            const infrastructureState = {
                ...
                resources: importedResources.map(resource => ({
                    ...
                    state: infrastructure_js_1.ResourceState.CREATED,
                    properties: {},
                    outputs: resource.outputs || {},
                    lastModified: new Date()
                })),
                status: infrastructure_js_1.InfrastructureStatus.PROVISIONED,
                createdAt: new Date(),
                updatedAt: new Date(),
                metadata: {
                    version: '1.0.0',
    ...
    async refreshState(infrastructureId) {
