...
const InfrastructureManager_js_1 = require("./InfrastructureManager.js");
const TemplateParser_js_1 = require("./TemplateParser.js");
const StateManager_js_1 = require("./StateManager.js");
const ResourceProvisioner_js_1 = require("./ResourceProvisioner.js");
const TemplateValidator_js_1 = require("./TemplateValidator.js");
const ChangeAnalyzer_js_1 = require("./ChangeAnalyzer.js");
const MetricsCollector_js_1 = require("../core/MetricsCollector.js");
...
describe('InfrastructureManager', () => {
    ...
    let mockLogger;
    beforeEach(() => {
        mockLogger = (0, winston_1.createLogger)({ silent: true });
        const templateParser = new TemplateParser_js_1.TemplateParser();
        const stateManager = new StateManager_js_1.StateManager(new StateManager_js_1.InMemoryStateStorage());
        const resourceProvisioner = new ResourceProvisioner_js_1.ResourceProvisioner();
        const templateValidator = new TemplateValidator_js_1.TemplateValidator();
        const changeAnalyzer = new ChangeAnalyzer_js_1.ChangeAnalyzer();
        const metricsCollector = new MetricsCollector_js_1.MetricsCollector(mockLogger);
        infrastructureManager = new InfrastructureManager_js_1.InfrastructureManager(templateParser, stateManager, resourceProvisioner, templateValidator, changeAnalyzer, metricsCollector);
    });
    describe('provisionInfrastructure', () => {
        it('should provision infrastructure from a valid template', async () => {
            const template = {
                ...
                version: '1.0.0',
                provider: infrastructure_js_1.CloudProvider.GCP,
                resources: [
                    {
                        type: infrastructure_js_1.ResourceType.COMPUTE_ENGINE,
                        name: 'web-server',
                        properties: {
                            cpu: '500m',
                            memory: '1Gi',
            ...
            expect(result.resources[0].resourceType).toBe(infrastructure_js_1.ResourceType.COMPUTE_ENGINE);
    ...
    describe('destroyInfrastructure', () => {
        it('should destroy existing infrastructure', async () => {
            ...
            const destroyResult = await infrastructureManager.destroyInfrastructure(provisionResult.infrastructureId);
    ...
    describe('getInfrastructureState', () => {
        it('should retrieve infrastructure state', async () => {
            ...
            expect(provisionResult.success).toBe(true);
            // Retrieve the state
            const state = await infrastructureManager.getInfrastructureState(provisionResult.infrastructureId);
        ...
        it('should throw error for non-existent infrastructure', async () => {
            await expect(infrastructureManager.getInfrastructureState('non-existent-id')).rejects.toThrow('Infrastructure non-existent-id not found');
        });
    });
...
//# sourceMappingURL=InfrastructureManager.test.js.map