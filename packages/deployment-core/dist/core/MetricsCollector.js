...
class MetricsCollector {
    ...
    updatePipelineAggregates(result) {
    ...
    calculateDORAMetrics(pipelineData, deploymentData) {
        // Deployment Frequency
        const deploymentFrequency = deploymentData.length > 0 ?
            deploymentData.length / Math.max(1, this.getDaysBetween(deploymentData[0].startTime, deploymentData[deploymentData.length - 1].startTime)) : 0;
    ...
    calculatePerformanceMetrics(pipelineData, buildData) {
    ...
    calculateQualityMetrics(buildData) {
    ...
    calculateReliabilityMetrics(pipelineData, deploymentData) {
        return {
    ...
    calculateTrends(pipelineData, buildData, deploymentData) {
        return {
    ...
    calculateMTTR(deploymentData) {
        ...
        failures.forEach(failure => {
            const nextSuccess = deploymentData.find(d => d.startTime > failure.startTime &&
                d.status === 'success' &&
    ...
    calculateAverageParallelization(pipelineData) {
    ...
    calculateCoverageTrend(buildsWithCoverage) {
        if (buildsWithCoverage.length < 2)
            return 'stable';
    ...
    calculateQualityTrend(buildsWithQuality) {
        if (buildsWithQuality.length < 2)
            return 'stable';
    ...
    calculateTrend(data, metric) {
        ...
        if (recentValue > olderValue * 1.1)
            return 'improving';
        if (recentValue < olderValue * 0.9)
            return 'declining';
        return 'stable';
    }
    generateAlerts(recentPipelines, _recentBuilds, _recentDeployments) {
        const alerts = [];
    ...
    // DORA classification methods
    classifyDeploymentFrequency(frequency) {
        if (frequency >= 1)
            return 'elite';
        if (frequency >= 0.14)
            return 'high'; // Weekly
