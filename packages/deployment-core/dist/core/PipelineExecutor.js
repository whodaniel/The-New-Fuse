...
class PipelineExecutor extends events_1.EventEmitter {
    ...
    async executeTask(task, stage, pipeline, executionId) {
        ...
        try {
            ...
            if (result.status === pipeline_js_1.PipelineStatus.FAILED && task.retryPolicy.enabled) {
    ...
    async executeDeployment(deployment) {
        ...
        try {
            ...
            const overallStatus = serviceResults.every(s => s.status === pipeline_js_1.PipelineStatus.SUCCESS) &&
                healthCheckResults.every(h => h.status === 'healthy') ?
                pipeline_js_1.PipelineStatus.SUCCESS : pipeline_js_1.PipelineStatus.FAILED;
            return {
                id: deploymentId,
                deploymentId: deployment.id,
    ...
    async evaluateTaskConditions(task, stage, pipeline) {
    ...
    async executeShellTask(task, taskId, startTime) {
        return new Promise((resolve) => {
            const command = task.command || task.script || '';
            const logs = [];
            let exitCode = 0;
            this.logger.debug(`Executing shell command: ${command}`, { taskId });
            const shellProcess = (0, child_process_1.spawn)('sh', ['-c', command], {
            ...
            shellProcess.on('close', (code) => {
                ...
                clearTimeout(timeout);
                this.taskTimeouts.delete(taskId);
                const status = exitCode === 0 ? pipeline_js_1.PipelineStatus.SUCCESS : pipeline_js_1.PipelineStatus.FAILED;
                resolve({
                    id: taskId,
                    taskId: task.id,
                    name: task.name,
    ...
    async executeDockerTask(task, taskId, startTime) {
    ...
    async executeKubernetesTask(task, taskId, startTime) {
    ...
    async executeTestTask(task, taskId, startTime) {
    ...
    async executeBuildTask(task, taskId, startTime) {
    ...
    async executeDeployTask(task, taskId, startTime) {
    ...
    async executeCustomTask(task, taskId, startTime) {
    ...
    async retryTask(task, stage, pipeline, executionId, lastResult) {
        const retryPolicy = task.retryPolicy;
        let attempt = 1;
        let result = lastResult;
        while (attempt < retryPolicy.maxAttempts && result.status === pipeline_js_1.PipelineStatus.FAILED) {
    ...
    async deployService(service, deployment) {
    ...
    async simulateRollback(deployment) {
