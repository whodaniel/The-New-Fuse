...
class PipelineValidator {
    ...
    validateStages(stages, errors) {
        ...
        stages.forEach((stage, index) => {
            ...
            if (!stage.name) {
                errors.push(`${stagePrefix}: Stage name is required`);
            }
            ...
            if (!Object.values(pipeline_js_1.StageType).includes(stage.type)) {
                errors.push(`${stagePrefix}: Invalid stage type: ${stage.type}`);
            }
    ...
    validateTasks(tasks, errors, stagePrefix) {
        ...
        tasks.forEach((task, index) => {
            ...
            if (!task.id) {
                errors.push(`${taskPrefix}: Task ID is required`);
            }
            ...
            if (!task.command && !task.script) {
                errors.push(`${taskPrefix}: Task must have either command or script`);
            }
    ...
    validateTaskParameters(task, errors, taskPrefix) {
        switch (task.type) {
            case 'docker':
    ...
    validateTriggers(triggers, errors) {
        triggers.forEach((trigger, index) => {
            const triggerPrefix = `Trigger ${index + 1}`;
            if (!Object.values(pipeline_js_1.TriggerType).includes(trigger.type)) {
            ...
            if (trigger.filters) {
                trigger.filters.forEach((filter, filterIndex) => {
    ...
    validateQualityGates(qualityGates, errors) {
        qualityGates.forEach((gate, index) => {
            ...
            if (gate.threshold === undefined || gate.threshold === null) {
                errors.push(`${gatePrefix}: Quality gate threshold is required`);
            }
            if (!gate.operator) {
    ...
    validateNotifications(notifications, errors) {
        notifications.forEach((notification, index) => {
            const notificationPrefix = `Notification ${index + 1}`;
            if (!notification.channels || notification.channels.length === 0) {
                errors.push(`${notificationPrefix}: At least one notification channel is required`);
            }
    ...
    validateStageDependencies(stages, errors) {
    ...
    validateTimeoutAndRetry(pipeline, errors) {
    ...
    validateSecurity(pipeline, errors) {
    ...
    validateResources(pipeline, errors) {
    ...
    validateStageConditions(conditions, errors, stagePrefix) {
        conditions?.forEach((condition, index) => {
            if (!condition.type) {
                errors.push(`${stagePrefix} - Condition ${index + 1}: Condition type is required`);
            }
    ...
    validateTaskConditions(conditions, errors, taskPrefix) {
        conditions?.forEach((condition, index) => {
            if (!condition.type) {
                errors.push(`${taskPrefix} - Condition ${index + 1}: Condition type is required`);
            }
            if (!condition.operator) {
                errors.push(`${taskPrefix} - Condition ${index + 1}: Condition operator is required`);
            }
            if (condition.value === undefined || condition.value === null) {
                errors.push(`${taskPrefix} - Condition ${index + 1}: Condition value is required`);
            }
        });
    }
    validateRetryPolicy(retryPolicy, errors, prefix) {
    ...
    validateTaskArtifacts(artifacts, errors, taskPrefix) {
        artifacts?.forEach((artifact, index) => {
            if (!artifact.name) {
                errors.push(`${taskPrefix} - Artifact ${index + 1}: Artifact name is required`);
            }
            if (!artifact.path) {
                errors.push(`${taskPrefix} - Artifact ${index + 1}: Artifact path is required`);
            }
            if (!artifact.type) {
                errors.push(`${taskPrefix} - Artifact ${index + 1}: Artifact type is required`);
            }
    ...
    validateDeploymentBasics(deployment, errors) {
        if (!deployment.id) {
    ...
    validateServices(services, errors) {
        services.forEach((service, index) => {
            ...
            if (service.replicas && service.replicas <= 0) {
                errors.push(`${servicePrefix}: Service replicas must be positive`);
            }
    ...
    validateDeploymentStrategy(strategy, errors) {
        if (!strategy.type) {
            errors.push('Deployment strategy type is required');
        }
        if (!Object.values(pipeline_js_1.DeploymentStrategy).includes(strategy.type)) {
            errors.push(`Invalid deployment strategy type: ${strategy.type}`);
        }
        // Validate strategy-specific parameters
        switch (strategy.type) {
            case pipeline_js_1.DeploymentStrategy.CANARY:
                if (strategy.canaryConfig) {
                    this.validateCanaryConfig(strategy.canaryConfig, errors);
                }
                break;
            case pipeline_js_1.DeploymentStrategy.BLUE_GREEN:
    ...
    validateCanaryConfig(canaryConfig, errors) {
        ...
        canaryConfig.steps?.forEach((step, index) => {
            if (step.weight === undefined || step.weight < 0 || step.weight > 100) {
                errors.push(`Canary step ${index + 1}: Weight must be between 0 and 100`);
            }
            if (!step.duration) {
                errors.push(`Canary step ${index + 1}: Duration is required`);
            }
        });
    }
    validateBlueGreenConfig(blueGreenConfig, errors) {
    ...
    validateHealthChecks(healthChecks, errors) {
        healthChecks.forEach((healthCheck, index) => {
            const healthCheckPrefix = `Health Check ${index + 1}`;
            if (!healthCheck.type) {
            ...
            if (healthCheck.periodSeconds && healthCheck.periodSeconds <= 0) {
                errors.push(`${healthCheckPrefix}: Period seconds must be positive`);
            }
            if (healthCheck.timeoutSeconds && healthCheck.timeoutSeconds <= 0) {
                errors.push(`${healthCheckPrefix}: Timeout seconds must be positive`);
            }
    ...
    validateRollbackPolicy(rollbackPolicy, errors) {
        if (rollbackPolicy && rollbackPolicy.enabled) {
            ...
            rollbackPolicy.triggers?.forEach((trigger, index) => {
                ...
                if (trigger.threshold !== undefined && trigger.threshold < 0) {
                    errors.push(`Rollback trigger ${index + 1}: Threshold cannot be negative`);
                }
            });
        }
    }
    validateApprovals(approvals, errors) {
        approvals.forEach((approval, index) => {
            const approvalPrefix = `Approval ${index + 1}`;
    ...
    // Helper methods
    hasCircularDependencies(stages) {
    ...
    containsDangerousCommands(command) {
    ...
    requiresPrivilegeEscalation(parameters) {
