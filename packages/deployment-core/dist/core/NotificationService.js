...
class NotificationService {
     ...
     */
    async notifyPipelineStart(pipeline, executionId) {
     ...
     */
    async notifyPipelineFailed(result) {
     ...
     */
    async notifyBuildComplete(result) {
     ...
     */
    async notifyDeploymentComplete(result) {
     ...
     */
    async notifyDeploymentFailed(result) {
     ...
     */
    async notifyRollbackComplete(result) {
    ...
    async sendNotifications(notifications, message) {
        for (const notification of notifications) {
            if (!notification.enabled) {
                continue;
            }
            // Check if this event type should trigger notifications
            const shouldNotify = notification.events.some((event) => event.type === message.type && event.enabled);
            if (!shouldNotify) {
                continue;
            }
    ...
    async sendSlackNotification(channel, message) {
        ...
        await axios_1.default.post(webhookUrl, slackMessage, {
            headers: {
    ...
    async sendWebhookNotification(channel, message) {
        ...
        await axios_1.default.post(webhookUrl, message, { headers });
    ...
    evaluateNotificationConditions(conditions, message) {
