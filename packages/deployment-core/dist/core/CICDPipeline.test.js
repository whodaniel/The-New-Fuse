...
const CICDPipeline_js_1 = require("./CICDPipeline.js");
const PipelineExecutor_js_1 = require("./PipelineExecutor.js");
const PipelineValidator_js_1 = require("./PipelineValidator.js");
const PipelineStorage_js_1 = require("./PipelineStorage.js");
...
describe('CICDPipeline', () => {
    ...
    beforeEach(() => {
        ...
        mockExecutor = new PipelineExecutor_js_1.PipelineExecutor(mockLogger);
        mockValidator = new PipelineValidator_js_1.PipelineValidator(mockLogger);
        mockStorage = new PipelineStorage_js_1.PipelineStorage(mockLogger);
        mockNotificationService = new NotificationService_js_1.NotificationService(mockLogger);
        mockMetricsCollector = new MetricsCollector_js_1.MetricsCollector(mockLogger);
        // Create pipeline instance
        pipeline = new CICDPipeline_js_1.CICDPipeline(mockExecutor, mockValidator, mockStorage, mockNotificationService, mockMetricsCollector, mockLogger);
    });
    describe('triggerBuild', () => {
        it('should trigger a build successfully', async () => {
            const trigger = {
                id: 'trigger-1',
                type: pipeline_js_1.TriggerType.PUSH,
                source: {
            ...
            expect(result.status).toBe(pipeline_js_1.PipelineStatus.SUCCESS);
        }, 30000);
        it('should handle build trigger failure', async () => {
            ...
            const result = await pipeline.triggerBuild(trigger);
            expect(result).toBeDefined();
            expect(result.status).toBe(pipeline_js_1.PipelineStatus.FAILED);
    ...
    describe('executePipeline', () => {
        ...
        it('should fail pipeline execution with invalid configuration', async () => {
            ...
            await expect(pipeline.executePipeline(pipelineDefinition)).rejects.toThrow('Pipeline validation failed');
        });
    });
    describe('validatePipeline', () => {
        ...
        it('should detect validation errors in pipeline definition', async () => {
            const invalidPipelineDefinition = {
                ...
                environment: {
                    name: 'test',
                    type: pipeline_js_1.EnvironmentType.TEST,
                    variables: {},
                    secrets: [],
                    resources: {
                        cpu: '100m',
    ...
    describe('monitorPipeline', () => {
        it('should return pipeline status for running pipeline', async () => {
            ...
            expect(Object.values(pipeline_js_1.PipelineStatus)).toContain(status);
        ...
        stages: [
            {
                id: 'build-stage',
                name: 'Build',
                type: pipeline_js_1.StageType.BUILD,
                dependencies: [],
                tasks: [
                    {
                        id: 'build-task',
            ...
            },
            {
                id: 'test-stage',
                name: 'Test',
                type: pipeline_js_1.StageType.TEST,
