...
exports.BlueGreenStrategy = void 0;
const DeploymentStrategy_js_1 = require("./DeploymentStrategy.js");
 ...
 */
class BlueGreenStrategy extends DeploymentStrategy_js_1.BaseDeploymentStrategy {
    constructor(logger) {
        super(logger);
    ...
    async deploy(config) {
        ...
        try {
            ...
            const progress = this.createInitialProgress(deploymentId, 'blue_green', config.services);
            // Determine current and target environments
            const currentEnv = this.getCurrentEnvironment(config.environment);
            const targetEnv = currentEnv === 'blue' ? 'green' : 'blue';
            this.logDeploymentStep(deploymentId, `Current environment: ${currentEnv}, Target environment: ${targetEnv}`);
            // Phase 1: Preparation
            this.updateProgress(deploymentId, {
                phase: DeploymentStrategy_js_1.DeploymentPhase.PREPARING,
            ...
            this.updateProgress(deploymentId, {
                phase: DeploymentStrategy_js_1.DeploymentPhase.DEPLOYING,
            ...
            for (let i = 0; i < config.services.length; i++) {
                ...
                const serviceResult = await this.deployServiceToEnvironment(service, targetEnv, config, deploymentId);
                ...
                if (serviceResult.status === pipeline_js_1.PipelineStatus.FAILED) {
                    throw new Error(`Service deployment failed: ${service.name}`);
                }
            }
            // Phase 3: Validation in target environment
            this.updateProgress(deploymentId, {
                phase: DeploymentStrategy_js_1.DeploymentPhase.VALIDATING,
            ...
            this.updateProgress(deploymentId, {
                phase: DeploymentStrategy_js_1.DeploymentPhase.PROMOTING,
            ...
            // Phase 7: Completion
            this.updateProgress(deploymentId, {
                phase: DeploymentStrategy_js_1.DeploymentPhase.COMPLETED,
            ...
            const result = {
                ...
                status: pipeline_js_1.PipelineStatus.SUCCESS,
                startTime,
                endTime,
                duration,
                environment: config.environment,
            ...
            this.updateProgress(deploymentId, {
                phase: DeploymentStrategy_js_1.DeploymentPhase.FAILED,
    ...
    async rollback(deploymentId, reason) {
        ...
        try {
            ...
            this.updateProgress(deploymentId, {
                phase: DeploymentStrategy_js_1.DeploymentPhase.ROLLING_BACK,
    ...
    async prepareTargetEnvironment(config, targetEnv, deploymentId) {
    ...
    async deployServiceToEnvironment(service, targetEnv, config, deploymentId) {
        ...
        try {
            ...
            if (serviceProgress) {
                serviceProgress.status = DeploymentStrategy_js_1.ServiceDeploymentStatus.DEPLOYING;
            }
            ...
            // Update service progress
            if (serviceProgress) {
                serviceProgress.status = DeploymentStrategy_js_1.ServiceDeploymentStatus.READY;
            ...
            const serviceProgress = progress?.services.find(s => s.name === service.name);
            if (serviceProgress) {
                serviceProgress.status = DeploymentStrategy_js_1.ServiceDeploymentStatus.FAILED;
            }
            return {
                name: service.name,
                status: pipeline_js_1.PipelineStatus.FAILED,
    ...
    async validateTargetEnvironment(config, targetEnv, deploymentId) {
    ...
    async switchTraffic(config, fromEnv, toEnv, deploymentId) {
    ...
    async cleanupOldEnvironment(config, oldEnv, deploymentId) {
    ...
    async switchTrafficBack(fromEnv, toEnv, deploymentId) {
        ...
        await this.updateLoadBalancer({}, fromEnv, toEnv);
        await new Promise(resolve => setTimeout(resolve, 5000));
        this.logDeploymentStep(deploymentId, `Traffic rollback completed`);
    }
    async validateRollback(env, deploymentId) {
