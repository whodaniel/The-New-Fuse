...
import { DeploymentConfig, DeploymentResult, DeploymentStrategy as StrategyType, HealthCheckResult } from '../types/pipeline.js';
/**
...
export declare abstract class BaseDeploymentStrategy extends EventEmitter implements IDeploymentStrategy {
    ...
    getHealthChecks(config: DeploymentConfig): Promise<HealthCheckConfig[]>;
    getProgress(deploymentId: string): Promise<DeploymentProgress>;
    protected createInitialProgress(deploymentId: string, strategy: StrategyType, services: any[]): DeploymentProgress;
    ...
    private executeExecHealthCheck;
    protected waitForServiceReady(serviceName: string, targetReplicas: number, timeout?: number): Promise<boolean>;
    private getServiceReadyReplicas;
    protected logDeploymentStep(deploymentId: string, message: string, level?: 'info' | 'warn' | 'error'): void;
}
