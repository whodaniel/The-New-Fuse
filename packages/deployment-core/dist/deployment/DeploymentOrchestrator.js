...
const RollingUpdateStrategy_js_1 = require("./RollingUpdateStrategy.js");
const BlueGreenStrategy_js_1 = require("./BlueGreenStrategy.js");
const CanaryStrategy_js_1 = require("./CanaryStrategy.js");
var ApprovalStatus;
...
class DeploymentOrchestrator extends events_1.EventEmitter {
     ...
     */
    async requestApproval(deploymentId, approvers, reason) {
    ...
    async rollbackDeployment(deploymentId, reason) {
        ...
        const strategy = this.getStrategy(result.services[0]?.name || 'rolling_update');
        return await strategy.rollback(deploymentId, reason);
    }
    /**
    ...
    initializeStrategies() {
        this.strategies.set(pipeline_js_1.DeploymentStrategy.ROLLING_UPDATE, new RollingUpdateStrategy_js_1.RollingUpdateStrategy(this.logger));
        this.strategies.set(pipeline_js_1.DeploymentStrategy.BLUE_GREEN, new BlueGreenStrategy_js_1.BlueGreenStrategy(this.logger));
        this.strategies.set(pipeline_js_1.DeploymentStrategy.CANARY, new CanaryStrategy_js_1.CanaryStrategy(this.logger));
    }
    getStrategy(type) {
    ...
    async validateDeployment(config, execution) {
    ...
    async handleApprovals(config, execution) {
        execution.logs.push('Handling deployment approvals');
        for (const approvalConfig of config.approvals) {
            if (!approvalConfig.required)
                continue;
            const approval = await this.requestApproval(execution.id, approvalConfig.approvers, `Deployment approval required for ${config.environment}`);
    ...
    async executeDeploymentGates(config, execution) {
    ...
    async postDeploymentValidation(result, execution) {
    ...
    async validateEnvironmentReadiness(config, execution) {
    ...
    async validateResourceAvailability(config, execution) {
