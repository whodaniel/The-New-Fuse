...
class BaseDeploymentStrategy extends events_1.EventEmitter {
    ...
    async getHealthChecks(config) {
        ...
        config.healthChecks.forEach(check => {
            healthChecks.push({
                name: check.name || 'deployment-health',
                type: check.type,
                endpoint: check.path,
                port: check.port,
                path: check.path,
    ...
    createInitialProgress(deploymentId, strategy, services) {
        const progress = {
            deploymentId,
            strategy,
    ...
    updateProgress(deploymentId, updates) {
        const progress = this.deployments.get(deploymentId);
        if (progress) {
    ...
    calculateTotalSteps(strategy, serviceCount) {
        switch (strategy) {
            case pipeline_js_1.DeploymentStrategy.ROLLING_UPDATE:
    ...
    async executeExecHealthCheck(check) {
    ...
    async waitForServiceReady(serviceName, targetReplicas, timeout = 300000) {
