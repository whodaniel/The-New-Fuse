...
exports.RollingUpdateStrategy = void 0;
 ...
 */
class RollingUpdateStrategy extends DeploymentStrategy_js_1.BaseDeploymentStrategy {
    constructor(logger) {
        super(logger);
    }
    async deploy(config) {
        ...
        try {
            ...
            const progress = this.createInitialProgress(deploymentId, 'rolling_update', config.services);
            // Phase 1: Preparation
    ...
    async rollback(deploymentId, reason) {
        ...
        try {
            ...
            for (const serviceProgress of progress.services) {
                ...
                serviceProgress.status = DeploymentStrategy_js_1.ServiceDeploymentStatus.ROLLING_BACK;
    ...
    async prepareDeployment(config, deploymentId) {
    ...
    async deployServiceRolling(service, config, deploymentId) {
        const serviceName = service.name;
        try {
            // Get rolling update parameters
            const maxUnavailable = this.parsePercentage(config.strategy.parameters?.maxUnavailable || '25%');
            ...
            this.logDeploymentStep(deploymentId, `Rolling update ${serviceName}: ${totalReplicas} replicas, batch size: ${batchSize}`);
            ...
            while (updatedReplicas < totalReplicas) {
                ...
                this.logDeploymentStep(deploymentId, `Updating batch of ${currentBatchSize} replicas for ${serviceName}`);
                // Deploy new replicas
                await this.deployReplicaBatch(serviceName, service, currentBatchSize);
                // Wait for new replicas to be ready
                const isReady = await this.waitForServiceReady(serviceName, updatedReplicas + currentBatchSize, 60000 // 1 minute timeout per batch
                );
                if (!isReady) {
                ...
                this.logDeploymentStep(deploymentId, `Updated ${updatedReplicas}/${totalReplicas} replicas for ${serviceName}`);
            }
            // Final health check
    ...
    async validateClusterResources(config) {
    ...
    async deployReplicaBatch(serviceName, service, batchSize) {
    ...
    async removeOldReplicas(serviceName, count) {
    ...
    async rollbackService(serviceName, previousVersion) {
    ...
    async waitForRollbackComplete(serviceNames) {
...
exports.RollingUpdateStrategy = RollingUpdateStrategy;
//# sourceMappingURL=RollingUpdateStrategy.js.map