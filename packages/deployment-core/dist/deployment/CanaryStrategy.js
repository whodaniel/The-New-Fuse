 ...
 */
class CanaryStrategy extends DeploymentStrategy_js_1.BaseDeploymentStrategy {
    constructor(logger) {
        super(logger);
    }
    async deploy(config) {
        ...
        try {
            ...
            const progress = this.createInitialProgress(deploymentId, 'canary', config.services);
    ...
    async prepareCanaryDeployment(config, deploymentId) {
    ...
    async deployCanaryService(service, config, deploymentId) {
    ...
    async executeProgressiveRollout(config, canaryConfig, deploymentId) {
        const steps = canaryConfig.steps;
        const analysis = canaryConfig.analysis || [];
        for (let i = 0; i < steps.length; i++) {
            const step = steps[i];
            const stepNumber = i + 1;
            this.logDeploymentStep(deploymentId, `Canary step ${stepNumber}/${steps.length}: ${step.weight}% traffic for ${step.duration}`);
            // Update progress
    ...
    async cleanupCanaryDeployment(config, deploymentId) {
