...
output_dir = "/Users/<owner>/Desktop/A1-Inter-LLM-Com/The-New-Fuse/apps/casin8-games/docs/extracted_flow_images_final"
...
for i, url in enumerate(urls):
    ...
    (async function() {{
        try {{
            window.__base64_result = 'PENDING';
            const res = await fetch('{url}', {{ redirect: 'follow' }});
            const blob = await res.blob();
            const reader = new FileReader();
            reader.onloadend = () => {{ window.__base64_result = reader.result; }};
            reader.readAsDataURL(blob);
        }} catch(e) {{
            window.__base64_result = 'ERROR: ' + e.toString();
        }}
    }})();
    """
    ...
    tell application "Google Chrome"
        ...
        repeat with w in windows
            repeat with t in tabs of w
                if URL of t starts with "https://labs.google" then
                    set targetTab to t
                    exit repeat
                end if
            end repeat
        ...
        if targetTab is not missing value then
            execute targetTab javascript "{js.replace('"', '\\"')}"
            return "OK"
        else
    ...
    proc1 = subprocess.run(["osascript", "-e", applescript], capture_output=True, text=True)
    if "ERROR_NOT_FOUND" in proc1.stdout:
        print("Flow project tab not found in Chrome. Please open https://labs.google/fx/tools/flow/project/be98b53f-bd41-4afc-b9e4-469f7744045e")
        break
    ...
    for _ in range(max_wait):
        ...
        tell application "Google Chrome"
            ...
            if targetTab is not missing value then
                return execute targetTab javascript "window.__base64_result"
            else
                return ""
            end if
        end tell
        """
        proc = subprocess.run(["osascript", "-e", check_script], capture_output=True, text=True)
        out = proc.stdout.strip()
        if out and out != "PENDING" and out != "undefined" and out != "missing value":
            result = out
            break

    if result.startswith("ERROR"):
        print(f"[{i+1}/{len(urls)}] Failed: {name} -> {result}")
        ...
        print(f"[{i+1}/{len(urls)}] Saved {filepath}")
    else:
        print(f"[{i+1}/{len(urls)}] Timeout downloading {name} -> Output length: {len(result)}")
