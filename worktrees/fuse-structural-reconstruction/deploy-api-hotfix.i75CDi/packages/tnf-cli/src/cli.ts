...
async function runHermesControl(args: string[] = []): Promise<void> {
  await runCommand("python3", ["/Users/<owner>/.hermes/hermes-agent/mcp_serve.py", ...args]);
}

function parseHermesTopLevelCommands(helpText: string): Set<string> {
...
let cachedHermesTopLevelCommands: Set<string> | null = null;

function getHermesTopLevelCommands(): Set<string> {
  if (cachedHermesTopLevelCommands) {
    return cachedHermesTopLevelCommands;
  }

  try {
    const result = spawnSync("hermes", ["--no-color", "--help"], {
      encoding: "utf8",
      env: process.env,
    });
    const output = `${result.stdout || ""}\n${result.stderr || ""}`;
    cachedHermesTopLevelCommands = parseHermesTopLevelCommands(output);
  } catch {
    cachedHermesTopLevelCommands = new Set();
  }

  return cachedHermesTopLevelCommands;
}

function resolveImplicitHermesArgs(argv: string[]): string[] | null {
  const subcommand = argv[2];
  if (!subcommand || subcommand === "help") {
    const helpTarget = argv[3];
    if (!helpTarget) return null;
    if (getTnfTopLevelCommands().has(helpTarget)) return null;
    if (!getHermesTopLevelCommands().has(helpTarget)) return null;
    return [helpTarget, "--help"];
  }

  if (getTnfTopLevelCommands().has(subcommand)) return null;
  if (!getHermesTopLevelCommands().has(subcommand)) return null;
  return argv.slice(2);
}
...
program
  .command("hermes")
  .description("Pass through any Hermes CLI command")
...
reports
  ...
  if (isHermesPassthroughArgv(process.argv)) {
    await runHermesPassthrough(process.argv.slice(3));
    return;
  }
  const implicitHermesArgs = resolveImplicitHermesArgs(process.argv);
  if (implicitHermesArgs) {
    await runHermesPassthrough(implicitHermesArgs);
    return;
  }    } catch (err: any) {
      console.error(chalk.red(`Error: ${err.message}`));
      process.exit(1);
