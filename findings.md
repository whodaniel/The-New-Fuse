...
- **`$TNF_ROOT/packages/tnf-cli/src/cli.ts`**
  - **`cli.ts:124`**:
    `Total OpenClaw top-level commands: ${chalk.cyan(String(report.totalOpenClawTopLevelCommands))}`
  - Suggests **TNF CLI inherits from OpenClaw**, which has its own slash-command
    system.

- **`$TNF_ROOT/apps/external/gemini-cli-source/packages/cli/src/utils/commands.ts`**
  - **`commands.ts`**: Defines `slashCommands` array (e.g., `/help`, `/exit`,
    `/clear`).
  - **`commands.ts:34`**: `export const slashCommands: Command[] = [...]`

- **`$TNF_ROOT/apps/external/gemini-cli-source/packages/cli/src/ui/hooks/useSlashCompletion.ts`**
  - **`useSlashCompletion.ts`**: React hook that **triggers dropdown on `/`
    keypress**.
  - **`useSlashCompletion.ts:14`**:
    `const handleInputChange = (input: string) => { if (input.startsWith("/")) setShowSuggestions(true); };`

- **`$TNF_ROOT/apps/external/gemini-cli-source/packages/cli/src/ui/components/SuggestionsDisplay.tsx`**
...
1. **TNF CLI Does Not Import `useSlashCompletion`**
   - The **Gemini CLI** (`gemini-cli-source`) has the slash-command logic, but
     **TNF CLI does not load it**.
...
2. **`SuggestionsDisplay` Component Not Mounted**
   ...
   - If the parent component (`<App>`) does not render it, the dropdown is
     invisible.
...
| Pros | Cons | | Clean TNF-specific design | More work | | No external
dependencies | |

### **Decision: Option A (Import Logic)**

- **Fastest path to restore functionality.**
- **TNF already inherits from OpenClaw/Gemini**, so this minimizes new
  dependencies.
...
- **Test:**
  `  tnf cli   Type`/`→ verify dropdown appears.   Navigate menu → verify commands execute.`
