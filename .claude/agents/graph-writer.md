...
## TNF Orthogonality Contract (mandatory for authority / fleet graphs)

Every TNF authority or agent-fleet diagram MUST encode these independent axes
(see `.agent/ROLE_DEFINITIONS.md` and
`/visualizations/graphs/dacc-role-platform-axes.html`):

| Axis           | Field                                                 | Notes                                           |
| -------------- | ----------------------------------------------------- | ----------------------------------------------- |
| Baton identity | `batonHolder`, `baton_identity` / `ORCHESTRATOR-{ts}` | Only master-clock holds the baton               |
| Hierarchy seat | `daccRole`                                            | director / orchestrator / broker / worker / …   |
| Work type      | `workerAction` / capabilities                         | May include orchestration without holding baton |
| Fulfillment    | `platform`                                            | antigravity, claude, pi, master-clock, …        |

**Reject / rewrite exports** that collapse platform into a role title (e.g.
labeling a node “Antigravity Orchestrator” as if platform were a DACC seat), or
that treat `cluster: orchestration` / persona `orchestrator-agent` as the
protocol baton.
