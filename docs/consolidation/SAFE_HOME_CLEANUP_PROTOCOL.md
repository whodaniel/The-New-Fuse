...
> **No unique data loss.** Backup off-machine first. Delete nothing until verified.

## Rules

1. **Never delete** `The-New-Fuse` or `~/.tnf` without a migration plan.
2. **Backup before touch** — any path not tier `RUNTIME` gets archived to cloud storage first.
3. **Verify duplicates** — use `diff` / checksums; monorepo must be the **superset** of home copies.
4. **Sensitive files** (`auth.txt`, `voice_bridge_cloud.env`, `.env*`) — backup encrypted; never push to public git.
...
Use `scripts/consolidation/personal-runtime-cleanup.sh` for **symlinks only** (no deletes of unique data).
