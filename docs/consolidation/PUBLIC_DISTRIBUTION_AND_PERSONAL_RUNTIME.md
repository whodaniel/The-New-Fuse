...
> **Goal:** Ship a clean `fuse-open-runtime` for the public, while keeping your Mac
...
Public users do **not** need voice bridge, `~/bin` copies, or emergency `~/apps` stubs.
...
- `apps/api`, `apps/api-gateway`, `apps/frontend`, `apps/relay-server`, `apps/backend` (stub orchestrator in OSS)
- `packages/*` (with proprietary files stubbed)
- `scripts/system/` — optional operator voice tools (included in OSS; not required to run hosted app)
...
- Proprietary apps: `nexus-orchestrator`, `picoclaw-overseer`, orchestrator module, etc.

**Optional in monorepo, not core product:** `apps/audio-trigger-kws-mvp` (pilot), `casin8-games`, `poker-room` (demos).
...
1. **One dev tree** — `The-New-Fuse` (remote: `the-new-fuse-next-gen`)
2. **One voice source** — `scripts/system/` (not `~/apps`, not `voice-bridge-package`)
...
See also: [HOME_DIRECTORY_CONSOLIDATION_MANIFEST.md](./HOME_DIRECTORY_CONSOLIDATION_MANIFEST.md), [REPO_SEPARATION.md](../REPO_SEPARATION.md).
