...
7. CloudRuntime one-command rollout for API + relay services:
   - `pnpm cloud-run:federation:gate-mode -- warn`
   - `pnpm cloud-run:federation:gate-mode -- enforce`
8. CloudRuntime phased canary rollout:
   - relay only:
     `APPLY_API=0 APPLY_RELAY=1 pnpm cloud-run:federation:gate-mode -- enforce`
   - api only:
     `APPLY_API=1 APPLY_RELAY=0 pnpm cloud-run:federation:gate-mode -- enforce`
