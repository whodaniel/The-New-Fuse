...
> **Canonical dev repo**: [`whodaniel/the-new-fuse-next-gen`](https://github.com/whodaniel/the-new-fuse-next-gen)

## Four layers

| Layer | Repos | Purpose |
| ----- | ----- | ------- |
| **L1 — Live dev** | `the-new-fuse-next-gen` | Combined monorepo; all development |
...
| Remote | GitHub | Role |
| ------ | ------ | ---- |
| `the-new-fuse-next-gen` | whodaniel/the-new-fuse-next-gen | **Canonical development** |
...
| Repo | Verdict | Report |
| ---- | ------- | ------ |
| the-new-fuse-next-gen | PASS | [REPO_PARITY_the-new-fuse-next-gen.md](./REPO_PARITY_the-new-fuse-next-gen.md) |
...
\*Open-runtime contains **contract stubs** at proprietary paths (expected). Audit passes when stubs are detected, not full implementations.

## Sync cadence (Phase 1)

1. Merge feature work to `main` on `the-new-fuse-next-gen`
2. Tag release when ready: `git tag vX.Y.Z && git push origin vX.Y.Z`
3. Run `pnpm run sync:repos:dry-run` then `pnpm run sync:repos`
4. CI workflow `.github/workflows/repo-sync.yml` syncs on push to `main` when `TNF_SYNC_PAT` is set
...
See also: [TAGS_BRANCHES_EXPORT.md](./TAGS_BRANCHES_EXPORT.md), [REPO_SEPARATION.md](../REPO_SEPARATION.md).
