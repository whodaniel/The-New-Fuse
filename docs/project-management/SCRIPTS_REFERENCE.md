...
This monorepo uses **Turborepo** for task orchestration and **pnpm** for package
management. Scripts are standardized across all packages to ensure consistency
and ease of use.
...
**Apps:** May use bundlers (Vite, Webpack) **Packages:** Usually TypeScript
compiler only
...
**Apps:** May run dev servers **Packages:** Usually watch mode compilation
...
Turborepo tasks are defined in `turbo.json`. These tasks are cached and
