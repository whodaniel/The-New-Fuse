...
Before write-capable code work, establish that the working tree points at `whodaniel/tnf-monorepo`. A historical folder name is not repository identity.
...
A provider-specific check may degrade a task only when that provider or capability is actually required/enlisted.
