...
| Provider routing                | Assimilation routes + harness context resolver                       | `data/harness/harness-config.json` → `provider_routing` |
| Context + compaction            | FRONTLOAD Stages A–C + compaction records                            | `scripts/harness/compaction-record.cjs`                 |
| Sandbox / isolation             | D11 + materialized seatbelt profile                                  | `scripts/harness/materialize-sandbox-profile.cjs`       |
...
| Tools / MCP / skills            | mcp_config + supply-chain checks in completeness verify              | `verify-harness-completeness.cjs`                       |
