...
- [TNF_SYNERGISTIC_ALIGNMENT_PROTOCOL.md](file:///Users/danielgoldberg/Desktop/A1-Inter-LLM-Com/TNF/The-New-Fuse/docs/protocols/TNF_SYNERGISTIC_ALIGNMENT_PROTOCOL.md):
  ...
  `AUTHORITY_TURNUP_RUNBOOK.md`<br/>• `agent-self-edit-protocol-v0.1.md` |
  **Authority Surface Guard:** Controls permissions and edit-lock rules for core
  harness files (`AGENTS.md`). | |
  [`DACC_PROTOCOL_MASTER_MANUAL.md`](./DACC_PROTOCOL_MASTER_MANUAL.md) | •
  `DACC_POML_MCP_A2A_INTEGRATION_BLUEPRINT.md` | **Deterministic Agent
  Communication:** Governs POML (Prompt Object Markup Language) and MCP
...
| [`TNF_CLUSTER_ORCHESTRATION_PROTOCOL.md`](./TNF_CLUSTER_ORCHESTRATION_PROTOCOL.md) | • `security.guard.ts`                 | **Multi-Tenant Domain Scoping:** Scopes core dev, agency client workspaces, and personal productivity bounds (`tenantId`). |
...
| [`TNF_TRANSPORT_LANE_SPEC.md`](./TNF_TRANSPORT_LANE_SPEC.md)                                                         | • `DispatchGuard.ts`<br/>• `check-federated-ws-channels.cjs`                                                                 | **Transport Lanes:** Federated WS channels (:3007), Redis pub/sub, and sub-director worker LIST queues — three non-interchangeable delivery paths.                                                                                                                                        |
| [`TNF_PROVIDER_RESOLUTION_COHERENCE.md`](./TNF_PROVIDER_RESOLUTION_COHERENCE.md)                                     | • `data/providers/catalog.json`<br/>• `provider-config.ts`<br/>• `model_resolver.py`                                         | **Provider Resolution:** one shared catalog behind every lane; why four drifting lists left the worker fleet unable to reach backends the CLI already used.                                                                                                                               |
| [`TNF_UNBOUNDED_GROWTH_AUDIT.md`](./TNF_UNBOUNDED_GROWTH_AUDIT.md)                                                   | • `rotate-tnf-logs.sh`<br/>• `check-federated-ws-channels.cjs`                                                               | **Unbounded Growth:** arrays and logs with no ceiling — the WS bridge OOM crash-loop, 5.9 GB of unrotated logs, and the ENOSPC they caused.                                                                                                                                               |
...
| [`TNF_EXTERNAL_TEAMMATE_RUNTIME_INTEROP.md`](./TNF_EXTERNAL_TEAMMATE_RUNTIME_INTEROP.md)                             | • `bridges/tnf-to-external-teammate-runtime.yml`<br/>• `TNF_EXTERNAL_TEAMMATE_HANDOFF_DEMO.md`<br/>• Grok Bot / Cowork peers | **External Teammate Runtimes:** Treat vendor digital-coworker products as peers; assign with lineage + policy; do not conflate with fleet models (e.g. Grok 4.6).                                                                                                                         |
...
| [`tnf-cron-governance-protocol-v0.1.md`](./tnf-cron-governance-protocol-v0.1.md)                                     | • `TNF_SWARM_MASTER_SCHEDULE.md`                                                                                             | **Cron Master Governance:** Manages 35 recurring background schedules across system and tenant categories.                                                                                                                                                                                |
