...
**Status:** ACTIVE **Scope:** System-Wide Master Operating Procedures
**Location:** /The-New-Fuse/docs/protocols/

This chart maps the consolidated ecosystem of memory and information processing
in TNF, merging historical solutions with the spatial MemPalace architecture.
...
1. **Ingestion (Zero Cost):** Verbatim data hits the `MemPalace Router` and is
   vaulted in a specific "Room."
2. **Analysis (Executable Intel):** Agents extract machine-actionable artifacts
   using the `Actionability Taxonomy`.
3. **Compound Linking (Karpathy Wiki):** Artifacts are saved to the Wiki with a
   `resource_pointer` back to the MemPalace verbatim source.
4. **Context Management (Pruning):** Active context only loads the Wiki
   artifact; the raw MemPalace source is lazy-loaded only when requested.
