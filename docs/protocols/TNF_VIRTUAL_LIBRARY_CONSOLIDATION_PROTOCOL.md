...
Establish one coherent source-of-truth model for Story Architect and Virtual
Library assets across:
...
- Monorepo mirror (derived copy):
  `~/Desktop/A1-Inter-LLM-Com/The-New-Fuse/apps/virtual-library-blueprints`
- Runtime surfaces (non-authoritative for story content): `~/.kilo`,
  `~/.opencode`, `~/.gemini`

## Data Authority

Story artifacts are authoritative only when persisted through the Virtual
Library data model:
...
1. Edit and validate in canonical repo
   (`~/Projects/virtual-library-blueprints`).
2. Run mirror dry-run: `scripts/autonomy/sync_virtual_library_mirror.sh`.
3. Apply mirror sync after verification:
   `scripts/autonomy/sync_virtual_library_mirror.sh --apply`.
4. Refresh audit map:
   `python3 scripts/autonomy/virtual_library_surface_audit.py`.
5. Use generated artifacts as routing anchors:
   `docs/protocols/storage/tnf-virtual-library-surface-map.json` and
...
- Treat `.kilo` and `.opencode` as execution surfaces only; they can enrich
  workflow, but cannot overrule canonical data or privacy policy.

## Operational Outcome

This protocol removes ambiguity between duplicated code paths and runtime state
directories, while preserving:
