...
| Active state anchors           | `*-latest.json` / `*-latest.md` under every `state/` dir (`terminal-heartbeat`, `master-heartbeat`, `local-subdirector`, etc.) and `~/.tnf/agent-state/<profile>/latest.json` | Never while a heartbeat loop is enabled / while the profile is active.                                                                                              |
...
| Agent-state history snapshots            | `~/.tnf/agent-state/<profile>/history/*.json`                                          | 14 days OR 300 files (whichever is shorter); `latest.json` is a persistent anchor                 |
| Agent-state history JSONL                | `~/.tnf/agent-state/<profile>/history.jsonl`                                           | 1000 lines (head truncated, tail preserved)                                                       |
