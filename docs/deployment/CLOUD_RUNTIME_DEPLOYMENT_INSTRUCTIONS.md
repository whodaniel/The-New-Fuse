...
# Complete CloudRuntime Deployment Instructions
...
This uses the CloudRuntime dashboard to create services, then deploys via CLI.

#### 1. Create Services in CloudRuntime Dashboard
...
# Deploy API
cd apps/api
CLOUD_RUNTIME_SERVICE=api cloud_runtime up --detach
cd ../..

# Deploy Backend
cd apps/backend
CLOUD_RUNTIME_SERVICE=backend cloud_runtime up --detach
cd ../..

# Deploy API Gateway
cd apps/api-gateway
CLOUD_RUNTIME_SERVICE=api-gateway cloud_runtime up --detach
cd ../..

# Deploy Frontend
cd apps/frontend
CLOUD_RUNTIME_SERVICE=frontend cloud_runtime up --detach
cd ../..
```
...
If you want CloudRuntime to auto-detect and deploy all services at once:

1. Ensure `nixpacks.toml` exists in the root with multi-service configuration
2. Push to GitHub and connect CloudRuntime to the repository
3. CloudRuntime will automatically detect and deploy all services

### Option C: GitHub Integration (Best for CI/CD)

1. **Connect Repository to CloudRuntime**
...
After deployment, configure these variables in the CloudRuntime Dashboard:
...
NODE_ENV=production
PORT=3002
API_URL=${{api.CLOUD_RUNTIME_PRIVATE_DOMAIN}}:3001
BACKEND_URL=${{backend.CLOUD_RUNTIME_PRIVATE_DOMAIN}}:3004
```
...
3. Check CloudRuntime logs for specific errors

### Port Conflicts

**Symptom**: Port already in use errors

**Solution**: CloudRuntime automatically assigns ports. Ensure your app reads from
`process.env.PORT`
...
2. Use CloudRuntime template variables: `${{Postgres.DATABASE_URL}}`
...
cd apps/api && cloud_runtime up --detach
...
# Restart a service
cloud_runtime service restart api
```
...
1. ✅ All 4 services show "Active" in CloudRuntime dashboard
