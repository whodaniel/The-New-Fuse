...
```bash
cloud_runtime up apps/zeroclaw-sandbox --path-as-root --ci --service zeroclaw-sandbox
cloud_runtime up apps/zeroclaw-sandbox --path-as-root --ci --service picoclaw-tester-v2
```
...
3. Update CloudRuntime vars for the matching service:
...
```bash
curl -sS https://zeroclaw-sandbox-production.thenewfuse.com/health | jq .
curl -sS https://zeroclaw-sandbox-production.thenewfuse.com/api/status | jq '.channels.Telegram,.provider,.model'

curl -sS https://picoclaw-tester-v2-production.thenewfuse.com/health | jq .
curl -sS https://picoclaw-tester-v2-production.thenewfuse.com/api/status | jq '.channels.Telegram,.provider,.model'
```
