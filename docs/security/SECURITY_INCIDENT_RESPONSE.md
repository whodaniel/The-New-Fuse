...
**Status:** ACTIVE - Immediate Action Required **Severity:** CRITICAL **Date
Detected:** 2025-11-18

## Summary

GitHub secret scanning has detected **6 publicly exposed secrets** in the
repository. These keys are in the public git history and must be considered
compromised.
...
**Impact:** Payment processing will fail until updated **Risk:** Unauthorized
charges possible
...
- Secret credentials were committed to git before proper `.gitignore` was in
  place
- Files committed: `service-account-key.json`, `firebase-credentials.json`
- Keys hardcoded in source files: `Settings.tsx`, `firebase.ts`,
  `docker-compose.yml`
...
**This is a CRITICAL security incident. Prioritize key rotation above all other
work.**
