...
console.log('=== Live Integration Test with Full Message Logging ===\n');
...
// Patch the internal WebSocket to log raw messages
const originalConnect = client.connect;
client.connect = async function(url) {
  // Call original connect
  const promise = originalConnect.call(this, url);
  
  // Wait a bit then access the internal ws
  setTimeout(() => {
    // Try to access private _ws property
    const ws = this._ws || this.socket;
    if (ws) {
      const origOnMessage = ws.onmessage;
      ws.onmessage = (event) => {
        console.log('[RAW RECV]', event.data);
...
async function runTest() {
  try {
    ...
    console.log('\nRegistration successful! Testing channel join...');
    
    const channelId = await client.joinChannel('test-channel');
    console.log('Joined channel:', channelId);
    
    // Send a test message
    await client.sendChannelMessage(channelId, { test: 'hello', timestamp: Date.now() });
