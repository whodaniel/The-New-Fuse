...
1. `mini-omni-inference` (Railway service)

- Runs mini-omni server (`/chat`).
- Internal/private endpoint preferred.

2. `kws-api` (Railway service)
...
## Railway: `kws-api` Environment

Set these in Railway service variables:

- `APP_HOST=0.0.0.0`
- `MINI_OMNI_MODE=native_chat`
- `MINI_OMNI_API_URL=https://mini-omni-inference-production.up.railway.app/chat`
...
## Railway CLI Commands (Exact Flow)
...
railway add --service kws-api

# Link current directory to service:
railway service kws-api

# Deploy using app-local railway.toml + Dockerfile
railway up apps/audio-trigger-kws-mvp --service kws-api --detach --path-as-root
```

Then set service vars in Railway dashboard (or CLI variables flow), matching the
list above.

### Current live services

- `kws-api`: `https://kws-api-production.up.railway.app`
- `mini-omni-inference`: `https://mini-omni-inference-production.up.railway.app`
...
- Configure:
  - `KWS_API_ORIGIN=https://kws-api-production.up.railway.app`
  - `EDGE_API_KEY=<secret>`
