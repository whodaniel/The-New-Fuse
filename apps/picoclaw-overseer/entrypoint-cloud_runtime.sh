#!/bin/sh
# CloudRuntime entrypoint for picoclaw - writes config from env vars then starts picoclaw
...
# CloudRuntime health checks expect port $PORT (usually 8080)
...
# Start HTTP health server on CloudRuntime's PORT for health checks
mkdir -p /tmp/www
  ...
  (
    while true; do
      curl -s -X POST "${TNF_WORKER_URL}/agent/heartbeat" \
        ...
        -d "{\"agentId\":\"${AGENT_ID}\",\"status\":\"healthy\",\"currentTask\":\"standing-by\",\"lastActivity\":$(date +%s)000,\"metadata\":{\"role\":\"${AGENT_ROLE}\",\"platform\":\"picoclaw-cloud_runtime\",\"routingTarget\":\"${ROUTING_TARGET}\",\"provider\":\"${PICOCLAW_AGENTS_DEFAULTS_PROVIDER:-openai}\",\"model\":\"${PICOCLAW_AGENTS_DEFAULTS_MODEL:-z-ai/glm-5:free}\"}}" \
        > /dev/null 2>&1 || true
...
resolve_searxng_base() {
  ...
  if [ -n "${CLOUD_RUNTIME_SERVICE_SEARXNG_URL:-}" ]; then
    printf 'https://%s' "${CLOUD_RUNTIME_SERVICE_SEARXNG_URL}"
    return
  fi
  printf ''
}
