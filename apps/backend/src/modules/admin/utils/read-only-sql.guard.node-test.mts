...
} from './read-only-sql.guard.ts';

function expectAllowed(sql: string) {
  assert.doesNotThrow(() => assertReadOnlyInspectionSql(sql));
}

function expectRejected(sql: string) {
  assert.throws(() => assertReadOnlyInspectionSql(sql), ReadOnlySqlViolation);
}

describe('assertReadOnlyInspectionSql adversarial suite', () => {
  it('allows plain SELECT', () => {
    expectAllowed('SELECT 1');
    assert.equal(assertReadOnlyInspectionSql('SELECT 1'), 'SELECT 1');
  });

  it('allows SELECT with dangerous words inside strings', () => {
  ...
  it('rejects comment and multi-line smuggling', () => {
