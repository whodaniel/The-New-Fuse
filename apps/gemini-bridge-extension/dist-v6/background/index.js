/******/ (() => {
  // webpackBootstrap
  /******/ 'use strict'; // ./src/v6/services/ai-studio/youtube-service.js
...
[
  ...
  /* harmony default export */ const youtube_service = youtubeService; // ./src/v6/shared/utils.ts
  ...
  class BackgroundService {
    ...
    connectToNode(nodeType, url) {
      ...
      try {
        ...
        console.log(
          `[GeminiBridge v7] Unable to connect to ${nodeType} - relay may not be running`
        );
        this.updateNodeStatus(nodeType, url, 'disconnected');
      }
    }
    ...
    flushPendingPageAgents() {
      ...
      while (this.pendingPageAgents.length > 0) {
        ...
        if (agent) {
          ...
          this.primaryConnection.send(JSON.stringify(regMessage));
          console.log(
            `[GeminiBridge v7] Registered queued Page Agent: ${agent.name} (${agent.id})`
          );
          // Auto-join channels
          for (const channelId of this.joinedChannels) {
            const joinMessage = {
    ...
    async createYouTubePlaylist(title, description) {
      ...
      const data = await youtube_service.createPlaylist(
        title,
    ...
    async startAutomationOrchestrator(
      queue,
      nextState,
      segmentDuration = 45,
      ...
      for (let videoIndex = 0; videoIndex < queue.length; videoIndex++) {
        ...
        try {
          ...
          if (!duration && video.url) {
            ...
            if (durationTab.id) {
              const durationResult = await this.sendTaskAndWait(
                durationTab.id,
                {
                  type: 'GET_DURATION',
                  url: video.url,
        ...
        switch (message.type) {
          ...
          case 'AI_STUDIO_PROGRESS': {
            ...
            chrome.storage.local.get(['processingState'], (result) => {
              const current = result.processingState || this.getDefaultProcessingState();
              const next = {
                ...current,
                isProcessing: true,
                isPaused: false,
                currentIndex: Number(data.currentIndex || current.currentIndex || 0),
                currentVideo: data.currentVideo || current.currentVideo || null,
                lastUpdated: Date.now(),
              };
              ...
              chrome.storage.local.set(
                {
                  ...
                  ai_video_processed_count:
                    Number(result.ai_video_processed_count || 0) + increment,
                },
                () => {
                  this.broadcastToTabs({ type: 'AI_VIDEO_PROCESSING_UPDATE', state: next });
          ...
          case 'AI_VIDEO_SET_PREFERENCES': {
            ...
            const segmentDuration = Math.max(
              5,
              Math.min(300, Number(message.segmentDuration || 45))
            );
