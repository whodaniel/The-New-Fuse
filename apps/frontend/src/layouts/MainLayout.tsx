...
const MainLayout: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-primary/30">
            ...
            <div className="flex items-center gap-8">
              ...
              {isAuthenticated && (
                <div className="hidden md:flex items-center gap-6 text-[10px] uppercase tracking-[0.2em] font-black text-white/60">
                  ...
                  <Link to="/dashboard/command-center" className="hover:text-primary transition-colors">Command Center</Link>
                  <Link to="/marketplace" className="hover:text-primary transition-colors">Marketplace</Link>
                  <Link to="/resources/skills" className="hover:text-primary transition-colors">Skills</Link>
                </div>
              )}
            </div>
               ...
               {isAuthenticated ? (
                 <div className="flex items-center gap-4">
                   <div className="flex flex-col items-end hidden sm:flex">
                     <span className="text-[10px] font-black uppercase tracking-wider text-primary">{user?.username || 'Pilot'}</span>
                     <span className="text-[8px] text-white/40 uppercase tracking-[0.2em]">Rank: Super Director</span>
                   </div>
                   <button 
                     onClick={logout}
      ...
      <footer className="w-full border-t border-white/5 bg-black/30 py-6">
        <div className="max-w-7xl mx-auto px-8 flex justify-between items-center">
          ...
          <div className="flex gap-4 items-center">
             ...
             <span className="font-mono text-[8px] text-primary uppercase tracking-[0.2em]">Node US-WEST-1: ONLINE</span>
          </div>
        </div>
      </footer>
    </div>
