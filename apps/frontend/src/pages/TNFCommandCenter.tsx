  ...
  return (
    <GlassCard className="p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <PremiumButton variant="ghost" size="sm" onClick={onRefresh} disabled={loading}>
...
const AgentActivityPanel: React.FC<{ activities: AgentActivity[] }> = ({ activities }) => {
        ...
        {loading ? (
          ...
          tasks.slice(0, 10).map((task) => (
            ...
            >
              ...
              {task.assignee && (
                <div className="text-xs text-gray-400 mt-1">Assigned: {task.assignee}</div>
              )}
            </div>
          ))
        )}
...
export const TNFCommandCenter: React.FC = () => {
  ...
  const fetchMeshHealth = useCallback(async () => {
    ...
    const totalSessions = onlineInstances.reduce((sum, i) => sum + (i.activeSessions || 0), 0);
    const avgLatency = onlineInstances.length
      ? Math.round(
          onlineInstances.reduce((sum, i) => sum + (i.latency || 0), 0) / onlineInstances.length
        )
      : 0;
