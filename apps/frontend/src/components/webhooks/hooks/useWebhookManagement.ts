...
export function useWebhookManagement() {
  ...
  const getAuthHeaders = useCallback(async () => buildAuthHeaders(), []);
  ...
  const registerWebhook = useCallback(
    async (request: WebhookRegistrationRequest): Promise<WebhookRegistrationResponse> => {
      setState((prev) => ({ ...prev, loading: true, error: null }));

      try {
        const response = await authFetch(`${apiBaseUrl}/webhooks/register`, {
          method: 'POST',
  ...
  const getWebhookStatus = useCallback(
    async (webhookId: string): Promise<WebhookStatusResponse> => {
      try {
        const response = await authFetch(`${apiBaseUrl}/webhooks/status/${webhookId}`, {
          headers: await getAuthHeaders(),
        });
  ...
  const retryFailedEvent = useCallback(
    async (eventId: string): Promise<void> => {
      ...
      try {
        const response = await authFetch(`${apiBaseUrl}/webhooks/events/${eventId}/retry`, {
          method: 'POST',
          headers: await getAuthHeaders(),
        });
  ...
  const loadConfigurations = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const response = await authFetch(`${apiBaseUrl}/webhooks/configurations`, {
        headers: await getAuthHeaders(),
      });
  ...
  const deleteWebhook = useCallback(
    async (webhookId: string): Promise<void> => {
      ...
      try {
        const response = await authFetch(`${apiBaseUrl}/webhooks/${webhookId}`, {
          method: 'DELETE',
          headers: await getAuthHeaders(),
        });
  ...
  const updateWebhook = useCallback(
    async (webhookId: string, updates: Partial<WebhookConfiguration>): Promise<void> => {
      setState((prev) => ({ ...prev, loading: true, error: null }));
  ...
  const testWebhook = useCallback(
    async (
      ...
      try {
        const response = await authFetch(`${apiBaseUrl}/webhooks/${webhookId}/test`, {
          method: 'POST',
          headers: await getAuthHeaders(),
  ...
  const getIntegrationStats = useCallback(async () => {
    try {
      const response = await authFetch(`${apiBaseUrl}/webhooks/stats`, {
        headers: await getAuthHeaders(),
      });
  ...
  const getConfigurationsBySource = useCallback(
  ...
  const getRecentDeliveryLogs = useCallback(
    (limit: number = 50) => {
