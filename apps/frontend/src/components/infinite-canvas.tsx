...
export function InfiniteCanvas() {
  ...
  const handleDragNode = (id, newPosition) => {
    setNodes(
      nodes.map((node: any) =>
        node.id === id ? Object.assign(Object.assign({}, node), { position: newPosition }) : node
      )
    );
  };
  ...
  const handlePan = react_1.useCallback(
    (event, info) => {
      setPosition((pos) => ({
  ...
  return (
    <div className="w-full h-full overflow-hidden relative">
      ...
      >
        {nodes.map((node: any) => (
          <pipeline_node_1.PipelineNode
            key={node.id}
            node={node}
            onDrag={(newPosition) => handleDragNode(node.id, newPosition)}
            onConnect={handleConnect}
          />
        ))}
        {connections.map((connection, index) => (
          <node_connection_1.NodeConnection key={index} from={connection.from} to={connection.to} />
        ))}
      </framer_motion_1.motion.div>
      <div className="absolute top-4 left-4 space-x-2">
        <button_1.Button onClick={() => handleZoom(1)} aria-label="Zoom in">
          <lucide_react_1.Plus className="h-4 w-4" />
        </button_1.Button>
        <button_1.Button onClick={() => handleZoom(-1)} aria-label="Zoom out">
          <lucide_react_1.Minus className="h-4 w-4" />
        </button_1.Button>
      </div>
      <div className="absolute bottom-4 left-4 space-x-2">
        <button_1.Button onClick={() => handleAddNode('huggingface')}>
          Add HuggingFace Node
        </button_1.Button>
        <button_1.Button onClick={() => handleAddNode('langchain')}>
          Add LangChain Node
        </button_1.Button>
