...
import { createDynamicValidator, validateN8nWorkflow } from './utils/validation';
const nodeTypes = {
...
export function WorkflowEditor() {
  ...
  const handleSaveWorkflow = useCallback(async () => {
    ...
    Object.keys(connectionsWithErrorOutputs).forEach((sourceNodeId) => {
      if (connectionsWithErrorOutputs[sourceNodeId].error) {
        ...
        n8nWorkflowJson.connections[sourceNodeId].error =
          connectionsWithErrorOutputs[sourceNodeId].error;
      }
    });
    ...
    try {
      const response = await authFetch('/api/n8n/workflow', {
        ...
        body: JSON.stringify(
          Object.assign(Object.assign({}, n8nWorkflowJson), { name: workflowName })
        ),
      });
      if (!response.ok) {
  ...
  const sortedNodeTypes = Object.entries(nodeTypesMetadata).sort(([, a], [, b]) =>
