...
export default function ChatContainer({
  ...
  const llmDialogTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  // Cleanup timeout on unmount
  useEffect(() => {
    return () => {
      if (llmDialogTimeoutRef.current) {
        clearTimeout(llmDialogTimeoutRef.current);
      }
    };
  }, []);

  // Handler for LLM provider selection with proper timeout cleanup
  const handleLLMProviderChange = useCallback((providerId: string) => {
    ...
    llmDialogTimeoutRef.current = setTimeout(() => setIsLLMDialogOpen(false), 500);
  }, []);
  ...
  useEffect(() => {
    ...
    function handleWSS() {
      try {
        ...
        // Define abort handler with reference to socket
        ...
        abortStreamHandler = handleAbortStream;
    ...
    // Cleanup function - removes event listeners and closes socket
    return () => {
