...
const WorkflowBuilderContent: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const applyGraph = (graph: {
      ...
      if (cancelled || !graph.nodes?.length) return;
      if (graph.name) setWorkflowName(graph.name);
      setNodes(attachHandlers(graph.nodes));
    ...
    void (async () => {
      try {
        // Hash router: query lives in the hash (`#/workflows?id=…`), not location.search.
        ...
        if (source === 'local-ai' && id) {
          ...
          if (res.ok) {
            ...
            applyGraph(
              { ...doc, savedAt: doc.updatedAt },
              `Loaded AI workflow ${id} into builder (${doc.nodes?.length || 0} nodes).`
            );
            return;
          }
        }
      ...
      try {
        ...
        applyGraph(
          draft,
          draft.savedAt
            ? `Restored local draft from ${new Date(draft.savedAt).toLocaleString()}.`
  ...
  }, [attachHandlers, fitView, setEdges, setNodes]);
