...
export class MarketplaceService implements OnModuleInit, OnModuleDestroy {
  ...
  async triggerResearchCrawl(input?: { command?: string; dryRun?: boolean }): Promise<{
    ...
    if (input?.command) {
      throw new BadRequestException('Dynamically specifying shell commands is disabled for security reasons.');
    }

    const runId = `crawl-${Date.now()}-${randomUUID().slice(0, 8)}`;
    const command = this.sanitizeText(process.env.CRAWL4AI_PIPELINE_COMMAND, MAX_DESCRIPTION_LENGTH);
