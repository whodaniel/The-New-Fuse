...
export type UnifiedRecordStatus = 'submitted' | 'queued' | 'in_progress' | 'under_review' | 'completed' | 'failed' | 'rejected' | 'archived';
...
export type UnifiedWorkLane = 'directive' | 'goal' | 'milestone' | 'realtime_broker_routing' | 'relay_federation' | 'tauri_sync' | 'redis_sync' | 'suggestion_vote' | 'changelog_suggestion' | 'kanban_delivery';
...
export interface FunctionalLink {
    targetId: string;
    linkType: 'depends_on' | 'blocks' | 'supports' | 'reviews' | 'derived_from' | 'related_to' | 'feedback_loop';
    weight: number;
    note?: string;
    createdAt: string;
}
...
export interface TimelineEvent {
    ...
    goalId?: string;
    planId?: string;
    eventType: 'record_created' | 'record_updated' | 'record_voted' | 'feedback_iteration_added' | 'functional_link_added' | 'goal_created' | 'goal_linked' | 'plan_created' | 'plan_linked' | 'milestone_updated' | 'historical_event';
    actor: string;
    timestamp: string;
...
//# sourceMappingURL=unified-ledger.types.d.ts.map