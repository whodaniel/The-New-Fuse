...
let ChatService = ChatService_1 = class ChatService {
    ...
    async create(userId, agentId, title) {
        try {
            ...
            }); // Type cast if repository input type is strict about optional fields
     ...
     */
    async addMessage(chatId, content, role, options) {
        try {
            // Create message
     ...
     */
    async generateAgentResponse(_prompt, agentId, userId) {
        try {
            ...
            throw new common_1.ServiceUnavailableException('Agent response generation is not configured on this endpoint. Configure provider keys and use adaptive provider routing via /api/agent-proxy/adaptive/:target.');
        }
        catch (error) {
            ...
            throw new common_1.ServiceUnavailableException('Failed to generate agent response. No production LLM backend is configured for this chat endpoint.');
        }
    }
    /**
