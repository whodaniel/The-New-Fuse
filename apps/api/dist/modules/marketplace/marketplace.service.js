...
let MarketplaceService = class MarketplaceService {
    constructor() {
        ...
        this.dbEnabled = true;
        this.dbClient = null;
        this.activeResearchRuns = new Set();
        this.researchRunStartedAt = new Map();
        this.seedItems = [
            {
                id: 'prim-native-vision-simd',
                slug: 'native-vision-simd',
                name: 'SIMD Vision Module',
                description: 'High-fidelity Tier-1 native vision capture/encode (MSS + TurboJPEG), achieving 34 FPS mirroring.',
                kind: 'skill',
                category: 'sensory',
            ...
            },
            {
                ...
                slug: 'jit-forge-llvm',
                name: 'LLVM JIT Forge',
                description: 'Hot-swappable native machine code (C/C++/Rust) compiler for 100x agent speedups.',
                kind: 'workflow',
                category: 'forge',
            ...
            },
            {
                ...
                description: 'High-performance Rust-based broadcast loops achieving ~49,000 msg/s throughput.',
                kind: 'mcp_server',
                category: 'nexus',
            ...
            },
            {
                ...
                description: 'Zero-trust gatekeeper for native code generation with heuristic syscall detection.',
                kind: 'skill',
                category: 'security',
            ...
            },
            {
                ...
                description: 'High-fidelity backbone model for complex hardware-intimate agent logic and native forge control.',
                kind: 'model',
            ...
            },
            {
                ...
                description: 'SIMD-optimized vector similarity using C++ AVX2 achieving ~610 Million dims/sec.',
                kind: 'skill',
                category: 'analytics',
                tags: ['cpp', 'avx2', 'simd'],
            ...
            },
            {
                ...
                description: 'Sub-millisecond acoustic signature detection module forged for the native kernel.',
                kind: 'skill',
                category: 'sensory',
            ...
            },
            {
                ...
                description: 'Secure isolation primitive for forged binaries using native macOS sandbox-exec rules.',
                kind: 'workflow',
                category: 'security',
    ...
    async getCatalog(query) {
        ...
        const filtered = items.filter((item) => {
            if (kind && item.kind !== kind)
                return false;
            if (status && item.publicationStatus !== status)
                return false;
    ...
    async searchResearchPrompts(input) {
        const empty = { items: [], total: 0, available: false };
    ...
    async getResearchSources(input) {
        const empty = { available: false, categories: [] };
    ...
    async getResearchSkillSources(input) {
        const empty = { available: false, categories: [] };
        const limitPerCategory = Math.max(1, Math.min(Number(input?.limitPerCategory) || 8, 20));
        await this.ensureInitialized();
        if (!this.dbEnabled || !this.dbClient) {
    ...
    async getResearchMcpSources(input) {
        const empty = { available: false, categories: [] };
    ...
    async searchResearchMcpServers(input) {
        const empty = {
            items: [],
            total: 0,
            available: false,
    ...
    async triggerResearchCrawl(input) {
        await this.ensureInitialized();
        if (!this.dbEnabled || !this.dbClient) {
            throw new common_1.BadRequestException('Marketplace DB unavailable');
        }
        const runId = `crawl-${Date.now()}-${(0, node_crypto_1.randomUUID)().slice(0, 8)}`;
        const command = this.sanitizeText(input?.command, MAX_DESCRIPTION_LENGTH) ||
        ...
        const child = (0, node_child_process_1.spawn)('sh', ['-lc', command], { stdio: ['ignore', 'pipe', 'pipe'] });
        let stdoutBuffer = '';
        let stderrBuffer = '';
        ...
        child.on('close', async (code) => {
            ...
            const error = code === 0 ? null : this.sanitizeText(stderrBuffer || `exit code ${code}`, 8000);
    ...
    async listResearchCrawlRuns(limit = 20) {
        await this.ensureInitialized();
        if (!this.dbEnabled || !this.dbClient) {
    ...
    async getItemById(id) {
        await this.ensureInitialized();
        const row = await database_1.drizzleMarketplaceCatalogRepository.findByIdOrSlug(id);
    ...
    async submitCatalogItem(input) {
        ...
        if (!normalizedKind) {
            throw new common_1.BadRequestException('kind is required and must be a valid catalog kind');
        }
    ...
    async transitionPublicationStatus(input) {
        ...
        if (!this.isTransitionAllowed(item.publicationStatus, input.toStatus)) {
            throw new Error(`Invalid publication transition: ${item.publicationStatus} -> ${input.toStatus}`);
        }
        const updated = {
            ...item,
    ...
    async ensureInitialized() {
        ...
        try {
            ...
            this.dbClient = (0, postgres_1.default)(connectionString, {
            ...
            const rowCount = await database_1.drizzleMarketplaceCatalogRepository.count();
            if (rowCount === 0) {
                for (const item of this.seedItems) {
                    await this.persistItem(item, false);
                }
            ...
            const message = error instanceof Error ? error.message : String(error);
            console.error(`MarketplaceService: failed to initialize database-backed catalog. (${message})`);
    ...
    async getAllItems() {
        await this.ensureInitialized();
        const rows = await database_1.drizzleMarketplaceCatalogRepository.findAll();
        return rows.map((row) => this.mapCatalogRowToItem(row));
    }
    async persistItem(item, upsert) {
        await this.ensureInitialized();
        try {
            if (upsert) {
                await database_1.drizzleMarketplaceCatalogRepository.upsert({
                    id: item.id,
                    slug: item.slug,
                    name: item.name,
            ...
            await database_1.drizzleMarketplaceCatalogRepository.insertIfMissing({
                id: item.id,
                slug: item.slug,
                name: item.name,
                description: item.description,
            ...
            if (code === '23505' ||
                ...
                throw new common_1.BadRequestException('A catalog item with a similar name already exists');
            }
            throw error;
        }
    }
    ...
    mapCatalogRowToItem(row) {
        ...
        return {
            ...
            publicationStatus: this.normalizeStatus(String(row.publicationStatus ?? row.publication_status)) || 'draft',
            launchUrl: row.launchUrl || row.launch_url ? String(row.launchUrl ?? row.launch_url) : undefined,
            avatarUrl: row.avatarUrl || row.avatar_url ? String(row.avatarUrl ?? row.avatar_url) : undefined,
            createdBy: row.createdBy || row.created_by ? String(row.createdBy ?? row.created_by) : undefined,
    ...
    validateSubmissionInput(input) {
        ...
        const createdBy = this.sanitizeText(input.createdBy || 'community', MAX_TEXT_LENGTH).toLowerCase();
        if (!name) {
    ...
    normalizeUrl(raw) {
        ...
        try {
            ...
            if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
                throw new common_1.BadRequestException('launchUrl must use http or https');
            }
            ...
            throw new common_1.BadRequestException('launchUrl must be a valid URL');
        }
    }
    normalizeUniqueStrings(values, limit) {
    ...
    generateUniqueSlug(baseName, existingItems) {
        const baseSlug = this.sanitizeText(baseName, MAX_TEXT_LENGTH)
            .toLowerCase()
            .replace(/[^a-z0-9]+/g, '-')
            .replace(/(^-|-$)/g, '') || 'item';
