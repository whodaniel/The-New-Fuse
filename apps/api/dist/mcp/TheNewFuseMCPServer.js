...
const jules_skill_1 = require("@the-new-fuse/jules-skill");
/**
...
class TheNewFuseMCPServer {
    constructor(isRemote = false) {
        ...
        this.julesClient = new jules_skill_1.JulesClient();
        ...
        this.server.setRequestHandler(types_js_1.ListToolsRequestSchema, async () => ({
            tools: [
                ...
                // Agent Delegate / LLM Proxy
                {
                    ...
                    description: 'Securely proxy an LLM request using a delegated grant token. Supports fixed provider mode or adaptive routing by target (centralized active/fallback policy).',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            provider: {
                                type: 'string',
                                description: 'LLM Provider for fixed mode (e.g. openai, anthropic). Optional if target is provided.',
                            },
                            target: {
                                type: 'string',
                                description: 'Adaptive routing target (e.g. zeroclaw-sandbox). If set, centralized routing policy is used.',
                            },
                ...
                // Agent Knowledge Banks (Definitions & Skills)
                {
                    ...
                    description: 'Access the core bank of agent definitions (.agent/agents, .claude/agents) and skills (.agent/skills). This allows discovery of persona templates and capabilities.',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            action: {
                ...
                */
                // Jules Coding Agent Tools (from @the-new-fuse/jules-skill)
                {
                    ...
                    inputSchema: {
                        ...
                        properties: {
                            task: {
                                type: 'string',
                                description: 'The task description/instructions for Jules. Be specific about what you want done.',
                            },
                            repository: {
                                type: 'string',
                                description: 'Optional: Repository in format "owner/repo". Defaults to current directory.',
                            },
                            workspace_context: {
                                type: 'string',
                ...
                },
                {
                    ...
                    description: 'List all Jules sessions. Returns information about active and completed coding sessions.',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            limit: {
                ...
                },
                {
                    ...
                    description: 'Pull the results of a completed Jules session. Optionally apply the patch to the local repository.',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            session_id: {
    ...
    async handleListChatRooms(args) {
    ...
    async handleCreateChatRoom(args) {
        if (this.services.chat) {
    ...
    async handleListAutomationTemplates(args) {
    ...
    async handleCreateAutomationFromTemplate(args) {
        if (this.services.claudeDev) {
    ...
    async handleGetPlatformStats(args) {
    ...
    async handleGetSystemHealth(_args) {
        if (this.services.monitoring) {
    ...
    async handleDelegateLlmRequest(args) {
        if (this.services.agentGrants) {
            const { provider, target, grantToken, payload } = args;
            try {
                let result;
                if (typeof target === 'string' && target.trim()) {
                    result = await this.services.agentGrants.adaptiveProxy(target.trim(), grantToken, payload);
                }
                ...
                throw new types_js_1.McpError(types_js_1.ErrorCode.InternalError, `LLM Delegation Failed: ${error.message || String(error)}`);
            }
        }
        throw new types_js_1.McpError(types_js_1.ErrorCode.InternalError, 'Agent Grants Service not initialized');
    }
    // Agent Knowledge Bank Handlers
    async handleGetAgentBankResources(args) {
        const { action, resourceType, bank = 'all', filePath } = args;
        const root = this.getWorkspaceRoot();
        const paths = [];
        if (resourceType === 'agents') {
            if (bank === 'tnf' || bank === 'all')
                paths.push(path.join(root, '.agent', 'agents'));
            if (bank === 'claude' || bank === 'all')
                paths.push(path.join(root, '.claude', 'agents'));
        }
        else if (resourceType === 'skills') {
            if (bank === 'tnf' || bank === 'all')
                paths.push(path.join(root, '.agent', 'skills'));
        ...
        if (action === 'list') {
            ...
            if (!filePath) {
                throw new types_js_1.McpError(types_js_1.ErrorCode.InvalidParams, 'filePath is required for read action');
            }
        ...
        throw new types_js_1.McpError(types_js_1.ErrorCode.MethodNotFound, `Action ${action} not supported`);
    }
    ...
    async handleGetPrompt(name, args) {
        switch (name) {
            case 'analyze_agent': {
                ...
                let agentStatus = { id: agentId, status: 'unknown' };
                if (this.services.agent) {
                    try {
