...
function validateEnvironment() {
    ...
    if (process.env.REDIS_URL && (process.env.REDIS_HOST || process.env.REDIS_PORT)) {
        warnings.push('⚠️  Both REDIS_URL and REDIS_HOST/PORT are set. REDIS_URL will take precedence.');
    }
    ...
    const hasCompleteWeb3Auth = process.env.WEB3AUTH_CLIENT_ID &&
        process.env.WEB3AUTH_JWT_SECRET &&
        process.env.ETHEREUM_RPC_URL;
    if (hasAnyWeb3Auth && !hasCompleteWeb3Auth) {
        warnings.push('⚠️  Incomplete Web3Auth configuration. All of WEB3AUTH_CLIENT_ID, WEB3AUTH_JWT_SECRET, and ETHEREUM_RPC_URL are required.');
    }
    return {
