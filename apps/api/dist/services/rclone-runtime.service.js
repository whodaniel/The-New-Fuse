...
var RcloneRuntimeService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.RcloneRuntimeService = void 0;
...
let RcloneRuntimeService = RcloneRuntimeService_1 = class RcloneRuntimeService {
    constructor(auditService, unifiedLedgerService) {
        this.auditService = auditService;
        this.unifiedLedgerService = unifiedLedgerService;
        this.logger = new common_1.Logger(RcloneRuntimeService_1.name);
        this.repoRoot = this.resolveRepoRoot();
        this.doctorScriptPath = path.join(this.repoRoot, 'scripts', 'tnf-rclone-doctor.cjs');
        this.workflowRunHistoryLimit = 120;
        this.maxOutputPreviewChars = 6000;
        this.defaultWorkflowTimeoutMs = 180000;
        this.workflowSnapshotAction = 'rclone.runtime.workflow.snapshot';
        this.ardriveConnectorEventAction = 'rclone.runtime.ardrive.connector.event';
        this.ardriveQueueMaxItems = 200;
        this.ardrivePreflightTtlMs = 30 * 60 * 1000;
        this.ardriveQuoteTtlMs = 15 * 60 * 1000;
        this.ardrivePaymentApiBase = String(process.env.TNF_ARDRIVE_TURBO_PAYMENT_API || 'https://payment.ardrive.io/v1').trim() ||
            'https://payment.ardrive.io/v1';
        this.ardrivePaymentTimeoutMs = this.clamp(Number(process.env.TNF_ARDRIVE_TURBO_PAYMENT_TIMEOUT_MS || 8000), 1000, 30000);
        this.ardriveWorkerEnabled = this.toBooleanEnv(process.env.TNF_ARDRIVE_CONNECTOR_WORKER_ENABLED, false);
        this.ardriveWorkerIntervalMs = this.clamp(Number(process.env.TNF_ARDRIVE_CONNECTOR_WORKER_INTERVAL_MS || 10000), 1000, 300000);
        this.ardriveWorkerBatchSize = this.clamp(Number(process.env.TNF_ARDRIVE_CONNECTOR_WORKER_BATCH_SIZE || 5), 1, 50);
        this.ardriveQueue = [];
        this.ardrivePreflights = new Map();
        this.ardriveWorkerRunning = false;
        this.ardriveWorkerLastTickAt = null;
        this.ardriveWorkerLastSummary = null;
        ...
        this.providerProfiles = [
            ...
            },
            {
                ...
                notes: 'No guaranteed native rclone backend path in this TNF runtime; recommended approach is a bridge connector.',
            ...
            },
            {
                ...
                notes: 'Blockchain storage requires content-addressed and transaction-aware flows beyond standard sync semantics.',
                docs: ['https://docs.ardrive.io/docs/arfs', 'https://upload.ardrive.io/api-docs'],
            },
        ];
        this.providerBlueprints = {
            pcloud: {
                id: 'pcloud',
                summary: 'Native rclone backend. Recommended default for TNF managed storage offload and migration workflows.',
                techStack: [
            ...
            degoo: {
                id: 'degoo',
                summary: 'No first-class rclone backend path. Use a compliance-safe bridge workflow instead of direct protocol emulation.',
                techStack: [
            ...
            ardrive: {
                id: 'ardrive',
                summary: 'ArDrive requires transaction-aware Arweave upload flows. Integrate via Turbo APIs/SDK, not vanilla rclone sync semantics.',
                techStack: [
        ...
        this.allowedSingleExtraFlags = new Set([
            '--fast-list',
            '--check-first',
            '--size-only',
            '--update',
        ...
        this.allowedValuedExtraFlags = new Set([
            '--exclude',
            '--include',
            '--max-age',
            '--min-age',
    ...
    async runArdriveTurboWorkerTick(options = {}) {
    ...
    async runArdriveTurboWorkerProcessOne(options = {}) {
        if (this.ardriveWorkerRunning) {
            return {
                ok: true,
                connector: 'ardrive_turbo_connector',
    ...
    getProviderBlueprint(providerId) {
    ...
    async preflightArdriveTurboUpload(input) {
        ...
        return {
            ...
            pricing: {
                ...
                quoteEndpoint: record.quoteEndpoint,
                mode: (record.quoteSource === 'live' ? 'live_quote' : 'estimated_fallback'),
            },
            queuePolicy: {
    ...
    enqueueArdriveTurboUpload(input) {
        ...
        if (!preflightId)
            throw new Error('preflightId is required');
        this.pruneArdrivePreflights();
    ...
    async transitionArdriveTurboQueueItem(input) {
        const actorId = String(input.actorId || 'admin').trim() || 'admin';
    ...
    async getWorkflowRunLogs(options = {}) {
    ...
    async runWorkflow(input) {
        ...
        const timeoutMs = this.clamp(Number(input.timeoutMs ?? this.defaultWorkflowTimeoutMs), 30000, 1200000);
        ...
        const child = (0, node_child_process_1.spawn)('rclone', args, {
            cwd: this.repoRoot,
            env: process.env,
        });
        const activeRun = {
    ...
    async pauseWorkflow(runId, actorId) {
        const active = this.getActiveRunOrThrow(runId);
        if (active.paused || active.entry.status === 'paused')
            return active.entry;
    ...
    async resumeWorkflow(runId, actorId) {
        const active = this.getActiveRunOrThrow(runId);
        if (!active.paused && active.entry.status !== 'paused')
            return active.entry;
        if (active.entry.status === 'stopping') {
    ...
    async stopWorkflow(runId, actorId) {
        const active = this.getActiveRunOrThrow(runId);
        if (active.stopRequested || active.entry.status === 'stopping')
            return active.entry;
    ...
    scheduleForceKill(runId, delayMs) {
    ...
    cleanupActiveRunTimers(active) {
        if (active.timeoutHandle)
            clearTimeout(active.timeoutHandle);
        if (active.stopEscalationHandle)
            clearTimeout(active.stopEscalationHandle);
    }
    resolveTerminalStatus(active, code, signal) {
    ...
    resolveTerminalExitCode(active, code, signal) {
        if (typeof code === 'number')
            return code;
        if (signal)
            return this.signalToExitCode(signal);
        if (active.timeoutTriggered)
            return 124;
        return 1;
    }
    ...
    finalizeRun(entry, status, exitCode) {
    ...
    async persistRunSnapshot(entry, event, actorOverride) {
        ...
        try {
            ...
            this.logger.warn(`Failed to write rclone audit snapshot: ${error.message}`);
        }
        try {
            ...
            this.logger.warn(`Failed to write rclone timeline snapshot: ${error.message}`);
        }
        entry.persistent = true;
        this.addOrUpdateInHistory(entry);
    }
    asPersistedSnapshot(entry) {
        return {
            ...entry,
            command: {
                ...entry.command,
    ...
    async getPersistentWorkflowRunLogs(limit) {
        try {
            ...
            this.logger.warn(`Failed to read persistent rclone logs: ${error.message}`);
            return [];
        }
    }
    parsePersistentRows(rows) {
        ...
        return Array.from(byRunId.values()).sort((a, b) => this.toEpochMs(b.updatedAt) - this.toEpochMs(a.updatedAt));
    }
    normalizePersistedRun(run, row) {
        ...
        return {
            ...
            finishedAt: run.finishedAt == null ? null : this.toIsoTimestamp(run.finishedAt, fallbackCreatedAt),
            durationMs: run.durationMs == null ? null : Number(run.durationMs),
            exitCode: run.exitCode == null ? null : Number(run.exitCode),
            command: {
                binary: 'rclone',
                args: Array.isArray(run.command?.args)
                    ? run.command.args.map((value) => String(value || ''))
                    : [],
                display: String(run.command?.display || 'rclone'),
            },
    ...
    mergeLogs(runtimeLogs, persistentLogs) {
        const map = new Map();
        ...
        return Array.from(map.values()).sort((a, b) => this.toEpochMs(b.updatedAt) - this.toEpochMs(a.updatedAt));
    }
    async runDoctorScript(args) {
        try {
    ...
    buildWorkflowCommandArgs(presetId, source, destination, input) {
        const args = this.baseWorkflowCommandArgs(presetId, source, destination);
        if (input.dryRun)
            args.push('--dry-run');
        if (input.checksum && !args.includes('--checksum'))
            args.push('--checksum');
    ...
    baseWorkflowCommandArgs(presetId, source, destination) {
        if (presetId === 'sync') {
    ...
    normalizeProviderId(value) {
        const normalized = String(value || '')
            .trim()
            .toLowerCase();
        if (normalized === 'pcloud')
            return 'pcloud';
        if (normalized === 'degoo')
            return 'degoo';
        if (normalized === 'ardrive')
            return 'ardrive';
        return null;
    }
    isArdriveWorkerProcessable(status) {
        return status === 'queued' || status === 'processing' || status === 'submitted';
    }
    async processArdriveWorkerCandidate(item, actorId, trigger) {
        try {
            ...
            if (this.isArdriveWorkerProcessable(item.status)) {
                try {
                    ...
                    return {
                        ...
                        transitioned,
                        failed: 1,
                        error: `${item.queueId}: ${message} | mark-failed error: ${markError.message || 'unknown'}`,
                    };
                }
            }
            return {
    ...
    startArdriveWorkerLoop() {
        if (this.ardriveWorkerTimer)
            return;
        ...
        this.logger.log(`ArDrive connector worker started (interval=${this.ardriveWorkerIntervalMs}ms, batch=${this.ardriveWorkerBatchSize})`);
    }
    stopArdriveWorkerLoop() {
        if (!this.ardriveWorkerTimer)
            return;
    ...
    async ensureLiveQuoteForArdriveQueueItem(item, actorId) {
        ...
        if (item.quoteSource !== 'live' || !item.quoteId || !item.quotedWinc) {
            throw new Error('Cannot transition to submitted without a live Turbo payment quote. Retry when payment API is reachable.');
        }
    }
    isArdriveQuoteExpired(quoteAt) {
        if (!quoteAt)
            return true;
    ...
    async fetchArdriveTurboQuote(fileSizeBytes) {
        ...
        try {
            ...
            return {
                ...
                quoteEndpoint: endpoint,
                warnings: [
                    `Live Turbo quote unavailable: ${error.message || 'unknown error'}`,
    ...
    normalizeArdriveQueueStatus(value) {
        const normalized = String(value || '')
            .trim()
            .toLowerCase();
    ...
    canTransitionArdriveQueue(current, next) {
        if (current === next)
            return true;
        if (current === 'queued')
            return next === 'processing' || next === 'failed';
        if (current === 'processing')
            return next === 'submitted' || next === 'failed';
    ...
    estimateTurboCredits(fileSizeBytes) {
        const mb = fileSizeBytes / (1024 * 1024);
        return Math.max(1, Math.ceil(mb));
    }
    pruneArdrivePreflights() {
        const now = Date.now();
    ...
    async persistArdriveConnectorEvent(event, actorId, payload) {
        try {
            ...
            this.logger.warn(`Failed to write ArDrive connector audit snapshot: ${error.message}`);
        }
        try {
            await this.unifiedLedgerService.createTimelineEvent({
                userId: actorId,
            ...
            this.logger.warn(`Failed to write ArDrive connector timeline snapshot: ${error.message}`);
        }
    }
    enforceProviderGuardrails(source, destination) {
        const aliases = [this.extractRemoteAlias(source), this.extractRemoteAlias(destination)].filter((value) => Boolean(value));
        for (const alias of aliases) {
            if (alias === 'degoo') {
                throw new Error('Degoo direct remote execution is disabled in TNF managed runtime. Use an official bridge workflow only.');
            }
            if (alias === 'ardrive' || alias === 'turbo' || alias === 'arweave') {
                throw new Error('ArDrive requires the custom Turbo connector path. Use provider blueprint and connector flow, not direct rclone execution.');
            }
        }
    }
    extractRemoteAlias(value) {
    ...
    previewOutput(value) {
        ...
        if (!text)
            return '';
        if (text.length <= this.maxOutputPreviewChars)
            return text;
...
exports.RcloneRuntimeService = RcloneRuntimeService;
exports.RcloneRuntimeService = RcloneRuntimeService = RcloneRuntimeService_1 = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [audit_service_1.AuditService,
        unified_ledger_service_1.UnifiedLedgerService])
], RcloneRuntimeService);
//# sourceMappingURL=rclone-runtime.service.js.map