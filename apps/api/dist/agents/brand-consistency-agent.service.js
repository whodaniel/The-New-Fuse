...
var BrandConsistencyAgentService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.BrandConsistencyAgentService = void 0;
const common_1 = require("@nestjs/common");
const event_emitter_1 = require("@nestjs/event-emitter");
const database_1 = require("@the-new-fuse/database");
const schema_1 = require("@the-new-fuse/database/drizzle/schema");
let BrandConsistencyAgentService = BrandConsistencyAgentService_1 = class BrandConsistencyAgentService {
    constructor(db, eventEmitter) {
        this.db = db;
        this.eventEmitter = eventEmitter;
        this.logger = new common_1.Logger(BrandConsistencyAgentService_1.name);
        // Agent identification
        this.agentId = 'brand-consistency-agent';
        this.agentName = 'BrandConsistencyGuardian';
        ...
        this.corePrompt = `You are the Brand Consistency Guardian for The New Fuse platform.
...
- Identify inconsistent component patterns`;
    ...
    async initializePromptTemplate() {
        try {
            ...
            const existing = await this.db.client.query.promptTemplates.findFirst({
                where: (0, database_1.eq)(schema_1.promptTemplates.name, `${this.agentName}-CorePrompt`),
            });
            if (!existing) {
                // Find any user to associate with (required by schema)
                // In a real system, this would be a system user or admin
                const anyUser = await this.db.client.query.users.findFirst();
                if (!anyUser) {
                    this.logger.warn('No users found in database, skipping prompt template creation');
                    return;
                }
                // Create initial prompt template
                const [template] = await this.db.client
                    .insert(schema_1.promptTemplates)
                    .values({
                    ...
                    .returning();
                if (template) {
                    await this.db.client.insert(schema_1.promptVersions).values({
                ...
                const latestVersion = await this.db.client.query.promptVersions.findFirst({
                    where: (0, database_1.eq)(schema_1.promptVersions.templateId, existing.id),
                    orderBy: [(0, database_1.desc)(schema_1.promptVersions.versionNumber)],
                });
                if (latestVersion) {
            ...
            if (this.isMissingPromptTemplateSchema(error)) {
                this.logger.log('Skipping prompt template bootstrap: prompt template schema is not fully available in this environment.');
                return;
            }
            this.logger.warn('Could not initialize prompt template in database: ' + error);
        }
    }
    isMissingPromptTemplateSchema(error) {
        const err = error;
        const message = err?.message?.toLowerCase() ?? String(error).toLowerCase();
        const hasSchemaSignal = err?.code === '42703' ||
            err?.code === '42P01' ||
            (message.includes('column') && message.includes('does not exist')) ||
            (message.includes('relation') && message.includes('does not exist'));
        const isPromptTemplateContext = message.includes('prompt_templates') ||
     ...
     */
    checkColorConsistency(code, issues) {
     ...
     */
    checkTypographyConsistency(code, issues) {
     ...
     */
    checkSpacingConsistency(code, issues) {
     ...
     */
    checkAnimationConsistency(code, issues) {
     ...
     */
    checkPatternConsistency(code, issues, suggestions) {
...
exports.BrandConsistencyAgentService = BrandConsistencyAgentService;
exports.BrandConsistencyAgentService = BrandConsistencyAgentService = BrandConsistencyAgentService_1 = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [database_1.DatabaseService,
        event_emitter_1.EventEmitter2])
], BrandConsistencyAgentService);
//# sourceMappingURL=brand-consistency-agent.service.js.map