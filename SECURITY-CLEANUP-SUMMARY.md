...
**Date:** 2025-11-17
**Action:** Removed hardcoded secrets and migrated to environment variables
...
```
/Users/<owner>/SECRETS-BACKUP.txt
```
...
**Deleted Completely:**
- `tools/legacy-files/firebase-credentials.json` (contained Google service account private keys)
...
**Note:** The full private key is in `/Users/<owner>/SECRETS-BACKUP.txt`
...
Since the repo was private and these secrets were only visible to you, rotation is optional but recommended:
...
2. **Firebase API Keys**
   ...
   - Rotate via: https://console.firebase.google.com/project/the-new-fuse-2025/settings/general

3. **Google Service Account**
   - Rotate via: https://console.cloud.google.com/iam-admin/serviceaccounts?project=the-new-fuse-2025
...
- [ ] **RUN:** `git log --all -- '*.env*' 'firebase-credentials.json'` to verify files removed from history
...
### Deleted
1. `tools/legacy-files/firebase-credentials.json` - Contained service account keys
...
**⚠️ WARNING:** These secrets are still in your git history. If you want to completely remove them before making the repo public, you have two options:
...
If rotating all credentials, you can safely make the repo public without cleaning history:
...
**Remember:** Never commit secrets to git. Always use environment variables for sensitive data!
