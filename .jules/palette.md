...
## 2024-08-20 - Adding focus-visible and aria-label to raw icon buttons
**Learning:** Adding `aria-label` and `focus-visible` states to raw HTML `<button>` elements (like icon-only toggles) is essential for screen reader and keyboard accessibility, especially when bypassing core design system components.
**Action:** Always verify that custom icon-only buttons include `aria-label` and explicit `focus-visible:ring-2` styles to maintain accessibility parity with the design system.
