...
## 2024-08-16 - Manual SQL Escaping vs Parameterization in Raw Queries
**Vulnerability:** The `StripeService` in `apps/api/src/modules/billing/stripe.service.ts` was executing raw SQL queries using `this.db.executeRaw` with string interpolation and a manual escaping function (`value.replace(/'/g, "''")`).
**Learning:** Relying on manual string replacement to escape single quotes is prone to errors, incomplete escaping, and bypasses, making it highly susceptible to SQL injection vulnerabilities. The standard, secure way to handle user input in raw SQL is to use prepared statements/parameterization, which is natively supported by the underlying driver used by `executeRaw`.
**Prevention:** Always use parameterized queries (e.g., `$1`, `$2`) and pass inputs as an array of arguments to `executeRaw` rather than manually escaping strings and interpolating them directly into the SQL query string.
